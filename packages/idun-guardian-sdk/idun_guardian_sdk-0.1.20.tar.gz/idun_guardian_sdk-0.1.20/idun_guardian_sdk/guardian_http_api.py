...
class GuardianHttpAPI:
    ...
    def download_sleep_report(self, recording_id: str) -> ReportResponse:
        ...
        """
        logger.warning(
            "This method will be deprecated in the future. Use download_sleep_report_by_id instead."
        )
    ...
    def download_daytime_report_by_id(self, report_id: str) -> ReportResponse:
        ...
        if resp.status_code != 200:
            raise APIRequestError(
                f"Failed to download daytime report with id {report_id}. {res.get('message')}"
            )
