...
def get_client_and_resource(
    ...
    client = session.client('s3', config=boto3.session.Config(signature_version='s3v4', connect_timeout=5, read_timeout=5), **settings)
    resource = session.resource('s3', config=boto3.session.Config(signature_version='s3v4', connect_timeout=5, read_timeout=5), **settings)
