...
from aixblock_core.core.utils.common import round_floats
...
class ViewSerializer(serializers.ModelSerializer):
        ...
        if "filter_group" not in data and conjunction:
            ...
            if "items" in filters:
                for f in filters["items"]:
                    ...
                    if v is None and o in ("equal", "not_equal", "less", "greater", "less_or_equal", "greater_or_equal", "contains", "not_contains", "regex"):
                        if t == "Number":
                            v = 0
                        else:
                            v = ""
    ...
    def update(self, instance, validated_data):
        with transaction.atomic():
            ...
            if is_private is not None and is_private != instance.is_private:
...
class DataManagerTaskSerializer(TaskSerializer):
    ...
    created_by = serializers.SerializerMethodField(required=False, read_only=True)
    updated_by = serializers.SerializerMethodField(required=False, read_only=True)
    reviewed_by = serializers.SerializerMethodField(required=False, read_only=True)
    reviewed_result = serializers.SerializerMethodField(required=False)
    qualified_by = serializers.SerializerMethodField(required=False, read_only=True)
    qualified_result = serializers.SerializerMethodField(required=False)
    assigned_to = serializers.SerializerMethodField(required=False, read_only=True)
    # label_config = serializers.CharField(source='project.label_config', read_only=True) #This is problem (140KB in a label config)
    ...
    def get_created_by(obj):
        return [{"user_id": obj.created_by_id}] if obj.created_by_id else []
    ...
    def get_reviewed_by(obj):
        return [{'user_id': obj.reviewed_by_id}] if obj.reviewed_by_id else []

    @staticmethod
    def get_reviewed_result(obj):
        return obj.reviewed_result if obj.reviewed_result in ["approved", "rejected"] else None

    @staticmethod
    def get_qualified_by(obj):
        return [{'user_id': obj.qualified_by_id}] if obj.qualified_by_id else []

    @staticmethod
    def get_qualified_result(obj):
        return obj.qualified_result if obj.qualified_result in ["qualified", "unqualified"] else None

    @staticmethod
    def get_assigned_to(obj):
        return [obj.assigned_to_id] if obj.assigned_to_id else []
