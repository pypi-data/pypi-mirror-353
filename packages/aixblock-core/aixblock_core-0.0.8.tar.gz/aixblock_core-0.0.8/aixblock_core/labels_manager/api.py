...
class LabelAPI(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Label.objects.filter(Q(organization=self.request.user.active_organization) |
            Q(organization_id__in=organization_join)).prefetch_related('links')
...
class LabelLinkAPI(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        organization_join = OrganizationMember.objects.filter(user_id=self.request.user.id).values_list("organization_id", flat = True)
        return LabelLink.objects.filter(Q(label__organization=self.request.user.active_organization)|Q(label__organization_id__in=organization_join)).annotate(
