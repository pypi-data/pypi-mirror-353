...
from aixblock_core.core.utils.exceptions import (
    AIxBlockValidationErrorSentryIgnored, AIxBlockXMLSyntaxErrorSentryIgnored
)
from aixblock_tools.core import label_config
...
def parse_config(config_string):
    ...
    """
    logger.warning("Using deprecated method - switch toaixblock_core.tools.label_config.parse_config!")
...
def _fix_choices(config):
    ...
    https://github.com/aixblock-3/aixblock/issues/1259
    '''
...
def validate_label_config(config_string):
    ...
    try:
        ...
        raise AIxBlockXMLSyntaxErrorSentryIgnored(str(exc))
        ...
        raise AIxBlockXMLSyntaxErrorSentryIgnored(error_message)

    # unique names in config # FIXME: 'name =' (with spaces) won't work
    all_names = re.findall(r'name="([^"]*)"', config_string)
    if len(set(all_names)) != len(all_names):
        raise AIxBlockXMLSyntaxErrorSentryIgnored('Label config contains non-unique names')
    ...
    for toName_ in toNames:
        for toName in toName_.split(','):
            if toName not in names:
                raise AIxBlockXMLSyntaxErrorSentryIgnored(f'toName="{toName}" not found in names: {sorted(names)}')
