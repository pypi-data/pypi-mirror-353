...
@login_required
def project_editor(request, pk,sub_path):
    return render(request, 'projects/editor.html')
@login_required
def project_store(request, pk,sub_path):
    return render(request, 'projects/crawler.html')
@login_required
def project_train(request, pk,sub_path):
    return render(request, 'projects/crawler.html')
@login_required
def project_health(request, pk,sub_path):
    return render(request, 'projects/crawler.html')
@login_required
    ...
    if request.GET.get('playground', '0') == '1':
        for key in task_data:
            if "/samples/time-series.csv" in task_data[key]:
                task_data[key] = "https://tool.wow-dao.ai" + task_data[key]
    return task_data
