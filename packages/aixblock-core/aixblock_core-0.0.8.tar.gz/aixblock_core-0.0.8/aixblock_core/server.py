...
from aixblock_core.core.utils.windows_sqlite_fix import windows_dll_fix
...
from aixblock_core.core.argparser import parse_input_args
...
def _app_run(host, port):
    ...
    call_command('runsslserver', '--noreload', http_socket)

def _worker_run():
    call_command('worker', '--rate_limit=0')
...
def initialize_database():
    ...
    call_command("create_superuser_with_password", "--username", settings.ADMIN_USERNAME, "--password", settings.ADMIN_PASSWORD, "--noinput", "--email", settings.ADMIN_EMAIL, verbosity=0)
    call_command('create_organization', settings.ADMIN_EMAIL, verbosity=0)
    call_command('seed_templates', verbosity=0)
    call_command('collectstatic', '--no-input', verbosity=0)

    print("Email: ", settings.ADMIN_EMAIL)
    print("Username: ", settings.ADMIN_USERNAME)
    print("Password: ", settings.ADMIN_PASSWORD)
...
def _create_user(input_args, config):
    ...
    try:
        user = User.objects.create_user(email=username.lower(), password=password)
        user.is_staff = True
    ...
    org = Organization.objects.first()
    if not org:
        org = Organization.create_organization(created_by=user, title='WowDAO')
    else:
        org.add_user(user)
...
def main():
    ...
    if input_args.data_dir:
        ...
        os.environ.setdefault("BASE_DATA_DIR", str(data_dir_path.absolute()))
    ...
    if input_args.command == 'version' or input_args.version:
        # from aixblock_core import __version__
        # print('\nAiXBlock version:', __version__, '\n')
    ...
    # init
        ...
        print('')
        print('AiXBlock has been successfully initialized.')
        if input_args.command != 'start':
            print('Start the server: aixblock start ' + input_args.project_name)
            return

    # start with migrations from old projects, '.' project_name means 'aixblock start' without project name
    elif input_args.command == 'start' and input_args.project_name != '.':
        from aixblock_core.core.old_ls_migration import migrate_existing_project
        from projects.models import Project
        sampling_map = {'sequential': Project.SEQUENCE, 'uniform': Project.UNIFORM,
                        'prediction-score-min': Project.UNCERTAINTY}
        ...
        if not _project_exists(input_args.project_name):
            ...
            if project_path.exists():
                print('Project directory from previous verion of aixblock found')
                ...
                print(
                    Fore.LIGHTYELLOW_EX +
                    '\n*** WARNING! ***\n'
                    + f'Project {input_args.project_name} migrated to AiXBlock Database\n'
            ...
            if not migrated:
                print(
                    ...
                    'Did you miss create it first with `aixblock init {project_name}` ?'.format(
                        project_name=input_args.project_name
                    )
                )
                return

    # on `start` command, launch browser if --no-browser is not specified and start AiXBlock server
    if input_args.command == 'start' or input_args.command is None:
        from aixblock_core.core.utils.common import start_browser
        ...
        if cert_file or key_file:
            logger.error("AiXBlock doesn't support SSL web server with cert and key.\n"
        ...
        from django.conf import settings
        url = ('https://127.0.0.1:' + str(internal_port)) if not host else host
        ...
        # worker_process = multiprocessing.Process(target=worker_run)
