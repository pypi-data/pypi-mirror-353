...
def predictions_to_annotations(project, queryset, request, **kwargs):
    if not request.user.is_superuser and not request.user.is_staff and not request.user.is_organization_admin:
        return {'processed_items': 0, 'detail': 'You do not have permission to perform this action.'}

    # request = kwargs['request']
    user = request.user
    # model_version = request.data.get('model_version')
    queryset = queryset.filter(predictions__isnull=False)
    predictions = Prediction.objects.filter(task__in=queryset, child_annotations__isnull=True).order_by('created_at').reverse()
    ...
    for result, model_version, task_id, prediction_id in predictions_values:
        if task_id in processed_task:
            continue

        tasks_ids.append(task_id)
        processed_task.append(task_id)
        body = {
            'result': result,
