...
from aixblock_core.core.utils.params import cast_bool_from_str
...
def get_fields_for_evaluation(prepare_params, user):
    ...
    if fields:
        from aixblock_core.data_manager.functions import TASKS
...
def add_result_filter(field_name, _filter, filter_expressions, project):
    ...
    # old approach
        ...
        if _filter.operator in [Operator.EQUAL, Operator.NOT_EQUAL]:
            ...
            filter_expressions.append(~Q(**{name + '__isnull': cast_bool_from_str(_filter.value)}))
            return 'continue'
...
def apply_filters(queryset, filters, project, request):
    ...
    for _filter in filters.items:
        ...
        # created_by
        result = add_user_filter(field_name == 'created_by', 'created_by', _filter, filter_expressions)
        if result == 'continue':
            continue

        # reviewed_by
        result = add_user_filter(field_name == 'reviewed_by', 'reviewed_by', _filter, filter_expressions)
        if result == 'continue':
            continue

        # qualified_by
        result = add_user_filter(field_name == 'qualified_by', 'qualified_by', _filter, filter_expressions)
        if result == 'continue':
            continue

        # assigned_to
        result = add_user_filter(field_name == 'assigned_to', 'assigned_to', _filter, filter_expressions)
        if result == 'continue':
            continue

        if field_name == 'storage_filename':
            field_name = 'file_upload__file'

        # reviewed_result
        if field_name == 'reviewed_result' or field_name == 'qualified_result':
            if 'contains' in _filter.operator or 'not_contains' in _filter.operator:
