...
def test_user_info(business_client):
    from aixblock_core.server import _get_user_info, _create_user
...
@pytest.mark.parametrize('command_line, result', [
    (['aixblock', 'user', '--username', 'test@test.com', '--password', '12345678'], None),
])
@pytest.mark.django_db
...
def test_string_is_url():
    from aixblock_core.core.utils.common import string_is_url
...
def test_get_client_ip():
    from aixblock_core.core.utils.common import get_client_ip
...
def test_timestamp_now():
    from aixblock_core.core.utils.common import timestamp_now
