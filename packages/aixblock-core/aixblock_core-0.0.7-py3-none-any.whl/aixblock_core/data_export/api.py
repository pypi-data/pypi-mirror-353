...
import django_rq
from .func_rq import process_export
...
class ExportFormatsListAPI(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        ...
        return Project.objects.filter(Q(organization=self.request.user.active_organization)|Q(organization_id__in=organization_join))
...
class ExportAPI(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        organization_join = OrganizationMember.objects.filter(user_id=self.request.user.id).values_list("organization_id", flat = True)
        return Project.objects.filter(Q(organization=self.request.user.active_organization) |
    ...
    def get(self, request, *args, **kwargs):
        ...
        custom_format = query_serializer.validated_data.get("custom_format", "{}")
        ...
        #     project, tasks, export_type, download_resources, request.GET, request.user.id
        # )
        ...
        # job = queue.enqueue(
        #     process_export,
        #     project.id, export_type, only_finished, download_resources, interpolate_key_frames, tasks_ids, request.user, request.GET
        # )
        import threading
        thread_export = threading.Thread(target=process_export, args=(project.id, export_type, only_finished, download_resources, interpolate_key_frames, tasks_ids, request.user, request.GET,))
        thread_export.start()
        return Response(f'Job is being processed', status=202)
        # return response
...
@method_decorator(
    ...
    decorator=swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                "custom_format": openapi.Schema(
                    type=openapi.TYPE_OBJECT,
                    description="Custom format JSON export",
                ),
            },
        ),
        tags=["Export"],
        operation_summary="export custom format json",
        responses={
            200: openapi.Response(
                description="Exported data",
                schema=openapi.Schema(
...
class ExportAPICustom(generics.CreateAPIView):
    permission_required = all_permissions.projects_change

    def get_queryset(self):
        organization_join = OrganizationMember.objects.filter(
...
@method_decorator(
    ...
    decorator=swagger_auto_schema(
        ...
        operation_description="""
        Retrieve a list of files exported from the AiXBlock UI using the Export button on the Data Manager page.
...
@method_decorator(
    ...
    decorator=swagger_auto_schema(
        ...
        """,
        request_body=ExportDatasetRequestSerializer
    ),
)
class ExportListAPI(generics.ListCreateAPIView):
    ...
    def post(self, request, pk):
        ...
        dataset = DatasetModelMarketplace.objects.get(pk=dataset_id)
        ...
        return Response({'message': 'Dataset upload request success'}, status=status.HTTP_200_OK)
