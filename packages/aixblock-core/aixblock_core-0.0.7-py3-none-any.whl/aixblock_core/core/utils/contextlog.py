...
class ContextLog(object):
    ...
    def _get_aixblock_core_env(self):
        env = {}
        for env_key, env_value in os.environ.items():
            if env_key.startswith('aixblock_core_'):
    ...
    def _secure_data(self, payload, request):
        ...
        if view_name == 'data_manager:dm-actions' and payload['values'].get('id') == 'next_task':
            ...
            for key in ["id", "inner_id", "cancelled_annotations", "total_annotations", "total_predictions", "updated_by", "created_at", "updated_at", "overlap", "comment_count", "unresolved_comment_count", "last_comment_updated_at", "project", "comment_authors", "queue", "reviewed_by", "reviewed_result", "is_in_review", "qualified_by", "qualified_result", "is_in_qc"]:
    ...
    def create_payload(self, request, response, body):
        ...
        payload = {
            ...
            'env': self._get_aixblock_core_env(),
