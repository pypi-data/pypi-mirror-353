...
class FileUpload(models.Model):
    ...
    def read_task_from_uploaded_file(self, object_key=False):
        logger.debug('Read 1 task from uploaded file {}'.format(self.file.name))
        if 'image' in self.project.data_types:
            data_type = 'image'
        elif 'audio' in self.project.data_types:
            data_type = 'audio'

        if settings.CLOUD_FILE_STORAGE_ENABLED:
            tasks = [{'data': {settings.DATA_UNDEFINED_NAME: self.file.name}}]
            if object_key:
                tasks = [{'data': {key: self.file.name if key == data_type else "" for key in self.project.data_types}}]

        else:
            tasks = [{'data': {settings.DATA_UNDEFINED_NAME: self.url}}]
        
            if object_key:
                tasks = [{'data': {key: self.url if key == data_type else "" for key in self.project.data_types}}]
    ...
    def read_tasks(self, file_as_tasks_list=True):
        ...
        try:
            ...
            # otherwise - only one object tag should be presented in label config
                ...
                tasks = self.read_task_from_uploaded_file(object_key=True)
    ...
    def load_tasks_from_uploaded_files(cls, project, file_upload_ids=None, formats=None, files_as_tasks_list=True, trim_size=None):
        ...
        is_audio_project = len(project.data_types) > 0 and 'audio' in project.data_types
        ...
        for file_upload in file_uploads:
            ...
            for task in new_tasks:
                ...
                if is_audio_project:
                    try:
                        import mutagen
                        file_path = file_upload.filepath.lstrip('/')
                        media_path = settings.MEDIA_ROOT.rstrip('/')
                        file_meta = mutagen.File(media_path + '/' + file_path)
                        task['data']['duration'] = file_meta.info.length
                        print('The duration of the file ' + file_path + ' is ' + file_meta.info.length.__str__() + "s")
