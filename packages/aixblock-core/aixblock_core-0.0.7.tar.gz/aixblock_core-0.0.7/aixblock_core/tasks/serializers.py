...
# from data_import.uploader import create_image_thumbnails, create_audio_ogg
...
class BaseTaskSerializer(FlexFieldsModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if project:
            ...
            # try:
            #     if 'text' in project.data_types:
            #         if 'image' in project.data_types:
            #             first_key = 'image'
            #         elif 'audio' in project.data_types:
            #             first_key = 'audio'
            #         elif 'video' in project.data_types:
            #             first_key = 'video'
            #
            #     if 'image' in data and data['image']:
            #         file_dir = data['image'].replace('/data/', '')
            #         small_file_path = os.path.dirname(file_dir) + '/small_' + os.path.basename(data['image'])
            #         if not FileUpload.objects.filter(file=small_file_path).exists():
            #             file_instance = FileUpload.objects.filter(file=file_dir).first()
            #             thread = threading.Thread(target=create_image_thumbnails, args=(project_minio, project, file_instance.file.path, file_instance.user))
            #             thread.start()
            #     elif 'audio' in data and data['audio']:
            #         file_dir = data['audio'].replace('/data/', '')
            #         ogg_file_name = os.path.splitext(file_dir)[0] + '.ogg'
            #         if not FileUpload.objects.filter(file=ogg_file_name).exists():
            #             file_instance = FileUpload.objects.filter(file=file_dir).first()
            #             thread = threading.Thread(target=create_audio_ogg, args=(project_minio, project, file_instance.file.path, file_instance.user))
            #             thread.start()
            #
            # except Exception as e:
            #     print(e)

            replace_task_data_undefined_with_config_field(data, project, first_key)
    ...
    def get_locked_by(self, obj):
        ...
        locking_user = obj.get_locking_user(user)
        return [locking_user.id] if locking_user else []
...
class TaskWithAnnotationsAndPredictionsAndDraftsSerializer(TaskSerializer):
    ...
    created_by = serializers.SerializerMethodField(default=[], read_only=True)
    reviewed_by = serializers.SerializerMethodField(default=[], read_only=True)
    reviewed_result = serializers.SerializerMethodField(default=None)

    def get_updated_by(self, task):
        return [{'user_id': task.updated_by_id}] if task.updated_by_id else []

    def get_created_by(self, task):
        return [{'user_id': task.created_by_id}] if task.created_by_id else []
    ...
    def get_reviewed_by(self, task):
        return [{'user_id': task.reviewed_by_id}] if task.reviewed_by_id else []

    def get_qualified_by(self, task):
        return [{'user_id': task.qualified_by_id}] if task.qualified_by_id else []

    @staticmethod
    def get_reviewed_result(task):
        return task.reviewed_result if task.reviewed_result in ["approved", "rejected"] else None

    @staticmethod
    def get_qualified_result(task):
        return task.qualified_result if task.qualified_result in ["qualified", "unqualified"] else None
...
class TaskQueueRequest(serializers.Serializer):
...
class TaskAdminViewSerializer(serializers.ModelSerializer):
