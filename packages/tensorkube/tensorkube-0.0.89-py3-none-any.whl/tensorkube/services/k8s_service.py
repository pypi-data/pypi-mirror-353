...
def generate_kubeconfig():
    ...
    kubeconfig_data = {
        ...
        "clusters": [{
            ...
            "name": f'arn:aws:eks:{region}:{get_aws_account_id()}:cluster/{cluster_name}'
        }],
        "contexts": [{
            "context": {
                "cluster": f'arn:aws:eks:{region}:{get_aws_account_id()}:cluster/{cluster_name}',
                "user": f'arn:aws:eks:{region}:{get_aws_account_id()}:cluster/{cluster_name}'
            },
            "name": f'arn:aws:eks:{region}:{get_aws_account_id()}:cluster/{cluster_name}'
        }],
        "current-context": f'arn:aws:eks:{region}:{get_aws_account_id()}:cluster/{cluster_name}',
        "kind": "Config",
        "users": [{
            "name": f'arn:aws:eks:{region}:{get_aws_account_id()}:cluster/{cluster_name}',
            "user": {
                "exec": {
                    ...
                    "env": [
                        {"name": "AWS_DEFAULT_REGION", "value": get_session_region()}, #TODO: is this env needed ?
                    ]
                }
            }
        }]
