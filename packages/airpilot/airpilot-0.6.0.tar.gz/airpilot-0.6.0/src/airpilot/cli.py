...
def cli(ctx: click.Context, version: bool) -> None:
    ...
    """
    if version:
        console.print(f"AirPilot CLI v{__version__}")
        return
...
def init_global_intelligence() -> None:
    ...
    if existing:
        console.print(f"[yellow]Warning: Found existing directories: {', '.join(existing)}[/yellow]")
    ...
    console.print("[green]SUCCESS: System-level intelligence control initialized successfully![/green]")
    console.print("\nNext steps:")
    console.print(f"• Configure your preferences in {airpilot_dir}/config.json")
    console.print(f"• Add global rules and prompts to {air_dir}/")
    console.print("• Run [cyan]air init[/cyan] in project directories to inherit from global")
...
def init_current_directory(force: bool) -> None:
    ...
    if air_dir.exists() and not force:
        if detect_air_standard(air_dir):
            console.print("[yellow]Found existing .air standard directory[/yellow]")
            if not Confirm.ask("Merge with existing .air directory?"):
                console.print("[yellow]Cancelled[/yellow]")
                return
        else:
            console.print("[yellow]Found non-standard .air directory[/yellow]")
            backup_path = backup_existing_air(air_dir)
            console.print(f"[blue]Backed up to: {backup_path}[/blue]")
    ...
    console.print("[green]SUCCESS: Project intelligence control initialized successfully![/green]")
    console.print("\nNext steps:")
    console.print("• Add project rules to .air/rules/")
    console.print("• Configure vendors in .airpilot")
    console.print("• Install AirPilot VSCode extension for automatic sync")
...
def init_new_project(project_name: str, force: bool) -> None:
    ...
    console.print(f"[green]SUCCESS: New project '{project_name}' created successfully![/green]")
...
def create_air_standard(air_dir: Path, is_project: bool = False) -> None:
    ...
    console.print(f"[dim]Creating .air standard structure in {air_dir}...[/dim]")
...
def init_git_if_needed(project_dir: Path) -> None:
    ...
    if not git_dir.exists():
        try:
            ...
            console.print("[green]SUCCESS: Initialized Git repository[/green]")
        except subprocess.CalledProcessError:
            console.print("[yellow]WARNING: Could not initialize Git (git not available)[/yellow]")
        except FileNotFoundError:
            console.print("[yellow]WARNING: Git not found - skipping Git initialization[/yellow]")
...
def backup_ai_vendors(project_dir: Path) -> None:
    ...
    if found_vendors:
        console.print(f"[blue]Found existing AI vendor directories: {', '.join(found_vendors)}[/blue]")
        console.print("[blue]These will be backed up by AirPilot VSCode extension on first sync[/blue]")
...
def license(ctx: click.Context) -> None:
    ...
    if ctx.invoked_subcommand is None:
        ...
        table = Table(title="AirPilot License Status")
        table.add_column("Property", style="cyan")
        table.add_column("Value", style="green")

        table.add_row("Plan", info["plan"].upper())
        table.add_row("Licensed", "Yes" if info["licensed"] else "No")
        ...
        if info.get("installed_at"):
            ...
            table.add_row("Installed", installed_date)

        console.print(table)

        if not info["licensed"]:
            console.print("\n[bright_blue]Premium Features Available:[/bright_blue]")
            console.print("• [blue]air sync[/blue] - Real-time vendor synchronization")
            console.print("• [blue]air cloud[/blue] - Cloud backup and sync (coming soon)")
            console.print("\n[bright_blue]To get a license:[/bright_blue]")
            console.print("1. Email: shaneholloman@gmail.com")
            console.print("2. Subject: AirPilot License Request")
            console.print("3. Include your name and use case")
            console.print("\n[bright_blue]Once you have your key:[/bright_blue]")
            console.print("air license install <your-license-key>")
...
def install(key: str) -> None:
    ...
    if license_manager.install_license(key):
        info = license_manager.get_license_info()
        console.print("[green]SUCCESS: License installed![/green]")
        console.print(f"[green]Plan: {info['plan'].upper()}[/green]")
        console.print(f"[green]Features: {', '.join(info['features'])}[/green]")
    else:
        console.print("[red]ERROR: Invalid license key[/red]")
        console.print("[yellow]If you need a license key, email shaneholloman@gmail.com[/yellow]")
        console.print("[yellow]Subject: AirPilot License Request[/yellow]")
...
def remove() -> None:
    ...
    if Confirm.ask("Remove license and revert to free plan?"):
        if license_manager.remove_license():
            console.print("[green]License removed successfully[/green]")
            console.print("[yellow]Reverted to free plan[/yellow]")
        else:
            console.print("[red]ERROR: Failed to remove license[/red]")
...
def sync() -> None:
    ...
    """
    console.print("[yellow]Starting real-time vendor synchronization...[/yellow]")
    console.print("[green]SUCCESS: Sync feature activated![/green]")
    console.print("[blue]This is a demo - full sync implementation coming soon[/blue]")
