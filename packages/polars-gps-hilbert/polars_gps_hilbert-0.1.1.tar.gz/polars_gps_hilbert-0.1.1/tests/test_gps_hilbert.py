...
def test_compute_bounds():
    ...
    bounds_df = df.select(gps.compute_bounds("lat", "lon", "ts").alias("bounds"))
    bounds_values = bounds_df["bounds"].to_list()
    
    # bounds_values = [lat_min, lat_max, lon_min, lon_max, ts_min, ts_max]
    assert bounds_values[0] == pytest.approx(34.0522)  # lat_min
    assert bounds_values[1] == pytest.approx(40.7128)  # lat_max
    assert bounds_values[2] == pytest.approx(-122.4194)  # lon_min
    assert bounds_values[3] == pytest.approx(-74.0060)  # lon_max
    assert bounds_values[4] == 1640995200  # ts_min
    assert bounds_values[5] == 1641002400  # ts_max
...
def test_invalid_coordinates():
    ...
    # Invalid coordinates should not raise error but produce valid indices
    # (Our implementation handles normalization)
    result = df.with_columns(
        gps.hilbert_index("lat", "lon", "ts").alias("idx")
    )
    assert "idx" in result.columns
...
def test_different_bit_sizes():
    ...
    # Test with different bit sizes and store results
    results = {}
    ...
    assert len(results[15][f"idx_15"].unique()) >= len(results[10][f"idx_10"].unique())
    assert len(results[21][f"idx_21"].unique()) >= len(results[10][f"idx_10"].unique())
