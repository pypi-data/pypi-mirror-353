...
class ClassesApi(PermissionsMixin, ModelClassMixin, ColumnInfoMixin, ObjectDataMixin):
    @classmethod
    async def get_classes(cls, user: BaseUser, *, include_count: bool) -> list[ClassInfo]:
        if AmsdalConfigManager().get_config().async_mode:
            return await cls._aget_classes(user, include_count=include_count)

        return await cls._get_classes(user, include_count=include_count)

    @classmethod
    async def _aget_classes(cls, user: BaseUser, *, include_count: bool = False) -> list[ClassInfo]:
        ...
        for class_item in classes:
            ...
            result.append(await cls.get_class(user, class_item, class_meta_item, include_count=include_count))

        return result

    @classmethod
    async def _get_classes(cls, user: BaseUser, *, include_count: bool = False) -> list[ClassInfo]:
    ...
    async def get_class_by_name(
        ...
        *,
        ...
        return await cls.get_class(user, class_item, class_meta_item, include_count=include_count)
    ...
    async def get_class(
        ...
        *,
        ...
        if AmsdalConfigManager().get_config().async_mode:
            return await cls._aget_class(user, class_item, class_meta_item, include_count=include_count)

        return await cls._get_class(user, class_item, class_meta_item, include_count=include_count)
    ...
    async def _aget_class(
        ...
        *,
        ...
        class_properties = await cls.aget_class_properties_by_class_and_meta(class_meta_item)
        data: dict[str, Any] = {
        ...
        if include_count:
            ...
            if has_read_permission:
                class_info.count = (
                    await model_class.objects.latest().filter(_metadata__is_deleted=False).count().aexecute()
                )
    ...
    async def _get_class(
        ...
        *,
        ...
        class_properties = cls.get_class_properties_by_class_and_meta(class_meta_item)
        data: dict[str, Any] = {
            'class': class_item.object_id,
            'count': 0,
            'properties': class_properties,
    ...
    def register_class(
        ...
        *,
        ...
        from amsdal.models.core.class_object_meta import ClassObjectMeta
        ...
        if not model_object:
            ...
            class_object_meta_object = ClassObjectMeta(  # type: ignore[call-arg]
                title=class_name,
                type=object_schema.type,
                properties=_props_meta,  # type: ignore[arg-type]
            ...
            if class_object_object.properties != _props:
                ...
                migrartion_executor._migrate_historical_data(
                    ...
                    class_name,
                    prior_version=class_object_object.get_metadata().prior_version,  # type: ignore[arg-type]
