...
async def _cache_class_list(request: Request, include_count: bool) -> tuple[dict[str, Any], str]:  # noqa: FBT001
    class_items = await ClassesApi.get_classes(request.user, include_count=include_count)
...
async def class_list(
    ...
    include_count: bool = Query(  # noqa: FBT001
        default=False,
        description='Include count of objects in each class',
    ),
) -> Response:
    ...
    result, etag = await _cache_class_list(request, include_count)
    headers['ETag'] = etag
