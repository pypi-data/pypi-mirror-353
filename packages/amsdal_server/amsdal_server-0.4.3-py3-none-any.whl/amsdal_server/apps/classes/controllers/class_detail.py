...
async def _cache_class_details(request: Request, class_name: str, include_count: bool) -> tuple[dict[str, Any], str]:  # noqa: FBT001
    _data = (await ClassesApi.get_class_by_name(request.user, class_name, include_count=include_count)).model_dump()
...
async def get_class(
    ...
    include_count: bool = Query(  # noqa: FBT001
        default=False,
        description='Include count of objects in the class',
    ),
) -> Response:
    ...
    result, etag = await _cache_class_details(request, class_name, include_count)
    headers['ETag'] = etag
