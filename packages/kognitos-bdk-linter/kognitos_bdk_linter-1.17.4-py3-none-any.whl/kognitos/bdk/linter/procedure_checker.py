...
class ProcedureChecker(BaseChecker):
    ...
    def visit_functiondef(self, node: nodes.FunctionDef | nodes.AsyncFunctionDef) -> None:
        if ProcedureChecker.is_procedure(node):
            ...
            docstring_inputs = [noun_phrase.to_snake_case() for input_concept in docstring.input_concepts for noun_phrase in getattr(input_concept, "noun_phrases", [])] + [
                param.name for param in docstring.params
