...
class Internal_RS485_slave_server_class:
    ...
    def add_slave(self,slave_id,slave_context):
        print(f"Adding slave with ID {slave_id} to the server")
        print(f"Slave context: {slave_context}")
    ...
    def run_server_function(self):
        while(1):
            if(self.stop_server_flag == 0):
                ...
                print(f"Active slaves context: {self.active_slave_dict}")
                ...
                self.server= StartSerialServer(context=ModbusServerContext(slaves=self.active_slave_dict,single=False),
                ...
                print(f"Started Modbus slave server on {self.hw_channal} with baudrate {self.baudrate}, parity {p}, stopbits {self.stopbits}, timeout {self.timeout}")
    ...
    def start_all_slaves(self):
        ...
        print("Starting all slaves in the server")
