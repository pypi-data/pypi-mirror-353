(
    ...
    let recordingData = args.recordingData || {};
    let elementsConfig = args.elementsConfig || [];
    let steps = args.recordingData?.processedSteps || [];
    ...
    function getDefinedHoverStyles(element) {
      if (!element || typeof element.matches !== 'function') {
        console.error("Invalid element provided or element.matches is not supported.");
        return {};
      }

      const hoverStyles = {};
      // 使用 Array.from 创建副本，以防在迭代期间样式表集合发生更改
      const sheets = Array.from(document.styleSheets);

      for (const sheet of sheets) {
        try {
          // sheet.rules (for IE < 9) or sheet.cssRules (for modern browsers)
          const rules = sheet.rules || sheet.cssRules;
          if (!rules) continue;

          // 迭代规则时也使用副本
          for (const rule of Array.from(rules)) {
            // 我们只关心 CSSStyleRule (类型为 1) 并且它有 selectorText
            if (rule.type !== CSSRule.STYLE_RULE || !rule.selectorText) {
              continue;
            }

            // CSS 规则可能包含多个选择器，用逗号分隔 (例如 "a:hover, button:hover")
            const selectors = rule.selectorText.split(',');

            for (const selectorString of selectors) {
              const trimmedSelector = selectorString.trim();

              // 查找选择器中的 :hover，使用 lastIndexOf 以处理类似 :focus:hover 的链式伪类
              const hoverIndex = trimmedSelector.lastIndexOf(':hover');

              if (hoverIndex === -1) {
                continue; // 此选择器部分不包含 :hover
              }

              // 获取 :hover 之前的部分作为基础选择器，用于匹配元素
              // 例如，对于 "div.myClass:hover"，baseSelector 是 "div.myClass"
              // 对于 ":hover"，baseSelector 是 ""
              const baseSelector = trimmedSelector.substring(0, hoverIndex);

              // 获取 :hover 之后的部分
              const suffix = trimmedSelector.substring(hoverIndex + ':hover'.length);
              const trimmedSuffix = suffix.trimStart(); // 去掉前导空格，以正确判断后续内容

              // 1. 过滤掉 :hover 应用于伪元素的情况 (例如 "element:hover::before")
              if (trimmedSuffix.startsWith('::')) {
                continue;
              }

              // 2. 过滤掉 :hover 应用于后代元素或兄弟元素的情况
              //    (例如 "element:hover span", "element:hover > child")
              //    如果 suffix 非空，且不以另一个伪类 ":" 或属性选择器 "[" 开头，
              //    那么它很可能是一个组合器或后代选择器。
              if (trimmedSuffix && !trimmedSuffix.startsWith(':') && !trimmedSuffix.startsWith('[')) {
                continue;
              }

              // 尝试匹配元素
              try {
                // 情况 A: 全局 :hover 规则 (例如 ":hover { color: red; }")
                // baseSelector 为空，并且原始选择器以 :hover 开头
                if (baseSelector === '' && trimmedSelector.startsWith(':hover')) {
                  // 检查元素是否匹配 :hover 后的附加条件 (例如 ":hover[disabled]")
                  let matchesExtraConditions = true;
                  if (trimmedSuffix) { // 如果存在如 "[disabled]" 或 ":focus" 这样的后缀
                    // 注意: element.matches(':focus') 只有在元素当前有焦点时才为 true。
                    // 此函数旨在获取“定义”的样式，即使那些附加状态当前未激活。
                    // 因此，我们假设如果基础部分匹配，这些样式就是相关的 :hover 样式。
                    // 更复杂的检查（如元素是否真的匹配 :focus）超出了此基本函数的范围。
                  }

                  if (matchesExtraConditions) {
                    for (let i = 0; i < rule.style.length; i++) {
                      const propName = rule.style[i];
                      hoverStyles[propName] = rule.style.getPropertyValue(propName);
                    }
                  }
                }
                // 情况 B: 针对特定元素或类的 :hover 规则 (例如 "div:hover", ".btn:hover")
                else if (baseSelector && element.matches(baseSelector)) {
                  // 与情况 A 类似，我们假设如果元素匹配 baseSelector，
                  // 那么 rule.style 中的样式是相关的 :hover 样式，
                  // 即使它们可能还依赖于 :hover 后面的其他链式伪类（如 :focus）。
                  for (let i = 0; i < rule.style.length; i++) {
                    const propName = rule.style[i];
                    hoverStyles[propName] = rule.style.getPropertyValue(propName);
                  }
                ...
                // 忽略无效选择器或 element.matches() 的其他错误
                // console.warn(`Error matching selector "${baseSelector}" from "${trimmedSelector}":`, e.message);
              }
            }
          }
        } catch (e) {
          ...
          if (e.name !== 'SecurityError') {
            // console.warn(`Cannot access CSS rules from stylesheet: ${sheet.href || '(inline stylesheet)'}`, e.message);
          }
        }
      }
    ...
    const isScrollableElement = (element) => {
        ...
        const isOverflowY = ['auto', 'scroll', 'overlay'].includes(overflowY);
        const isOverflowX = ['auto', 'scroll', 'overlay'].includes(overflowX);
        const canScrollY = element.scrollHeight > element.clientHeight;
        const canScrollX = element.scrollWidth > element.clientWidth;
        return (isOverflowY && canScrollY) || (isOverflowX && canScrollX);
      };
    ...
    function isInteractiveElement(element) {
      if (!element || element.nodeType !== Node.ELEMENT_NODE) {
        ...
        const steps = recordingData.processedSteps || [];

        const config = elementsConfig.find(config => window.location.href.includes(config.domain))

        if(config) {
            const elementConfig = config.elements.find(el => {
                const isMatchClassName = element.className && el.className?.includes?.(element.className);
                const isMatchId = element.id && el.id && element.id === el.id
                return isMatchClassName || isMatchId
            })

            if(elementConfig) {
                console.log(elementConfig,element, 'elementConfig')
                return {
        ...
        if (steps.length > 0 && element.nodeType === Node.ELEMENT_NODE) {
            const step = steps.find(step => {
                const eventInfo = step.eventInfo || {}
                const stepNodeId = eventInfo.id || eventInfo.target?.attributes?.id
                const stepNodeClassName = (eventInfo.className || eventInfo.target?.attributes?.class || '')?.split(' ')
                const stepNodeTagName = eventInfo.tagName || eventInfo.target?.tagName || ''
                const isMatchURL = (step.url || '').includes(window.location.origin);
                const isMatchTagName = stepNodeTagName.toLowerCase() === element.tagName.toLowerCase();
                const isMatchId = stepNodeId && element.id && stepNodeId === element.id
                const elementClassList = [...element.classList]
                const isMatchClassName = stepNodeClassName && element.className && stepNodeClassName?.length === elementClassList?.length && stepNodeClassName?.every(v => elementClassList?.includes?.(v))

                const dataAttribute = element.dataset
                const elementDataAttributeKeys = Object.keys(dataAttribute).map(v => `data-${v}`) || []
                const stepDataAttributeKeys = Object.keys(eventInfo?.dom?.attributes || {}).filter(v => v.startsWith('data-')) || []


                const domSelector = eventInfo?.domSelector && document.querySelector(eventInfo?.domSelector)

                const isMatchDomSelector = domSelector && domSelector === element

                // if(eventInfo.tagName === 'use') {
                //     console.log(element, elementDataAttributeKeys, stepDataAttributeKeys, 'elementDataAttributeKeys, stepDataAttributeKeys')
                // }

                // if(isMatchClassName) {
                //   console.log(element, step,element.className, element.classList,isMatchTagName, isMatchURL && isMatchTagName && (isMatchId || isMatchClassName) ,'isMatchClassName')
                // }

                const isMatchDataAttribute = stepDataAttributeKeys.length === elementDataAttributeKeys.length && stepDataAttributeKeys.every(v => elementDataAttributeKeys.includes(v))

                // if(eventInfo?.domSelector && isMatchDomSelector) {
                //   return
                // }
                if(isMatchDomSelector) {
                  return true
                }
                // if (isMatchURL && isMatchTagName && (isMatchId || isMatchClassName) && element.style.display !== 'none') {
            ...
            if (step) {
                const desc = JSON.stringify(step.domOperationInfo)
                return {
                    type: step.event,
                    desc: desc ? `Element Comment: ${desc}` : undefined,
                };
            }
        }
      ...
      function doesElementHaveInteractivePointer(element) {
        ...
        const hoverStyle = getDefinedHoverStyles(element)

        if(interactiveCursors.has(hoverStyle.cursor)) {
          return true
        }
    ...
    function handleHighlighting(nodeData, node, parentIframe, isParentHighlighted) {
      ...
      if (nodeData.isRecording) {

        shouldHighlight = true;
      }

      if(node.classList.contains('count')) {
        console.log(nodeData, shouldHighlight, node, 'nodeData')
      }
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (nodeData.isVisible) {
          ...
          if (nodeData.isTopElement) {
            ...
            isScrollElement = typeof interactive === 'object' && interactive.type?.toLowerCase?.() === 'scroll'

            nodeData.isInteractive = !!interactive;
            nodeData.isRecording = typeof interactive === 'object'
            nodeData.isScrollElement = isScrollElement
            const desc = interactive?.desc
            nodeData.desc = desc || undefined;
            // Call the dedicated highlighting function
            nodeWasHighlighted = handleHighlighting(nodeData, node, parentIframe, isScrollElement? false :isParentHighlighted);
          }
        }
      }
      ...
      if (node.tagName) {
        ...
        // Handle rich text editors and contenteditable elements
          ...
          for (const child of node.childNodes) {
            ...
            const passHighlightStatusToChild = isScrollElement ? false : (nodeWasHighlighted || isParentHighlighted)
