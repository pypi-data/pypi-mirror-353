...
class BrowserContext:
    ...
    async def _initialize_session(self):
        ...
        # Add listener for new page events immediately after creating context
    ...
    def _add_new_page_listener(self, context: PlaywrightBrowserContext):
        ...
        async def on_page(page: Page):
            ...
            targets = await self._get_cdp_targets()
            # 筛选 type=page 的 targets
            page_targets = [target for target in targets if target.get('type') == 'page' and target.get('url') == page.url]
            if page_targets:
                self.state.target_id = page_targets[0]['targetId']
    ...
    async def _create_context(self, browser: PlaywrightBrowser):
        ...
        # logger.info(f'Creating context for browser: {browser.contexts}')
    ...
    async def take_screenshot(self, marked: bool = True, save_path: str = None) -> str:
        """
    ...
    async def create_new_tab(self, url: str | None = None) -> None:
        ...
        logger.info(f'Page Targets-------------: {page_targets}')
        
        if page_targets:
            self.state.target_id = page_targets[0]['targetId']
        elif targets:
            self.state.target_id = targets[0]['targetId']
        for target in targets:
    ...
    async def _get_current_page(self, session: BrowserSession) -> Page:
        ...
        # active_page = None
        # for target in targets:
        #     if target['type'] == 'page' and target['attached']:
        #         logger.info(f'target----: {target}')
        #         if target.get('isPage', False) and target.get('isInitialPage', False):
        #                 for page in session.context.pages:
        #                     logger.info(f'targetPage: {page.url} {target["url"]}')
        #                     if page.url == target['url']:
        #                         active_page = page
        #                         break
        # logger.info(f'Active page: {active_page}')
