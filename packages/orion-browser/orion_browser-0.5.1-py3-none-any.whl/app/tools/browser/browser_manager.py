...
class BrowserManager:
    ...
    async def execute_action(self, cmd: BrowserActionRequest) -> BrowserActionResult:
        ...
        try:
            ...
            # markdown = markdownify.markdownify(await page.content())
            ...
            clean_screenshot_save_path = screenshot_save_path.replace('.webp', '_clean.webp')
            ...
            try:
                clean_screenshot = await self.browser_context.take_screenshot(
                    marked=False,
                    save_path=clean_screenshot_save_path,
                )
            except ScreenshotError as e:
            ...
            await self.upload_screenshots(cmd, clean_screenshot, clean_screenshot)
            ...
            return BrowserActionResult(
                ...
                marked_screenshot_path=screenshot_save_path,
                screenshot_uploaded=bool(cmd.screenshot_presigned_url),
                clean_screenshot_uploaded=bool(cmd.clean_screenshot_presigned_url),
                clean_screenshot_path=clean_screenshot_save_path.replace(DEFAULT_WORKING_DIR, ''),
                screenshot_path=clean_screenshot_save_path,
