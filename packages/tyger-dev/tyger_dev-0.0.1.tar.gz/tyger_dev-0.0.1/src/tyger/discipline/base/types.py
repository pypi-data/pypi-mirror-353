...
from tyger.discipline.types import TypeSystem, TypeException
from tyger.errors.errors import TypeMismatchError
    ...
    def check_slice_type(self, value_type, slice_type):
        match value_type.__qualname__:
            case list.__qualname__:
                if slice_type.__qualname__ != tslice.__qualname__ and not self.consistent(slice_type, int):
            ...
            case dict.__qualname__:
                ...
                if not self.consistent(expected_key, slice_type):
                    raise TypeError("Expected key of type {expected_key}, got {slice_type}")
    ...
    def check_annotation_type(self, ret_ann_type):
        match ret_ann_type.__qualname__:
            case type.__qualname__ | types.GenericAlias.__qualname__:
                return
            case _:
                raise TypeException("Invalid annotation")

    def get_builtin_types(self):
        return {
            "int": type, "str": type, "float": type, "dict": types.GenericAlias, "list": types.GenericAlias,
            "tuple": types.GenericAlias, "bool": type
        }
