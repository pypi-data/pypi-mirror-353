...
def setup_cli_parser() -> argparse.ArgumentParser:
    ...
    input_group.add_argument(
        "--stdin",
        action="store_true",
        help="Read code from standard input instead of files",
    )
    ...
    cli_parser.add_argument(
        "--json",
        action="store_true",
        help="Export errors in JSON format to stdout",
    )
...
def process_single_file(
    ...
    """
    Process a single file and return any errors.
    """
    ...
    target_module, deps_asts, errors = driver.run_2(file_ast)

    if errors and json_output:
        error_data = []
        for error in errors:
            error_info = {"type": error.__class__.__name__, "message": str(error)}

            # Try to extract additional information if available
            if hasattr(error, "lineno"):
                error_info["lineno"] = error.lineno
            if hasattr(error, "end_lineno"):
                error_info["end_lineno"] = error.end_lineno
            if hasattr(error, "col_offset"):
                error_info["col_offset"] = error.col_offset
            if hasattr(error, "end_col_offset"):
                error_info["end_col_offset"] = error.end_col_offset

            if hasattr(error, "filename"):
                error_info["filename"] = error.filename
            else:
                # Add the filename if not already present
                error_info["filename"] = full_file_path
        ...
        # TODO: Include elaboration errors in the output
        return output
...
def process_from_stdin(
    discipline: str, phases: List, elaborate: bool, json_output: bool = False
) -> Optional[Dict[str, Any]]:
    """
    Process code from standard input and return any errors.
    """
    code = sys.stdin.read()
    file_parser = Parser()
    file_ast = file_parser.parse_string(code)

    # Set filename to a special value to indicate stdin
    file_ast.filename = "<stdin>"
    ...
    try:
        ...
        if cli_args.stdin:
            phases = _setup_phases(
                discipline, type_system, cli_args.elaborate, "<stdin>"
            )
            result = process_from_stdin(
                discipline, phases, cli_args.elaborate, cli_args.json
            )
...
def _setup_phases(
    ...
    if dependency_phase is None:
        ...
        phases = [DependencyCollectionPhase(file_dir)]
    else:
        phases = [dependency_phase]
