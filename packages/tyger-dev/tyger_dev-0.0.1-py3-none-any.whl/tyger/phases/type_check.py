...
from tyger.discipline.types import Context, TypeException, TypeSystem
...
def collect_assigned_vars(nodes: list[ast.stmt]) -> list[str]:
    vars = []
    for node in nodes:
        match node:
            case ast.Assign(targets, value):
                for t in targets:
                    match t:
                        ...
                        # raise Exception(f"Unsupported target: {t}")
...
class TypingPhase(Phase):
    ...
    def type_check_expr(
        ...
        match node:
            case ast.Name(idx):
                ...
                if not t:
                    if hasattr(builtins, idx):
                        t = self.type_system.get_default_type()  # unk
                    else:
                        raise TypeException(f"Unknown variable: {idx}")
            ...
            case ast.Lambda(args, body):
                ...
                if context and context.expected:
                    ...
                    if len(args.args) < len(dom):
                        raise TypeException(
                            f"Type mismatch, invalid argument size: {len([unk for _ in args.args])} != {len(dom)}"
                        )
                else:
            ...
            case ast.BinOp(left, op, right):
                ...
                except NotFoundTypeError as nfe:
                    self.errors.append(
                        NotFoundTypeError(
    ...
    def type_check_stmt(
        ...
        match node:
            ...
            case ast.AnnAssign(target, annotation, value, simple):
                match target:
                    case ast.Name(idx):
                        if idx in linear:
                            self.errors.append(
                                RedefineVariableError(node, idx, self.current_file_path)
                            )

                        ann_ty = self.type_check_expr(annotation, linear, context)
                        self.type_system.check_annotation_type(ann_ty)
            ...
            case ast.ClassDef(name, bases, keywords, body, decorator_list):
                if len(bases) > 0:
                    raise TypeException(
                        f"Unsupported operation: {node} for nont-empty bases"
                    )
                else:
                    ...
                    if name.name in module_env:
                        ...
                        try:
                            ...
                            raise AttributeError(
                                f"Module {module_name} does not have attribute {name.name}"
                            )
                ...
                for name in name:
                    ...
                    if module_name.startswith(
                        ...
                        if imported_discipline == running_discipline:
                            ...
                            raise TypeException(
                                f"Type discipline mismatch: imported {imported_discipline} when running {running_discipline}"
                            )
        ...
        try:
            if module_name not in self.namespaces:
                if module_name.startswith("tyger"):
                    ...
                    module: ast.Module = self.deps[module_name]
                    # Try to get the file path for this module from deps
                    if hasattr(module, "filename") and module.filename:
                        self.file_paths[module_name] = module.filename
                        self.current_file_path = module.filename

                    module_env: Environment = self.type_check_stmts(
    ...
    def type_check_stmts(
        ...
        for func in func_defs:
            ...
            assigned = collect_assigned_vars(func.body)
