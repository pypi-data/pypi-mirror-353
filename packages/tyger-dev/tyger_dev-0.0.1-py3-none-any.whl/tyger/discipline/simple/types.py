...
from tyger.errors.errors import NotFoundTypeError

primitives: dict[type, list[type]] = {
    type(ast.Add()): [Callable[[int, int], int], Callable[[float, float], float], Callable[[str, str], str]],
    ...
    type(ast.Lt()): [Callable[[int, int], bool], Callable[[float, float], bool], Callable[[str, str], bool]],
    type(ast.Eq()): [Callable[[int, int], bool], Callable[[float, float], bool], Callable[[str, str], bool]],
    type(ast.Gt()): [Callable[[int, int], bool], Callable[[float, float], bool], Callable[[str, str], bool]],
    type(ast.LtE()): [Callable[[int, int], bool], Callable[[float, float], bool], Callable[[str, str], bool]],
    type(ast.GtE()): [Callable[[int, int], bool], Callable[[float, float], bool], Callable[[str, str], bool]],
...
class SimpleTypeSystem(BaseTypeSystem):
    ...
    def type_of(self, v: Any) -> type:
        ...
        :param v: the value
        :return: the type of the value. Here we rely on the 'type' builtin primitive function
        """
        return type(v)
    ...
    def type_of_bin_primitive(self, op, lty, rty) -> Callable:
        ...
        if str_op == type(ast.Add()) and lty.__qualname__ == 'list' and lty.__qualname__ == rty.__qualname__:
        ...
        raise NotFoundTypeError(op, lty, rty, str_op)
        ...
        match (a.__qualname__, b.__qualname__):
            ...
            case (Callable.__qualname__, Callable.__qualname__):
                ...
                if len(list(zip(dom2, dom1))) > 0:
                    dom = reduce(lambda x, y: x and y, map(lambda p: self.consistent(p[0], p[1]), zip(dom2, dom1))) if (
                            len(dom1) > 0 and len(dom2) > 0) else True
                else:
                    dom = True
                cod = self.consistent(cod1, cod2)
                length = (len(dom1) == len(dom2) or
                          (len(dom1) < len(dom2) and dom1[-1].__qualname__ == unk.__qualname__) or
                          (len(dom2) < len(dom1) and dom2[-1].__qualname__ == unk.__qualname__))
                return length and dom and cod
            case (tuple.__qualname__, tuple.__qualname__) | (list.__qualname__, list.__qualname__):
                left = get_args(a)
                right = get_args(b)
            ...
            case (dict.__qualname__, dict.__qualname__):
                ...
                return self.consistent(left_key, right_key) and self.consistent(left_val, right_val)
            case (aqn, bqn):
                return aqn == bqn
        return False
