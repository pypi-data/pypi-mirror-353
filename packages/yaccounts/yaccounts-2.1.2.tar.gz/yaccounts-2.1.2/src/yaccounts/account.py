...
from .run_report import run_activity_report, run_grant_report
from .yutils import TermColors, print_color, retry, workday_str_amount_to_float, error
...
class Account:
    def __init__(self, account_code, year):
        ...
        if not (
            self.account_code.startswith("GR") or self.account_code.startswith("AC")
        ):
            error(
                f"Account code {self.account_code} does not start with 'GR' or 'AC'. "
    ...
    def get_workday_data(self, dr):
        ...
        if actuals_cached_path.exists() and CACHE_WORKDAY_DATA:
            ...
            # self.actuals = self.actuals_df.set_index("Category").to_dict(orient="index")
        else:
    ...
    def _get_actuals_data(self, dr):
        ...
        for month in month_year_list:
            ...
            for category in data:
                ...
                # If month is January, also update budget and previous actuals
                if month.month == 1:
