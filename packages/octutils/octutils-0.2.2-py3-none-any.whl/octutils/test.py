...
def upsert_dataset_lineage(results,DatasetID):
    ...
    if not(errors.isEmpty()):
        errors = errors.dropDuplicates()
    spark_model.write.format("delta").mode("overwrite").option("replaceWhere", f"DatasetID = '{DatasetID}'").saveAsTable("oct_model_v1")
    spark_relationships.write.format("delta").mode("overwrite").option("replaceWhere", f"DatasetID = '{DatasetID}'").saveAsTable("oct_relationships")
    spark_roles.write.format("delta").mode("overwrite").option("replaceWhere", f"DatasetID = '{DatasetID}'").saveAsTable("oct_roles_v1")
    spark_tables.write.format("delta").mode("overwrite").option("replaceWhere", f"Dataset_ID = '{DatasetID}'").saveAsTable("oct_tables")
    spark_columns.write.format("delta").mode("overwrite").option("replaceWhere", f"Dataset_ID = '{DatasetID}'").saveAsTable("oct_column")
    spark_measures.write.format("delta").mode("overwrite").option("replaceWhere", f"Dataset_ID = '{DatasetID}'").saveAsTable("oct_measures")
    spark_expressions.write.format("delta").mode("overwrite").option("replaceWhere", f"Dataset_ID = '{DatasetID}'").saveAsTable("oct_expression")
    errors.write.format("delta").mode("overwrite").option("replaceWhere", f"Item_ID = '{DatasetID}'").saveAsTable("oct_errors")
    return "Operation Completed"
