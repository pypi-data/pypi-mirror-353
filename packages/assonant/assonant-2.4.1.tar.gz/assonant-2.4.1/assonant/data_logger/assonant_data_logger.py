...
class AssonantDataLogger:
    ...
    def __init__(
        ...
        if data_schema_file_path is None:
            # No data schema file path passed? - Build it based on beamline name
            self.data_schema_file_path = self._auto_build_data_schema_file_path(beamline_name=beamline_name)
        else:
    ...
    def _auto_build_data_schema_file_path(self, beamline_name: BeamlineName) -> str:
        """Build path within storage to schema file automatically based on beamline_name.

        Args:
            beamline_name (BeamlineName): Enum containing the respective Beamline Name.

        Returns:
            str: Path within storage where schema file is stored.
        """
        path_builder = BeamlinePathBuilder()
        path_builder.set_beamline_name(beamline_name.value.lower())
        return path_builder.build_path_to_data_schema()
    ...
    def _mount_log_file_name(self, experiment_stage: ExperimentStage, acquisition_moment: AcquisitionMoment):
        """Mount log file name based on passed parameters.

        Args:
            experiment_stage (ExperimentStage): ExperimentStage Enum reflecting current experimental stage.
            acquisition_moment (AcquisitionMoment): AcquisitionMoment Enum reflecting current experimental stage
            acquisition moment.
        """
        return "_".join([experiment_stage.value.lower(), acquisition_moment.value.lower()])
    ...
    def collect_and_log_pv_data(self, experiment_stage: ExperimentStage, acquisition_moment: AcquisitionMoment):
        ...
        if acquisition_moment in self._supported_acquisition_moments_for_auto_collection:
            ...
            self.log_collected_pv_data(
                experiment_stage=experiment_stage, acquisition_moment=acquisition_moment, pv_data=acquired_pv_data
            )
        else:
            raise AssonantDataLoggerError(
    ...
    def log_collected_pv_data(
        self, experiment_stage: ExperimentStage, acquisition_moment: AcquisitionMoment, pv_data: Dict[str, Any]
    ):
        ...
        entry = AssonantEntryFactory.create_entry(self.beamline_name)
        entry = self._wrap_components_into_entry(hierarchized_components, entry)

        log_file_name = self._mount_log_file_name(experiment_stage, acquisition_moment)
        self.file_writer.write_data(self.log_file_path, log_file_name, entry)
    ...
    def log_bluesky_data_from_json_file(
        self, experiment_stage: ExperimentStage, acquisition_moment: AcquisitionMoment, json_file_path: str
    ):
        ...
        # print(pv_data)
        self.log_collected_pv_data(experiment_stage, acquisition_moment, pv_data)
