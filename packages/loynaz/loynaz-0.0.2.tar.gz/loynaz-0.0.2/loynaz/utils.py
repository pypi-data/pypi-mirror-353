...
def string_result_format(result, count_result=False):
  """
  Returns a string indicating the edge dominating set.
  
  Args:
    result: None if the graph is empty, the edge dominating set otherwise.
    count_result: Count the number of edges in the edge dominating set (default is False).

  Returns:
    - "Empty Graph" if result is None, "Edge Dominating Set Found a, b, c, ...." otherwise.
  """
  if result:
    if count_result:
        return f"Edge Dominating Set Size {len(result)}"
    else:
        formatted_string = f'{", ".join(f"({x + 1}, {y + 1})" for x, y in result)}'
        return f"Edge Dominating Set Found {formatted_string}"
  else:
     return "Empty Graph"
...
def is_edge_dominating_set(G, D):
    """Check if a set of edges D is an edge dominating set of G."""
    # Convert D to a set for faster lookups (if it isn't already)
    D_edges = set(D)
    
    # Check every edge in G
    for u, v in G.edges():
        # If the edge is in D, skip
        if (u, v) in D_edges or (v, u) in D_edges:
            continue
        
        # Check if any adjacent edge is in D
        u_has_adjacent = any((u, x) in D_edges or (x, u) in D_edges for x in G.neighbors(u))
        v_has_adjacent = any((v, x) in D_edges or (x, v) in D_edges for x in G.neighbors(v))
        
        # If neither endpoint is part of an edge in D, D is not dominating
        if not (u_has_adjacent or v_has_adjacent):
            return False
