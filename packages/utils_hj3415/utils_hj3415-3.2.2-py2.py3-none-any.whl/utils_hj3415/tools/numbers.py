...
def parse_won(text: str) -> int:
    """
    한국 원화 단위가 포함된 문자열을 정수(원 단위)로 변환합니다.

    지원하는 단위:
        - "원": 그대로 반환 (예: "100원" → 100)
        - "만원": 10,000 곱함 (예: "2,000만원" → 20,000,000)
        - "억원": 100,000,000 곱함 (예: "3.5억원" → 350,000,000)
        - "조원": 1,000,000,000,000 곱함 (예: "1.25조원" → 1,250,000,000,000)

    쉼표(,)는 자동으로 제거되며, 소수점도 지원됩니다.

    예외 처리:
        - 지원하지 않는 단위가 포함된 경우 `ValueError` 예외를 발생시킵니다.

    Args:
        text (str): "3,498,506억원" 또는 "1.2조원"과 같은 문자열

    Returns:
        int: 원(₩) 단위의 정수

    Raises:
        ValueError: 인식할 수 없는 단위가 포함된 경우
    """
    units = {
        "원": 1,
        "만원": 10_000,
    ...
    for unit, factor in units.items():
        if unit in text:
            number = text.replace(",", "").replace(unit, "").strip()
            return int(float(number) * factor)

    raise ValueError(f"알 수 없는 단위 포함: {text}")
