...
def global_setup():
  print(f"\nAPI Version: {ida_domain.VersionInfo.api_version_full}")
  print(f"SDK Version: {ida_domain.VersionInfo.sdk_version}")
...
def test_function(test_env):
  ...
  func = all_functions.get_first()
  assert func is not None
  assert db.functions.get_name(func) == "add_numbers"

  func = all_functions.get_next()
  assert func is not None
...
def test_strings(test_env):
  ...
  assert string == (True, "Hello, IDA!\n")

  string = db.strings.get_at(0x11d)
  assert string == (True, "Sum: Product: \n")
...
def test_names(test_env):
  ...
  assert name == (True, "_start")

  name = db.names.get_at(0x150)
  assert name == (True, "newline_len")

  assert db.names.get_at(db.entry_point) == (True, "_start")
...
def test_api_examples(test_env):
  ...
  script_path = Path(__file__).parent.parent / "examples" / "traverse.py"
  ...
  assert result.returncode == 0, "Example traverse.py failed to run"
