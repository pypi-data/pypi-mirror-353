...
class SDLC_DATABRICKS():
    ...
    def generate_response(self, prompt):
        ...
        try:
            ...
            print(f"LLM API response status code: {str(response.status_code)}")
            # Check and print response
            if response.status_code == 200:
                self.llm_response = json.loads(response.text)['candidates'][0]['content']['parts'][0]['text']
                print("Successfully received response from LLM.")
                return self.llm_response
            else:
                print(f"LLM API call failed with status {str(response.status_code)}. Retrying...")
                time.sleep(10)
                self.generate_response(prompt)
        except Exception as e:
            print(f"Exception during LLM API call: {str(e)}")

    def pdf_extractor(self):
        print("##---STEP-1: Extracting PDF content.---##")
        try:
            # Step 1: Read all PDF files as binary
            df_pdf = self.spark.read.format("binaryFile").load(self.FILE_PATH)

            # Step 2: Define a Pandas UDF to extract text
            ...
            # Step 4: Collect text into a single Python variable
            all_text_list = df_with_text.select("text").rdd.flatMap(lambda x: x).collect()
            self.all_pdf_text = "\n".join(all_text_list)
            print("Successfully extracted PDF content.")
            return self.all_pdf_text
        except Exception as e:
            print(f"Error while extracting content from PDF: {str(e)}")
    ...
    def functional_document(self):
        ...
        try:
            ...
            print("Successfully generated functional document.")
            return self.functional_output
        except Exception as e:
            print(f"Error while generating functional document: {str(e)}")
    ...
    def technical_document(self):
        ...
        try:
            ...
            print("Successfully generated technical document.")
            return self.technical_output
        except Exception as e:
            print(f"Error while generating technical document: {str(e)}")
    ...
    def generate_pyspark_code(self):
        ...
        try:
            ...
            print(f"Error while generating pyspark code: {str(e)}")

    def parse_llm_output(self):
        print("Parsing LLM output, generating code directory.")
        try:
    ...
    def clean_files_dict(self):
        ...
        try:
            ...
            for path, content in self.llm_parser.items():
                ...
                print("Successfully cleaned code directory.")
            return self.cleaned
        except Exception as e:
            print(f"Error during code directory cleaning: {str(e)}")
    ...
    def push_to_github(self, commit_message="Initial commit by library"):
        ...
        try:
            ...
            print(f"Code directory pushed to repository: {GITHUB_REPO},{GITHUB_BRANCH} successfully.")
            return True, "Pushed successfully."

        except requests.exceptions.HTTPError as e:
            print(f"Exception raised during git-push: {str(e)}")
