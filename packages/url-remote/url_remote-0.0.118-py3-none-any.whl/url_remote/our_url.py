...
class OurUrl:
    ...
    def endpoint_url(
        ...
        """
        environment_name, port = OurUrl.get_actual_environment_and_port(component_name, environment_name, default_port)
        ...
        if path_parameters:
            path_params_string = "/".join(
                [urllib.parse.quote_plus(str(val)) for val in path_parameters.values()]
            )
