...
def agregat(df):
    ...
    # cari kolom numerik yang diapit kolom object ---
    col_types = df.dtypes
    object_cols = [i for i, dt in enumerate(col_types) if dt == 'object']
    ...
    value_columns = [col for col in candidate_cols if pd.api.types.is_numeric_dtype(df[col])]
    ...
    if not value_columns:
        ...
        try:
            ...
            value_columns = [df.columns[i] for i in indices if 0 <= i < len(df.columns)]
