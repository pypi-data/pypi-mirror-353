...
@click.option(
    ...
    default=None,
    help=(
        "Top-level key in TOML to isolate (e.g., extract variables "
...
def cli(
    ...
    # Define umber of variables (for dependency chains) + a safety margin.
    ...
    effective_env.update(resolved_vars)
    ...
    if verbose:
        if resolved_vars:
            output_vars = "\n".join(
                f"  {k}={v}" for k, v in resolved_vars.items()
            )
            click.secho(
            ...
            click.secho(
                ...
                "config (or selected namespace is empty).",
                fg="yellow",
                err=True,
            )
        click.secho(
