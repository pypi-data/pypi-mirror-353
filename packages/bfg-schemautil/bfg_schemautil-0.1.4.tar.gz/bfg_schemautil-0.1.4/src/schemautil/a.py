...
def make_array(item, nullable):
    return {
        "type": nullable_type("array", nullable),
        "items": make_type(item, False),
    }
...
def make_type(value, nullable):
    match value:
        case []:
            raise ValueError("Empty list is not a valid type")
        case list(arr) if all(
            isinstance(item, str) and item.startswith("@") for item in arr
        ):
            return make_enum([item.removeprefix("@") for item in arr], nullable)
        case [item]:
            return make_array(item, nullable)
        ...
        case str(t) if t.startswith("@"):
            return make_enum([t.removeprefix("@")], nullable)
