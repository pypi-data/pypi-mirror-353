...
#        self.initConsoleWidget()
    ...
    def initHistogramsWidget(self):
        ...
        self.widgets['cascade'] = pg.GraphicsLayoutWidget()
        self.widgets['longitudinal'] = self.widgets['cascade'].addPlot()
        self.widgets['cross'] = self.widgets['cascade'].addPlot()
        self.widgets['cascade'].nextRow()
        self.widgets['departure_angle'] = self.widgets['cascade'].addPlot()
        self.widgets['angle_distribution'] = self.widgets['cascade'].addPlot()
        self.docks['distributions'].addWidget(self.widgets['cascade'])
    ...
    def add_state_info(self,key):
        if key == 'ProductionHeader':
            ...
            self.state_photons['Photons'] = {'n_total': self.photons.n_tracks, 'n_steps': self.photons.n_steps}
    ...
    def update_data(self):
        self.eventHeader = self.h5reader.gEvent.evtHeader
        self.photons     = self.h5reader.gEvent.photons
        self.hits        = self.h5reader.gEvent.hits
        self.particles   = self.h5reader.gEvent.particles
        self.tracks      = self.h5reader.gEvent.tracks
        self.add_state_info('EventHeader')
        self.add_state_info('Photons')
...
if __name__ == '__main__':
    ...
    p.add_argument('--min_energy_for_tracks', type=float, default=0)
