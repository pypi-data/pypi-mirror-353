...
This file is used not only internally by dbxconv, but also by bladerunner, in order to pass proper parameters to dbxconv when invoked from Remorph.

Remorph itself gets the list of supported source techs (with a user-friendly name) from a config stored in lsp/config.yml. In Remorph lingo, they are named dialects.
...
In order to fix discrepancies, you should:
 ...
 - adjust the friendly/internal name in bladerunner/src/databricks/labs/remorph/bladerunner/helpers.py
