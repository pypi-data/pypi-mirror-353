...
def _local_graph(T=None, T_family=None, T_kwargs=None):
    if T_family is None:
        if T is None:
            raise ValueError(
                "Either the target graph T, or its lattice specication T_family and T_kwargs, must be provided"
            )
        return T.copy()
    else:
        if T_kwargs is None:
            T_kwargs = {}
        if T is not None:
            T_kwargs.update({"edge_list": T.edges, "node_list": T.nodes})
        if T_family == "chimera":
            return dnx.chimera_graph(**T_kwargs)
        elif T_family == "pegasus":
            return dnx.pegasus_graph(**T_kwargs)
        elif T_family == "zephyr":
            return dnx.zephyr_graph(**T_kwargs)
        else:
            raise ValueError("Unsupported dwave_networkx graph")
...
def find_sublattice_embeddings(
    ...
    *,
    ...
    Args:
        S: The source graph to embed.
        T: The target graph in which to embed. Either the graph should be of type
            chimera, pegasus or zephyr and generated by dwave_networkx, or the
            graph must be cast to one of these types using `T_family` and `T_kwargs`.
            If `T` is None, then `T` is generated as a defect free graph.
        ...
        sublattice_size: Parameterizes the tile when it is not provided
           ...
           provides a lower bound based on a fast feasibility filter. Note that
           the tile is created with default graph parameters (integer labeled variables).
        max_num_emb: Maximum number of embeddings to find.
        ...
        use_tile_embedding: A single embedding for a tile is generated, and then
            ...
            By default the setting is False unless ``tile==S`` or a tile_embedding
            is provided.
        tile_embedding: When provided, this should be an embedding from the source
            to the tile. If not provided it is generated by the embedder method, or
            fixed as a 1:1 mapping (in the special case that S==tile). Note
        ...
        T_family: 'chimera', 'pegasus' or 'zephyr' arguments result in the target graph
            being cast to the corresponding dwave_networkx graph family.
        T_kwargs: The arguments passed to the graph generator. It is necessary to specify the
            shape parameters m (rows) and perhaps n (columns) or t (tile).
            If T is not None, `T_kwargs` is updated with the nodelist and edgelist of T.
    Raises:
        ValueError: If the target graph ``T`` is not of type zephyr, pegasus, or chimera and
            T_family, T_kwargs are not specified.
    ...
    if use_filter and tile is None:
        feasibility_bound = lattice_size_lower_bound(
            S=S, T=_T, one_to_one=not one_to_iterable
        )
    ...
    if use_tile_embedding is None:
        use_tile_embedding = tile == S or tile_embedding is not None  # Trivial 1:1

    if use_tile_embedding and tile_embedding is None and S == tile:
        if one_to_iterable:
            tile_embedding = {
                i: (i,) for i in tile.nodes
            }  # 1 to 1 mapping is sufficient
        else:
            tile_embedding = {i: i for i in tile.nodes}  # 1 to 1 mapping is sufficient
        use_filter = False  # Unnecessary
    ...
    if max_num_emb is None:
        max_num_emb = _T.number_of_nodes() // S.number_of_nodes()
    else:
        max_num_emb = min(_T.number_of_nodes() // S.number_of_nodes(), max_num_emb)
