...
def create_status_button(
    ...
    """
    Create a button that changes appearance based on item status.

    Args:
        item_key: Key in item data to check for status
        status_map: Mapping of status values to button properties
        default_status: Default status if key not found

    Returns:
        ButtonWidget with dynamic styling

    Example:
        create_status_button(
            item_key="status",
            status_map={
                "active": {"css_class": "btn btn-success", "label": "Active"},
                "inactive": {"css_class": "btn btn-secondary", "label": "Inactive"},
                "pending": {"css_class": "btn btn-warning", "label": "Pending"}
            }
        )
    """

    def get_status_property(item, property_name):
        if not item or item_key not in item:
            status = default_status
        else:
            status = item[item_key]

        return status_map.get(status, status_map.get(default_status, {})).get(property_name, "")

    return ButtonWidget(
        url="#",
        label=lambda item: get_status_property(item, "label"),
        css_class=lambda item: get_status_property(item, "css_class"),
        disabled=True
    )
