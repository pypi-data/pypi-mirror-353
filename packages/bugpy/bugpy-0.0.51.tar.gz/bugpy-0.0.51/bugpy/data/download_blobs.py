...
def collect_one_from_bucket(file_location, bucket_name):
    """Streams a file from an S3 bucket."""
    config = Config(
        retries=dict(max_attempts=5, mode='standard'),
        read_timeout=60,
        connect_timeout=10,
        max_pool_connections=os.cpu_count() * 4
    )
    s3 = boto3.client("s3", config=config)
    response = s3.get_object(Bucket=bucket_name, Key=file_location)
...
def download_filelist(filelist, output_dir, aws_bucket, flatten_filestructure=False, redownload=False,
