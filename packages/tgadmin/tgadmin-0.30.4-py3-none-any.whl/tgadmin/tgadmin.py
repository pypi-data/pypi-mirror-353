...
def delete_object(client, textgrid_uris, recursive: bool = False, force: bool = False):
    ...
    if not force:
        click.echo(f'found {len(all_uris)} objects:')
        ...
        for tguri in all_uris[:limit]:
            ...
            if len(res) > 0:
                ...
                click.secho(
                    f' - {tguri}: not found - may be a dead reference in aggregation', fg='magenta'
                )

        if len(all_uris) > limit:
            click.echo(f' ...and ({len(all_uris) - limit}) more objects')
...
def put_aggregation(
    ...
    TextgridURI of first file is {res['tguri']}.
    """)
...
Use this for update? hit "y" for update or "n" to do a fresh upload""",
    ...
    if fresh and not project_id:
        click.secho('please provide a project ID for new uploads')
        return
