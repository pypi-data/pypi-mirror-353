...
class TGimport:
    ...
    def upload(self, filenames: list[str]) -> dict:
        ...
        return {
            'objects_uploaded': len(self._imex.as_map()),
            'imex_location': self._imex.get_location().as_posix(),
            'tguri': self._imex.get(PurePath(filenames[0])),
        }
    ...
    def _rewrite_with_linkrewriter_cli(self, path: PurePath, metadata: TextgridObject) -> str:
        ...
        imex_loc = self._imex.get_location().as_posix()
    ...
    def _rewrite_with_python(self, path: PurePath, metadata: TextgridObject) -> str:
        ...
        Returns:
            str: content of file after rewrite
        """
        path_prefix = self._imex.get_path_prefix()
