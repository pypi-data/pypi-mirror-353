...
class BaseRepository:
    ...
    async def get_all(self, find: ListFilter, collection_name: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if find.pre_filters:
            if hasattr(find.pre_filters, "model_dump"):
                ...
                pre_filters = {k: v for k, v in find.pre_filters.items() if v is not None}
            pipeline.append({"$match": self._build_filter_condition(pre_filters, collection_name,is_expr=False)})
    ...
    def _build_reference_stage(
        ...
        """Build reference stages for MongoDB aggregation pipeline with truly dynamic depth"""
        ...
        def process_reference(ref, current_pipeline):
            ...
            if sort_fields:
                for sort_item in sort_fields:
                    direction = ASCENDING if sort_item.direction == SortDirection.ASC else DESCENDING
                    ref_sort_dict[sort_item.field] = direction

            path_parts = local_path.split(".")
            is_nested = len(path_parts) > 1
            local_key = path_parts[-1]
            embed_field = alias.split(".")[-1]
            
            # Check if alias has dots (nested alias)
            has_nested_alias = "." in alias

            if is_nested:
                # Step 1: Extract all local keys from nested structures
                id_var = f"_ids_{embed_field}"
                
                # Build dynamic ID extraction based on path depth
                current_pipeline.append({
                    "$set": {
                        id_var: self._build_dynamic_id_extraction(path_parts[:-1], local_key)
                    }
                })

                # Step 2: Lookup based on collected IDs
                lookup_pipeline = [
                    {
                        "$match": {
                            "$expr": {
                ...
                # Add sorting to the lookup pipeline if needed
                if ref_sort_dict:
                ...
                current_pipeline.append({
                    "$lookup": {
                        ...
                        "let": {"local_ids": f"${id_var}"},
                        "pipeline": lookup_pipeline,
                        "as": joined_var
                    }
                })

                # Step 3: Embed matching items back into nested structures
                if has_nested_alias:
                    # For nested alias, embed back into nested structure
                    alias_parts = alias.split(".")
                    root_field = alias_parts[0]
                    embed_expr = self._build_dynamic_embedding(alias_parts, joined_var, foreign_field, local_key, is_single)
                    current_pipeline.append({
                        "$set": {
                            root_field: embed_expr
                        }
                    })
                else:
                    # For single alias, collect all matched references from nested structure and flatten
                    # Build expression to collect all matches from the nested structure
                    if len(path_parts) > 1:
                        # Handle nested local_field with single alias
                        collect_expr = self._build_nested_reference_collection(path_parts[:-1], local_key, joined_var, foreign_field)
                    else:
                        # Handle simple local_field with single alias
                        collect_expr = f"${joined_var}"
                    
                    # Apply single object logic if needed
                    if is_single:
                        collect_expr = {
                            "$cond": {
                                "if": {"$gt": [{"$size": collect_expr}, 0]},
                                "then": {"$arrayElemAt": [collect_expr, 0]},
                                "else": None
                            }
                        }
                ...
                current_pipeline.append({"$unset": [id_var, joined_var]})

            else:
                # Flat join (original logic)
                lookup_pipeline = [
                    {
                        "$match": {
                            "$expr": {
                ...
                if is_single:
                    current_pipeline.append({
                        "$set": {
                            alias: {
                                "$cond": {
                                    "if": {"$gt": [{"$size": f"${alias}"}, 0]},
                                    "then": {"$arrayElemAt": [f"${alias}", 0]},
                                    "else": None
                                }
                            }
                        }
    ...
    def _build_dynamic_id_extraction(self, path_parts, local_key):
        """Build dynamic ID extraction for any depth"""
        if len(path_parts) == 1:
            return {
                "$map": {
                    "input": f"${path_parts[0]}",
                    "as": "item",
                    "in": f"$$item.{local_key}"
                }
            }
            ...
            return {
                "$reduce": {
                    ...
                    "in": {
                        "$concatArrays": [
                            "$$value",
                            self._extract_nested_ids("$$this", path_parts[1:], local_key)
                        ]
                    }
                }
            }
    ...
    def _extract_nested_ids(self, parent_expr, remaining_path, local_key):
        ...
        if len(remaining_path) == 1:
            return {
                "$map": {
                    "input": f"{parent_expr}.{remaining_path[0]}",
                    "as": "nested_item",
            ...
            return {
                "$reduce": {
                    ...
                    "in": {
                        "$concatArrays": [
                            "$$value",
                            self._extract_nested_ids("$$this", remaining_path[1:], local_key)
                        ]
                    }
                }
            }

    def _build_dynamic_embedding(self, alias_parts, joined_var, foreign_field, local_key, is_single):
        """Build dynamic embedding for any depth"""
        if len(alias_parts) == 1:
            # This shouldn't happen for nested references, but handle it
            return f"${alias_parts[0]}"
        
        # Start with the root array
        return self._build_nested_embedding(f"${alias_parts[0]}", alias_parts[1:], joined_var, foreign_field, local_key, is_single)

    def _build_nested_embedding(self, current_expr, remaining_alias_parts, joined_var, foreign_field, local_key, is_single):
        """Recursively build nested embedding expressions"""
        if len(remaining_alias_parts) == 1:
            # Base case: embed the final field
            embed_field = remaining_alias_parts[0]
            
            # Build the filter expression for matching references
            filter_expr = {
                "$filter": {
                    "input": f"${joined_var}",
                    "as": "j",
            ...
            # If single object is requested, convert array to single object
            if is_single:
                final_expr = {
                    "$cond": {
                        "if": {"$gt": [{"$size": filter_expr}, 0]},
                        "then": {"$arrayElemAt": [filter_expr, 0]},
                        "else": None
                    }
                }
            else:
                final_expr = filter_expr
            ...
            next_field = remaining_alias_parts[0]
            return {
                "$map": {
                    "input": current_expr,
                    "as": "parent",
                    "in": {
                        "$mergeObjects": [
                            "$$parent",
                            {
                                next_field: self._build_nested_embedding(
                                    f"$$parent.{next_field}", 
                                    remaining_alias_parts[1:], 
                                    joined_var, 
                                    foreign_field, 
                                    local_key,
                                    is_single
    ...
    def _build_nested_reference_collection(self, path_parts, local_key, joined_var, foreign_field):
        """Build expression to collect references from nested paths for single aliases"""
        # First, collect all the IDs from the nested structure
        def build_id_collection(current_path, remaining_path):
            if len(remaining_path) == 0:
                # Base case: collect all IDs at this level
                return {
                    "$map": {
                        "input": current_path,
                        "as": "item",
                ...
                # Recursive case: go deeper and flatten
                next_field = remaining_path[0]
                return {
                    "$reduce": {
                        "input": current_path,
                        "initialValue": [],
                        "in": {
                            "$concatArrays": [
                                "$$value",
                                build_id_collection(f"$$this.{next_field}", remaining_path[1:])
                            ]
                        }
                    }
                }
        
        # Collect all IDs from the nested structure
        all_ids = build_id_collection(f"${path_parts[0]}", path_parts[1:])
        
        # Filter the joined data to only include items with matching IDs
        return {
            "$filter": {
                "input": f"${joined_var}",
                "as": "j",
                "cond": {
                    "$in": [f"$$j.{foreign_field}", all_ids]
                }
            }
        }