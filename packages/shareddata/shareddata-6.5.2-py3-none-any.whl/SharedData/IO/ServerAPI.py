...
def delete_collection(database, period, source, tablename, request):
    ...
    if filter_query:        
        filter_parsed = json.loads(filter_query)        
        collection = shdata.collection(database, period, source, tablename, user=user, create_if_not_exists=False)
        result = collection.collection.delete_many(filter_parsed)
        ...
        success = shdata.delete_collection(database, period, source, tablename, user=user, create_if_not_exists=False)
        if success:
            return Response(status=204)
        else:
            return Response(status=404)
