...
class ClientAPI:
    ...
    def raise_for_status(response):
        ...
        including the status code, error reason, and the URL of the request. If JSON is missing, prints and returns
        all available info from the response.
        ...
        # If we cannot get a JSON error message, try to gather as much info as possible
        if not error_msg or error_msg.strip() == "":
            ...
            error_msg = "\n".join(error_details)
        if 400 <= status < 500:
            raise Exception(f"{status} Client Error: {error_msg} for url: {url}")
        elif 500 <= status < 600:
            raise Exception(f"{status} Server Error: {error_msg} for url: {url}")
        else:
            return
    @staticmethod
