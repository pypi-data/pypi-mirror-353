...
@main.command()
@click.option('--output-dir', '-o', default='.', help='Output directory for Docker files')
...
def package(output_dir, base_image, agent_name):
    ...
    try:
        packager.create_dockerfile()
        packager.create_entrypoint()
        click.echo(f"Docker files created in {output_dir}")
        click.echo(f"Build with: docker build -t {agent_name} {output_dir}")
