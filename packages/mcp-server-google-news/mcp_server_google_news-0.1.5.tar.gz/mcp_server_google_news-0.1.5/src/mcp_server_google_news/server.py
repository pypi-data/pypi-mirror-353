...
@mcp.tool()
async def google_news_search(query: str, limit: int = 10, hl: str = "ja", gl: Optional[str] = None) -> List[Dict[str, Any]]:
    """
    ...
    # Validate hl parameter
    if not isinstance(hl, str) or not hl:
        raise McpError(ErrorData(
            code=INVALID_PARAMS,
            message="Language code (hl) must be a non-empty string"
        ))
...
@mcp.tool()
async def google_news_topics(topic_id: str = "TOP", limit: int = 10, hl: str = "ja", gl: Optional[str] = None) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if topic_id == "TOP":
            ...
            url = f"{GOOGLE_NEWS_TOPICS_URL}/{topic_id}?{urlencode(params)}"
