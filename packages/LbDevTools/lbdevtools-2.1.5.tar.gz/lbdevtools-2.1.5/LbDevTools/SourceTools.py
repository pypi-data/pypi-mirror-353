...
def report(filenames, inverted=False, target=None, license="GPL-3.0-only"):
    ...
    print("\n- ".join(filenames))
    if not inverted:
        license_arg = f" --license={license}" if license != "GPL-3.0-only" else ""
        if target:
            print(
                f"\nYou can fix the {len(filenames)} files without copyright statement "
                f"with:\n\n  $ lb-check-copyright --porcelain {target}{license_arg} "
                f"| xargs -r lb-add-copyright{license_arg}\n"
            )
        else:
            print(
...
def to_comment(text, lang_family="#", width=80):
    ...
    * c                                                                           *
    ...
    if lang_family == "c":
        ...
        msg = f"invalid language family: {lang_family}"
...
def get_yapf_format_cmd(version=YAPF_VERSION):
    ...
    if found_version != version:
        msg = f"wrong yapf version {found_version} ({version} required)"
        raise CommandNotFound(msg)
    return cmd
...
def check_copyright():
    ...
    parser = ArgumentParser(
        description="""
    Check that each git tracked source file contains a copyright statement.
    """
    )
    parser.add_argument(
        "reference",
    ...
    for path in args.files:
        if not args.force and not to_check(path):
            if not args.pre_commit:
                print(
                    f"warning: cannot add copyright to {path} (file type not supported)"
                )
        elif has_copyright(path):
            if not args.pre_commit:
                print(f"warning: {path} already has a copyright statement")
...
_USE_PRE_COMMIT_MSG = """error: repository {} has a pre-commit configuration, use pre-commit instead of lb-format

you can let it run automatically at every commit or invoke it by hand
for example:
    $ pre-commit run --all-files
    $ pre-commit run --files file1 file2...

see https://pre-commit.com/#pre-commit-run"""


def _check_pre_commit(path):
    git_top = git_root(path)
    if git_top and os.path.exists(os.path.join(git_top, ".pre-commit-config.yaml")):
        print(_USE_PRE_COMMIT_MSG.format(git_top))
        exit(1)
...
def clang_format():
    ...
    try:
        ...
        exit(f"{os.path.basename(sys.argv[0])}: internal command {err}")
