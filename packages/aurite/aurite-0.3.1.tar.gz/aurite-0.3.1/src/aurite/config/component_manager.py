...
class ComponentManager:
    ...
    def _load_packaged_defaults(self):
        ...
        try:
            # Find the spec for the top-level 'aurite' package to locate the project root.
            spec = importlib.util.find_spec("aurite")
            if not spec or not spec.origin:
                logger.error(
                    "Could not find the 'aurite' module spec. Packaged defaults will not be loaded."
                )
                return

            # spec.origin is .../src/aurite/__init__.py. We need the project root.
            # .parent -> .../src/aurite
            # .parent.parent -> .../src
            # .parent.parent.parent -> project root
            project_root = Path(spec.origin).parent.parent.parent

            # The directory containing the packaged component JSON files
            packaged_configs_dir = project_root / "src" / "aurite" / "packaged" / "component_configs"
            ...
            for component_type, (model_class, id_field) in COMPONENT_META.items():
                ...
                # are relative to the project root. So, we use the calculated project_root
                # as the base for resolving them.
                self._load_components_from_dir(
                    component_type,
                    model_class,
                    id_field,
                    component_dir_path,
                    base_path_for_resolution=project_root,
