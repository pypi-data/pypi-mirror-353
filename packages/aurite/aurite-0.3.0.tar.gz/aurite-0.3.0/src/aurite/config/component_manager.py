...
class ComponentManager:
    ...
    def _load_packaged_defaults(self):
        ...
        try:
            # Find the file system path to the 'aurite.packaged' directory using its spec.
            # This provides a concrete Path object instead of a Traversable, which is crucial
            # for subprocess calls that need a real file system path.
            spec = importlib.util.find_spec("aurite.packaged")
            if not spec or not spec.origin:
                logger.error(
                    "Could not find the 'aurite.packaged' module spec. Packaged defaults will not be loaded."
                )
                return

            # The 'origin' for a package is its __init__.py file, so .parent gives the directory.
            packaged_root = Path(spec.origin).parent
            ...
            for component_type, (model_class, id_field) in COMPONENT_META.items():
                ...
                self._load_components_from_dir(
                    ...
                    id_field,
                    component_dir_path,
                    base_path_for_resolution=packaged_root,  # This is now a concrete Path
