...
class ComponentManager:
    ...
    def _load_packaged_defaults(self):
        ...
        try:
            ...
            # For path resolution, we need a concrete file system path to the project root.
            # We can get this from the spec of the top-level 'aurite' package.
            spec = importlib.util.find_spec("aurite")
            if not spec or not spec.origin:
                raise RuntimeError("Could not find 'aurite' package spec.")

            # spec.origin is .../src/aurite/__init__.py, so three parents up is the project root.
            project_root_path = Path(spec.origin).parent.parent.parent
            ...
            for component_type, (model_class, id_field) in COMPONENT_META.items():
                ...
                # The base path for resolving file paths within the JSONs is the project root.
                self._load_components_from_dir(
                    component_type,
                    model_class,
                    id_field,
                    component_dir_path, # This is a Traversable
                    base_path_for_resolution=project_root_path, # This is a concrete Path
                    is_override_allowed=False,
                )
        except (ModuleNotFoundError, RuntimeError) as e:
            logger.error(f"Could not load packaged defaults: {e}. This may happen if the package is not installed correctly.")
        except Exception as e:
            logger.error(f"An unexpected error occurred while loading packaged defaults: {e}", exc_info=True)
