...
import buggpt
import buggpt.debug
from buggpt.client import AsyncClient, ChatCompletion, ImagesResponse, ClientResponse, convert_to_provider
from buggpt.providers.response import BaseConversation, JsonConversation
from buggpt.client.helper import filter_none
from buggpt.image import is_data_an_media, EXTENSIONS_MAP
from buggpt.image.copy_images import get_media_dir, copy_media, get_source_url
from buggpt.errors import ProviderNotFoundError, ModelNotFoundError, MissingAuthError, NoValidHarFileError
from buggpt.cookies import read_cookie_files, get_cookies_dir
from buggpt.providers.types import ProviderType
from buggpt.providers.response import AudioResponse
from buggpt.providers.any_provider import AnyProvider
from buggpt import Provider
from buggpt.gui import get_gui_app
from buggpt.tools.files import supports_filename, get_async_streaming
from .stubs import (
...
class AppConfig:
    ...
    buggpt_api_key: Optional[str] = None
...
class Api:
    def __init__(self, app: FastAPI) -> None:
        ...
        self.get_buggpt_api_key = APIKeyHeader(name="buggpt-api-key")
    ...
    async def get_username(self, request: Request) -> str:
        ...
        is_correct_password = secrets.compare_digest(
            current_password_bytes, AppConfig.buggpt_api_key.encode()
        )
    ...
    def register_authorization(self):
        if AppConfig.buggpt_api_key:
            print(f"Register authentication key: {''.join(['*' for _ in range(len(AppConfig.buggpt_api_key))])}")
        @self.app.middleware("http")
        async def authorization(request: Request, call_next):
            if AppConfig.buggpt_api_key is not None or AppConfig.demo:
                try:
                    user_buggpt_api_key = await self.get_buggpt_api_key(request)
                except HTTPException:
                    user_buggpt_api_key = await self.security(request)
                    if hasattr(user_buggpt_api_key, "credentials"):
                        user_buggpt_api_key = user_buggpt_api_key.credentials
                path = request.url.path
                if path.startswith("/v1") or path.startswith("/api/") or (AppConfig.demo and path == '/backend-api/v2/upload_cookies'):
                    if user_buggpt_api_key is None:
                        return ErrorResponse.from_message("BugGPT API key required", HTTP_401_UNAUTHORIZED)
                    if AppConfig.buggpt_api_key is None or not secrets.compare_digest(AppConfig.buggpt_api_key, user_buggpt_api_key):
                        return ErrorResponse.from_message("Invalid BugGPT API key", HTTP_403_FORBIDDEN)
                elif not AppConfig.demo and not path.startswith("/images/") and not path.startswith("/media/"):
                    if user_buggpt_api_key is not None:
                        if not secrets.compare_digest(AppConfig.buggpt_api_key, user_buggpt_api_key):
                            return ErrorResponse.from_message("Invalid BugGPT API key", HTTP_403_FORBIDDEN)
    ...
    def register_routes(self):
        ...
        async def read_root_v1():
            return HTMLResponse('BugGPT API: Go to '
        ...
        async def models():
            return {
                ...
                "data": [{
                    ...
                    "created": 0,
                    "owned_by": "",
                    "image": isinstance(model, buggpt.models.ImageModel),
                    "provider": False,
        ...
        async def model_info(model_name: str) -> ModelResponseModel:
            if model_name in buggpt.models.ModelUtils.convert:
                model_info = buggpt.models.ModelUtils.convert[model_name]
                return JSONResponse({
                    'id': model_name,
                    'object': 'model',
                    'created': 0,
        ...
        async def get_media(filename, request: Request):
            ...
            if not os.path.isfile(target) and mime_type is not None:
                ...
                if source_url is None:
                    backend_url = os.environ.get("BUGGPT_BACKEND_URL")
                    if backend_url:
...
def format_exception(e: Union[Exception, str], config: Union[ChatCompletionsConfig, ImageGenerationConfig] = None, image: bool = False) -> str:
    last_provider = {} if not image else buggpt.get_last_provider(True)
...
def run_api(
    ...
    **kwargs
    ...
    print(f'Starting server... [buggpt v-{buggpt.version.utils.current_version}]' + (" (debug)" if debug else ""))
