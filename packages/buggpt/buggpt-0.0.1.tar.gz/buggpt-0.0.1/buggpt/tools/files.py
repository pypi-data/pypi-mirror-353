...
def supports_filename(filename: str):
    if filename.endswith(".pdf"):
        if has_pypdf2:
            ...
            return True
        raise MissingRequirementsError(f'Install "pypdf2" requirements | pip install -U buggpt[files]')
    elif filename.endswith(".docx"):
        if has_docx:
            return True
        elif has_docx2txt:
            return True
        raise MissingRequirementsError(f'Install "docx" requirements | pip install -U buggpt[files]')
        ...
        if not has_beautifulsoup4:
            raise MissingRequirementsError(f'Install "beautifulsoup4" requirements | pip install -U buggpt[files]')
        return True
...
def spacy_refine_chunks(source_iterator):
    if not has_spacy:
        raise MissingRequirementsError(f'Install "spacy" requirements | pip install -U buggpt[files]')
