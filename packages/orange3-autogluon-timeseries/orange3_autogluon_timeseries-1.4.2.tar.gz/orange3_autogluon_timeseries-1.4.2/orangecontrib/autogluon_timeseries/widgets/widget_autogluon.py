...
class AutoGluonWorker(QThread):
    ...
    def run(self):
        ...
        try:
            self.log("=== НАЧАЛО АСИНХРОННОГО ОБУЧЕНИЯ ===")
                ...
                if HOLIDAYS_AVAILABLE:
                    try:
                        ...
                        if len(unique_dates) > 0:
                            ...
                            self.log(f"Добавлены праздники: {df_sorted['is_holiday'].sum()} дней")
                    except Exception as e:
                        self.log(f"Ошибка добавления праздников: {str(e)}")
                else:
                    self.log("Праздники отключены: библиотека holidays не установлена")
                    self.log("Установите: pip install holidays")
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # Обучение модели
                self.log("Запуск обучения AutoGluon...")
                ...
                try:
                    ...
                    if "observations" in error_msg:
                        self.log("Обработка ошибки с количеством наблюдений...")
                        ...
                        if min_ts_len < 10:
                            ...
                            if len(long_enough_ids) > 0:
                                ...
                                self.log(f"Отфильтровано до {len(long_enough_ids)} рядов")
                                predictor.fit(ts_data, **fit_args)
                            else:
                    ...
                    try:
                        ...
                        if future_df_list:
                            ...
                            if HOLIDAYS_AVAILABLE:
                                ...
                                self.log("Подготовлены будущие праздники для прогноза")
                            else:
                                self.log("Будущие праздники пропущены: библиотека holidays недоступна")
                                known_covariates_for_prediction = None
                    except Exception as e:
                        self.log(f"Ошибка подготовки будущих праздников: {str(e)}")
                ...
                try:
                    ...
                    if hasattr(predictions, 'index') and hasattr(predictions.index, 'nlevels') and predictions.index.nlevels == 2:
                        ...
                        self.log(f"Числовые ID в прогнозе: {forecast_numeric_ids.tolist()}")
                        ...
                        for numeric_id in forecast_numeric_ids:
                            ...
                            self.log(f"--- Обработка ID: {numeric_id_str} ---")
                        ...
                        if all_forecast_data:
                            ...
                            self.log(f"Итоговый прогноз: {len(pred_df)} записей")
                            self.log(f"=== ПОСЛЕ CONCAT ===")
                            self.log(f"Колонки pred_df: {list(pred_df.columns)}")
                ...
                try:
                    if leaderboard is not None and not leaderboard.empty:
                        ...
                        if 'WeightedEnsemble' in best_model_name:
                            ...
                            self.log(f"Состав ансамбля: {len(ensemble_data)} компонентов")
...
class OWAutoGluonTimeSeries(OWWidget):
    ...
    def update_progress(self, progress, message):
        ...
        self.progress_widget.setValue(progress)         # ← исправлено
        self.progress_label.setText(f"{progress}% - {message}")  # ← исправлено
        QCoreApplication.processEvents()
