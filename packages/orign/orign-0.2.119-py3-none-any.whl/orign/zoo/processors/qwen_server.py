...
def init():
    ...
    plumbed_model.set_adapter(None)  # Deactivate by default
...
def smart_adapter_loading_for_inference(
    ...
    # If the requested adapter's weights are already in the persistent adapter, do nothing.
    if state.currently_loaded_adapter == adapter_name:
        print(
            f"[METRIC] [Hotswap] Adapter '{adapter_name}' is already loaded. No action needed."
        )
        return False
    ...
    adapter_config_path = os.path.join(persistent_adapter_path, "adapter_config.json")

    # A simple check for existence. More robust would be to check metadata/timestamps.
    if not os.path.exists(adapter_config_path):
        print(
            f"[METRIC] [Hotswap] Weights for '{adapter_name}' not found locally at '{persistent_adapter_path}'. Downloading..."
        )
        ...
        print(f"[METRIC] [Hotswap] Found cached weights for '{adapter_name}'.")
        # Update access time for the cached item
        _update_item_access(adapter_name, "adapters", persistent_adapter_path)
    # --- End of on-disk weight management ---

    print(
        f"[METRIC] [Hotswap] Hotswapping weights from '{persistent_adapter_path}' into persistent adapter '{state.persistent_adapter_name}'"
    )
    ...
    try:
        ...
        hotswap_adapter(model, persistent_adapter_path, state.persistent_adapter_name)
...
def infer_qwen_vl(
    ...
    if load_adapter:
        ...
        if state.base_model.active_adapter is not None:
            ...
            state.base_model.set_adapter(None)
        else:
            print(
    ...
    try:
        ...
        pass
    print(
        f"[METRIC] Active adapter(s) after FastVisionModel.for_inference(): {active_after_for_inference}"
    )
...
def QwenVLServer(
    ...
    image: str = "public.ecr.aws/d8i6n0n1/orign/unsloth-server:c2caa58",  # "us-docker.pkg.dev/agentsea-dev/orign/unsloth-infer:latest"
