...
class Construct:
    ...
    def __ask_for_server(self):
        ...
        if "code" in rsp and rsp["code"] == "0" and "server" in rsp:
            ...
            self.logger.info("获取数据库服务器地址成功，服务器地址:" + self.__sql_ser_addr)
        else:
            msg = "分配数据库服务器地址失败,服务器响应:" + str(rsp)
    ...
    def query(self, query, page=1, sort_type=1, sort_key=""):
        ...
        if not self.__check_server():
            return rsp
        url = "%s/sql?&mode=sql&&token=%s&query=%s&page=%s&sort_type=%s&sort_key=%s" % (
    ...
    def kline(self, code, cate="stock", fq="前复权", type="day", limit=240):
        ...
        if not self.__check_server():
            return rsp
        url = "%s/sql?&mode=kline&token=%s&code=%s&cate=%s&type=%s&fq=%s&limit=%s" % (
    ...
    def industry(self):
        ...
        if not self.__check_server():
            return rsp
        url = "%s/sql?&mode=industry&token=%s" % (
            self.__sql_ser_addr, self.__token)
        try:
            res = requests.get(url=url)
            rsp = res.json()
    ...
    def minute(self, code: str, end_day: str, limit: int):
        ...
        if not self.__check_server():
            return rsp
        url = "%s/sql?&mode=minute&token=%s&code=%s&end_day=%s&limit=%s" % (
    ...
    def order_book(self, code: str, offset=0):
        ...
        if not self.__check_server():
            return rsp
        url = "%s/sql?&mode=order_book&token=%s&code=%s&offset=%s" % (
    ...
    def level_queue(self, code):
        ...
        if not self.__check_server():
            return rsp
        url = "%s/sql?&mode=level_queue&token=%s&code=%s" % (
    ...
    def download_history(self, year: str, save_path=''):
        ...
        if not self.__check_server():
            return
        url = "%s/sql?&mode=history&token=%s&year=%s.zip&stamp=%s" % (
