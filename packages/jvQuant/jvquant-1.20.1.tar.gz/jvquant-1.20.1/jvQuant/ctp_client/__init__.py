...
class Construct:
    ...
    def __init__(self, token: str, ctp_acc: str, ctp_pwd: str, ticket_path="", auto_relogin=True,
        ...
        try:
            with open(ticket_path, 'r', encoding='utf-8') as file:
                ...
                if ctp_acc == old_acc:
                    ...
                    if expire_remain > 0:
                        ticket_valid = True
                        self.logger.warning("使用指定ticket，剩余有效期:{}秒。".format(expire_remain))
        ...
        self.th_handle = threading.Thread(
    ...
    def __ask_for_server(self):
        ...
        if "code" in rsp and rsp["code"] == "0" and "server" in rsp:
            ...
            self.logger.info("获取CTP服务器地址成功，服务器地址。" + self.__ctp_ser_addr)
        else:
            msg = "分配CTP服务器地址失败,服务器响应:" + str(rsp)
