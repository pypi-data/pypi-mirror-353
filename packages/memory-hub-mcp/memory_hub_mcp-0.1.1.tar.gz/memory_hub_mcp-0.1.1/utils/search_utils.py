...
from services import qdrant_client, get_embedding, get_chat_completions_endpoint
...
def validate_version_logic(chunks: List) -> Dict[str, any]:
    ...
    from handlers.memory_handlers import safe_int_conversion
...
async def generate_chunk_keywords(chunk_text: str, client: httpx.AsyncClient) -> List[str]:
    ...
    try:
        ...
        if len(chunk_text) > MAX_CHUNK_LENGTH_FOR_KEYWORDS:
            ...
            print(f"WARN: Truncated chunk text for keyword extraction ({MAX_CHUNK_LENGTH_FOR_KEYWORDS} chars)")
