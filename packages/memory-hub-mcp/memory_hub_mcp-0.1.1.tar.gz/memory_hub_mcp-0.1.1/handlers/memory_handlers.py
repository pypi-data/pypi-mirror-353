...
from services import qdrant_client, get_embedding, synthesize_search_results
...
async def add_memory(memory_item: MemoryItemIn, client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    try:
        ...
        if not memory_item.metadata.get("app_id"):
            raise HTTPException(status_code=400, detail="app_id is required in metadata")
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Content chunking failed: {str(e)}")
        ...
        if not chunks:
            ...
            if not memory_item.content.strip():
                raise HTTPException(status_code=400, detail="Content is empty or only whitespace.")
        ...
        for i, chunk_text in enumerate(chunks):
            if not chunk_text: # Skip empty chunks
                continue
            try:
                embedding = await get_embedding(chunk_text, client)
                
                # Generate chunk-specific keywords using Gemma
                chunk_keywords = await generate_chunk_keywords(chunk_text, client)
        ...
        if not points_to_upsert:
            ...
            raise HTTPException(status_code=500, detail="No data could be prepared for storage after chunking/embedding.")
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Storage in Qdrant failed: {str(e)}")
...
async def search_memories(search_request: MemorySearchRequest, client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    try:
        ...
        try:
            expanded_query = await expand_query_with_keywords(search_request.query_text, search_request.metadata_filters, client)
        ...
        try:
            query_embedding = await get_embedding(expanded_query, client)
        except Exception as e:
        ...
        if ENABLE_GEMMA_SUMMARIZATION:
            try:
                # Limit chunks for summarization to prevent context overflow
                summarization_chunks = retrieved_chunks_for_response[:min(MAX_SUMMARIZATION_CHUNKS, len(retrieved_chunks_for_response))]
                synthesized_summary = await synthesize_search_results(search_request.query_text, summarization_chunks, client)
                print(f"INFO: Gemma summarization enabled and completed")
            except Exception as e:
