...
class BlogDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["related_videos"] = (
            category.news.exclude(id=context["object"].id)[:4] if category else []
        )
        context["object_comments_count"] = (
            context["object"].comments.filter(is_accepted=True).count()
        )
        ...
        if self.object.tags.first():
            context["seo_tags"] = ",".join(
                [tag.title for tag in self.object.tags.all()]
            )
        return context
...
class BlogView(ListView):
    ...
    queryset = (
        BlogPost.objects.select_related("writer")
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        if context["category_pk"]:
            ...
            context["url_page"] = (
                ...
                + str(cate.id)
                + "/"
                + slugify(cate.title, True)
                + "/page/"
            )
        elif context["tag_pk"]:
            tag = get_object_or_404(BlogTag, pk=context["tag_pk"])
            context["url_page"] = (
                "/blog/tag/" + str(tag.id) + "/" + slugify(tag.title, True) + "/page/"
            )
        else:
