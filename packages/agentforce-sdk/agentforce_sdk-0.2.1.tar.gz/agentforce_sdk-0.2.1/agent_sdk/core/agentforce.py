...
<ApexClass xmlns="http://soap.sforce.com/2006/04/metadata">
    ...
    def get_agent_by_name(self, agent_name: str) -> Dict:
        ...
        """
        self._check_auth()
        sanitized_agent_name = MetadataGenerator._sanitize_name(agent_name)
        agent_records: List = self.sf.query(f"SELECT Id, DeveloperName, MasterLabel, Description FROM GenAiPlannerDefinition WHERE MasterLabel = \'{sanitized_agent_name}\'").get("records", [])
    ...
    def retrieve_metadata(self, agent_name: str | None = None) -> Dict:
        ...
        try:
            ...
            if agent_name:
                sanitized_agent_name = MetadataGenerator._sanitize_name(agent_name)
                bot_query += f" WHERE MasterLabel = '{sanitized_agent_name}' OR DeveloperName = '{sanitized_agent_name}'"
