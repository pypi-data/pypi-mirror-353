...
class Config:
    ...
    def get_auth_token(cls) -> str:
        """Get the API authentication token from environment variable.

        Returns:
            str: API authentication token

        Raises:
            ValueError: If AGENTFORCE_API_AUTH_TOKEN environment variable is not set
        """
        token = os.environ.get("AGENTFORCE_API_AUTH_TOKEN")
        if not token:
            raise ValueError(
                "AGENTFORCE_API_AUTH_TOKEN environment variable must be set"
            )
        return token
