...
class JiraClient(UtilityBase):
    ...
    def __init__(
        ...
        :param api_key: Jira API key for authentication.
        :param jira_url: Base URL of the Jira instance.
        :param retry_cnt: Number of retries in case of a failed request.
        :param verbose: If true debug messages are logged during the operations. Exceptions, Errors and Warnings are always logged.
        :param log_urls: If true all the requested urls are logged
    ...
    async def _fetch_json(self, url: str) -> Optional[Dict[str, Any]]:
        """
        Fetches JSON data from Jira. Requires session to be started first.

        :param url: The Jira URL that returns a JSON object as a response.
        :return: The response as a python dictionary or None in case of a failed retrieval.
        """
    ...
    async def _read_issue_worklog(self, issue_id: str) -> Optional[Dict[str, Any]]:
        """
        Fetches the worklog details for a Jira issue.
        
        :param issue_id: The Jira issue ID.
        :return: Worklog JSON data as a dictionary, or None if an error occurs.
        """
    ...
    async def _read_issue_comments(self, issue_id: str) -> Optional[Dict[str, Any]]:
        """
        Fetches the comments for a Jira issue.
        
        :param issue_id: The Jira issue ID.
        :return: Comments JSON data as a dictionary, or None if an error occurs.
        """
    ...
    async def start_session(self):
        """Explicitly initializes the aiohttp session. Must be called once before making requests."""
        if self.session is None:
    ...
    async def read_custom_jql_keys(self, custom_jql: str) -> list[str]:
        """
        Retrieves all issue keys for a custom JQL query

        :param custom_jql: The JQL query as a string.
        :return: The result of the JQL query.
        """
        issue_keys = []
        start_at = 0
    ...
    async def read_issue(
        ...
        """
        Fetches the details of a Jira issue, with optional inclusion of changelog, comments, and worklog data.
        ...
        :param issue_id: The unique identifier of the Jira issue to fetch.
        :param read_changelog: If True, fetches the issue changelog and includes it in the result. Defaults to False.
        :param read_comments: If True, fetches the issue comments and includes them in the result. Defaults to False.
        :param read_worklog: If True, fetches the issue worklog and includes it in the result. Defaults to False.
        :param changelog_filter: An optional list of field names to filter the changelog. Only changes to these fields
                                will be included in the result. Filtering is case-insensitive. Ignored if 
                                `read_changelog` is False.
        :return: A dictionary representing the full issue data with optionally merged changelog, comments, 
                and worklog. Returns None if the issue cannot be fetched or if an error occurs.
        """
    ...
    async def read_board_id(self, board_name: str) -> int | None:
        """
        Asynchronously retrieves the ID of a board given its name.
        ...
        :param board_name: The name of the board to look up.
        :return: The ID of the board if found, otherwise None.
        """
    ...
    async def read_sprint_list(self, 
        ...
        """
        Asynchronously retrieves a list of active and closed sprints for a given Jira board.
        ...
        :param origin_board: If True, only sprints where `originBoardId` matches the board_id are returned.
        :param name_filter: If provided, only sprints whose names include this substring will be returned.
        :return: A list of sprint dictionaries with data as returned by the Jira API. Returns None on failure.
        """
        sprint_list = []
        start_at = 0
