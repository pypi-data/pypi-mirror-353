...
class ContentScanner(UtilityBase):
    """
    A Class for searcing in the content of files.
    """
    ...
    def __init__(
        ...
        """
        Initializes the content scanner with search patterns and scanning settings.

        :param string_patterns: List of simple string patterns to match.
        :param regex_patterns: List of regular expression patterns to match.
        :param case_sensitive: Flag indicating if the match should be case-sensitive.
        :param max_results: Optional maximum number of results to return per file.
        :param verbose: If True, logs matches during the content search.
    ...
    def _init_text_extensions(self) -> Set[str]:
        """
        Initializes the set of recognized plain text file extensions.

        :return: A set of file extensions treated as plain text.
        """
        return {
            # Code
    ...
    async def scan_files(self, file_paths: List[Union[str, Path]]) -> AsyncGenerator:
        """
        Asynchronously scans multiple files for matching content.

        :param file_paths: List of file paths to scan.
        :yield: Dictionaries with file, line, and line number of matches.
        """
    ...
    def _scan_file(self, file_path: Path) -> List[Dict[str, Union[str, int]]]:
        """
        Determines the file type and scans it appropriately.

        :param file_path: Path object representing the file.
        :return: List of dictionaries with match results.
        """
        try:
    ...
    def _match_line(self, line: str) -> bool:
        """
        Checks if a line matches any of the defined patterns.

        :param line: A line of text.
        :return: True if the line matches, else False.
        """
    ...
    def _scan_plain_text(self, file_path: Path) -> List[Dict[str, Union[str, int]]]:
        """
        Scans plain text files line-by-line.

        :param file_path: Path to the file.
        :return: List of dictionaries containing matches.
        """
        matches = []
        try:
    ...
    def _scan_csv(self, file_path: Path) -> List[Dict[str, Union[str, int]]]:
        """
        Scans CSV/TSV files for matching cell values.
    ...
    def _scan_excel(self, file_path: Path) -> List[Dict[str, Union[str, int]]]:
        """
        Scans Excel files for matching cell content.
    ...
    def _scan_docx(self, file_path: Path) -> List[Dict[str, Union[str, int]]]:
        """
        Scans .docx files for matching paragraph text.
    ...
    def _scan_pdf(self, file_path: Path) -> List[Dict[str, Union[str, str]]]:
        """
        Scans PDF files page-by-page and line-by-line.
