...
class MissionStats(ServiceAbstract):
    ...
    def _setup_connection(self):
        ...
        try:
            ...
            self.channel.queue_declare(queue="last_data_mission_id", durable=True)
    ...
    def get_mission_stats_from_rabbitmq(self, mission_id: Union[int,str]) -> List[Dict[str, Any]]:
        """
    ...
    def start_mission(
        ...
        try:
            ...
            # TODO Later
            self._publish_mission_to_hq(mission_detail)
    ...
    def on_mission_data_arrive(
        ...
        if isinstance(mission_id, str):
            if mission_id!=data_buffer_key and not mission_id.isdigit():
                raise Exception("Invalid mission_id provided, it should be an integer or '_all_'")
        ...
        new_mission_stats = self.get_mission_stats_from_rabbitmq(mission_id)
    ...
    def on_mission_data_publish( # NOT in use
        self,
        mission_id: Union[int,str],
        size: int,
        data_type: str,
    ...
    def _publish_stats_to_hq(self, mission_id: Union[int, str]):  # PUBLISHER FUNCTION 1
        try:
            ...
            mission_data_list = self.get_mission_stats_from_rabbitmq(mission_id)
            ...
            if mission_read_error is not None:
                return False
            if current_mission and str(current_mission.get("id")) == mission_id:
                mission_detail = current_mission
                # self.delete_current_mission(mission_id) # TODO
            else:
                mission_detail = None

            message_priority = 1 if mission_detail is None else 3
            ...
            self.rabbit_mq.publish(fileinfo_message, message_priority)
    ...
    def _publish_mission_to_hq(  # PUBLISHER FUNCTION 2
