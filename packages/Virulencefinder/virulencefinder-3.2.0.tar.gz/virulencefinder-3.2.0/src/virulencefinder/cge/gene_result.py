...
class GeneResult(dict):
    def __init__(self, res_collection, res, ref_db_name, ref_db_sub, conf, alignments= None):
        self["type"] = "seq_region"

        complete_ref_db_name = f"{res_collection.get_db_key(ref_db_name)[0]}:{ref_db_sub}"
        self["ref_database"] = [complete_ref_db_name]
 ...
 """
        if conf.extended_output is True:
