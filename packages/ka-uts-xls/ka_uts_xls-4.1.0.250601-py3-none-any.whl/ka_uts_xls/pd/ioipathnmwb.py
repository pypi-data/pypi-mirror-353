...
class IoiPathnmWb:
    ...
    def read_wb_to_aod(
        ...
        aod: TnAoD = PdIoiPathWb.read_wb_to_aod(path, sheet, **kwargs_wb)
        return aod
    ...
    def read_wb_to_doaod(
        ...
        path = PathNm.sh_path(pathnm, kwargs)
        doaod: TnDoAoD = PdIoiPathWb.read_wb_to_doaod(path, **kwargs_wb)
        return doaod
    ...
    def read_wb_to_aod_or_doaod(
        ...
        path = PathNm.sh_path(pathnm, kwargs)
        obj: TnAoD_DoAoD = PdIoiPathWb.read_wb_to_aod_or_doaod(path, sheet, **kwargs_wb)
        return obj
    ...
    def read_wb_to_df(
        ...
        path = PathNm.sh_path(pathnm, kwargs)
        return PdIoiPathWb.read_wb_to_df(path, sheet, **kwargs_wb)

    @staticmethod
    def read_wb_to_dodf(
            pathnm: TyPathnm, sheet: TnSheet, kwargs: TyDic, **kwargs_wb) -> TnDoPdDf:
        path = PathNm.sh_path(pathnm, kwargs)
        return PdIoiPathWb.read_wb_to_dodf(path, sheet, **kwargs_wb)
    ...
    def read_wb_to_df_or_dodf(
        ...
        path = PathNm.sh_path(pathnm, kwargs)
        obj: TnPdDf_DoPdDf = PdIoiPathWb.read_wb_to_df_or_dodf(path, sheet, **kwargs_wb)
        return obj
