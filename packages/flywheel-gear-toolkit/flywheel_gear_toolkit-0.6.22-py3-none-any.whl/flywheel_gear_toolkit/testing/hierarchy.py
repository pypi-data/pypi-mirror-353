...
class MockContainerMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.analyses = []
        self.files = []
        self._update = None  # to store updates made to the container when calling `update`
...
def make_session(label, n_acqs=1, n_files=1, par=None):
    ...
    for i in range(n_acqs):
        ...
        acquisitions.append(make_acquisition(f"acq-{i}-{label}", n_files=n_files, par=parents))
...
def make_subject(label, n_ses=1, n_acqs=1, n_files=1, par=None):
    ...
    for i in range(n_ses):
        ...
        sessions.append(
            make_session(f"ses-{i}-{label}", n_acqs=n_acqs, n_files=n_files, par=parents)
        )
...
def make_project(label="test", n_subs=1, n_ses=1, n_acqs=1, n_files=1, par={"group": "test"}):
    ...
    for i in range(n_subs):
        ...
        subjects.append(
            make_subject(f"sub-{i}", n_ses=n_ses, n_acqs=n_acqs, n_files=n_files, par=parents)
        )
