...
class Manifest:
    ...
    def __init__(self, manifest=None):
        ...
        elif not manifest:  # look into current directory
            self._path = (Path(".") / "manifest.json").absolute()
            if not self._path.exists():
                raise FileNotFoundError(f"manifest.json not found in cwd ({os.getcwd()}")
    ...
    def _is_docker_images_match(self):
        ...
        if docker_image_1 and docker_image_2 and docker_image_1 != docker_image_2:
            errors += ['"custom.gear-builder.image" is different from "custom.docker-image"']
        return errors
