...
class Dataflow:
    ...
    def secret(self, secret_name: str):
        ...
        try:
            if runtime and slug:
                full_path = f"{runtime}/{slug}/{vault_path}"
                value = self.secrets_manager.get_secret_value_by_key(full_path, "", secret_name)
                if value is not None:
                    return value
                global_path = f"{runtime}/global/{vault_path}"
                value = self.secrets_manager.get_secret_value_by_key(global_path, "", secret_name)
                if value is not None:
                    return value
            return self.secrets_manager.get_secret_value_by_key(vault_path, user_name, secret_name)
