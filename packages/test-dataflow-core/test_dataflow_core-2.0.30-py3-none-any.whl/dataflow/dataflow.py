...
class Dataflow:
    ...
    def variable(self, variable_name: str):
        ...
        try:
            ...
            if runtime:
                ...
                print(f"[Dataflow.variable]: {response.status_code} - {response.text}")
    ...
    def secret(self, secret_name: str):
        ...
        try:
            if runtime and slug:
                vault_path = f"{runtime}/{slug}/{vault_folder}"
                value = self.secrets_manager.get_secret_value_by_key(vault_path, "", secret_key=secret_name)
                if value is not None:
                    print("rutime sluf condition passed")
                    return value
                vault_path = f"{runtime}/global/{vault_folder}"
                value = self.secrets_manager.get_secret_value_by_key(vault_path, "", secret_key=secret_name)
                if value is not None:
                    print("runtime globals condition passed")
                    return value
                vault_path = f"{user_name}/{vault_folder}"
                print("username condition passed")
            return self.secrets_manager.get_secret_value_by_key(vault_path, "", secret_key=secret_name)
        except Exception as e:
            print(f"[Dataflow.secret] Failed to fetch secret '{secret_name}': {e}")
            return None
