...
from .generated.models.graph_models_pb2 import Camera
...
class GstMpegtsDecoderPipeline(threading.Thread):
    ...
    @staticmethod
    def create_element(element: str, name: str, properties: Optional[Dict[str, Any]] = None) -> Gst.Element:
    ...
    def __init__(
        ...
        self._capsfilter_string = "video/x-raw(memory:NVMM), format=RGBA"
    ...
    def _create_parser_elements_with_caps(self, caps_string: str) -> Tuple[Gst.Element, Gst.Element]:
        ...
        parser = self.create_element(f"{codec}parse", f"{self.camera.id}_{codec}_parse")

        parser_filter = self.create_element(
            element="capsfilter",
    ...
    def _attach_sink_elements(self):
        ...
        if self._output_fps > 0:
            ...
            self.videorate = self.create_element(
                element="videorate",
    ...
    def _create_elements(self):
        ...
        self.appsrc = self.create_element(
            element="appsrc",
        ...
        self.tsparse = self.create_element(
            element="tsparse",
        ...
        self.demuxer = self.create_element(
            element="tsdemux",
        ...
        self.inter_queue = self.create_element(
            element="queue",
            name=f"{self.camera.id}_input_queue"
        )

        self.inter_queue_2 = self.create_element(
            element="queue",
            name=f"{self.camera.id}_inter_queue_2",
        )

        self.decoder = self.create_element(
            element="nvv4l2decoder",
            name=f"{self.camera.id}_decoder",
        )

        mem_type = int(pyds.NVBUF_MEM_CUDA_UNIFIED)
        self.videoconvert = self.create_element(
        ...
        self.capsfilter = self.create_element(
            element="capsfilter",
        ...
        self.appsink = self.create_element(
            element="appsink",
