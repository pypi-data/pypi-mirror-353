...
def remove_gaps_from_original_l_series(l1, l2, F_high, F_low, gap_mask_1hz):
    """
    Removes corresponding high-frequency regions from l1 and l2 based on a 1 Hz gap mask.

    Parameters:
        l1, l2          : original high-frequency time series (e.g., 100 Hz)
        F_high          : original high frequency (e.g., 100)
        F_low           : low sampling rate used for downsampling (e.g., 1)
        gap_mask_1hz    : boolean array of shape (T_low,) where False means "gap"

    Returns:
        l1_cleaned, l2_cleaned : high-frequency time series with "gap" regions removed
    """
    if F_high <= F_low:
        return l1, l2
    ...
    # Repeat each mask value 'factor' times to cover corresponding high-freq region
    gap_mask_high = np.repeat(gap_mask_1hz, factor)

    # Ensure the mask and signal are the same length
    min_len = min(len(l1), len(gap_mask_high))
    l1_cleaned = l1[:min_len][gap_mask_high[:min_len]]
    l2_cleaned = l2[:min_len][gap_mask_high[:min_len]]
