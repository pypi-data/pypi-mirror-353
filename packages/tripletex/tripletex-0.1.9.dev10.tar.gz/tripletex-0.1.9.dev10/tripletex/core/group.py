...
from crudclient import ResourceGroup
...
class TripletexGroup(ResourceGroup):
    """
    Tripletex-specific ResourceGroup base class.
    Includes the same special serialization handling as TripletexCrud.
    """
    ...
    def _dump_data(self, data: Any, partial: bool = False, validation_model: Optional[Type] = None) -> Any:
        """
        Override to ensure Pydantic models are serialized with by_alias=True.
        This is critical for converting snake_case to camelCase for the API.

        Parameters:
            data: The data to dump
            partial: Whether this is a partial update
            validation_model: Optional model to use for validation (passed by ResourceGroup)
        """
        ...
        if model_to_use:
            ...
            # Ensure super call passes along partial and validation_model correctly
    ...
    def _convert_to_model(self, data: Any) -> Any:
        ...
        Returns:
            An instance of the datamodel or the raw JSON dictionary.
        ...
        # Tripletex wraps single object responses in a 'value' field
        cleaned_data = validated_data.get("value", validated_data)

        return self._datamodel(**cleaned_data) if self._datamodel else cleaned_data
