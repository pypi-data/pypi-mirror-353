...
class TripletexCrud(Crud[T]):
    ...
    def _dump_data(self, data: Any, partial: bool = False, validation_model: Optional[Type[T]] = None) -> Any:
        # Check if this endpoint has special update mode that allows partial updates
        update_mode = getattr(self, "_update_mode", None)

        # If we have Tripletex-specific models and are in a special update mode, use our logic
        if update_mode == "no_resource_id" and isinstance(data, dict):
            # For no_resource_id mode, we don't validate strictly - just serialize with by_alias
            logger.debug(f"Using no_resource_id mode for data dumping (partial={partial})")
            # Directly serialize the dict with by_alias=True without validation
            # This preserves the existing behavior for endpoints like company
            result = {}
        ...
        # Prioritize the validation_model passed from the operation if it exists.
        # This aligns with breaking changes in crudclient.
        model_to_use: Optional[type] = validation_model

        if not model_to_use:
            # Fallback to the old logic if no explicit validation_model is provided.
        ...
        if model_to_use:
            ...
            return super()._dump_data(data, partial=partial, validation_model=validation_model)
