...
class CFlarumOper(CFlarumJson):
    ...
    #     # 调试通过 discussion_id 获取回帖，发现存在翻页问题而不全
    #     sParam = f'?filter[discussion]={sDiscussionId}'
    #     oJson = self._get_posts(sParam)
    #     self.debug_json_print_object(oJson, r'E:\tmp\get_posts_dict_by_discussion_id-13.json')

    # def get_all_posts_dict_by_discussion_id(self, sDiscussionId):
    #     # 调试通过 discussion_id 获取回帖，发现存在翻页问题而不全
    #     # dictPosts = {}
    #     lsReturn = []
    #     sParam = f'?filter[discussion]={sDiscussionId}'
    ...
    #             return True
    #         return False
    #     sUrl = self.api_posts(sParam)
    #     self.page_get_call(sUrl, _cbHandle)
    #     self.debug_json_print_object(lsReturn, r'E:\tmp\get_all_posts_dict_by_discussion_id.json')
