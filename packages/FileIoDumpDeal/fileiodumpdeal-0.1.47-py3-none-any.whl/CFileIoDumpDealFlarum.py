...
class CFileIoDumpDealFlarum(CFileIoDumpDealBase):
    ...
    def DumpFile4Query(self, sQueryDir):
        ...
        dictTitleByDiscussionId = self.oFlarum.page_get_all_discussions_by_tag_slug(self.oFlarum.sFlarumTagFocus)
    ...
    def NotiFile4Query(self, sQueryDir):
        ...
        for sNotificationId, dictInfo in dictNotiInfo.items():
            ...
            if sDiscussionId and sPostId:
                ...
                if listDictChat:
                    # sContent = dictInfo.get('post_content', '')
                    # dictLast = listDictChat[-1]
                    # sContentLast = dictLast.get('sContent', '')
                    # sContentLast = sContentLast.strip()
                    # if sContentLast.startswith('@'):  # 以@开头，则表示是新任务，不需要之前的交互历史
                    #     listDictChat = listDictChat[-1:]
                    sContent = json.dumps(listDictChat, indent=4)
...
def mainFileIoDumpDealFlarum():
    ...
    if len(sys.argv) >= 2:
        ...
        PrintTimeMsg('Usage: DumpDealFlarum dump/deal <sMcpTaskDir> [<sFlarumWorkDir>]')
