...
import monitizer.benchmark.datasets.FashionMNIST.fashionmnist_dataloaders as fashionmnist_data
from monitizer.benchmark.datasets.datasets import get_fashionmmnist, get_fashionmnist_set, get_dataloader


class FashionMNISTDataset(Dataset):
    def __init__(self, name: str, data_folder: str = './data', root='monitizer/benchmark/',
                 num_workers=os.cpu_count() - 1):
        super().__init__(name, get_fashionmmnist, data_folder, root, num_workers=num_workers)

        self.preprocessing_transformers = [transforms.ToTensor()]
        self.get_dataset = get_fashionmmnist
        self.test_set = get_fashionmnist_set("test",
                                      transforms.Compose(self.preprocessing_transformers),
                                      self.validation_split, self.data_folder)
        self.train_set = get_fashionmnist_set("train",
                                       transforms.Compose(self.preprocessing_transformers),
                                       self.validation_split, self.data_folder)
        self.validation_set = get_fashionmnist_set("val",
                                            transforms.Compose(self.preprocessing_transformers),
                                            self.validation_split, self.data_folder)
    ...
    def get_OOD_data_specific(self, name, usecase) -> DataLoader:
        if name == "WrongPrediction/FGSM":
            return fashionmnist_data.get_fashionmnist_fgsm_dataloader(usecase, data_folder=self.data_folder,
