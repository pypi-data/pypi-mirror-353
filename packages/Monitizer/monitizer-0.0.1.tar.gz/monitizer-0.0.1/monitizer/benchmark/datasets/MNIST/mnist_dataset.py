...
import monitizer.benchmark.datasets.MNIST.mnist_dataloaders as mnist_data
from monitizer.benchmark.datasets.datasets import get_mnist, get_mnist_set, get_dataloader
...
class MNISTDataset(Dataset):
    def __init__(self, name: str, data_folder: str = './data', root='monitizer/benchmark/',
        ...
        self.get_dataset = get_mnist
        self.test_set = get_mnist_set("test",
                                      transforms.Compose(self.preprocessing_transformers),
                                      self.validation_split, self.data_folder)
        self.train_set = get_mnist_set("train",
                                       transforms.Compose(self.preprocessing_transformers),
                                       self.validation_split, self.data_folder)
        self.validation_set = get_mnist_set("val",
                                            transforms.Compose(self.preprocessing_transformers),
                                            self.validation_split, self.data_folder)
    ...
    def get_OOD_data_specific(self, name, usecase) -> DataLoader:
        if name == "NewWorld/CIFAR10":
            return mnist_data.get_mnist_cifar10_dataloader(usecase, data_folder=self.data_folder,
            ...
            return mnist_data.get_mnist_dtd_dataloader(usecase, data_folder=self.data_folder,
                                                       root=self.specification_root, num_workers=self.num_workers)
        elif name == "UnseenEnvironment/SVHN":
            return mnist_data.get_mnist_svhn_dataloader(usecase, data_folder=self.data_folder,
                                                        root=self.specification_root, num_workers=self.num_workers)
        elif name == "UnseenObject/FashionMNIST":
            return mnist_data.get_mnist_fashionmnist_dataloader(usecase, data_folder=self.data_folder,
                                                                root=self.specification_root,
                                                                num_workers=self.num_workers)
        elif name == "UnseenObject/KMNIST":
            return mnist_data.get_mnist_kmnist_dataloader(usecase, data_folder=self.data_folder,
                                                          root=self.specification_root, num_workers=self.num_workers)
        elif name == "WrongPrediction/FGSM":
            return mnist_data.get_mnist_fgsm_dataloader(usecase, data_folder=self.data_folder,
                                                        root=self.specification_root, num_workers=self.num_workers)
        else:
    ...
    def get_all_subset_names(self) -> list:
        generated_ood = ["Noise/Gaussian", "Noise/SaltAndPepper", "Perturbation/Contrast",
                         ...
                         "Perturbation/Light"]
        specific_ood = ["NewWorld/CIFAR10", "NewWorld/DTD", "UnseenEnvironment/SVHN", "UnseenObject/FashionMNIST",
                        "UnseenObject/KMNIST", "WrongPrediction/FGSM"]
