...
import monitizer.benchmark.datasets.SVHN.svhn_dataloaders as svhn_data
from monitizer.benchmark.datasets.datasets import get_svhn, get_svhn_set, get_dataloader


class SVHNDataset(Dataset):
    def __init__(self, name: str, data_folder: str = './data', root='monitizer/benchmark/',
                 num_workers=os.cpu_count() - 1):
        super().__init__(name, get_svhn, data_folder, root, num_workers=num_workers)
        self.get_dataset = get_svhn
        self.preprocessing_transformers = [
            transforms._presets.ImageClassification(crop_size=32, resize_size=32, mean=(0, 0, 0), std=(1, 1, 1))]
        self.test_set = get_svhn_set("test", transforms.Compose(self.preprocessing_transformers), data_folder=self.data_folder)
        self.train_set = get_svhn_set("train", transforms.Compose(self.preprocessing_transformers), data_folder=self.data_folder)
        self.validation_set = get_svhn_set("val", transforms.Compose(self.preprocessing_transformers), data_folder=self.data_folder)
    ...
    def get_OOD_data_specific(self, name, usecase) -> DataLoader:
        if name == "WrongPrediction/FGSM":
            return svhn_data.get_svhn_fgsm_dataloader(usecase, data_folder=self.data_folder,
