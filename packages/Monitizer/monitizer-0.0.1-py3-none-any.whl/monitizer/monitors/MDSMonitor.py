...
#                with torch.no_grad():
    ...
    def get_mean_stds(self, train_input, activations, train_labels):
        ...
        for layer in activations:
            means = []
            #stds = []
            std = torch.zeros(size=(activations[layer].shape[-1], activations[layer].shape[-1]))
            for label in self._labels:
                relevant_features = activations[layer][train_labels == label]
                mean = torch.mean(relevant_features, dim=0)
                # Variant 1
                #std = group_lasso.fit(relevant_features.detach().numpy() - mean.detach().numpy())
                #temp_precision = group_lasso.precision_
                #std = torch.from_numpy(temp_precision).float().cuda()
                ...
                # Variant 3
                std+= (relevant_features-mean).T.mm(relevant_features-mean)
                means.append(mean)
                #stds.append(torch.Tensor(std))
            self._means[layer] = torch.stack(means)
            self._stds[layer] = torch.eye(std.shape[0]) * 1e-6
