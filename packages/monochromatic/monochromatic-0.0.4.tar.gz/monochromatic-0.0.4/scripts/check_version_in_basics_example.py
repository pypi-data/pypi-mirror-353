...
the latest version of monochromatic. This saves us from forgetting to update that
...
def main(changes: str, the_basics: str) -> None:
    ...
    version_examples = [
        ...
        if "$ monochromatic --version" in code_block.string
    ]
    ...
    for tag in tags:
        for version_example in version_examples:
            if tag in version_example and tag != latest_tag:
                print(
                    "Please set the version in the ``monochromatic --version`` "
