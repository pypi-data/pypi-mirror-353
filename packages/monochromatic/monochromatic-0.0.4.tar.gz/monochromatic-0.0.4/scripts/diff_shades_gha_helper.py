"""Helper script for psf/monochromatic's diff-shades Github Actions integration.

diff-shades is a tool for analyzing what happens when you run monochromatic on
...
https://monochromatic.readthedocs.io/en/latest/contributing/gauging_changes.html#diff-shades
"""
...
DOCS_URL: Final = (
    "https://monochromatic.readthedocs.io/en/latest/"
    "contributing/gauging_changes.html#diff-shades"
)
USER_AGENT: Final = f"psf/monochromatic diff-shades workflow via urllib3/{urllib3.__version__}"
SHA_LENGTH: Final = 10
GH_API_TOKEN: Final = os.getenv("GITHUB_TOKEN")
REPO: Final = os.getenv("GITHUB_REPOSITORY", default="psf/monochromatic")
...
def get_pypi_version() -> Version:
    data = http_get("https://pypi.org/pypi/monochromatic/json")
...
def config(event: Literal["push", "pull_request"]) -> None:
    ...
    if event == "push":
        ...
        # Push on main, let's use PyPI monochromatic as the baseline.
