...
def multiple_sequence_alignment(e_value, in_genome_assembly_path, out_genome_assembly_path,cpus,input_num_alignments,input_temd):
    ...
    result = is_file_size_exceeded(out_genome_assembly_path+'/Step2_extend_blast_input_rename.fa',10)
    ...
    if not input_temd=='':
        os.system('python ' + script_dir + '/paf2blast6_chunking2_parallel3_ordered_parts.py ' + out_genome_assembly_path + '/Step3_blast_output.paf ' + out_genome_assembly_path+' -t '+str(cpus)+' -d '+input_temd)
    else:
...
def main():
    ...
    '''
    if not input_temd=='':
        input_temd=input_temd+'/temd'
        if not os.path.exists(input_temd):
            os.makedirs(input_temd)
    '''
