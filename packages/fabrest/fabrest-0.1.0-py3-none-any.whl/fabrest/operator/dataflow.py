...
class DataflowOperator(
    ...
    def __init__(self, workspace_id, item_type="Dataflow", credential=None):
    ...
    def schedule_apply_change(self, item_id):
        ...
        if not sched_operator:
            sched_operator = ScheduleOperator(
                ...
                item_id=item_id,
                job_type=JobType.ApplyChange.value,
    ...
    def schedule_execute(self, item_id):
        cached = self._cache_operator["schedule_execute"]
        sched_operator = cached.get(item_id)
        if not sched_operator:
            sched_operator = ScheduleOperator(
                workspace_id=self.workspace_id,
                item_id=item_id,
                item_type=self.item_type,
                job_type=JobType.Execute.value,
                credential=self.credential,
            )
            cached[item_id] = sched_operator
        return sched_operator