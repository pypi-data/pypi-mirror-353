...
- 🦘 Use it to boost code editors like Cursor, Windsurf or VSCode extensions like Roo Code, Cline, Copilot or Augment with intuitive Git management and AI-assisted file editing capabilities and say goodbye to those pesky diff application failures wasting your tool calls or `Roo having trouble...` breaking your carefully engineered automation workflows. Aider seems to get diffs right!
  - For [Roo Code](https://github.com/RooCodeInc/Roo-Code), place [.roomodes](https://github.com/daoch4n/zen-ai-mcp-devtools/blob/main/.roomodes) in your repo root to enable advanced multi-mode workflows ineraction:
    - `🤖 AI Code`, `👽 AI Architect`, and `👾 AI Debug` modes delegate all code changes to the `ai_edit` tool and enforce strict MCP tool usage.
    - `🛸 AI Orchestrator` mode coordinates complex tasks by breaking them into logical subtasks and delegates each subtask using the `new_task` tool, providing comprehensive, context-rich instructions for each specialized AI mode (ai-code, ai-debug, ai-architect). For questions or clarifications, it uses the default Ask mode. Each subtask receives explicit instructions to only perform the outlined work and signal completion with `attempt_completion`—do not instruct subtasks to commit results, as Aider handles commits automatically. The orchestrator tracks all subtasks, synthesizes results, and suggests workflow improvements.
