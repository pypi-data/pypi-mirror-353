...
def test_correctly_setup():
	...
	atuin_db_path = Path('~/.local/share/atuin/history.db').expanduser().as_posix()
	connection = sqlite3.connect(atuin_db_path)
...
export PATH={LIMINAL_BIN}:$PATH
	...
	def add(self):
		...
		content_to_add = f"""
### Liminal tools ---
# info: Activates liminal shell extension and tools. Managed through `liminal_cl` command. Learn more at url
...
def uninstall_atuin():
	run_command(['rm', '-r', Path.home() / '.atuin'])
	run_command(['rm', '-r', Path.home() / '.local/share/atuin'])
