...
from .user import UserAlternative, User
from .utils import (
    format_notification,
    to_date,
)
...
class Guest(User):
    ...
    def new_from_data(self, data):
        ...
        guest = Guest(admin_username=self._admin_username, admin_password=self._admin_password, **new_guest)
        return guest
    ...
    def create(
        ...
        host_users = UserAlternative().search_users(username=host_username)
        if not host_users:
            raise ValueError(f"no such host: {host_username}")

        host_user = host_users[0]
        if host_user:
            host_leitzahl = host_user.leitzahl
        else:
            raise ValueError(f"no such host: {host_username}")
