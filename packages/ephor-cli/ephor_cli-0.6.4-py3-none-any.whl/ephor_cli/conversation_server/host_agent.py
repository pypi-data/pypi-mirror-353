...
"""
    ...
    def send_task(self, agent_name: str, message: str):
        ...
        if self.current_message and hasattr(self.current_message, 'additional_kwargs'):
            current_attachments = self.current_message.additional_kwargs.get("attachments", [])
        ...
        # Copy processed attachment content from current message if available
        if self.current_message and hasattr(self.current_message, 'content') and isinstance(self.current_message.content, list):
            for item in self.current_message.content:
                # Skip text content (we're replacing it with delegation text)
            ...
            if isinstance(m, AIMessage) and m.tool_calls:
                for tool_call in m.tool_calls:
                    ...
                    if (
                        ...
                        and tool_call_args.get("message") == getattr(message, "content", None)
                    ):
                        return tool_call["id"]
        return None
    ...
    async def _send_task(self, agent_name: str, message: BaseMessage):
        ...
        if self.current_message:
            if isinstance(self.current_message.content, str):
                ...
                text_parts = [part.get("text", "") for part in self.current_message.content if part.get("type") == "text"]
        ...
        request: TaskSendParams = TaskSendParams(
            ...
            message=Message(
                role="user",
                parts=langchain_content_to_a2a_content(getattr(message, "content", str(message))),
                metadata=metadata,
            ),
    ...
    async def astream(
        ...
        print(
            f"Streaming agent with message: {message}, current state: {self._agent.get_state(config)}"
        )
