...
class DashboardView(HTMLMixin, SQLAlchemyMixin, SimpleView):  # pylint: disable=locally-disabled,abstract-method
    ...
    def do_before_response(self, **kwargs):
        ...
        self.response_context["users_disabled"] = (
            self.dbquery(UserModel).filter(UserModel.enabled.is_(False)).order_by(UserModel.createtime.desc()).all()
        )
        self.response_context["users_nomemberships"] = (
            self.dbquery(UserModel).filter(~UserModel.memberships.any()).order_by(UserModel.createtime.desc()).all()
        )
        self.response_context["users_noroles"] = (
            self.dbquery(UserModel).filter(UserModel.roles == []).order_by(UserModel.createtime.desc()).all()
        )
        self.response_context["users_nologin"] = (
            self.dbquery(UserModel)
            .filter(
                or_(
        ...
        self.response_context["groups_disabled"] = (
            self.dbquery(GroupModel).filter(GroupModel.enabled.is_(False)).order_by(GroupModel.createtime.desc()).all()
        )
        self.response_context["groups_nomembers"] = (
            self.dbquery(GroupModel).filter(~GroupModel.members.any()).order_by(GroupModel.createtime.desc()).all()
        )
        self.response_context["groups_nomanagers"] = (
            self.dbquery(GroupModel).filter(~GroupModel.members.any()).order_by(GroupModel.createtime.desc()).all()
        )
        self.response_context["groups_nonetworks"] = (
            self.dbquery(GroupModel).filter(~GroupModel.networks.any()).order_by(GroupModel.createtime.desc()).all()
        )

        self.response_context["filters_disabled"] = (
            self.dbquery(FilterModel)
            .filter(FilterModel.enabled.is_(False))
            .order_by(FilterModel.createtime.desc())
            .all()
        )
        self.response_context["filters_nohits"] = (
            self.dbquery(FilterModel).filter(FilterModel.hits == 0).order_by(FilterModel.createtime.desc()).all()
        )
        self.response_context["filters_future"] = (
            self.dbquery(FilterModel)
            .filter(FilterModel.valid_from > (datetime.datetime.utcnow() + datetime.timedelta(days=14)))
            .order_by(FilterModel.createtime.desc())
            .all()
        )
        self.response_context["filters_expired"] = (
            self.dbquery(FilterModel)
        ...
        self.response_context["settingsrep_redirected"] = (
            self.dbquery(SettingsReportingModel)
            .filter(SettingsReportingModel.redirect is True)
            .order_by(SettingsReportingModel.createtime.desc())
            .all()
        )
        self.response_context["settingsrep_modenone"] = (
            self.dbquery(SettingsReportingModel)
            .filter(SettingsReportingModel.mode == mentat.const.REPORTING_MODE_NONE)
            .order_by(SettingsReportingModel.createtime.desc())
            .all()
        )
        self.response_context["settingsrep_emailsnotset"] = (
            self.dbquery(SettingsReportingModel)
            .filter(
                SettingsReportingModel.emails_info == []
                and SettingsReportingModel.emails_low == []
                and SettingsReportingModel.emails_medium == []
                and SettingsReportingModel.emails_high == []
        ...
        action_menu.add_entry(
            ...
            endpoint="users.show",
            hidetitle=True,
            legend=lambda **x: lazy_gettext(
                "View details of user account &quot;%(item)s&quot;",
