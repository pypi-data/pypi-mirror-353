...
class NeosphereClient(asyncio.Queue):
    ...
    async def get_medias_base64(self, *media_ids):
        media_list = {}
        if len(media_ids) == 1 and isinstance(media_ids[0], list):
            media_id_list = media_ids[0]
        else:
        ...
        if self.media_handler:
            for media_id in media_id_list:
                try:
                    x = self.media_handler.get_media_base64(media_id)
                except Exception as e:
                    logger.error(f"Failed to get media base64 {media_id}: {e}")
                    continue
                media_list[media_id] = x
        else:
            logger.error(f"No media handler registered, to get base64 for {media_id}.")
        return media_list
    ...
    async def save_media_base64(self, parent_id, base64_data, content_type)->str:
        if self.media_handler:
