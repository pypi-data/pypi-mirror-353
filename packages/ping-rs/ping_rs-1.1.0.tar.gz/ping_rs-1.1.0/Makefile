.DEFAULT_GOAL := all
sources = python/ping_rs tests

mypy-stubtest = uv run python -m mypy.stubtest ping_rs._ping_rs
...
lint-python:
	...
	uv run griffe dump -f -d google -LWARNING -o/dev/null python/ping_rs
	$(mypy-stubtest)
...
testcov: build-coverage
	...
	coverage-prepare html python/ping_rs/*.so
