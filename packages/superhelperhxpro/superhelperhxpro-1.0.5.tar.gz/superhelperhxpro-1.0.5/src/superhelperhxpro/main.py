...
# --- organize_photos function ---
def organize_photos(base_folder):
    """
    Organizes photos and videos in a base folder into a nested hierarchy:
    base_folder/YEAR/MONTH_NAME/DATE/Photos
    """
    if not os.path.isdir(base_folder):
        print(f"Error: Base folder '{base_folder}' not found.")
        return

    print(f"Organizing photos and videos in '{base_folder}' into a YEAR/MONTH/DATE/Photos structure...")
    organized_count = 0
    skipped_count = 0

    # List files to avoid issues if subfolders are created during iteration
    files_to_process = [f for f in os.listdir(base_folder) if os.path.isfile(os.path.join(base_folder, f))]

    for filename in files_to_process:
        filepath = os.path.join(base_folder, filename)
        
        file_ext = os.path.splitext(filename)[1].lstrip('.').lower()
        # Include common image and video formats
        if file_ext not in ('jpg', 'jpeg', 'png', 'gif', 'mp4', 'mov', 'avi', 'heic', 'webp'):
            skipped_count += 1
            print(f"   Skipping '{filename}': Not a recognized photo/video file type.")
            continue

        try:
            # Use modification time as a robust way to get a date for the file
            mod_time = datetime.fromtimestamp(os.path.getmtime(filepath))
            
            # Create the nested folder structure
            year_folder = mod_time.strftime("%Y") # e.g., "2024"
            month_folder = mod_time.strftime("%B").upper() # e.g., "MARCH" (full month name, uppercase)
            date_folder = mod_time.strftime("%Y-%m-%d") # e.g., "2024-03-10"

            # Construct the full destination path
            # FOLDER/YEAR/MONTH_NAME/DATE/Photos
            dest_sub_path = os.path.join(year_folder, month_folder, date_folder, "Photos")
            dest_folder = os.path.join(base_folder, dest_sub_path)
            
            os.makedirs(dest_folder, exist_ok=True) # Create all necessary folders
            ...
            if os.path.exists(dest_filepath):
                ...
                print(f"   Conflict: '{filename}' already exists. Renaming to '{new_filename}'")
    ...
    print(f"\nFinished. Total files organized: {organized_count}. Files skipped: {skipped_count}.")
...
def _get_file_info(filepath):
    """
    Retrieves basic file information and handles common errors.
    Returns a dictionary with size, modification time, and extension.
    ...
    try:
        ...
        return {
            ...
            "accessed_datetime": datetime.fromtimestamp(stat.st_atime),
...
def _is_file_older_than(file_mod_datetime, days):
    """Checks if a file (based on its modification datetime) is older than a given number of days."""
    return datetime.now() - file_mod_datetime > timedelta(days=days)

def _get_file_hash(filepath, hash_algo=hashlib.sha256, chunk_size=65536):
    ...
    try:
        ...
        # print(f"Warning: Could not read file '{filepath}' for hashing.")
        return None

# --- Super Command: clutter-advisor ---

def clutter_advisor_logic(folder_to_scan): # Renamed to avoid direct call if main() changes
    """
    Intelligently scans a folder and suggests files/folders that are candidates
    for cleanup, based on fixed internal heuristics.
    No options are provided to the user, the logic is internal.
    """
    if not os.path.isdir(folder_to_scan):
        print(f"Error: Folder '{folder_to_scan}' not found or is not a directory.")
        return

    print(f"Analyzing '{folder_to_scan}' for potential clutter...\n")

    # Fixed Heuristics (Internal Logic - No external options)
    OLD_FILE_DAYS = 90  # Files modified/accessed older than 90 days
    LARGE_FILE_MB = 50  # Files larger than 50 MB
    VERY_LARGE_FILE_GB = 1 # Files larger than 1 GB (for archiving suggestions)
    COMMON_TEMP_EXTENSIONS = {'tmp', 'log', 'bak', 'old', 'part', 'crdownload', '$$$'}
    INSTALLER_EXTENSIONS = {'exe', 'msi', 'dmg', 'iso'} # Added .iso as common installer type
    MEDIA_EXTENSIONS = {'mp4', 'mov', 'avi', 'mkv', 'flv', 'wmv', 'mp3', 'wav', 'flac', 'jpg', 'png', 'gif'} # Added image extensions

    suggested_for_deletion = []
    suggested_for_archiving = []
    suggested_for_review = [] # For duplicates, temporary files, etc.
    estimated_reclaimable_bytes = 0

    file_hashes = collections.defaultdict(list) # To detect exact duplicates

    for root, dirs, files in os.walk(folder_to_scan):
        # Identify empty subfolders
        if not dirs and not files and root != os.path.abspath(folder_to_scan):
        ...
        for filename in files:
            ...
            # Skip common hidden/system/config files that are usually not clutter
            if filename.startswith('.') or filename.startswith('~') or filename.lower() in (
                'thumbs.db', 'desktop.ini', '._.ds_store', '.localized'
            ):
                continue
            ...
            file_size_gb = info["size"] / (1024 * 1024 * 1024)
            
            is_old = _is_file_older_than(info["accessed_datetime"], OLD_FILE_DAYS)
            is_large = file_size_mb > LARGE_FILE_MB
            is_very_large = file_size_gb >= VERY_LARGE_FILE_GB

            is_temp_file = info["extension"] in COMMON_TEMP_EXTENSIONS
            is_installer = info["extension"] in INSTALLER_EXTENSIONS
            is_media = info["extension"] in MEDIA_EXTENSIONS

            current_file_reasons = []

            # Rule 1: Old and Large Files
            if is_old and is_large:
                current_file_reasons.append(f"Old & Large ({OLD_FILE_DAYS} days, {LARGE_FILE_MB}MB)")
                
            # Rule 2: Common Temporary Files
            if is_temp_file:
                current_file_reasons.append("Temporary File")

            # Rule 3: Old Installers
            if is_installer and is_old:
                current_file_reasons.append(f"Old Installer ({OLD_FILE_DAYS} days)")
            elif is_installer and is_large and not is_old:
                current_file_reasons.append(f"Large Installer ({LARGE_FILE_MB}MB)")

            # Rule 4: Very Large Media Files (suggest archiving)
            if is_media and is_very_large:
                current_file_reasons.append(f"Very Large Media ({VERY_LARGE_FILE_GB}GB)")
            
            # Grouping suggestions
            if any(reason in current_file_reasons for reason in ["Old & Large", "Temporary File", "Old Installer", "Large Installer"]):
                suggested_for_deletion.append({
                    "path": filepath,
                    "size_mb": round(file_size_mb, 2),
                    "modified": info["modified_datetime"].strftime("%Y-%m-%d"),
                    "reason": ", ".join([r for r in current_file_reasons if r in ["Old & Large", "Temporary File", "Old Installer", "Large Installer"]]),
                    "type": "file",
                    "size": info["size"]
                })
                estimated_reclaimable_bytes += info["size"]
            
            if "Very Large Media" in current_file_reasons:
                suggested_for_archiving.append({
                    "path": filepath,
                    "size_gb": round(file_size_gb, 2),
                    "modified": info["modified_datetime"].strftime("%Y-%m-%d"),
                    "reason": "Very Large Media",
                    "type": "file"
                })
    ...
    # After iterating all files, check for exact duplicates
    for file_hash, paths in file_hashes.items():
        if len(paths) > 1:
            for duplicate_path in paths[1:]:
                is_already_in_deletion = any(item['path'] == duplicate_path for item in suggested_for_deletion)
                if not is_already_in_deletion:
                    suggested_for_review.append({
                        "path": duplicate_path,
                        "reason": "Exact Duplicate",
                        "original": paths[0],
                        "type": "file"
                    })
                    duplicate_info = _get_file_info(duplicate_path)
                    if duplicate_info:
                        estimated_reclaimable_bytes += duplicate_info["size"]


    # --- Output Report ---

    print("--- SuperHelperXPro Clutter Advisor Report ---\n")

    if not suggested_for_deletion and not suggested_for_archiving and not suggested_for_review:
        print("No significant clutter patterns detected in this folder based on current analysis.\n")
        print("Your computer is looking tidy! ✨")
        return

    if suggested_for_deletion:
        print("Suggested for Deletion (Likely no longer needed):\n")
        for item in suggested_for_deletion:
            if item['type'] == 'file':
                print(f"  •  {item['path']} ({item['size_mb']} MB, Modified: {item['modified']})")
            else:
                print(f"  •  {item['path']} (Empty Folder)")
            print(f"     - Reason: {item['reason']}\n")

    if suggested_for_archiving:
        print("Suggested for Archiving (Very large files that might need long-term storage):\n")
        for item in suggested_for_archiving:
            print(f"  •  {item['path']} ({item['size_gb']} GB, Modified: {item['modified']})")
            print(f"     - Reason: {item['reason']}\n")

    if suggested_for_review:
        print("Suggested for Review (Possible duplicates or complex cases):\n")
        for item in suggested_for_review:
            if item['reason'] == "Exact Duplicate":
                print(f"  •  {item['path']}")
                print(f"     - Reason: {item['reason']} of '{item['original']}'\n")
            else:
                print(f"  •  {item['path']}")
                print(f"     - Reason: {item['reason']}\n")

    total_reclaimable_mb = round(estimated_reclaimable_bytes / (1024 * 1024), 2)
    if total_reclaimable_mb > 0:
        print(f"\nEstimated space reclaimable by acting on 'Suggested for Deletion' and 'Exact Duplicate' items: **{total_reclaimable_mb} MB**\n")
    else:
        print("\nNo direct space reclaimable from suggestions (e.g., only archiving or review items).\n")


    print("--- End of Clutter Advisor Report ---")
    print("\nThis report is for your review. No actions have been taken.")
    print("You can use other SuperHelperXPro commands like 'remove-item' or 'smart-archive' after reviewing.")

def main():
    # Only one main parser for the entire CLI tool
    ...
    # All commands (subparsers) are added to this single parser
    ...
    # Organize Photos Command (Moved into the *single* subparsers definition)
    organize_photos_parser = subparsers.add_parser(
        "organize-photos",
        help="Organizes photos into a YEAR/MONTH_NAME/DATE/Photos folder structure."
    )
    organize_photos_parser.add_argument(
        "folder",
        help="Path to the base folder containing photos to organize (e.g., 'C:/Users/YourName/Pictures')."
    )

     # Define the 'clutter-advisor' command
    clutter_advisor_parser = subparsers.add_parser(
        "clutter-advisor",
        help="Clear clutter effortlessly. Get intelligent suggestions for files to clean up."
    )
    clutter_advisor_parser.add_argument(
        "folder",
        help="The path to the folder you want SuperHelperXPro to analyze for clutter."
    )
    ...
    # Dispatch commands
    if args.command == "clutter-advisor":
        clutter_advisor_logic(args.folder)
    ...
    elif args.command == "organize-photos":
        organize_photos(args.folder)
    else:
        parser.print_help()
