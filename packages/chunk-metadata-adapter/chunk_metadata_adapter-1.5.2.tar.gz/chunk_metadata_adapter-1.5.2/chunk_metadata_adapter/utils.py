...
def semantic_to_flat_value(value, field, field_name):
    ...
    # Объект (pydantic.BaseModel и др.)
...
def str_to_list(value, separator=',', allow_none=True)->List[str]:
    ...
    - If value is list — return its copy.
    ...
    if isinstance(value, list):
        return list(value)
    if not isinstance(value, str):
        raise ValueError(f"value must be a string or list, got: {type(value)}")
...
def dict_prop_to_flat_dict(value, separator='=', allow_none=True)->dict:
    ...
    - If value is dict — flatten nested dicts: data[ИмяСвойства.КлючВСловаре] = ЗначениеКлючаВСвойстве
    - If key has no dot — just key and value.
    - If value is not dict — raise ValueError.
    """
    if value is None:
        return {}
...
def flat_dict_to_dict_prop(value, separator='=', allow_none=True)->dict:
    ...
    - If key has no dot — just key and value.
    - If key has one dot — nested dict.
    - If key has more than one dot — raise ValueError.
    """
    if value is None:
        return {}
    if not isinstance(value, dict):
    ...
    for k, v in value.items():
        if '.' not in k:
            ...
            raise ValueError(f"value must be a flat dict, got: {value}")
    return result

def autofill_min_length_str_fields(data, model_fields):
