...
class TestSemanticChunk:
    ...
    def test_sha256_always_autofilled(self):
        data = {
            "uuid": str(uuid.uuid4()),
            "type": "DocBlock",
            "text": "abc",
        ...
        assert err is None
        assert obj is not None
        expected_sha256 = hashlib.sha256(data["text"].encode()).hexdigest()
        print(f"[TestSemanticChunk.test_sha256_always_autofilled] obj: {obj}")
        assert obj.sha256 == expected_sha256
...
class TestFlatSemanticChunk:
    ...
    def test_minimal_initialization(self):
        ...
        chunk,errors = FlatSemanticChunk.validate_and_fill(
            uuid=uuid_val,
            type="DocBlock",
            text="Test content\nSecond line\nThird line\n",
            language="markdown",
        ...
        assert chunk is not None and errors is None
        ...
        # Check defaults
        assert isinstance(chunk.source_id,str)
        ...
        assert chunk.source_lines_start == 0
        # source_lines_end должен быть равен числу строк в тексте + start
        n_lines = chunk.text.count('\n')
        if chunk.text and not chunk.text.endswith('\n'):
            n_lines += 1
        assert chunk.source_lines_end == chunk.source_lines_start + n_lines
        assert chunk.tags == ""
        assert chunk.link_related == ChunkId.default_value()
        assert chunk.link_parent == ChunkId.default_value()
        assert chunk.quality_score == 0.0
        assert chunk.coverage == 0.0
        assert chunk.cohesion == 0.0
        assert chunk.boundary_prev == 0.0
        assert chunk.boundary_next == 0.0
    ...
    def test_full_initialization(self):
        ...
        print(f"[TestFlatSemanticChunk.test_full_initialization] chunk.sha256: {chunk.sha256}")
        assert chunk.sha256 == hashlib.sha256(chunk.text.encode()).hexdigest()
    ...
    def test_conversion_to_semantic(self):
        ...
        parent_id = str(uuid.uuid4())
        timestamp = datetime.now(timezone.utc).isoformat()
        text_val = "Test content"
        summary_val = "Test summary"
        sha256_val = hashlib.sha256(text_val.encode()).hexdigest()
        ...
        assert semantic_chunk.text == text_val
        assert semantic_chunk.summary == summary_val
        assert semantic_chunk.sha256 == sha256_val
    ...
    def test_conversion_from_semantic(self):
        ...
        assert flat_chunk.quality_score is None
        assert flat_chunk.coverage is None
        assert flat_chunk.cohesion is None
        assert flat_chunk.boundary_prev is None
        assert flat_chunk.boundary_next is None
...
class TestSemanticChunkValidateAndFill:
    def test_valid_minimal(self):
        ...
        assert obj.cohesion is None
        assert obj.boundary_prev is None
        assert obj.boundary_next is None
...
class TestFlatSemanticChunkValidateAndFill:
    ...
    def test_from_semantic_chunk_edge(self):
        ...
        with pytest.raises(Exception):
            FlatSemanticChunk.from_semantic_chunk(Dummy())
    ...
    def test_to_semantic_chunk_edge(self):
        ...
        assert obj.source_path == ""
        assert obj.chunking_version == "1.0"
        assert obj.link_parent == ChunkId.default_value()
        assert obj.link_related == ChunkId.default_value()
        assert obj.quality_score == 0.0
        assert obj.coverage == 0.0
        assert obj.cohesion == 0.0
        assert obj.boundary_prev == 0.0
        assert obj.boundary_next == 0.0 