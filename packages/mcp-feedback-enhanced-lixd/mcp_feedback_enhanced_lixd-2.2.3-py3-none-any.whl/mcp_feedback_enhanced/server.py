...
async def interactive_feedback(
    ...
    host: Annotated[str, Field(description="Web UI 主機名，默認使用環境變數 MCP_HOST 或 127.0.0.1")] = None,
    port: Annotated[int, Field(description="Web UI 端口號，默認使用環境變數 MCP_PORT 或 8765")] = None
) -> List:
    """
    收集用戶的互動回饋，支援文字和圖片
    ...
    Web UI 配置：
    - 可通過 MCP_HOST 環境變數或 host 參數指定主機名
    - 可通過 MCP_PORT 環境變數或 port 參數指定端口號
    ...
    Args:
        ...
        host: Web UI 主機名，優先於環境變數 MCP_HOST
        port: Web UI 端口號，優先於環境變數 MCP_PORT
    ...
    try:
        ...
        # 選擇適當的介面
        if use_web_ui:
            result = await launch_web_ui_with_timeout(project_directory, summary, timeout, host, port)
        else:
...
async def launch_web_ui_with_timeout(project_dir: str, summary: str, timeout: int, host: str = None, port: int = None) -> dict:
    """
    ...
    Args:
        ...
        summary: AI 工作摘要
        timeout: 超時時間（秒）
        host: 主機名，默認使用環境變數 MCP_HOST 或 127.0.0.1
        port: 端口號，默認使用環境變數 MCP_PORT 或 8765
        
    Returns:
        dict: 收集到的回饋資料
    """
    debug_log(f"啟動 Web UI 介面，超時時間: {timeout} 秒，主機: {host or os.getenv('MCP_HOST', '127.0.0.1')}，端口: {port or os.getenv('MCP_PORT', '8765(預設)')}")
    
    try:
        # 使用新的 web 模組
        from .web import launch_web_feedback_ui
        
        # 傳遞 timeout, host, port 參數給 Web UI
        return await launch_web_feedback_ui(project_dir, summary, timeout, host=host, port=port)
