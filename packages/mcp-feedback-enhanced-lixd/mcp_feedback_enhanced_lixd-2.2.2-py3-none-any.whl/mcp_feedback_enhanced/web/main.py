...
class WebUIManager:
    ...
    def __init__(self, host: str = None, port: int = None):
        ...
        if env_port and env_port.isdigit():
            ...
            preferred_port = 8765  # 默認端口
            
        # 優先使用環境變數，其次是參數，最後嘗試使用默認值或找一個可用端口
...
def get_web_ui_manager(host: str = None, port: int = None) -> WebUIManager:
    """獲取 Web UI 管理器實例
    
    Args:
        host: 指定的主機名，優先級高於環境變數
        port: 指定的端口，優先級高於環境變數
    
    Returns:
        WebUIManager: Web UI 管理器實例
    """
    global _web_ui_manager
    if _web_ui_manager is None:
        _web_ui_manager = WebUIManager(host=host, port=port)
    return _web_ui_manager


async def launch_web_feedback_ui(project_directory: str, summary: str, timeout: int = 600, 
    ...
    """
    manager = get_web_ui_manager(host=host, port=port)
