...
from .core import ShangshanAIError
from .core import HttpClient, SHANGSHANAI_DEFAULT_MAX_RETRIES
...
class ShangshanAI(HttpClient):
    ...
    def __init__(
        ...
        if api_key is None:
            api_key = os.environ.get("SHANGSHANAI_API_KEY")
        if api_key is None:
            raise ShangshanAIError("未提供api_key，请通过参数或环境变量提供")
        self.api_key = api_key
        self._disable_token_cache = disable_token_cache

        if base_url is None:
            base_url = os.environ.get("SHANGSHANAI_BASE_URL")
        if base_url is None:
            base_url = f"https://shangshan.mwr.cn/open"
