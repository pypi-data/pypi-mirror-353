...
from operror.op_error import OpError, ErrorBuilder
...
class DBClient:
    ...
    def insert(self, data: str):
        try:
            ...
            raise ErrorBuilder.new_internal_error().with_message("DB insert failed").build(OpError) from e
...
def test_build_op_error():
    ...
    e = ErrorBuilder.new_internal_error().with_message("internal error").build(MyOpError)
...
def test_op_error_str():
    ...
    # 测试带模块名的错误信息
    e = ErrorBuilder.new_internal_error().with_module("test_module").with_message("internal error").build()
    assert str(e) == "OpError(module=test_module, status=(code=INTERNAL_ERROR(13), message='internal error'))"
    
    # 测试带 case 的错误信息
    e = ErrorBuilder.new_internal_error().with_case(ErrorCase("1001", Code.INTERNAL_ERROR)).with_message("internal error").build()
    assert str(e) == "OpError(module=none, status=(code=INTERNAL_ERROR(13), case=1001, message='internal error'))"
    
    # 测试带 details 的错误信息
    e = ErrorBuilder.new_internal_error().with_details({"key": "value"}).with_message("internal error").build()
    assert str(e) == "OpError(module=none, status=(code=INTERNAL_ERROR(13), message='internal error', details={'key': 'value'}))"
    
    # 测试完整错误信息
    e = ErrorBuilder.new_internal_error()\
...
def test_build_error_with_kwargs():
    ...
    # 测试传入关键字参数
    e = ErrorBuilder.new_internal_error().with_message("internal error").build(key1="value1")
    assert isinstance(e, OpError)
    assert e.status.code == Code.INTERNAL_ERROR
    assert e.status.message == "internal error"
