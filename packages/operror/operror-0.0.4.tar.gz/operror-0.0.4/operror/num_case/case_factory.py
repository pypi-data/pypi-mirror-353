...
from operror.num_case.code_mapper import CodeMapper
from operror.num_case.num_case import NumCase
from operror.num_case.num_range import NumRange
from operror.op_status import Code
from operror.utils import check_arg
...
class CaseFactory:
    ...
    def __init__(self,
        ...
        self.app_code_range = NumRange(0, int(math.pow(10, num_digits_for_app_code)) - 1)
        self.module_code_range = NumRange(0, int(math.pow(10, num_digits_for_module_code)) - 1)
        self.case_code_range = NumRange(0, int(math.pow(10, num_digits_for_case_code)) - 1)
    ...
    def _create(self, status_code: Code, case_code: int) -> NumCase:
        ...
        # )
        if code_seg is None:
            raise ValueError(f"code_mapper doesn't define a CaseCodeSegment for given status_code {status_code}")
        check_arg(
    ...
    def build_case_id(self, case_code: int) -> str:
        ...
        if self.num_digits_for_app_code > 0:
            id_parts.append(self._pad_left_zeros(
                self.app_code, self.num_digits_for_app_code
            ))
        if self.num_digits_for_module_code > 0:
            id_parts.append(self._pad_left_zeros(
                self.module_code, self.num_digits_for_module_code
            ))
        id_parts.append(self._pad_left_zeros(
            case_code, self.num_digits_for_case_code
        ))
    ...
    def new_data_corrupted(self, case_code: int) -> NumCase:
        return self._create(Code.DATA_CORRUPTED, case_code)