...
def deserialize_result(serialized_data: Dict[str, Any]) -> Any:
    ...
    if result_type == "none":
        ...
        try:
            ...
            log_message(f"Failed to deserialize Pydantic model {class_name}: {str(e)}", "WARNING")
            return data
        ...
        if cached_data and isinstance(cached_data, WorkflowCache):
            if not cached_data.traceback:
                ...
                try:
                    ...
                    log_message(f"Failed to deserialize cached result for {method_name}: {str(e)}", "WARNING")
        ...
        with CaptureOutput() as output:
            ...
            try:
                ...
                try:
                    ...
                    log_message(f"Failed to serialize result for {method_name}: {str(e)}", "WARNING")
        ...
        if cached_data and isinstance(cached_data, WorkflowCache):
            if not cached_data.traceback:
                ...
                try:
                    ...
                    log_message(f"Failed to deserialize cached workflow result for {workflow_name}: {str(e)}",
                                "WARNING")
        ...
        with CaptureOutput() as output:
            ...
            try:
                ...
                try:
                    ...
                    log_message(f"Failed to serialize workflow result for {workflow_name}: {str(e)}", "WARNING")
                    # Fallback to basic JSON serialization
                    try:
                        result_json = json.dumps(result)
                    except:
