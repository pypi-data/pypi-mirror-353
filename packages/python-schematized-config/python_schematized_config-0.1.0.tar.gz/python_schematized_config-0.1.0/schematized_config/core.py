...
class ConfigValidator(object):
    DEFAULT_STORAGE_DRIVER: AbstractFileSystem = LocalFileSystem(".")
    ...
    def load_json(
        cls,
        json_source: Union[str, dict] = None,
        ...
        if isinstance(json_source, str):
            with storage_driver.open(json_source, mode="r") as ifile:
    ...
    def get_default_json_schema(cls, storage_driver: AbstractFileSystem = None) -> dict:
        ...
        if cls.CONFIG_VALIDATOR_JSON_SCHEMA_ENVVAR_NAME in os.environ:
            expected_json_schema_path = os.environ[
                cls.CONFIG_VALIDATOR_JSON_SCHEMA_ENVVAR_NAME
            ]
            with storage_driver.open(expected_json_schema_path, mode="r") as ifile:
                return json.load(ifile)
        return None
    ...
    def __init__(
        ...
        if isinstance(json_schema, (str, dict)):
            self._json_schema = self.__class__.load_json(
                json_schema, storage_driver=self.storage_driver
            )
    ...
    def load_validated_config(
        cls, json_schema: Union[str, dict], config: dict, **kwargs
    ) -> dict:
        return cls(json_schema, **kwargs).load_config(config)

    @classmethod
    def load_validated_environment(cls, json_schema: Union[str, dict] = None, **kwargs) -> dict:
    ...
    def load_dotenv(
        ...
        if dotenv_path:
            with storage_driver.open(dotenv_path, mode="r") as ifile:
        ...
        json_schema_dict = (
            cls.load_json(json_schema, storage_driver=storage_driver) or {}
        )
        for key in json_schema_dict.get("properties", []):
            ...
            if key in config and config[key] != os.environ[key]:
                logger.debug(
                    f'os.environ key "{key}" overriding value present in {dotenv_path}'
                )
