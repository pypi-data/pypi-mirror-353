...
class Config:
    ...
    PAGE_SIZE = 5  # Number of items to show per page
...
class AccountPoolCli:
    ...
    def _display_account_info(self, account: Dict) -> None:
        ...
        print(f"📱 Phone Number:    {main_number}")
        ...
        if locked:
            print(f"🔒 Locked Features: {', '.join(locked) if locked else 'None'}")
        
        print("=" * 50)

    @staticmethod
    def _paginated_display(items: List[any], display_func, title: str = "") -> bool:
        """Display items with pagination. Returns True if user completed viewing."""
        if not items:
        ...
        while current_index < total_items:
            ...
            if current_index < total_items:
                ...
                print(f"\n--- Showing {end_index}/{total_items} items ---")
        ...
        try:
            ...
            if accounts_list:
                ...
                if retrieved_account_id:
                    print(f"   ✓ Found account ID: {retrieved_account_id}")
                    ...
                    self._display_account_info(account_summary)
            else:
    ...
    def _display_account_types(self, account_settings: List[Dict], filter_keyword: Optional[str], brand: str) -> None:
        ...
        completed = self._paginated_display(account_settings, display_account_type, title)
    ...
    def _show_cache_status(self) -> None:
        ...
        if account_types:
            ...
            # Show more account types if there are many
            if len(account_types) <= 10:
                print(f"\nAll cached account types:")
                ...
                print(f"\nUse 'ap types' to view all types with pagination")
        else:
