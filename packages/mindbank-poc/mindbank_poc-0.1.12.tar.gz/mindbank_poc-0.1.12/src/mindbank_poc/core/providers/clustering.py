    ...
    async def cluster(
        ...
        **runtime_cfg
        ...
        try:
            ...
            # Обрабатываем малые кластеры - объединяем их в "Mixed Topics"
            if small_clusters:
                ...
                results[999] = {  # Специальный ID для смешанного кластера
                    "segment_ids": [seg.id for seg in small_clusters],
                    "centroid": centroid.tolist(),
                    "keywords": keywords,
                    "title": f"Mixed Topics ({len(small_clusters)} segments)" if len(small_clusters) > 1 else title,
                    "summary": summary,
                    "size": len(small_clusters)
                }
                
                logger.info(f"Mixed cluster: {len(small_clusters)} segments from small clusters")
