...
from parsomics_core.entities.files.gene_annotation import (
    ...
    GeneAnnotationEntryTransactions,
)
...
_routing_helpers = RoutingHelpers(
    ...
    transactions=GeneAnnotationEntryTransactions(),
)
router = APIRouter(
    prefix="/entry",
    tags=["files/gene_annotation/entry"],
    responses={
...
@router.get("/item/{key}", response_model=GeneAnnotationEntryPublic)
def get_entry(
    *,
...
@router.post("/items/", response_model=list[GeneAnnotationEntryPublic])
def get_entries(
    *,
    session: Session = Depends(get_session_ro),
    keys: list[int],
...
@router.get("/", response_model=list[GeneAnnotationEntryPublic])
def read_entries(
    *,
    session: Session = Depends(get_session_ro),
    offset: int = 0,
