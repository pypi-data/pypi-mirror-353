...
def normalize_dates(text):
    matches = extractor.extract_bengali_dates(text)
    matches = sorted(list(set(matches)), key=len, reverse=True)
    for match in matches:
        normalized_date = methods.date_to_word(match)
...
def normalize_distance(text):
    matches = extractor.extract_distance(text)
    matches = sorted(list(set(matches)), key=len, reverse=True)
    for match in matches:
        normalized_distance = methods.distance_to_word(match)
...
def normalize_phonenumbers(text):
    mobile_numbers = extractor.extract_mobile_numbers(text)
    mobile_numbers = sorted(list(set(mobile_numbers)), key=len, reverse=True)
    for number in mobile_numbers:
        normalized = methods.phone_number_to_word(number)
...
def normalize_numbers(text):
    ...
    for number in numbers:
        normalized = methods.number_to_word(number)
        text = text.replace(number, normalized)
    return text
...
def normalize_time(text):
    ...
    for t in times:
        normalized = methods.time_to_word(t, text)
        if normalized != t:
...
def normalize_taka(text):
    takas = extractor.extract_taka_amounts(text)
    takas = sorted(list(set(takas)), key=len, reverse=True)
    for taka in takas:
        normalized = methods.taka_to_word(taka)
        if normalized != taka:
...
def normalize_percentage(text):
    ...
    for percentage in percentages:
        normalized = methods.percentage_to_word(percentage)
...
def normalize_temperatures(text):
    temperatures = extractor.extract_temperatures(text)
    temperatures = sorted(list(set(temperatures)), key=len, reverse=True)
    for temperature in temperatures:
        normalized = methods.temperature_to_word(temperature)
...
def normalize_ratio(text):
    ratios = extractor.extract_ratios(text)
...
def normalize_ordinal(text):
    words = extractor.extract_ordinals(text)
    words = sorted(list(set(words)), key=len, reverse=True)
    for word in words:
        normalized = methods.ordinal_to_word(word)
        if normalized != word:
...
def normalize_year(text):
    words = extractor.extract_years_with_context(text)
    words = sorted(list(set(words)), key=len, reverse=True)
    for word in words:
        normalized = methods.year_to_word(word)
        if normalized != word :
             text = text.replace(word, normalized)
    return text
