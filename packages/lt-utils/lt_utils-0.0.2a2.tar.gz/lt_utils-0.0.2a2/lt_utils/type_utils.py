...
def is_pathlike(
    ...
    begin = isinstance(entry, (str, PathLike, bytes))
    assert (
...
def is_path_valid(entry: PathLike, validate: bool = False) -> bool:
    if not is_pathlike(entry, validate):
        return False
    result = Path(entry).exists()
    assert not validate or result, f'The path "{entry}" is not a valid path!'
    return result
