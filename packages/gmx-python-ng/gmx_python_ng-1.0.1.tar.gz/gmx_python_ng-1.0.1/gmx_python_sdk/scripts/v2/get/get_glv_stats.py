from ..gmx_utils import get_glv_reader_contract, get_token_balance_contract
...
class GlvStats(GetData):
    ...
    def _get_data_processing(self):
        ...
        for glv_market_address in glv_info_dict.keys():
            ...
            for market_address in glv_info_dict[glv_market_address]["glv_market_addresses"]:
                ...
                oracle_prices = self._get_oracle_prices(market_address, index_token_address, return_tuple=True)
                index_token_prices += [oracle_prices[0]]

                oracle_prices_dict = OraclePrices(self.config.chain).get_recent_prices()

                long_token_price_dict = oracle_prices_dict[glv_info_dict[glv_market_address]["long_address"]]
                short_token_price_dict = oracle_prices_dict[glv_info_dict[glv_market_address]["short_address"]]
                ...
                market_token_balance = self._get_glv_composition(glv_market_address, market_address)
                gm_price = (
                    self.GMPrices_obj._make_market_token_price_query(
                        [
                            market_address,
                            index_token_address,
                ...
                glv_markets_metadata[market_address] = {
    ...
    def _get_glv_info_list(self):
        ...
        raw_output = (
            get_glv_reader_contract(self.config)
            .functions.getGlvInfoList(self.data_store_contract_address, 0, 10)
            .call()
        )
    ...
    def _get_glv_composition(self, glv_address: str, market_address: str):
        ...
        market_token_contract = get_token_balance_contract(self.config, market_address)
        market_token_balance = market_token_contract.functions.balanceOf(glv_address).call() / 10**18
