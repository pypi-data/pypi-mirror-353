...
class MemorySecurityManager:
    ...
    def __init__(self, config: MemorySystemConfig, agent_security_profile: str = "standard"):
        self.config = config
    ...
    def _get_security_level(self, entry: MemoryEntry) -> str:
        """Determine security level based on memory type and agent security profile"""
        
        # If agent security profile is minimal, always use minimal for memory validation
        # This ensures consistency with the agent's security validation behavior
        if self.agent_security_profile == "minimal":
            return "minimal"
        
        # For other security profiles, apply the original logic but cap at agent's level
        ...
        # Cap the security level at the agent's security profile
        security_hierarchy = ["minimal", "standard", "high", "maximum"]
        agent_level_index = security_hierarchy.index(self.agent_security_profile) if self.agent_security_profile in security_hierarchy else 1
        base_level_index = security_hierarchy.index(base_level) if base_level in security_hierarchy else 1
        
        # Use the lower of the two (more permissive)
        final_level_index = min(agent_level_index, base_level_index)
        return security_hierarchy[final_level_index]
