...
class MemoryManager:
    ...
    def __init__(self, config: Optional[MemorySystemConfig] = None, agent_security_profile: str = "standard"):
        """
        Initialize the memory manager.
        
        Args:
            config: Optional configuration. If None, uses default settings.
            agent_security_profile: The agent's overall security profile (minimal, standard, high, maximum)
        """
        self.config = config or self._create_default_config()
        self.agent_security_profile = agent_security_profile
        
        # Initialize storage backend based on configuration
        self.storage = self._create_storage_backend()
        self.security = MemorySecurityManager(self.config, agent_security_profile)
        ...
        logger.info(f"MemoryManager initialized with {backend_name} storage backend and {agent_security_profile} security profile")
    ...
    def sync_initialize(self) -> bool:
        """Synchronous wrapper for initialize() - Jupyter/Colab compatible"""
        try:
            import asyncio
            
            # Check if we're in a Jupyter/Colab environment with existing loop
            try:
                # Try to get existing loop (Jupyter/Colab case)
                ...
                try:
                    ...
                    task = loop.create_task(self.initialize())
                ...
                # No existing loop, safe to create new one
    ...
    def sync_store(self, user_id: str, content: str, memory_type: MemoryType = MemoryType.WORKING,
        ...
        """Synchronous wrapper for store() - Jupyter/Colab compatible"""
        try:
            import asyncio
            ...
            try:
                ...
                try:
                    ...
                    nest_asyncio.apply()
                    # Create task in current loop
                    task = loop.create_task(self.store(user_id, content, memory_type, priority, access_level, tags, key))
                    result = loop.run_until_complete(task)
                    return result
                except ImportError:
                    # nest_asyncio not available, run in thread pool
                    ...
                    def run_async():
                        ...
                        try:
                            asyncio.set_event_loop(new_loop)
                            return new_loop.run_until_complete(self.store(user_id, content, memory_type, priority, access_level, tags, key))
                        finally:
                            new_loop.close()
                ...
                # No existing loop, safe to create new one
                result = asyncio.run(self.store(user_id, content, memory_type, priority, access_level, tags, key))
                return result
    ...
    def sync_retrieve(self, user_id: str, query: str, 
        ...
        """Synchronous wrapper for retrieve() - Jupyter/Colab compatible"""
        try:
            import asyncio
            ...
            try:
                ...
                try:
                    ...
                    nest_asyncio.apply()
                    # Create task in current loop
                    task = loop.create_task(self.retrieve(user_id, query, memory_types, limit))
                    result = loop.run_until_complete(task)
                    return result
                except ImportError:
                    # nest_asyncio not available, run in thread pool
                    ...
                    def run_async():
                        ...
                        try:
                            asyncio.set_event_loop(new_loop)
                            return new_loop.run_until_complete(self.retrieve(user_id, query, memory_types, limit))
                        finally:
                            new_loop.close()
                ...
                # No existing loop, safe to create new one
                result = asyncio.run(self.retrieve(user_id, query, memory_types, limit))
                return result
