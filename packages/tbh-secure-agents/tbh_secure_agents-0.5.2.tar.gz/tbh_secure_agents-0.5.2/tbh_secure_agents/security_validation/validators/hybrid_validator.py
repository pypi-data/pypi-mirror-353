...
class HybridValidator(SecurityValidator):
    ...
    def _should_use_llm(self, text: str, security_level: str) -> bool:
        ...
        if security_level == "standard":
            ...
            # NOTE: Reduced patterns for cybersecurity experts - be less aggressive
            complex_patterns = [
                # Only check for obvious prompt injection attempts
                r"disregard\s+previous|ignore\s+all|forget\s+everything|bypass\s+security",
                # Only check for clear malicious code patterns
                r"__import__\s*\(\s*['\"]os['\"]|exec\s*\(\s*input|eval\s*\(\s*input",
                # Only very suspicious encoded content
                r"base64.*decode.*exec|decode.*eval.*exec",
                # Very long text that might hide malicious content
                r".{2000,}"  # Increased threshold from 500 to 2000
            ]
            ...
            # Reduced random LLM usage from 20% to 5% for standard security
