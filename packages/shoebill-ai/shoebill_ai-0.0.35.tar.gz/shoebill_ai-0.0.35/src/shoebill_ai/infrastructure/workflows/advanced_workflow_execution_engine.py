...
class AdvancedWorkflowExecutionEngine(WorkflowExecutionEngine):
    ...
    def _execute_agent_node(self, 
        ...
        # Execute the agent
        result = agent.process(input_data, context.to_dict())
        ...
        if 'result' not in result:
            ...
            # Otherwise, use the entire result as the result
            else:
    ...
    def _execute_output_node(self, 
        ...
        target_input = node.config.get('target_input', 'result')

        if target_input in input_data:
            # Use the specific input value
    ...
    async def _execute_subworkflow_node(self, 
        ...
        if context.current_execution_id:
            context.subworkflow_contexts[node.node_id] = context.current_execution_id
