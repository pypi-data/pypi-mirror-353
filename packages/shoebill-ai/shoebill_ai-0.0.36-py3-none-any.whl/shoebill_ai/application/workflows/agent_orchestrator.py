...
class AgentOrchestrator:
    ...
    def add_variable_node(self, 
                         ...
                         config: Dict[str, Any] = None) -> Optional[WorkflowNode]:
        """
        Add a variable node to a workflow.
        ...
        Args:
            workflow_id: The ID of the workflow
            name: The name of the node
            variable_name: The name of the variable to store
            description: Optional description of the node
            config: Optional additional configuration for the node
    ...
    def add_condition_node(self, 
        ...
        Args:
            workflow_id: The ID of the workflow
            name: The name of the node
            condition: The condition to evaluate as a string, e.g., "== 10" or "< 5"
                The condition will be applied to the input value(s)
            value_type: The type of value to compare, either "string" or "numeric"
            true_branch: List of node IDs to execute if the condition is true
            false_branch: List of node IDs to execute if the condition is false
            description: Optional description of the node
            config: Optional additional configuration for the node
        ...
        # Parse the condition string to extract operator and right value
        condition = condition.strip()

        # Extract the operator and right value from the condition string
        if condition.startswith("=="):
            operator = "=="
            right_value = condition[2:].strip()
            ...
            operator = "<"
            right_value = condition[1:].strip()
        elif condition.startswith(">"):
            operator = ">"
            right_value = condition[1:].strip()
        else:
            raise ValueError(f"Invalid condition format: {condition}. Must start with ==, !=, <, >, <=, or >=.")

        # Convert the right value to the appropriate type
        if value_type == "numeric":
            try:
                right_value = float(right_value)
            except ValueError:
                raise ValueError(f"Invalid numeric value: {right_value}")

        # Create the condition dictionary
        condition_dict = {
            "left": "input",  # Default key for the input value
            "operator": operator,
            "right": right_value
        }

        node_config['condition'] = condition_dict
        node_config['true_branch'] = true_branch or []
        node_config['false_branch'] = false_branch or []
        node_config['value_type'] = value_type
    ...
    def add_foreach_node(self, 
        ...
        Args:
            workflow_id: The ID of the workflow
            name: The name of the node
            collection_key: The key in the input data that contains the collection to iterate over
            body_nodes: List of node IDs to execute for each item in the collection
            item_key: The key to use for the current item in the input data for body nodes
            parallel: Whether to execute the body nodes in parallel
            store_results: Whether to store the results of each iteration
            aggregate_results: Whether to aggregate the results of all iterations
            description: Optional description of the node
            config: Optional additional configuration for the node
        ...
        # Only set these values if they're not already in the config
        if 'collection_key' not in node_config:
            node_config['collection_key'] = collection_key
        if 'body_nodes' not in node_config:
            node_config['body_nodes'] = body_nodes
        if 'item_key' not in node_config:
            node_config['item_key'] = item_key
        if 'parallel' not in node_config:
            node_config['parallel'] = parallel
        if 'store_results' not in node_config:
            node_config['store_results'] = store_results
        if 'aggregate_results' not in node_config:
            node_config['aggregate_results'] = aggregate_results

        # Debug the config
        print(f"DEBUG: add_foreach_node config = {node_config}")
    ...
    def add_loop_node(self, 
        ...
        Args:
            workflow_id: The ID of the workflow
            name: The name of the node
            condition: The condition to evaluate for continuing the loop. Can be either:
                - A simple comparison in the format "left_value operator right_value"
                - A dictionary with keys "left_value", "operator", and "right_value"
            body_nodes: List of node IDs to execute in each iteration
        ...
        if 'condition' not in node_config:
            node_config['condition'] = condition
        if 'body_nodes' not in node_config:
            node_config['body_nodes'] = body_nodes
        if 'max_iterations' not in node_config:
            node_config['max_iterations'] = max_iterations

        # Set optional configuration if not already in config
        if 'store_results' not in node_config:
            node_config['store_results'] = store_results
        if 'aggregate_results' not in node_config:
            node_config['aggregate_results'] = aggregate_results

        print(f"DEBUG: add_loop_node config = {node_config}")
