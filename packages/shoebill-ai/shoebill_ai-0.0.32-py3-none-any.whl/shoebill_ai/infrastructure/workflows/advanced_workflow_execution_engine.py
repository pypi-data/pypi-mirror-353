...
class AdvancedWorkflowExecutionEngine(WorkflowExecutionEngine):
    ...
    async def execute_node(self, 
        ...
        input_data = await self._prepare_node_input(workflow, node, context)
    ...
    async def _prepare_node_input(self, workflow: Workflow, node: WorkflowNode, context: WorkflowExecutionContext) -> Dict[str, Any]:
        """
        ...
        for edge in input_edges:
            ...
            # Check if source_value is a coroutine and await it
            if asyncio.iscoroutine(source_value):
                self.logger.debug(f"Awaiting coroutine in _prepare_node_input for edge {edge.edge_id}")
                source_value = await source_value
