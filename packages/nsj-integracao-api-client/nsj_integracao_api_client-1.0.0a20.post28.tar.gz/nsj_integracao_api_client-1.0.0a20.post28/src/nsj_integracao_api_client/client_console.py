...
if not ONLY_CONSOLE:
    ...
    from PyQt5.QtWidgets import QApplication, QTableWidgetItem, QCheckBox, QTableWidget
...
class ClientConsole:
    ...
    def ativar_grupos_empresariais(self, grupos: List[str], env: integrador.Environment):
        self.mensagem(f"Ativando grupos empresariais: {grupos}")
        with InjectorFactory() as injector:
...
#color=94 if _is_console else 0
    ...
    def executar_integracao(
        ...
        with InjectorFactory() as injector:
            if entidades:
                ...
                _, tempo = self.get_integrador(injector, env, forca_tenant, verboso, data_no_log).executar_integracao()
            else:
    ...
    def executar_carga_inicial(
        ...
        with InjectorFactory() as injector:
            _, tempo = self.get_integrador(injector, env, forca_tenant, verboso, data_no_log).executar_carga_inicial(entidades)
            self.mensagem(tempo)
    ...
    def _acompanhamento_carga_inicial_presenter(self, view: acompanhamento_window.Ui_FormAcompanhamento):
        ...
        def _carga_inicial():
            try:
                self.executar_carga_inicial([], self._env, self._tenant_espelho, True, True)
            except Exception as e:
                _msg = e.args[0] if e.args and len(e.args) > 0 else str(e)
                self.mensagem(_msg)
                self.mensagem_erro(_msg)
    ...
    def _acompanhamento_integracao_presenter(self, view: acompanhamento_window.Ui_FormAcompanhamento):
        ...
        def _integracao():
            try:
                self.executar_integracao([], self._env, self._tenant_espelho, True, True)
            except Exception as e:
                _msg = e.args[0] if e.args and len(e.args) > 0 else str(e)
                self.mensagem(_msg)
                self.mensagem_erro(_msg)
