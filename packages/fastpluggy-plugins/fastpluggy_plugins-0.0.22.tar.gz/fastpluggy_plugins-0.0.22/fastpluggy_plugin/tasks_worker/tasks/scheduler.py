            ...
            with session_scope() as db:
                ...
                if not all_tasks:
                    ...
                    for sched in all_tasks:
                        try:
                            expected = sched.expected_next_run
                            print(f"[SCHEDULER] Expected next run for {sched.name}: {expected} (now : {now_utc}")
                            if expected is None:
                                # No schedule configured (cron and interval both null)
                                continue
