...
def _parse_time_range(
    ...
    """
    Parse start and end time parameters supporting both ISO format and relative times.
    ...
    Returns:
      A tuple (parsed_start, parsed_end), where each is either a datetime (naive, UTC)
    ...
    parsed_start = parse_single_time(start_time)
    parsed_end = parse_single_time(end_time)
...
def _fetch_reports_by_task(
    ...
    #    extra_context->>'schedule_id'  yields TEXT; we'll cast it to Integer.
...
def _has_active_filters(criteria: FilterCriteria) -> bool:
...
class TaskData(BaseModel):
    ...
    @classmethod
    def from_db(cls, sched: ScheduledTaskDB, reports: List[TaskReportDB]) -> "TaskData":
        """
        ...
        runs = []
        for report in reports[:30]:  # Limit to last 30 runs
            activity = ActivityData.from_report(report, name)
        ...
        if len(incident_list) < 30:
            incident_list.extend(["good"] * (30 - len(incident_list)))
        incidents = incident_list[:30]  # Ensure we don't exceed 30
...
class ActivityData(BaseModel):
    ...
    @classmethod
    def from_report(cls, report: TaskReportDB, task_name: str) -> "ActivityData":
        """
...
class MonitorData(BaseModel):
    ...
    def create(
            cls,
            scheduled_tasks: List[ScheduledTaskDB],
            reports_by_task: Dict[str, List[TaskReportDB]],
    ) -> "MonitorData":
        """
        ...
        for sched in scheduled_tasks:
            ...
            task_data = TaskData.from_db(sched, reports)
        ...
        for report in recent_activities:
            try:
                task_id_int = int(report.task_id)
                task_name = task_name_map.get(task_id_int, f"Task #{report.task_id}")
            except (ValueError, TypeError):
                task_name = f"Task #{report.task_id}"

            activity = ActivityData.from_report(report, task_name)
