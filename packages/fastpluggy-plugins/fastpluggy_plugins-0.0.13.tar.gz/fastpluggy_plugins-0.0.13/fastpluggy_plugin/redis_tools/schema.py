...
class SlowLogEntry(BaseModel):
    id: int = Field(..., description="Unique slowlog entry ID")
    timestamp: int = Field(..., description="Unix timestamp when the command was executed")
    execution_time: int = Field(..., description="Execution time in microseconds")
    command: List[str] = Field(..., description="Array of command and arguments as run by Redis")
