...
from ..repository.schedule_monitoring import MonitorData, _build_filter_info, \
    _has_active_filters, _fetch_reports_by_task, _fetch_scheduled_tasks, FilterCriteria, \
    _parse_time_range
...
def scheduled_task_monitoring(
        ...
        task_name: Optional[str] = Query(None, description="Filter by task name (partial match)"),
        start_time: Optional[str] = Query(None, description="Start time (ISO format or relative like '1h', '7d')"),
        end_time: Optional[str] = Query(None, description="End time (ISO format or relative like '1h', '7d')"),
        # Data limits
        max_reports_per_task: int = Query(100, ge=1, le=200, description="Max reports per task"),
        max_activities: int = Query(50, ge=1, le=1000, description="Max activities for timeline"),
):
    """
    ...
    try:
        ...
        parsed_start_time, parsed_end_time = _parse_time_range(start_time, end_time)
        ...
        if not scheduled_tasks:
            ...
            reports_by_task = _fetch_reports_by_task(
                db, scheduled_tasks, max_reports_per_task, filter_criteria
            )
        ...
        return view_builder.generate(
            ...
            title=f'Scheduled Task Monitoring{" - Filtered" if _has_active_filters(filter_criteria) else ""}',
        )
        ...
        logger.error(f"Error in scheduled_task_monitoring: {e}", exc_info=True)

        # Return minimal error state
        empty_monitor_data = MonitorData.create(
            scheduled_tasks=[],
            reports_by_task={},
        )
