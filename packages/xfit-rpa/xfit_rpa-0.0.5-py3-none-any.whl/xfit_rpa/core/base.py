...
class XExecutor:
    ...
    def execute(self, action: 'XAction', context: XContext):
        ...
        self._execute_locator(element, action)

    def _resolve_locator(self, action: 'XAction', context: XContext) -> Any:
        """解析元素定位器"""
        _method_name = action.selector.get("method", None)
        if not _method_name:
            _method_name = 'find_by_xpath'
        if context.locator is not None:
            _method = getattr(context.locator, _method_name, None)
            locator = _method(action.selector.get('value'))
        else:
            _method = getattr(context.page, _method_name, None)
            locator = _method(action.selector.get('value'))
        return locator

    def _execute_locator(self, locator, params: {}, action: 'XAction'):
        # 根据不同类型执行操作
        if callable(action.action):
            action.action()
        elif isinstance(action.action, dict):
            if action.action.type == ActionType.CLICK:
                locator.click()
            elif action.action.type == ActionType.HOVER:
                locator.hover()
            elif action.action.type == ActionType.FILL:
                locator.fill(params.get("value", ""))
            elif action.action.type == ActionType.SCROLL:
                locator.scroll_into_view_if_needed()
            elif action.action.type == ActionType.WAIT:
                locator.wait_for(timeout=params.get("timeout", 5000))
            # ...其他类型处理...
            elif action.action.type != ActionType.CONTAINER:
                raise NotImplementedError(f"Action type {action.action.type} not implemented")
...
class XAction(metaclass=_ClassMeta):
    ...
    def __post_init__(self):
        ...
        if not self.action:
            self.action = {type: ActionType.CONTAINER}
    ...
    def _do_execute(self, context):
        ...
        if context.executor:
            # 使用执行器执行动作
            context.executor.execute(self, context)
...
class XApp(metaclass=_ClassMeta):
    ...
    def run(self, page=None, executor=None, global_params: Dict[str, Any] = None):
        context = XContext(
            page=page,
            executor=executor,
...
class XEngine:
    ...
    executor_class: Type[XExecutor] = XExecutor
    ...
    def run_all(self, page: [Any, None] = None, global_params: Dict[str, Any] = {}, executor=None):
        """并行运行所有App"""
        if executor is None and self.executor_class:
        ...
        if len(self.account_list) > 0:
            for account_info in self.account_list:
                ...
                for app in self.apps:
                    tasks.append(app.run(page, executor, account_params))
        else:
            for app in self.apps:
                tasks.append(app.run(page, executor, global_params))
