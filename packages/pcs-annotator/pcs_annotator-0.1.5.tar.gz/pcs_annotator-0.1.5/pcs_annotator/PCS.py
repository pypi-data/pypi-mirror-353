...
class PCS:
    ...
    def get_pcs_label(self, llm_labels, w_llms=None, w_mrs=None, possible_labels=None):
        ...
        for i in range(len(w_llms)):
            # Step 1: Rank MRs based on their weights
            mrs_indices = np.argsort(w_mrs)[::-1]  # Sort in descending order
            
            # Get the labels from each MR for this LLM
            mr_predictions = [llm_labels[i][j].strip().lower() for j in range(len(w_mrs))]
            
            # Reorder predictions by MR weight
            ranked_predictions = [mr_predictions[j] for j in mrs_indices]
            
            # Step 2-4: Implement the voting logic for 4 MRs
            selected_label = None
            
            # Check if top two MRs choose the same label
            if ranked_predictions[0] == ranked_predictions[1]:
                selected_label = ranked_predictions[0]
            # Check if the other two MRs (3rd and 4th) choose the same label
            elif ranked_predictions[2] == ranked_predictions[3]:
                selected_label = ranked_predictions[2]
            else:
                # Default to first (highest weighted) MR's label
                selected_label = ranked_predictions[0]
            
            # Convert the selected label to one-hot encoding
            pcs_label_llm = np.zeros(num_labels)
            if selected_label:
                for k in range(num_labels):
                    if selected_label == possible_labels[k].strip().lower():
                        pcs_label_llm[k] = 1.0
                        break
