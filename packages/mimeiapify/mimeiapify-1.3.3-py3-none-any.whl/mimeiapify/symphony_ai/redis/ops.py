    ...
    async with RedisClient.connection() as redis:
        try:
            ...
            if not isinstance(value, (str, bytes, int, float)):
                    ...
                    logger.warning(f"RedisCoreMethods.set received non-primitive type for key '{key}'. Attempting str conversion. Type: {type(value)}")
                    value = str(value)
...
async def delete(*keys: str) -> int:
    ...
    async with RedisClient.connection() as redis:
        try:
            ...
            logger.error(f"Redis DELETE error for keys starting with '{keys[0]}...': {e}", exc_info=True)
            return 0
            ...
            async with redis.pipeline(transaction=True) as pipe:
                ...
                for k, v in mapping.items():
                    if not isinstance(v, (str, bytes, int, float)):
                        logger.warning(f"RedisCoreMethods.hset_with_expire received non-primitive type for field '{k}' in mapping for key '{key}'. Attempting str conversion. Type: {type(v)}")
...
async def hincrby_with_expire(key: str, field: str, increment: int, ttl: int) -> Tuple[Optional[int], bool]:
    ...
    async with RedisClient.connection() as redis:
        try:
            ...
            incr_res = int(results[0]) if results and len(results) > 0 and results[0] is not None else None
            expire_res = bool(results[1]) if results and len(results) > 1 else False
            return incr_res, expire_res
        except Exception as e: # Catches WRONGTYPE from HINCRBY within execute or connection errors
...
async def rpush_and_sadd(list_key: str, list_values: Sequence[str], set_key: str, set_members: Sequence[str]) -> Tuple[Optional[int], Optional[int]]:
    ...
    if not list_values or not set_members:
        logger.warning(f"rpush_and_sadd called with empty list_values or set_members. ListKey: '{list_key}', SetKey: '{set_key}'. Skipping pipeline.")
    ...
    async with RedisClient.connection() as redis:
        try:
            ...
            for v in list_values:
                if not isinstance(v, (str, bytes, int, float)):
                        logger.warning(f"RedisCoreMethods.rpush_and_sadd received non-primitive type in list_values for key '{list_key}'. Attempting str conversion. Type: {type(v)}")
            ...
            for m in set_members:
                if not isinstance(m, (str, bytes, int, float)):
                        logger.warning(f"RedisCoreMethods.rpush_and_sadd received non-primitive type in set_members for key '{set_key}'. Attempting str conversion. Type: {type(m)}")
            ...
            logger.error(f"Redis RPUSH+SADD pipeline error for list '{list_key}', set '{set_key}': {e}", exc_info=True)
...
async def scan_keys(match_pattern: str, cursor: Union[str, bytes, int] = 0, count: Optional[int] = None) -> Tuple[Union[str, int], List[str]]:
    ...
    async with RedisClient.connection() as redis:
        try:
            ...
            next_cursor, keys_batch = await redis.scan(cursor=current_cursor, match=match_pattern, count=count)
            ...
            if isinstance(next_cursor, bytes):
                next_cursor = next_cursor.decode('utf-8')
                    ...
                    next_cursor = str(next_cursor) # Return as string for consistency with potential byte cursor

            # Ensure keys are strings (should be if decode_responses=True)
            keys_batch_str = [k if isinstance(k, str) else k.decode('utf-8') for k in keys_batch]

            return next_cursor, keys_batch_str
        except Exception as e:
            logger.error(f"Redis SCAN error for pattern '{match_pattern}' with cursor '{current_cursor}': {e}", exc_info=True)
            return '0', [] # Return '0' cursor and empty list to signal completion/error
...
async def hset(key: str, field: Optional[str] = None, value: Optional[str] = None, mapping: Optional[Mapping[str, str]] = None) -> int:
    ...
    if mapping is None and (field is None or value is None):
        logger.error(f"HSET requires either 'field' and 'value', or 'mapping' for key '{key}'")
    ...
    async with RedisClient.connection() as redis:
        try:
            ...
            if mapping:
                ...
                for k, v in mapping.items():
                    if not isinstance(v, (str, bytes, int, float)):
                        logger.warning(f"RedisCoreMethods.hset received non-primitive type for field '{k}' in mapping for key '{key}'. Attempting str conversion. Type: {type(v)}")
                        checked_mapping[k] = str(v)
                    else:
                ...
                # Handle single field/value
                    if not isinstance(value, (str, bytes, int, float)):
                        logger.warning(f"RedisCoreMethods.hset received non-primitive type for field '{field}' for key '{key}'. Attempting str conversion. Type: {type(value)}")
...
async def rpush(key: str, *values: str) -> int:
    ...
    async with RedisClient.connection() as redis:
        try:
                ...
                for v in values:
                    if not isinstance(v, (str, bytes, int, float)):
                        logger.warning(f"RedisCoreMethods.rpush received non-primitive type in values for key '{key}'. Attempting str conversion. Type: {type(v)}")
...
async def lpop(key: str, count: Optional[int] = None) -> Optional[Union[str, List[str]]]:
    ...
    async with RedisClient.connection() as redis:
        try:
            ...
            logger.error(f"Redis LPOP error for key '{key}' with count {count}: {e}", exc_info=True)
            return None
...
async def sadd(key: str, *members: str) -> int:
    ...
    async with RedisClient.connection() as redis:
        try:
            ...
            for m in members:
                    if not isinstance(m, (str, bytes, int, float)):
                        logger.warning(f"RedisCoreMethods.sadd received non-primitive type in members for key '{key}'. Attempting str conversion. Type: {type(m)}")
...
async def srem(key: str, *members: str) -> int:
    ...
    async with RedisClient.connection() as redis:
        try:
            ...
            for m in members:
                    if not isinstance(m, (str, bytes, int, float)):
                        logger.warning(f"RedisCoreMethods.srem received non-primitive type in members for key '{key}'. Attempting str conversion. Type: {type(m)}")
                        checked_members.append(str(m))
                    else:
                        checked_members.append(m)
