...
class GrowwAPI:
    ...
    @staticmethod
    def get_access_token(api_key: str, totp: str) -> dict:
        """
        Args:
            api_key (str): Bearer token or API key for the Authorization header.
            totp (str): The TOTP code as a string.
        Returns:
        ...
        if response.status_code == 400:
            try:
                msg = response.json().get('error', {}).get('displayMessage', 'Bad Request')
            except Exception:
                msg = 'Bad Request'
            raise GrowwAPIException(code=str(response.status_code), msg=f"Groww API Error 400: {msg}")
        if response.status_code in GrowwAPI._ERROR_MAP:
            raise GrowwAPI._ERROR_MAP[response.status_code]()
        if not response.ok:
            raise GrowwAPIException(code=str(response.status_code), msg="The request to the Groww API failed.")
