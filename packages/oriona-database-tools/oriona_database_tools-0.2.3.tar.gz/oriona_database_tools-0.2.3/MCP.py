...
class MCPDatabaseComponent(Component):
    ...
    description = "Connect to database tools via MCP server for secure SQL query execution, table exploration, and data analysis with built-in safety features."
    icon = "Mcp"
    name = "MCPDatabase"
            ...
            if field_name in ("command", "sse_url", "mode"):
                try:
                    await self.update_tools(
                        ...
                        database_url=build_config["database_url"]["value"],
                    )
    ...
    async def _update_tool_config(self, build_config: dict, tool_name: str) -> None:
        ...
        if not self.tools:
            await self.update_tools(
                ...
                env=build_config["env"]["value"],
                headers=build_config["headers_input"]["value"],
                database_url_env=build_config["database_url_env"]["value"],
            )
    ...
    async def update_tools(
        ...
        try:
            ...
            # Ensure env is always a list
            env = env or []
            
            # Add database URL to environment if provided
            if database_url and database_url.strip():
                # Remove any existing DATABASE_URL entries first
                env = [var for var in env if not var.startswith('DATABASE_URL=')]
                env_var_string = f"DATABASE_URL={database_url}"
                env.append(env_var_string)
                logger.info(f"Added DATABASE_URL to environment (length: {len(database_url)} characters)")
            else:
                logger.warning("No database_url specified, MCP server may fail if it requires DATABASE_URL")
            
            # Final env list debug
            logger.info(f"Final env list length: {len(env)}")
                ...
                if not self.stdio_client.session:
                    try:
                        ...
                        if env:
                            ...
                            has_database_url = any(var.startswith('DATABASE_URL=') for var in env)
                            logger.info(f"DATABASE_URL present in environment: {has_database_url}")
                        else:
                        ...
                        if "DATABASE_URL environment variable is required" in error_msg:
                            msg = (
                                f"Database connection error: The MCP server requires a DATABASE_URL environment variable. "
                                f"Please ensure '{database_url_env or 'DATABASE_URL'}' is set in your Langflow environment. "
