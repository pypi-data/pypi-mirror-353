...
class MCPDatabaseComponent(Component):
    display_name = "MCP Database"
    description = "Connect to database tools via MCP server for secure SQL query execution, table exploration, and data analysis with built-in safety features. Supports PostgreSQL, MySQL, SQLite, and BigQuery."
    icon = "Mcp"
    name = "MCPDatabase"
    
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.schema_inputs: list[InputTypes] = []
        self.stdio_client: MCPStdioClient = MCPStdioClient()
        self.sse_client: MCPSseClient = MCPSseClient()
        self.tools: list = []
        self.tool_names: list[str] = []
        self._tool_cache: dict = {}  # Cache for tool objects
    ...
    inputs = [
        ...
        MessageTextInput(
            ...
            value="uvx oriona-database-tools",
            show=True,
            refresh_button=True,
        ),
        MessageTextInput(
        ...
        MessageTextInput(
            ...
            info="Database connection URL (PostgreSQL, MySQL, SQLite) - Not needed for BigQuery",
            value="",
            show=True,
            advanced=False,
        ),
        MessageTextInput(
            ...
            display_name="BigQuery Project ID",
        ...
        MessageTextInput(
            name="bigquery_credentials_base64",
            display_name="BigQuery Credentials (Base64)",
            info="Base64 encoded service account JSON (optional, uses default credentials if not provided)",
            value="",
            show=False,
            advanced=False,
            password=True,
        ...
        MessageTextInput(
            ...
            info="Default dataset name (optional)",
            value="",
            show=False,
            advanced=True,
        ),
        MessageTextInput(
            ...
            display_name="BigQuery Location",
            info="BigQuery location/region",
            value="US",
            show=False,
            advanced=True,
        ),
        ...
        DropdownInput(
            ...
            options=["Component", "Tool"],
            value="Component", 
            info="Select whether to use as component or tool",
            show=False,
            real_time_refresh=True,
        ),
        DropdownInput(
    ...
    def _build_environment_variables(self, env: list[str], database_type: str) -> list[str]:
        """Build environment variables based on database type."""
        env = env or []
        
        if database_type == "BigQuery":
            # Remove any existing database-related environment variables (including DATABASE_URL)
            env = [var for var in env if not any(var.startswith(prefix) for prefix in [
                'DATABASE_URL=', 'BIGQUERY_PROJECT_ID=', 'BIGQUERY_CREDENTIALS_BASE64=',
                'BIGQUERY_DATASET=', 'BIGQUERY_LOCATION='
            ])]
            
            # Add BigQuery-specific environment variables (DATABASE_URL not needed)
            if hasattr(self, 'bigquery_project_id') and self.bigquery_project_id:
                env.append(f"BIGQUERY_PROJECT_ID={self.bigquery_project_id}")
            
            if hasattr(self, 'bigquery_credentials_base64') and self.bigquery_credentials_base64:
                env.append(f"BIGQUERY_CREDENTIALS_BASE64={self.bigquery_credentials_base64}")
            
            if hasattr(self, 'bigquery_dataset') and self.bigquery_dataset:
                env.append(f"BIGQUERY_DATASET={self.bigquery_dataset}")
            
            if hasattr(self, 'bigquery_location') and self.bigquery_location:
                env.append(f"BIGQUERY_LOCATION={self.bigquery_location}")
                
        else:  # SQL databases
            # Remove any existing BigQuery environment variables
            env = [var for var in env if not any(var.startswith(prefix) for prefix in [
                'BIGQUERY_PROJECT_ID=', 'BIGQUERY_CREDENTIALS_BASE64=',
                'BIGQUERY_DATASET=', 'BIGQUERY_LOCATION='
            ])]
            
            # Add DATABASE_URL if provided
            if hasattr(self, 'database_url') and self.database_url and self.database_url.strip():
                env = [var for var in env if not var.startswith('DATABASE_URL=')]
                env.append(f"DATABASE_URL={self.database_url}")
        ...
        try:
            if field_name == "mode":
                ...
                if field_value == "Stdio":
                    ...
                    build_config["database_type"]["show"] = True
                    # Show appropriate database fields based on current database_type
                    current_db_type = build_config.get("database_type", {}).get("value", "SQL")
                    self._update_database_fields_visibility(build_config, current_db_type)
                    ...
                    build_config["database_type"]["show"] = False
                    self._hide_all_database_fields(build_config)
                    build_config["sse_url"]["show"] = True
                    build_config["sse_url"]["value"] = "MCP_SSE"
                    build_config["headers_input"]["show"] = True
                    return build_config
                    
            elif field_name == "database_type":
                self._update_database_fields_visibility(build_config, field_value)
                # Reset tool options when database type changes
                build_config["tool"]["options"] = []
                return build_config
                
            if field_name in ("command", "sse_url", "mode", "database_type", "database_url", 
                             "bigquery_project_id", "bigquery_credentials_base64"):
                try:
                    await self.update_tools(
                        ...
                        database_type=build_config.get("database_type", {}).get("value", "SQL"),
                    )
    ...
    def _update_database_fields_visibility(self, build_config: dict, database_type: str) -> None:
        """Update visibility of database fields based on database type."""
        if database_type == "BigQuery":
            # Hide SQL fields
            build_config["database_url"]["show"] = False
            # Show BigQuery fields
            build_config["bigquery_project_id"]["show"] = True
            build_config["bigquery_credentials_base64"]["show"] = True
            build_config["bigquery_dataset"]["show"] = True
            build_config["bigquery_location"]["show"] = True
        else:  # SQL
            # Show SQL fields
            build_config["database_url"]["show"] = True
            # Hide BigQuery fields
            build_config["bigquery_project_id"]["show"] = False
            build_config["bigquery_credentials_base64"]["show"] = False
            build_config["bigquery_dataset"]["show"] = False
            build_config["bigquery_location"]["show"] = False
    
    def _hide_all_database_fields(self, build_config: dict) -> None:
        """Hide all database-related fields."""
        build_config["database_url"]["show"] = False
        build_config["bigquery_project_id"]["show"] = False
        build_config["bigquery_credentials_base64"]["show"] = False
        build_config["bigquery_dataset"]["show"] = False
    ...
    async def update_tools(
        ...
        try:
            ...
            if database_type is None:
                database_type = getattr(self, 'database_type', 'SQL')
            
            # Build environment variables based on database type
            env = self._build_environment_variables(env, database_type)
            ...
            if mode == "Stdio":
                if not self.stdio_client.session:
                    try:
                        # Debug: Log the command and environment variables being passed
                        logger.info(f"Connecting to MCP server with command: {command}")
                        logger.info(f"Database type: {database_type}")
                        if env:
                            env_preview = [f"{var.split('=')[0]}=<redacted>" if '=' in var else var for var in env]
                            logger.info(f"Environment variables being passed: {env_preview}")
                        else:
                            logger.info("No environment variables being passed")
                        
                        # Validate environment variables format
                        if env:
                            for var in env:
                                if '=' not in var:
                                    logger.warning(f"Invalid environment variable format: {var} (should be KEY=VALUE)")
                        
                        # Pass environment variables as list (original format)
                        self.tools = await self.stdio_client.connect_to_server(command, env)
                    except ValueError as e:
                        error_msg = str(e)
                        if "DATABASE_URL environment variable is required" in error_msg:
                            if database_type == "SQL":
                                msg = (
                                    f"SQL Database connection error: Please provide a valid DATABASE_URL. "
                                    f"Example: postgresql://user:pass@localhost:5432/mydb"
                                )
                            else:  # This shouldn't happen for BigQuery, but just in case
                                msg = (
                                    f"Configuration error: DATABASE_URL is not needed for BigQuery. "
                                    f"Please ensure BigQuery Project ID is provided instead."
                                )
                        elif "BIGQUERY_PROJECT_ID is required" in error_msg:
                            msg = (
                                f"BigQuery connection error: Please provide a valid BigQuery Project ID. "
                                f"This is required when using BigQuery database type."
                            )
                        else:
                            msg = f"Error connecting to MCP server: {error_msg}"
                        logger.exception(msg)
                        raise ValueError(msg) from e
