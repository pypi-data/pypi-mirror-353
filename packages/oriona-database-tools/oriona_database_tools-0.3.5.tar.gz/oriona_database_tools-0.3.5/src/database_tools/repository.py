...
class DatabaseRepository:
    ...
    def get_table_columns(self, table_name: str) -> List[Dict[str, Any]]:
        ...
        # Special handling for BigQuery to avoid REGEXP_EXTRACT issues with STRUCT types
        if self.config.is_bigquery:
            try:
                # Parse table name to get dataset and table
                if '.' in table_name:
                    dataset, table = table_name.split('.', 1)
                else:
                    dataset = self.config.bigquery_config.dataset if self.config.bigquery_config else None
                    table = table_name
                
                if not dataset:
                    # Fall back to standard method if no dataset specified
                    return self._get_columns_standard(table_name)
                ...
                SELECT 
                    ...
                    is_nullable,
                    column_default
                FROM `{self.config.bigquery_config.project_id}.{dataset}.INFORMATION_SCHEMA.COLUMNS`
                ...
                logger.warning(f"Failed to get BigQuery columns via INFORMATION_SCHEMA: {e}")
                # Fall back to standard method
                return self._get_columns_standard(table_name)
        else:
            return self._get_columns_standard(table_name)
    
    def _get_columns_standard(self, table_name: str) -> List[Dict[str, Any]]:
        """Get column information using standard SQLAlchemy inspector."""
        columns = []
    ...
    def get_primary_keys(self, table_name: str) -> List[str]:
        ...
        # BigQuery doesn't have traditional primary keys, return empty list
        if self.config.is_bigquery:
            return []
    ...
    def get_foreign_keys(self, table_name: str) -> List[Dict[str, Any]]:
        ...
        # BigQuery doesn't have traditional foreign keys, return empty list
        if self.config.is_bigquery:
            return []
