...
class Settings(BaseSettings):
    ...
    def get_database_config(self) -> DatabaseConfig:
        ...
        if self.database_url.startswith("bigquery://") or self.bigquery_project_id:
            if not self.bigquery_project_id:
                raise ValueError("BIGQUERY_PROJECT_ID is required for BigQuery connections")
            ...
            # If using BigQuery, construct the connection URL
            if self.database_url.startswith("bigquery://"):
                ...
                self.database_url = f"bigquery://{self.bigquery_project_id}{dataset_part}"
