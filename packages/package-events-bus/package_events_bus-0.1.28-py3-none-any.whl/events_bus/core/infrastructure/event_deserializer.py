...
class EventDeserializer:
    ...
    def _get_event_type_from_handler(
        ...
        if not isinstance(handler, BaseHandler):
            raise TypeError(f"Expected an instance of BaseHandler, got {type(handler)}")   # noqa: E501
    ...
    def get_event_type_from_class_handler(
        ...
        if method is None:
            raise ValueError(f"Handler {cls.__name__} does not have a 'handle' method.")  # noqa: E501
    ...
    def _validate_and_extract_data(json_obj: dict) -> dict:
        ...
        if "detail" not in json_obj:
            raise ValueError(
                'Invalid JSON format: "detail" key not found.'
            )
        data = json_obj["detail"]
        missing_keys = [
            key for key in ["attributes", "id", "occurred_on"] if key not in data  # noqa: E501
        ]
        if missing_keys:
            raise ValueError(
    ...
    def get_event_type_json(msg: str) -> str:
        ...
        if "detail-type" not in json_obj:
            raise ValueError('Invalid JSON format: "detail-type" key not found.')  # noqa: E501
