...
from .operations import filter_collection_with_ids, save_df_local
...
async def input_fun(julius_app, collections: List[Tuple[str, ...]], logger: Logger) -> Tuple[bool, List[Optional[Tuple[Optional[str], ...]]], Dict[str, Any]]:
    async def __input_fun(collections) -> Tuple[bool, List[Optional[Tuple[Optional[str], ...]]]]:
        ...
        if any([x is None for x in collections]):
            raise EntityException(f"collection not found in config (app id={julius_app.app_id})")
        ...
        collections = []
        if input_mode == InputWrapper.INPUT_DOC:        # TODO remove?
            log_debug(f"{julius_app._input_id} in {julius_app.app_id} started", logger)
            ...
            for i, jdf in enumerate(jdfs_list):
                if isinstance(jdf, DF):
                    ...
                    if julius_app.input_df_by_args:
                        ...
                        if len(ids_list) != len(df_collections):
                            raise EntityException(f"wrong input result collections size: expected {len(df_collections)}, found {len(ids_list)}")
            ...
            if isinstance(new_dfs, Tuple) or isinstance(new_dfs, DFS) or isinstance(new_dfs, List) and len(new_dfs) > 0 and\
                ...
                for new_df_i in new_dfs:
                    if isinstance(new_df_i, OBJ):
                        ...
                        assert isinstance(new_df_i, pd.DataFrame) or isinstance(new_df_i, Doc) or isinstance(new_df_i, Docs) or issubclass(new_df_i.__class__, BaseModel) or isinstance(new_df_i, Dict) or isinstance(new_df_i, List), f"processor should return pd.DataFrame/OBJ/Doc/BaseModel/Dict or list/tuple/DFS of them, found {type(new_df_i)}"
