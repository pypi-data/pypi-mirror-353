...
from ETL.commons.reformat_data import (
    reformat_dict_data_field_types, none_empty_dict_fields, check_for_dict_data_expected_fields, uningested_fields
)

set_logger_level_error(locals())
script_path = f"{os.path.abspath('')}/all_ms_integration_json.py"
etl_data_name = "ms_integration_json"


def get_source_files(env):
    breadcrumbs = ["landing/financial_statement/ms-integration/aj_bell"]
...
def preprocess_dict_data(dict_data: dict, table_schema: StructType, unappendable_parent_keys: list) -> list:
    dict_list = normalize_dict_data(dict_data, unappendable_parent_keys)
...
def etl_steps(spark: SparkSession, file_path: str, data_collected: dict) -> Tuple[list, dict]:
    ...
    dict_list = preprocess_dict_data(dict_data, table_schema, table_class.unappendable_parent_keys)
