...
from ETL.scripts.asset.df_utils.morningstar_IFLS_regional_asset_allocation_df_utils import ColumnClasses, MorningstarXMLTable, db_name

script_path = f"{os.path.abspath('')}/morningstar_IFLS_regional_asset_allocation.py"
...
def create_cols_list_for_star_expand(unq_id, new_list_of_cols, final_df):
    '''
    function performs a star expand on all columns not in unq_id list

    arg:
    unq_id -> list of columns that do not require exploding
    new_list_of_cols -> empty list which is used to append values within the for loop
    final_df -> Dataframe containing all data
...
def clean_data(spark: SparkSession, sc: SparkContext, nested_col_name: str, field_name: list) -> Tuple[DataFrame, float]:
    ...
    unq_id = ['isin', 'msid']
    new_list_of_cols = []
    create_cols_list_for_star_expand(unq_id, new_list_of_cols, final_df)
    final_df = final_df.select(*new_list_of_cols)

    final_df = explode_array_and_map_columns(final_df)
    
    unq_id = ['isin', 'msid', 'RegionId']
    new_list_of_cols = []
    create_cols_list_for_star_expand(unq_id, new_list_of_cols, final_df)
    final_df = final_df.select(*new_list_of_cols)
...
@log_general_info(
    env=locals(),
    etl_data="morningstar_IFLS_regional_asset_allocation",
