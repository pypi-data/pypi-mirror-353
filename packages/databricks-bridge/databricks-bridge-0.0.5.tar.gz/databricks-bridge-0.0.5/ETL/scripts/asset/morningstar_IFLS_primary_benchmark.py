...
from ETL.scripts.asset.df_utils.morningstar_IFLS_primary_benchmark_df_utils import ColumnClasses, MorningstarXMLTable, db_name

script_path = f"{os.path.abspath('')}/morningstar_primary_benchmark.py"
...
def clean_data(spark: SparkSession, sc: SparkContext, nested_col_name: str, field_name: list) -> Tuple[DataFrame, float]:
    query = f"""
            ...
            , fund_share_class__fund_id
            , fund_share_class__id
            , LEFT(RIGHT(file_path, INSTR(REVERSE(file_path),'/')-1), INSTR(RIGHT(file_path, INSTR(REVERSE(file_path), '/')-1), '.') - 1) AS isin 
            from loaded.asset_morningstar_xml_isin_dataoutput where fund_share_class__id is not null and {nested_col_name} is not null
            """
    ...
    for row in data:
        ...
        json_data['fund_share_class__fund_id'] = row['fund_share_class__fund_id']
        json_data['fund_share_class__id'] = row['fund_share_class__id']
    ...
    final_df = df.select(unq_id, 'fund_share_class__fund_id', 'fund_share_class__id')
    ...
    exploded_df = final_df.select(final_df.isin
                                , explode(final_df.columns[-1]).alias('exploded_HoldingDetail'))
 
    exploded_df = exploded_df.select(exploded_df.isin
                                    , exploded_df.exploded_HoldingDetail.Id.alias('PrimaryBenchmark_Id')
                                    , exploded_df.exploded_HoldingDetail.SecurityName.alias('PrimaryBenchmark_Name')
                                    , exploded_df.exploded_HoldingDetail.Weighting.alias('PrimaryBenchmark_Weighting'))
    
    final_df = final_df.join(exploded_df
                            , final_df.isin == exploded_df.isin
                            , "left").drop(final_df.isin).drop(final_df.fund_share_class__fund__fund_basics_fund_share_class__fund__fund_basics_PrimaryBenchmark_HoldingDetail)
    
    # drop rows with null isin
    final_df = final_df.where(final_df.isin.isNotNull())
...
def rename_df(df: DataFrame, schema: StructType) -> DataFrame:
    ...
    for col_name in df_snake_case.columns:
        df_snake_case = df_snake_case.withColumnRenamed(col_name, col_name.replace('fund_share_class__','').replace('fund__fund_basics_fund__fund_basics__','').replace('__', '_'))
...
@log_general_info(
    env=locals(),
    etl_data="morningstar_primary_benchmark",
