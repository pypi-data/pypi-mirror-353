...
from ETL.scripts.asset.df_utils.morningstar_IFLS_asset_allocation_df_utils import ColumnClasses, MorningstarXMLTable, db_name

script_path = f"{os.path.abspath('')}/morningstar_IFLS_asset_allocation.py"
...
def clean_data(spark: SparkSession, sc: SparkContext, nested_col_name: str, field_name: list) -> Tuple[DataFrame, float]:
    ...
    for i in field_name:
        ...
        final_df = final_df.select(final_df.isin, final_df.msid, final_df.exploded_BreakdownValue.type.alias('type'), final_df.exploded_BreakdownValue.value.alias('value'))
...
def rename_df(df: DataFrame, schema: StructType) -> DataFrame:
    for col_name in df.columns:
        new_col_name = col_name.split('_')[-1]
...
@log_general_info(
    env=locals(),
    etl_data="morningstar_IFLS_asset_allocation",
