...
from pyspark.sql.functions import arrays_zip, explode, split, col
...
from ETL.scripts.asset.df_utils.morningstar_IFLS_geo_exp_df_utils import ColumnClasses, MorningstarXMLTable, db_name

script_path = f"{os.path.abspath('')}/morningstar_IFLS_geo_exp.py"
...
def clean_data(spark: SparkSession, sc: SparkContext, nested_col_name: str, field_name: list) -> Tuple[DataFrame, float]:
    ...
    for i in field_name:
        ...
        final_df = final_df.withColumnRenamed(final_df.columns[-1], 'BreakdownValue')
        final_df = final_df.select(final_df.isin
                                    , explode(final_df.BreakdownValue).alias('exploded_BreakdownValue'))
        final_df = final_df.select(final_df.isin
                                    , final_df.exploded_BreakdownValue.GeographicZoneBreakdownValue.type.alias('GeoZone_type')
                                    , final_df.exploded_BreakdownValue.GeographicZoneBreakdownValue.value.alias('GeoZone_value'))
        final_df = final_df.withColumn("new"
                                        , arrays_zip("GeoZone_type", "GeoZone_value")).withColumn("new"
                                                                                                    , explode("new")).select("isin"
                                                                                                                            , col("new.GeoZone_type").alias("GeoZone_type")
                                                                                                                            , col("new.GeoZone_value").alias("GeoZone_value"))
...
@log_general_info(
    env=locals(),
    etl_data="morningstar_IFLS_geo_exp",
