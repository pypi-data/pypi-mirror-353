...
from pyspark.sql.functions import arrays_zip, explode_outer, expr
...
from ETL.scripts.asset.df_utils.morningstar_IFLS_Industry_Group_df_utils import ColumnClasses, MorningstarXMLTable, db_name

script_path = f"{os.path.abspath('')}/morningstar_IFLS_Industry_Group.py"
...
def etl_steps(spark: SparkSession, sc: SparkContext, column_classes: list, data_sources_duration_s: float) -> float:
    ...
    renamed_df = renamed_df.withColumnRenamed("type", "industry_group_type")\
                           .withColumnRenamed("value", "industry_group_value")
...
@log_general_info(
    env=locals(),
    etl_data="morningstar_IFLS_Industry_Group",
