...
from pyspark.sql.functions import arrays_zip, explode_outer, col
...
from ETL.scripts.asset.df_utils.morningstar_IFLS_equity_style_df_utils import ColumnClasses, MorningstarXMLTable, db_name

script_path = f"{os.path.abspath('')}/morningstar_IFLS_equity_style.py"
...
def clean_data(spark: SparkSession, sc: SparkContext, nested_col_name: str, field_name: list) -> Tuple[DataFrame, float]:
    ...
    for row in data:
        '''
        for loop expands the the json data and creates a list named data_list to be used to create a Dataframe with struct fields
        '''
    ...
    final_df = df.select(unq_id) 
    for i in field_name:
        '''
        for loop extracts all the specified fields in the original dataframe ready for the transformations and creates a Dataframe
        '''
        full_path = f'''{nested_col_name}.{nested_col_name}.{i}'''
        extracted_df = df.select(unq_id, full_path)
        final_df = final_df.join(extracted_df, final_df[unq_id] == extracted_df[unq_id], "left").drop(final_df[unq_id]).dropDuplicates()

    for i in range(2):
        '''
        for loop explodes all the nested columns. This needs to be done twice for this dataset hence why range() is set to 2. 
        '''
        final_df = final_df.withColumn("EquityStyleCoreValue", explode_outer(final_df.EquityStyleCoreValue)) \
                        .withColumn("SalePosition", explode_outer(final_df.SalePosition))

    final_df = final_df.select(unq_id
                                      , 'EquityStyleCoreValue.SizeScore'
                                      , 'EquityStyleCoreValue.StyleScore'
                                      , 'SalePosition')

    final_df = final_df.withColumn("new", arrays_zip("SizeScore", "StyleScore", "SalePosition"))\
                            .withColumn("new", explode_outer("new"))\
                            .select("isin", col("new.SizeScore").alias("SizeScore"), col("new.StyleScore").alias("StyleScore"), col("new.SalePosition").alias("SalePosition"))
    
    for i in range(2):
        '''
        for loop explodes all the nested columns. This needs to be done twice for this dataset hence why range() is set to 2. 
        '''
        final_df = final_df.withColumn("SizeScore", explode_outer(final_df.SizeScore)) \
                            .withColumn("StyleScore", explode_outer(final_df.StyleScore))
...
@log_general_info(
    env=locals(),
    etl_data="morningstar_IFLS_equity_style",
