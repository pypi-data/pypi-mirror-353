...
from ETL.scripts.asset.df_utils.morningstar_IFLS_credit_rating_df_utils import ColumnClasses, MorningstarXMLTable, db_name

script_path = f"{os.path.abspath('')}/morningstar_IFLS_credit_rating.py"
...
def clean_data(spark: SparkSession, sc: SparkContext, nested_col_name: str, field_name: list) -> Tuple[DataFrame, float]:
    ...
    for i in field_name:
        ...
        final_df = final_df.select(final_df.isin, explode(final_df.BreakdownValue).alias('exploded_BreakdownValue'))
        final_df = final_df.select(final_df.isin, final_df.exploded_BreakdownValue.type.alias('credit_rating_type'), final_df.exploded_BreakdownValue.value.alias('credit_rating_value'))
...
@log_general_info(
    env=locals(),
    etl_data="morningstar_IFLS_credit_rating",
