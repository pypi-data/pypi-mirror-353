...
from odrive.high_rate_capturer import high_rate_capture, high_rate_capture_start, HighRateCapturer, TimestampFmt
...
__all__ = [
    ...
    'to_sync', 'to_async',
    'high_rate_capture', 'high_rate_capture_start',
    'HighRateCapturer', 'TimestampFmt',
]
...
async def request_state(axis: AsyncObject, state: AxisState):
    ...
    while True:
        ...
        if time_passed >= timeout:
            raise TimeoutError(f"Axis failed to enter {repr(state)} after {time_passed} s: {await format_errors(axis._dev)}")
