...
class Request(PackageEnvInfo):
    ...
    def __init__(self, test_env: str = None):
        ...
        self.url = None
        self._env = test_env if test_env else PackageEnvInfo.TestEnv
    ...
    def re_param(self, url, **kwargs):
        ...
        """
        if "http" not in url:
            self.url = PackageEnvInfo.build_url(url, self._env)
        else:
            self.url = url

        if "headers" not in kwargs:
            self.session.headers = PackageEnvInfo.build_header(url, self._env)
    ...
    def _custom_logger(self, response):
        ...
        file_path = re.search(rf"{re.escape(str(PackageEnvInfo.project_root_dir))}(.+)", frame_info.filename).group(1)
    ...
    def format_res_log(res: Response) -> dict:
        ...
        if res.request.method == "GET":
            ...
            } if res.request.headers.get("content-type") == "application/json" else {
                "request": {
                    "url": str(res.request.url),
                    "headers": dict(res.request.headers)
                },
