...
# JSON ultra-rapide avec fallback
try:
    import orjson as json
    JSON_LOADS = json.loads
    JSON_DUMPS = lambda obj: json.dumps(obj).decode('utf-8')
    print("🚀 orjson activé pour JSON ultra-rapide")
except ImportError:
    import json
    JSON_LOADS = json.loads
...
# uvloop pour performance maximale (Linux/macOS)
try:
    import uvloop
    UVLOOP_AVAILABLE = True
    print("🚀 uvloop disponible pour event loop ultra-rapide")
except ImportError:
...
def setup_event_loop():
    """Configure l'event loop optimal"""
    if UVLOOP_AVAILABLE:
        uvloop.install()
        print("🚀 uvloop installé pour performance maximale")

if __name__ == "__main__":
    # Configuration du logging
    logging.basicConfig(level=logging.INFO)
    
    # Setup event loop optimal
    setup_event_loop()
