...
# Import custom tools
# Import upload folder tool
from box_folder_upload_tool import box_upload_folder_tool
# Import chunked upload tool for large files
from box_chunked_upload_tool import box_chunked_upload_tool, box_chunked_upload_new_version_tool
...
# Shared link tools are defined within this file
@mcp.tool()
...
@mcp.tool()
async def box_upload_folder_recursive_tool(
    ctx: Context,
    ...
    """Recursively upload a local folder and all its contents to Box.

    This tool will create folder structures matching the local directory structure
    and upload all files within them. It handles duplicate files by updating them
    with new versions and creates any missing folders.

    Args:
        local_folder_path (str): Path on the server filesystem to the folder to upload.
        box_folder_id (str): The ID of the destination Box folder. Defaults to root ("0").
    ...
    Returns:
        str: Summary of the upload operation including statistics on files/folders created and any errors.
    """
    return await box_upload_folder_tool(
        ctx,
        local_folder_path,
        box_folder_id,
        include_patterns,
...
@mcp.tool()
async def box_chunked_upload_file_tool(
    ctx: Context,
    file_path: str,
    folder_id: str = "0",
    new_file_name: str = "",
    ...
    """
    Upload a large file (>50MB) or video to Box using the chunked upload API.
    This method is more reliable for large files and allows for better error recovery.
    ...
    Args:
        ...
        num_threads (int): Number of threads to use for parallel uploading (1-10).
    
    Returns:
        str: Information about the uploaded file as a JSON string.
    """
    # Validate parameters
    if num_threads < 1 or num_threads > 10:
        return json.dumps({
            "status": "error",
            "message": "Number of threads must be between 1 and 10."
        })
    
    if chunk_size < 20 * 1024 * 1024:
        chunk_size = 20 * 1024 * 1024  # Enforce minimum 20MB chunk size

    return await box_chunked_upload_tool(
        ctx,
        file_path,
        folder_id,
        new_file_name,
...
async def box_chunked_upload_new_version_tool(
    ...
    """
    Upload a new version of a large file (>50MB) or video to Box using the chunked upload API.
    ...
    Returns:
        str: Information about the updated file as a JSON string.
    """
    # Validate parameters
    if num_threads < 1 or num_threads > 10:
        return json.dumps({
    ...
    return await box_chunked_upload_new_version_tool(
        ctx,
        file_path,
        file_id,
        new_file_name,
