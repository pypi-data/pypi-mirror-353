...
def configure_command() -> None:
    ...
    cfg["region"] = input("region: ") or cfg.get("region", "europe-west2")
    cfg["intellithing_key"] = input("intellithing_key: ") or cfg.get("intellithing_key")
    while True:
        secret_input = input("Secret (format: xxx-xxx, only letters/numbers, e.g., app-key1): ").strip()
        if not secret_input:
