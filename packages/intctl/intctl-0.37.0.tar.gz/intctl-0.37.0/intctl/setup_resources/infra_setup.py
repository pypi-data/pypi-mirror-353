    ...
    for _ in range(60):  # up to 5 minutes
        ...
        with Spinner("Checking peering status..."):
            peer_status = run(
                ...
                f"--format='value(name,state,peerNetwork)'"
            )

        if peer_status.returncode == 0:
            lines = peer_status.stdout.strip().splitlines()
            for line in lines:
                try:
                    name, state, peer_net = line.strip().split()
                    if "servicenetworking" in name and "servicenetworking" in peer_net and state == "ACTIVE":
                        print("✅ VPC peering is ACTIVE.")
                        break
                except ValueError:
                    continue  # In case of malformed line
            else:
