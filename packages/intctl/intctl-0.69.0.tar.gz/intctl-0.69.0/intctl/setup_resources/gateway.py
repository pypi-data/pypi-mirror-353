...
def setup_https_gateway(cfg: dict, status: StatusManager):
    ...
    # 1. Ensure the GKE Gateway Controller is enabled AND VERIFIED
    ...
    if update_result.returncode != 0:
        print("❌ Failed to send 'enable addon' command. Check permissions.")
        print(update_result.stderr)
        status.fail("https_gateway: gcloud update-addons command failed")
        raise typer.Exit(1)

    print("✅ GKE Gateway addon enabled. Verifying controller installation...")

    print("⏳ Waiting for the GKE Gateway controller to become ready...")
    for i in range(30): # Wait up to 10 minutes
        with Spinner(f"Checking for controller pod... (Attempt {i+1}/30)"):
            check_pods_result = run("kubectl get pods -n gke-gateway-system -o json")
            if check_pods_result.returncode == 0:
                pods_json = json.loads(check_pods_result.stdout)
        ...
        print("   Please check the cluster's status in the GCP console or run:")
        print("   kubectl get pods -n gke-gateway-system")
...
metadata:
  ...
  annotations:
    # THIS is the only thing that links the Gateway to the ManagedCertificate.
...
spec:
  ...
  - name: https
    ...
    port: 443
    tls:
      # The 'tls' block for a ManagedCertificate MUST only contain 'mode: Terminate'.
      # NO certificateRefs. NO options.
      mode: Terminate
    allowedRoutes:
      namespaces:
        from: All
