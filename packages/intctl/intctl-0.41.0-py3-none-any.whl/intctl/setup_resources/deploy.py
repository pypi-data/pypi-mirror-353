...
def transfer_and_deploy(cfg: dict, status: StatusManager):
    ...
    # Get the static IP for the gateway manager
    gateway_ip = ""
    gateway_address_name = f"gateway-manager-ip-{workspace}".lower()
    print(f"🔍 Fetching static IP for address '{gateway_address_name}'...")
    with Spinner(f"Fetching IP for {gateway_address_name}..."):
        ip_result = run(
            f"gcloud compute addresses describe {gateway_address_name} "
            f"--region={target_location} --project={target_project} --format='value(address)'"
        )
    if ip_result.returncode == 0:
        gateway_ip = ip_result.stdout.strip()
        print(f"✅ Found static IP: {gateway_ip}")
    else:
        print(f"❌ Could not find pre-configured static IP for '{gateway_address_name}'. Deployment may fail or use an ephemeral IP.")
        status.fail("deploy", f"Static IP for {gateway_address_name} not found.")
        return
    ...
    for service in tqdm(images, desc="☸️ Deploying to Kubernetes", unit="service"):
        ...
        if os.path.exists(deploy_yaml):
            ...
            with open(deploy_yaml, "r") as f:
                ...
                if service == "gateway-manager":
                    raw_yaml = raw_yaml.replace("__STATIC_IP__", gateway_ip)
        ...
        for manifest in manifests:
            with open(manifest, "r") as f:
                content = f.read()
                if service == "gateway-manager":
                    content = content.replace("__STATIC_IP__", gateway_ip)
