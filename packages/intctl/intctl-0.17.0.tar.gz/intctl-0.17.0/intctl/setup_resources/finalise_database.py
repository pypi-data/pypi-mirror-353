...
def finalise_database(cfg: dict, status) -> None:
    ...
    try:
        ...
        if loop.is_running():
            # For environments like Cloud Shell, Jupyter, etc.
            # Use `ensure_future()` and let it run
            task = loop.create_task(execute_sql())
            # Optional: wait explicitly using `await` if inside an async function
            # Or just rely on the event loop managing it if top-level
        else:
