...
class RestWrapper(BaseRestWrapper):
    ...
    def copy(self, other: 'BaseRestWrapper'):
        ...
        self.__rest_ = other.rest
        self.__tag = other.tag
        self.__wrapper_tag = other.wrapper_tag
        self.__api_stats_do = other.api_stats_do

    def clone(self) -> 'BaseRestWrapper':
        new = RestWrapper(self.tag, self.wrapper_tag, self.__rest_.clone())
        new.__api_stats_do = self.__api_stats_do
        return new
    ...
    def request(self, api: str, method: HttpMethod or str = None,
                ...
                hooks: Hooks = None, show_len: int = None, opts: RestOptions = None) -> Callable[[Any], Callable[[tuple[Any, ...], dict[str, Any]], RestResponse]]:
    ...
    def __parser_rest(self, func, args, this: Optional[ApiAware] = None):
        if self.__rest_ is None:
            if (func is not None and inspect.ismethod(func) and isinstance(api_aware := func.__self__, ApiAware)) \
                ...
                self.__parser_rest_handler(api_aware)
            elif this is not None and isinstance(this, ApiAware):
                self.__parser_rest_handler(this)

    def __parser_rest_handler(self, api_aware: ApiAware):
        bean: BaseContextBean = api_aware.context.get_bean(api_aware.tag())
        if bean is None:
