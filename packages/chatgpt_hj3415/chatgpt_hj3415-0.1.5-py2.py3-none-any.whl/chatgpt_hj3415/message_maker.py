...
def make_messages(code:str, last_days:int = 60) -> List[dict]:
    ...
    content = (f"{name}({code})의 다음의 데이터(재무분석, 투자지표, 업종비교, 주가)와 그 외의 관련 정보(산업 동향, 경기 사이클 등)를 바탕으로,"
               f"현재 기업의 상황을 항목별로 구체적으로 분석해줘. 또한, 향후 6개월간 주가에 대해 합리적인 시나리오를 보수적/낙관적 관점에서 각각 하나 이상 제시해줘.\n"
               f"{csv_103}"
               f"{csv_104}"
               f"{csv_106}"
               f"{chart_data}")
