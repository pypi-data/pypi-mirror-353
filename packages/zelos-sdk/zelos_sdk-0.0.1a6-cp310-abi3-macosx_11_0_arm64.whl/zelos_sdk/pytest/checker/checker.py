...
class Checker(ContextBase):
    ...
    def _add_to_board(self, item: CheckerItem) -> None:
        ...
        self._board.add_row(
            Text(datetime.utcfromtimestamp(item.time_ns // 1e9).isoformat(), style=dim_style),
    ...
    def _close(self) -> None:
        ...
        if self._checks:
            console = Console()
            # This is here to provide some separation between the board
            # and the rest of the logs
            console.print()
            console.print(self._board)
            log.debug("\n%s", log_table(self._board))
