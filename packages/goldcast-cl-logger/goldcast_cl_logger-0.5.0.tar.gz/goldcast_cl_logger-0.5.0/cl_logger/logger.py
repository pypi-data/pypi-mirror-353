        ...
        try:
            ...
            if "%" in msg and "args" in kwargs:
                ...
                try:
                    ...
                    self.fallback_logger.warning(
                        ...
                        extra={"error": str(e), "message": msg, "args": args},
                        exc_info=True,
                    )
            ...
            if processed_extra is not None:
                try:
                    ...
                    # If anything goes wrong with extra handling, log it and continue without extra
                    self.fallback_logger.warning(
            ...
            if "{" in msg and "}" in msg:
                try:
                    ...
                    if all(
                        ...
                        for expr in f_string_exprs:
                            try:
                                ...
                                if value is None:
                                    # If not in extra, try to evaluate in the current scope
                    ...
                    # If anything goes wrong in f-string parsing, log it and continue
                    self.fallback_logger.warning(
                        "Failed to process f-string expressions",
                        extra={"error": str(e), "message": msg},
                        exc_info=True,
                    )
            ...
            try:
                # Remove 'args' from kwargs if it exists to avoid conflict with logger.log() parameter
            ...
            # If anything goes wrong at the top level, use fallback logger with minimal processing
            try:
                self.fallback_logger.error(
