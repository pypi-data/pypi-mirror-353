...
class FacilitatorClient:
    ...
    async def verify_payment(
        ...
        try:
            ...
            is_local_facilitator = "localhost" in self.base_url or "127.0.0.1" in self.base_url
    ...
    async def settle_payment(self, payment_header: str, payment_requirements: PaymentRequirements) -> SettleResponse:
        ...
        try:
            ...
            return await self.settle_payment_object(decoded_payment, payment_requirements)
    ...
    async def verify_and_settle_payment(self, payment_header: str, payment_requirements: PaymentRequirements) -> tuple[VerifyResponse, SettleResponse]:
        ...
        if not verify_response.is_valid:
            failed_settle = SettleResponse(success=False, error_reason="Verification failed")
            return verify_response, failed_settle
        
        # Immediately settle using the same decoded payment
        decoded_payment = self._decode_payment_header(payment_header)
        settle_response = await self.settle_payment_object(decoded_payment, payment_requirements)
                ...
                "payload": {
                    ...
                    "authorization": {
                        **parsed_payment["payload"]["authorization"],
                        ...
                        "validAfter": parsed_payment["payload"]["authorization"]["validAfter"], 
                        "validBefore": parsed_payment["payload"]["authorization"]["validBefore"],
                    }
                }
            }
    ...
    async def settle_payment_object(self, decoded_payment: dict, payment_requirements: PaymentRequirements) -> SettleResponse:
        ...
        try:
            ...
            nonce = decoded_payment.get('payload', {}).get('authorization', {}).get('nonce', 'unknown')
