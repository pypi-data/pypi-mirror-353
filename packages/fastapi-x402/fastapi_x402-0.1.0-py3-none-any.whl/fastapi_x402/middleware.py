...
from .core import get_config, requires_payment, get_payment_config, requires_payment_by_name, get_payment_config_by_name
...
class PaymentMiddleware(BaseHTTPMiddleware):
        ...
        try:
            ...
            if response.status_code < 400 and self.auto_settle:
                ...
                if settle_response.tx_status == "SETTLED":
                    ...
                    print(f"DEBUG: Settlement successful! Transaction: {settle_response.tx_hash}")
                else:
                ...
                print(f"DEBUG: Route failed with status {response.status_code}, skipping settlement")
            ...
            return JSONResponse(
                ...
                content={
                    ...
                    "accepts": [self._build_payment_requirements(request, payment_config).model_dump()],
                },
                headers={
                    "Content-Type": "application/json",
                }
    ...
    def _build_payment_requirements(
        ...
        return PaymentRequirements(
            ...
            mimeType="",
            payTo=config.pay_to,
            maxTimeoutSeconds=payment_config.get("expires_in") or config.default_expires_in or 300,
