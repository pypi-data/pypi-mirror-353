...
class ChainlitAdapter(SDKAdapter):
    ...
    async def process_tool_calls(self,run_id: str, tool_calls: Dict[int, ChainlitToolCallWithLLMResponse], is_sequence_agent: bool = False, sequence_agent_graph_items:List[GraphItem] = [], reported_sequence_sub_agent_ids: set = set(), on_node_enter: Optional[Callable] = None, on_node_error: Optional[Callable] = None, check_if_agent_has_dedicated_workers: Optional[Callable] = None, get_agent_emoji: Optional[Callable] = None, agent_custom_functions: list[GraphItem] = [], local_tools: Dict[str, Callable] = None) -> str | None:
        """
        ...
        if is_sequence_agent and not self.agent.id in reported_sequence_sub_agent_ids:
            ...
            if get_agent_emoji:
                emoji = get_agent_emoji(agent_id=self.agent.id)
                if emoji:
                    agent_name = f"{emoji} {agent_name}"
        ...
        for raw_tool_call in tool_calls.values():
            ...
            if tool_agent_id:
                ...
                if sub_agent_node:
                    ...
                    if get_agent_emoji:
                        emoji = get_agent_emoji(agent_id=sub_agent_node.item_id)
                        if emoji:
                            tool_name = f"{emoji} {tool_name}"
