...
class Publisher(typing.Generic[IN, OUT], Subject[IN, OUT]):
    ...
    def __init__(
        self,
        source: typing.Union[
            typing.Callable[[], collections.abc.Awaitable[IN]],
            typing.Callable[[], IN],
        ],
        interval: float = 5,
        maxsize: int = 0,
        pipe: typing.Optional[PipeFunction[IN, OUT]] = None,
    ) -> None:
    ...
    def on_unsubscribe(self) -> None:
        self._counter -= 1
        if self._counter <= 0 and self._update_task:
            self.__unset()
        super().on_unsubscribe()
