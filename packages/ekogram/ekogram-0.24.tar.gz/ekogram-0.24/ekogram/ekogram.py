...
class FreeGpt:
    ...
    def chat(self, messages: dict = None, model: str = "Qwen/Qwen3-235B-A22B", temperature: float = 0.7, max_tokens: int = None, timeout: int = 80) -> str:
    ...
    def conver(self, prompt, model_index=0):
        '''Генерация ответа используя Toolbaz\nДоступные модели: gemini-2.5-flash, gemini-2.0-flash-thinking, gemini-2.0-flash, gemini-1.5-flash", o3-mini, gpt-4o-latest, gpt-4o, grok-3-beta, grok-2-1212, Llama-4-Maverick, Llama-4-Scout, Llama-3.3-70B, deepseek-r1, toolbaz_v3.5_pro, toolbaz_v3, mixtral_8x22b, L3-70B-Euryale-v2.1, midnight-rose, unity, unfiltered_x'''
        try:
            HEADERS = {
                "accept": "*/*",
            ...
            MODELS = ["gemini-2.5-flash", "gemini-2.0-flash-thinking", "gemini-2.0-flash", "gemini-1.5-flash", "o3-mini", "gpt-4o-latest", "gpt-4o", "grok-3-beta", "grok-2-1212", "Llama-4-Maverick", "Llama-4-Scout", "Llama-3.3-70B", "deepseek-r1", "toolbaz_v3.5_pro", "toolbaz_v3", "mixtral_8x22b", "L3-70B-Euryale-v2.1", "midnight-rose", "unity", "unfiltered_x"]
            if prompt is None:
                raise ValueError("Забыли указать prompt")
            elif model_index > len(MODELS)-1 or model_index < 0:
    ...
    def flux_1(self, prompt: str = None, width: int = 1024, height: int = 1024, seed: int = None, enhance: bool = True, private: bool = True, nologo: bool = True, model: str = 'flux', filename: str = 'image.jpg') -> bool:
        '''Генерация фотографии с помощью flux-1.5'''
        try:
            if prompt is None:
                raise ValueError("Забыли указать prompt")
            ...
            print(f"FreeAI(flux_1): {e}")
            return False
    
    def flux_2(self, prompt, filename='image.jpg'):
        '''Генерация фотографии с помощью flux-2'''
        try:
            if prompt is None:
                raise ValueError("Забыли указать prompt")
            prompt = prompt.replace(" ", "%20")
            ...
            headers={'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/134.0.0.0 Safari/537.36', 'referer': 'https://lusion.regem.in/'})
            if resp.status_code == 200:
...
class FreeImg:
    def sdxl(self, prompt, filename="image.jpg"):
        '''Генерация фотографии с помощью Stable Diffusion XL'''
        try:
            if prompt is None:
                raise ValueError("Забыли указать prompt")
            session_hash = "".join(random.choices(string.ascii_letters + string.digits, k=16))
            payload = {"data": [prompt, "(deformed, distorted, disfigured:1.3), poorly drawn, bad anatomy, wrong anatomy, extra limb, missing limb, floating limbs, (mutated hands and fingers:1.4), disconnected limbs, mutation, mutated, ugly, disgusting, blurry, amputation", True, 0, 1024, 1024, 3, 15, True], "event_data": "null", "fn_index": 3, "trigger_id": 4, "session_hash": session_hash}
            resp = requests.post('https://kingnish-sdxl-flash.hf.space/gradio_api/queue/join?', json=payload)
            answer = requests.get(f'https://kingnish-sdxl-flash.hf.space/gradio_api/queue/data?session_hash={session_hash}', stream=True)
            ...
            print(f"FreeImg(sdxl): {e}")
            return False

    def dalle_3(self, prompt, filename="image.jpg"):
        '''Генерация фотографии с помощью Dall-E 3'''
        try:
            if prompt is None:
                raise ValueError("Забыли указать prompt")
            session_hash = "".join(random.choices(string.ascii_letters + string.digits, k=16))
            payload = {"data": [prompt, "(deformed, distorted, disfigured:1.3), poorly drawn, bad anatomy, wrong anatomy, extra limb, missing limb, floating limbs, (mutated hands and fingers:1.4), disconnected limbs, mutation, mutated, ugly, disgusting, blurry, amputation", True, 0, 1024, 1024, 6, True], "event_data": "null", "fn_index": 3, "session_hash": session_hash, "trigger_id": 6}
            resp = requests.post('https://ehristoforu-dalle-3-xl-lora-v2.hf.space/queue/join?__theme=system', json=payload)
            answer = requests.get(f'https://ehristoforu-dalle-3-xl-lora-v2.hf.space/queue/data?session_hash={session_hash}', stream=True)
            if answer.status_code == 200:
                for line in answer.iter_lines():
                    if line:
                        decoded_line = line.decode('utf-8')
            ...
            print(f"FreeImg(dalle_3): {e}")
            return False

    def nsfw(self, prompt, filename="image.jpg"):
        '''Генерация фотографии с помощью Flux-uncensored'''
        try:
            if prompt is None:
                raise ValueError("Забыли указать prompt")
            session_hash = "".join(random.choices(string.ascii_letters + string.digits, k=16))
            ...
            answer = requests.get(f'https://heartsync-nsfw-uncensored-video2.hf.space/gradio_api/queue/data?session_hash={session_hash}', stream=True)
            if answer.status_code == 200:
                for line in answer.iter_lines():
                    if line:
                        decoded_line = line.decode('utf-8')
