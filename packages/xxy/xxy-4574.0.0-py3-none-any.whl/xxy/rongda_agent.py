...
from typing import Annotated, Any, Dict, List, TypedDict, Union, Literal, Tuple
import aiohttp
...
async def get_rongda_doc(doc_id: str) -> str:
    ...
    report = await download_report_html(session, FinancialReport(title=None, content=None, dateStr=None, security_code=None, downpath=None, htmlpath=doc_id))
...
async def search_rongda_doc(
    ...
    results = await comprehensive_search(
        session, company_code, content_keywords, title_keywords
    )
    return [
        RongdaDoc(doc_id=result.htmlpath, title=result.title, content_clip=result.content)
...
def create_search_agent():
    ...
    search_system_prompt = """You are a financial document search agent. Your job is to find the most relevant financial documents for the user's question.
...
- Start with 1 short keywords, if returned too many results, refine by adding more keywords
            ...
            for i, tool_call in enumerate(response.tool_calls, 1):
                ...
                if tool_func:
                    try:
                        ...
                        if "company_code" in tool_args and tool_args["company_code"] != state["company_code"]:
                            logger.warning(f"🔧 LLM tried to modify company codes!")
                            logger.warning(f"   LLM provided: {tool_args['company_code']}")
                            logger.warning(f"   Using original: {state['company_code']}")
                        ...
                        logger.exception(
                            f"❌ Error executing search tool {tool_name}"
                        )
                        state["messages"].append(
                            ToolMessage(
...
async def process_document_question(
    user_question: str, company_code: List[str], doc_id: str = None
) -> str:
    """
...
if __name__ == "__main__":
    ...
    async def main():
        result = await process_document_question(
            user_question="用中文，搜索 2023年年度报告 中'分行业情况'里不同“行业”的营业收入和营业成本，汇总到一张表格上\n",
            company_code=["603259 药明康德"],
        )
        print(result)
