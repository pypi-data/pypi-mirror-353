...
from xxy.rongda_agent import process_document_question, process_document_question_with_doc_id
...
async def command_rongda(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if args.doc_id:
            ...
            result = await process_document_question_with_doc_id(args.doc_id, args.question)
        else:
            # Search and analyze
...
Examples:
  ...
  python -m xxy rongda "分析这份报告的主要财务指标" --doc-id "report123.html"
    ...
    if hasattr(args, 'question') and hasattr(args, 'company_codes') and hasattr(args, 'doc_id'):
