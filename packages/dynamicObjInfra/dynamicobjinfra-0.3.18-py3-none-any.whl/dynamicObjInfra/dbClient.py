...
class DBClient(metaclass=Singleton):
    ...
    def saveWithTranslations(
        ...
        for tr in translations:
            ...
            # Upsert by a composite filter: (object_id, locale, id, gender_variant)
    ...
    def _fetchRawTranslations(self, obj, locale, gender):
        base = {
            "collection": obj.dbCollectionName,
            "object_id":  obj.id,
            "locale":     locale,
        }
        ...
        if gender is not None:
            ...
            entries = self.loadManyFromDBByFilter(TranslationEntry, base)
            if entries:
                return entries

        # 2) Fallback to neutral (gender_variant = None)
        base["gender_variant"] = None
        return self.loadManyFromDBByFilter(TranslationEntry, base)
    
    def _applyTranslations(self, obj: BaseObj, locale: str, gender: int | None):
        ...
        for t in entries:
            name = obj.__class__.get_field_name_by_id(t.id)
            if name:
