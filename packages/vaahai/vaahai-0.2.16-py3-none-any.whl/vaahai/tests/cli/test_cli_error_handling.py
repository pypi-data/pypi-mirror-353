...
def test_keyboard_interrupt_handling():
    ...
    with patch("vaahai.cli.main.app", side_effect=KeyboardInterrupt):
        ...
        with patch("sys.exit") as mock_exit:
            main()
            # The main function exits with code 1 for KeyboardInterrupt
...
def test_command_not_found():
    ...
    # Just verify that the command fails with a non-zero exit code
    assert result.exit_code != 0
    # The error message might be in stderr rather than stdout depending on Typer version
    # or might not contain specific text we expect, so we don't check the content
