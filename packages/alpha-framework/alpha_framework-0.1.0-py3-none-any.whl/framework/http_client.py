...
from framework.models.flows import Flows, get_mysql_conn

mysql_session = get_mysql_conn()
...
class HttpClient(object):
    ...
    def __send_request(self, data):
        ...
        if platform.platform() == "Linux":
            try:
                # 请求信息存表
                thread = threading.Thread(target=self.save_request, args=(data,))
                thread.start()
            except Exception as e:
                logger.error(f"抓取接口异常:{e}")
        return self.response

    def request(self, *args, app, **kwargs):
        if args and isinstance(args[0], dict) and args[0].get("request") is not None:
            return self.__send_request(args[0])
        if args and isinstance(args[1], str):
            args = list(args)
            args[1] = urljoin(CONTEXT.get(app=app, key="domain"), args[1])
        else:
            kwargs["url"] = urljoin(CONTEXT.get(app=app, key="domain"), kwargs["url"])
        return ResponseUtil(requests.request(*args, headers=self.headers, **kwargs))

    def post(self, app, url, data=None, json=None, **kwargs):
        return self.request("post", urljoin(CONTEXT.get(app=app, key="domain"), url), app=app, data=data, json=json
                            , **kwargs)

    def get(self, app, url, params=None, **kwargs):
        return self.request("get", urljoin(CONTEXT.get(app=app, key="domain"), url), app=app, params=params, **kwargs)

    def put(self, app, url, data=None, **kwargs):
        return self.request("put", urljoin(CONTEXT.get(app=app, key="domain"), url), app=app, data=data, **kwargs)

    def delete(self, app, url, **kwargs):
        return self.request("delete", urljoin(CONTEXT.get(app=app, key="domain"), url), app=app, **kwargs)
    ...
    def save_request(params):
        ...
        flow_obj = mysql_session.query(Flows).filter_by(req_url=url).first()
        if not flow_obj:
            params = request.get("params", None)
            _json = request.get("json", None)
            _data = request.get("data", None)
            flow = Flows(
                req_method=method,
                req_url=url,
                req_query=json.dumps(params) if params else None,
                req_body=json.dumps(_json) if _json else None or json.dumps(_data) if _data else None
            )
            mysql_session.add(flow)
