...
from labels.parsers.cataloger.golang.cataloger import on_next_golang
...
@parametrize_sync(
    ...
    cases=[
        ["/path/to/go.mod", "parse-go-mod"],
        ["/path/to/nested/go.mod", "parse-go-mod"],
    ],
)
def test_on_next_golang_with_valid_files(file_path: str, expected_parser: str) -> None:
    source = from_iterable([file_path])
    received: list[Request] = []

    on_next_golang(source).subscribe(on_next=received.append)
...
def test_on_next_golang_with_invalid_file() -> None:
...
def test_on_next_golang_exception_calls_on_error() -> None:
    source = reactivex.from_iterable([None])
    received: list[Request] = []
    errors: list[Exception] = []

    on_next_golang(source).subscribe(  # type: ignore[arg-type]
