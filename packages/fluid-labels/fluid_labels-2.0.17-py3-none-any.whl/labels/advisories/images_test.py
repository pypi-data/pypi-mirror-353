...
from labels.advisories.images import (
...
@mock.patch("labels.advisories.images.DATABASE.get_connection")
def test_get_package_advisories(connect_to_db: mock.MagicMock) -> None:
    connection = get_mocked_connection(":memory:")
    connect_to_db.return_value = connection
    try:
        result = get_package_advisories("npm", "test-package", "1.1.0", "ubuntu:20.04")
...
def test_get_vulnerabilities_with_matching_versions(connect_to_db: mock.MagicMock) -> None:
    ...
    connect_to_db.return_value = connection
    try:
        result = get_vulnerabilities("npm", "test-package", "1.1.0", "ubuntu:20.04")
...
def test_get_vulnerabilities_with_empty_params(connect_to_db: mock.MagicMock) -> None:
    ...
    connect_to_db.return_value = connection
    try:
        result = get_vulnerabilities("npm", "", "1.1.0", "ubuntu:20.04")
