...
class S02MSIL1ABProductConversion:
    ...
    def convert_s2msi_l1_safe_product(self, **kwargs: Any) -> DataTree:
        ...
        for detector_id in self.params.detector_ids:
            ...
            for band_name in self.params.band_names:
                ...
                larger_chunk_size = {"alt": chunk_size[0] * 4, "act": chunk_size[1]}
                band_img_data: xr.DataArray = xr.DataArray(
                    full_image_data_arr,
                    dims=("alt", "act"),
                ).chunk(larger_chunk_size)
    ...
    def handle_detector_swath_footprint_construction(
        ...
        # detector_group_attrs = build_detector_footprint_(grouped_granule_metadata_paths_dict)
        detector_swath_footprint = build_detector_swath_footprint_(
        ...
        alt_60m_chunk_size: int = ALT_CHUNK_SIZES["b01"] * 4
        ...
        new_product[vname_detector_footprint_longitudes] = detector_footprint_longitudes_xda
        new_product[vname_detector_footprint_longitudes].attrs = {
        ...
        new_product[vname_detector_footprint_latitudes] = detector_footprint_latitudes_xda
        new_product[vname_detector_footprint_latitudes].attrs = {
...
def open_image_data(img_data_path: Path, chunks: Any = True) -> xr.DataArray:
    ...
    # Open the raster
    img_data_raster = rxr.open_rasterio(img_data_path, chunks=DEFAULT_CHUNKS)
...
def build_detector_swath_footprint_(
