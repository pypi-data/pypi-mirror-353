...
FALLBACK_PSD_VERSION = 14  # used only if PSD location is not found in input paths, so normally never used.
DEFAULT_CHUNKS: Any = {}

S2L1CProductMetadata = dict[str, Any]
ResolutionType = Literal[10, 20, 60]

DEFAULT_CHUNK_SIZE: dict[ResolutionType | int, int] = {
    10: 1830,
    20: 915,
    60: 305,
}
...
L2A_NATIVE_RESOLUTION: ResolutionType = 20  # in meters
...
BandNameToResolutionMapping: dict[BandNameType | str, ResolutionType] = {
    "b01": 60,
    "b02": 10,
    "b03": 10,
    "b04": 10,
...
class S02MSITileZarrPathRenderer:
    ...
    @property
    def detector_footprint_(self) -> PurePosixPath:
...
class AccessorXML:
    ...
    def findoptional(self, path: str) -> Any:
        """
    ...
    def findoneattr(self, path: str) -> Any:
        """
    ...
    def findalltext(self, path: str) -> list[Any]:
        """
...
def open_s2msi_tile_safe_product(
    safe_path: Path,
    *,
    product_level: ProductLevelType | str | None,
    ...
    **kwargs: Any,
    ...
    xdt.attrs.update(attrs_from_xml)  # type: ignore
...
def convert_xml_metadata(safe_path: Path, *, product_level: Literal["L1C", "L2A"] | str) -> dict[str, Any]:
    """
    ...
    xml_accessor = AccessorXML(
        roots=roots,
        namespaces=get_xml_namespaces(product_level=product_level, roots=roots, fallback_version=FALLBACK_PSD_VERSION),
    )
...
def get_xml_namespaces(*, product_level: str, roots: Any, fallback_version: int = 14) -> dict[str, dict[str, str]]:
    """
    Get XML namespaces
    ...
    schemas = {}
    schema_root = f"https://psd-{fallback_version}.sentinel2.eo.esa.int/PSD"
    default_schemas = {
...
def read_product_raster_data(
    ...
    *,
    product_level: str,
    eo_band_metadata: list[dict[str, Any]],
    perform_bitpacking: bool = True,
    default_chunks: Any = DEFAULT_CHUNKS,
) -> DataTree:
    """
    ...
    if product_level == "L1C":
        ...
        for band_file_path in sorted(safe_path.glob("GRANULE/*/IMG_DATA/*.jp2")):
            ...
            if band_name in BAND_NAMES:
                ...
                var_path = f"r{get_band_name_to_resolution_mapping(band_name)}m"
                xdt[var_path][band_name] = raster
            elif band_name == "tci":
    ...
    for band_file_path in sorted(safe_path.glob("GRANULE/*/QI_DATA/MSK_CLASSI*")):
        ...
        available_masks = ("OPAQUE", "CIRRUS", "SNOW_ICE")
    ...
    root_xdt[str(S02MSITileZarrPathRenderer().detector_footprint_)] = xdt
...
def open_raster(
    ...
    *,
    ...
    resolution: ResolutionType | int | None = None,
    ...
    if resolution is not None:
        if isinstance(raster, list):
            raster = [raster_i.chunk(get_chunk_size(resolution)) for raster_i in raster]
        else:
...
def read_product_datastrip_metadata(safe_path: Path) -> dict[str, Any]:
    """
...
def analyze_tie_points_grid_from_xml(grid: Any) -> Any:
    """
...
def xarrayize_xml_table(grid: Any, grid_info: GridInfo, crs: str, angle_type: str) -> xr.DataArray:
    """
...
def validate_granule_metadata(grids: dict[str, Any]) -> bool:
    """
...
def get_chunk_size(resolution: ResolutionType | int) -> tuple[int, int, int]:
    """
    ...
    """
    return 1, DEFAULT_CHUNK_SIZE[resolution], DEFAULT_CHUNK_SIZE[resolution]
...
def get_band_name_to_resolution_mapping(band_name: str) -> ResolutionType:
    """
