...
from typing import Any, Iterable, List, Mapping, Optional, Sequence, Type, cast
...
class EOProcessorWorkFlow:
    ...
    def __init__(
        ...
        index = 0
        self._punits_indices = {}
        self._indexed_unit_workflow = {}
        ...
        for workflow_unit in workflow_units:
            if not workflow_unit.active:
                break
            self._punits_indices[workflow_unit.identifier] = index
            self._indexed_unit_workflow[index] = workflow_unit
            index += 1
        ...
        self._workflow = [self._indexed_unit_workflow[o] for o in order]
    ...
    def run_workflow(
        ...
        if eoqc is not None:
            ...
            _eoqc_processor: EOQCProcessor = EOQCProcessor(
                config_folder=(eoqc.config_folder if eoqc.config_folder is not None else "default"),
                parameters=eoqc.parameters,
                update_attrs=(eoqc.update_attrs if eoqc.update_attrs is not None else True),
                report_path=eoqc.report_path,
                config_path=eoqc.config_path,
                additional_config_folders=eoqc.additional_config_folders,
            )
        else:
        ...
        self._run(
            ...
            validation_mode=EOConfiguration().triggering__validate_mode,
        )
    ...
    @staticmethod
    def validate_output_products(products: MappingDataType) -> None:
            ...
            try:
                ...
                if dask_monitor is not None:
                    ...
                    if (
                        ...
                        raise DaskMonitorCriticalError(f"Cluster is in error state ! {[s.name for s in cluster_state]}")
    ...
    def _close_store_instances(
        ...
        for output_unit_id, store_results in store_instances.items():
            self._logger.debug(f"id : {output_unit_id}")
            self._logger.debug(f"Stores: {store_results}")
