...
class DaskContext:
    ...
    def __init__(
        ...
        # setup cluster
        if self._cluster_type == ClusterType.LOCAL:
            """
        ...
        elif self._cluster_type == ClusterType.SSH:
            from dask.distributed import SSHCluster

            self._cluster = SSHCluster(**self._cluster_config)
        elif self._cluster_type == ClusterType.KUBERNETES:
            try:
        ...
        elif self._cluster_type == ClusterType.PBS:
            try:
        ...
        elif self._cluster_type == ClusterType.SGE:
            try:
                ...
                raise PackageNotFoundError("Package dask_jobqueue should be installed.")
        elif self._cluster_type == ClusterType.LSF:
            try:
                ...
                raise PackageNotFoundError("Package dask_jobqueue should be installed.")
        elif self._cluster_type == ClusterType.SLURM:
            try:
                ...
                raise PackageNotFoundError("Package dask_jobqueue should be installed.")
        elif self._cluster_type == ClusterType.YARN:
            try:
        ...
        elif self._cluster_type == ClusterType.GATEWAY:
            self._setup_gateway_cluster()
        elif self._cluster_type == ClusterType.ADDRESS:
            if address is None:
        ...
        elif self._cluster_type == ClusterType.CUSTOM:
            self._setup_custom_cluster()
        elif self._cluster_type is None:
