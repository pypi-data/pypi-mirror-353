...
class Download(MagmaVar):
    ...
    def download(self, params: dict = None) -> dict:
        ...
        try:
            ...
            if len(response['data']) == 0:
                raise ValueError(f'❌ No data found for {self.volcano_code} :: {response}')
    ...
    def extract(self) -> Self:
        ...
        Returns:
            self: Self
        """
        self.df = ext(events=self.events)
        return self
    ...
    def _to(self, filetype: str, filename: str = None, verbose: bool = True) -> str | None:
        """Save wrapper
        ...
        self.df.to_csv(path, index=False) if filetype == 'csv' else self.df.to_excel(path, index=False)
