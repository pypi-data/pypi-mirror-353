...
{t('cli_examples')}
  ...
  vm-balancer --config /path/to/.env    {t('cli_example_config')}
    ...
    parser.add_argument(
        '--once',
        action='store_true',
        help=t('cli_once_help')
    )
    
    parser.add_argument(
        '--interval',
        type=int,
        help=t('cli_interval_help')
    )
    
    parser.add_argument(
        '--cluster-ids',
        type=str,
        help=t('cli_cluster_ids_help')
    )
    
    parser.add_argument(
        '--version',
        action='version',
        version=f'VM Balancer {__version__}'
    )
    ...
    try:
        ...
        print(f"🔄 Mode: {t('balancer_dry_run_mode') if args.dry_run else 'Production'}")
        
        if args.once:
            print(f"⚡ {t('balancer_once_mode')}")
            asyncio.run(balancer.run_once())
        else:
            print(f"🔄 {t('balancer_interval_mode', interval=balancer.balance_interval)}")
