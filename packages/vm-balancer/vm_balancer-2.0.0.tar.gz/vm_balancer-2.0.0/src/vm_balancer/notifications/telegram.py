...
class TelegramNotifier:
    ...
    def notify_migration_start(self, vm_name: str, source_node: str, target_node: str) -> None:
        ...
        message = f"🔄 <b>{t('telegram_migration_started')}</b>\n\n" \
    ...
    def notify_migration_success(self, vm_name: str, source_node: str, target_node: str, duration: float) -> None:
        ...
        message = f"✅ <b>{t('telegram_migration_completed')}</b>\n\n" \
                 f"{t('telegram_vm_label')} <code>{vm_name}</code>\n" \
                 f"{t('telegram_from_label')} <code>{source_node}</code>\n" \
                 f"{t('telegram_to_label')} <code>{target_node}</code>\n" \
    ...
    def notify_migration_failure(self, vm_name: str, source_node: str, target_node: str, error: str) -> None:
        ...
        message = f"❌ <b>{t('telegram_migration_failed')}</b>\n\n" \
                 f"{t('telegram_vm_label')} <code>{vm_name}</code>\n" \
                 f"{t('telegram_from_label')} <code>{source_node}</code>\n" \
                 f"{t('telegram_to_label')} <code>{target_node}</code>\n" \
    ...
    def notify_balance_cycle_start(self, cluster_count: int, dry_run: bool = False) -> None:
        ...
        mode = f"🧪 {t('telegram_dry_run_mode')}" if dry_run else f"🔄 {t('telegram_live_mode')}"
        message = f"{mode} <b>{t('telegram_balance_cycle_started')}</b>\n\n" \
    ...
    def notify_balance_cycle_complete(self, total_migrations: int, dry_run: bool = False) -> None:
        ...
        mode = f"🧪 {t('telegram_dry_run_mode')}" if dry_run else f"✅ {t('telegram_completed_mode')}"
        message = f"{mode} <b>{t('telegram_balance_cycle_finished')}</b>\n\n" \
