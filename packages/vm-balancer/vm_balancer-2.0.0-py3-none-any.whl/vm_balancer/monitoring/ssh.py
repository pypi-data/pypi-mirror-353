...
class SSHMonitor:
    ...
    def __init__(self, username: str, private_key_path: Optional[str] = None, 
                 password: Optional[str] = None, timeout: int = 10, hosts_mapping: Optional[dict] = None):
    ...
    async def get_load_average(self, host: str, port: int = 22, username: Optional[str] = None) -> Optional[Tuple[float, float, float]]:
        ...
        try:
            ...
            async with asyncssh.connect(**connect_options) as conn:
                ...
                if len(parts) >= 3:
                    ...
                    logging.debug(t('ssh_load_average_debug', 
                                    host=host, 
                                    load_1m=load_1m, 
                                    load_5m=load_5m, 
                                    load_15m=load_15m))
            ...
            logging.debug(t('ssh_connection_failed', host=host, port=port, error=str(e)))
    ...
    async def monitor_nodes(self, nodes: List[NodeInfo]) -> None:
        ...
        for node in nodes:
            if node.ssh_host and node.ssh_host.strip():  # Check that ssh_host is not empty
    ...
    async def _monitor_single_node(self, node: NodeInfo) -> None:
        ...
        try:
            load_avg = await self.get_load_average(node.ssh_host, node.ssh_port, node.ssh_user)
            if load_avg:
                node.load_average_1m, node.load_average_5m, node.load_average_15m = load_avg
