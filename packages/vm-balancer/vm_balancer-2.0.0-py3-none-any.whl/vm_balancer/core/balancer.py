...
class VMBalancer:
    ...
    def find_overloaded_nodes(self, nodes: List[NodeInfo]) -> List[NodeInfo]:
        ...
        for node in nodes:
            ...
            if not node.is_maintenance and (cpu_overloaded or memory_overloaded):
                logging.info(t('node_overloaded', node_name=node.name, 
    ...
    def setup_ssh_monitoring(self, nodes: List[NodeInfo], ssh_hosts_mapping: Optional[Dict[str, str]] = None) -> None:
        ...
        for node in nodes:
            ...
            if node.ssh_host:  # Already has SSH host from API
                nodes_configured += 1
                logging.debug(f"SSH monitoring for node {node.name}: host={node.ssh_host}:{node.ssh_port}, user={node.ssh_user} (from API)")
            elif ssh_hosts_mapping and node.name in ssh_hosts_mapping:
                node.ssh_host = ssh_hosts_mapping[node.name]
                nodes_configured += 1
                logging.debug(f"SSH monitoring for node {node.name}: host={node.ssh_host} (from name mapping)")
            elif ssh_hosts_mapping and node.id in ssh_hosts_mapping:
                node.ssh_host = ssh_hosts_mapping[node.id]
                nodes_configured += 1
                logging.debug(f"SSH monitoring for node {node.name}: host={node.ssh_host} (from ID mapping)")
            else:
                # Try to use node name as hostname (fallback)
                node.ssh_host = node.name
                logging.debug(f"SSH monitoring for node {node.name}: host={node.ssh_host} (fallback to node name)")
        
        if nodes_configured > 0:
            logging.info(f"SSH monitoring configured for {nodes_configured}/{len(nodes)} nodes")
    ...
    async def update_nodes_load_average(self, nodes: List[NodeInfo]) -> None:
        ...
        nodes_with_ssh = [node for node in nodes if node.ssh_host and node.ssh_host.strip()]
        ...
        for node in nodes_with_ssh:
            if node.ssh_available:
                successful_ssh += 1
                logging.info(f"Node {node.name}: SSH load_avg={node.load_average_1m:.2f}, "
                           f"effective_load={node.effective_cpu_load:.2f}, "
                           f"api_allocation={node.cpu_allocation_ratio:.2f}")
            else:
                logging.debug(f"Node {node.name}: SSH monitoring failed, using API data (allocation={node.cpu_allocation_ratio:.2f})")
        
        if successful_ssh > 0:
            logging.info(f"SSH monitoring successful for {successful_ssh}/{len(nodes_with_ssh)} nodes")
        else:
            logging.warning("SSH monitoring failed for all nodes, falling back to API data")
    ...
    async def run_once(self) -> None:
        ...
        try:
            ...
            for cluster in filtered_clusters:
                ...
                logging.info(f"Cluster {cluster.name}: Found {len(overloaded_nodes)} overloaded node(s)")
                ...
                for source_node in overloaded_nodes:
                    if migrations_performed >= self.max_migrations_per_cycle:
                        logging.info(f"Maximum migrations per cycle reached ({self.max_migrations_per_cycle})")
                        break
                    ...
                    if not node_vms:
                        logging.debug(f"No VMs found on overloaded node {source_node.name}")
                        continue
                    ...
                    if not vm_to_migrate:
                        logging.debug(f"No suitable VM found for migration from node {source_node.name}")
                        continue
                ...
                logging.info(f"Cluster {cluster.name}: Completed {migrations_performed} migration(s)")
            
            # Send cycle completion notification
            if self.telegram_notifier:
                await self.telegram_notifier.send_cycle_completed(len(filtered_clusters))
    ...
    async def run(self) -> None:
        ...
        logging.info(f"Starting VM Balancer in continuous mode (interval: {self.balance_interval}s)")
        
        while True:
            try:
                await self.run_once()
                
                logging.info(f"Waiting {self.balance_interval} seconds until next cycle...")
    ...
    async def migrate_vm(self, vm: VMInfo, source_node: NodeInfo, target_node: NodeInfo) -> bool:
        """Migrate a VM from source to target node"""
        if self.dry_run:
            logging.info(t('migration_dry_run', vm_name=vm.name, source_node=source_node.name, target_node=target_node.name))
        else:
            logging.info(t('migration_start', vm_name=vm.name, source_node=source_node.name, target_node=target_node.name))
        
        # Notify migration start
        if self.telegram_notifier:
            await self.telegram_notifier.notify_migration_start(vm.name, source_node.name, target_node.name)
        
        if self.dry_run:
            # In dry run mode, just log what would be done
            logging.info(t('migration_success', vm_name=vm.name))
            if self.telegram_notifier:
                await self.telegram_notifier.notify_migration_success(vm.name, source_node.name, target_node.name, 0)
            ...
            if self.api.migrate_vm(vm.id, target_node.id, self.migration_timeout):
                ...
                # Notify successful migration
                if self.telegram_notifier:
                    await self.telegram_notifier.notify_migration_success(vm.name, source_node.name, target_node.name, migration_duration)
                ...
                logging.error(t('migration_failed', vm_name=vm.name, error='Migration failed'))
                # Add to blacklist to prevent retry attempts for 24 hours
                ...
                # Notify failed migration
                if self.telegram_notifier:
                    await self.telegram_notifier.notify_migration_failure(vm.name, source_node.name, target_node.name, "Migration failed")
                return False
