...
def site(host: str, 
    ...
    def export_table(table_name):
        ...
        if fmt == 'xlsx' and HAS_PANDAS:
            ...
            output = io.StringIO()
            writer = csv.DictWriter(output, fieldnames=column_names, quoting=csv.QUOTE_ALL)
            writer.writeheader()
            for row in rows:
            ...
            # Получаем текстовое содержимое, затем кодируем его в байты с BOM
            csv_output = '\ufeff' + output.getvalue()
            csv_bytes = csv_output.encode('utf-8-sig')
