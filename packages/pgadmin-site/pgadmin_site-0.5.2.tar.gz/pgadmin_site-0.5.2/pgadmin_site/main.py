    ...
    def index():
        if card:
            ...
            if not card_template:
                return render_template('error.html', error=f"Шаблон карточки '{card}' не найден.", bkg=bkg, sec=sec, acc=acc, foto_enabled=foto_enabled, foto_movable=foto_movable, foto_url=foto_url)
            ...
            return render_template('card_svg.html', card_template=card_template, data=data, card_name=card, abs=abs, eval_formula=eval_formula, bkg=bkg, sec=sec, acc=acc, foto_enabled=foto_enabled, foto_movable=foto_movable, foto_url=foto_url)
        else:
            try:
                table_names = inspector.get_table_names()
                return render_template('index.html', tables=table_names, current_db=database, bkg=bkg, sec=sec, acc=acc, foto_enabled=foto_enabled, foto_movable=foto_movable, foto_url=foto_url)
            except SQLAlchemyError as e:
                logger.error(f"Ошибка при подключении к базе данных: {str(e)}")
                return render_template('error.html', error=str(e), bkg=bkg, sec=sec, acc=acc, foto_enabled=foto_enabled, foto_movable=foto_movable, foto_url=foto_url)
            ...
            with engine.connect() as connection:
                ...
                try:
                    ...
                    try:
                        ...
                        row = row_result
            return render_template('edit_row.html', table_name=table_name, row=row, columns=columns, card_name=card_name)
        except SQLAlchemyError as e:
    ...
    def add_row(table_name):
        ...
        try:
            ...
            if request.method == 'POST':
                ...
                return redirect(url_for('view_table', table_name=table_name, card=card_name))
            return render_template('add_row.html', table_name=table_name, columns=columns, card_name=card_name)
        except SQLAlchemyError as e:
        ...
        for fig in card_template:
            ...
            if db_col and db_table and db_col not in (None, '', 'None') and db_table not in (None, '', 'None'):
                if db_table not in used_tables:
                    used_tables.append(db_table)
        if not used_tables:
            return render_template('error.html', error='Не удалось определить основную таблицу для удаления.')
        main_table = used_tables[0]
