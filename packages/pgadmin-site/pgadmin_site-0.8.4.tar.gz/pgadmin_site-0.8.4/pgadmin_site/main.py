...
def site(host: str, 
        ...
        def foto_position():
            ...
            if request.method == 'POST':
                ...
                if data.get('deleted') is True:
                    with open(position_file, 'w') as f:
                        json.dump({'x': 0, 'y': 0, 'deleted': True}, f)
                    return jsonify({'status': 'deleted'})
                # Если просто перемещение
                else:
                    # Сохраняем x, y, но не меняем deleted (если был)
                    try:
                    ...
                    pos['deleted'] = pos.get('deleted', False)
                    with open(position_file, 'w') as f:
                        json.dump(pos, f)
    ...
    def export_table(table_name):
        ...
        if fmt == 'xlsx' and HAS_PANDAS:
            ...
            output = io.StringIO(newline='', encoding='utf-8') # Указываем кодировку utf-8
            writer = csv.DictWriter(output, fieldnames=column_names)
            writer.writeheader()
            for row in rows:
                writer.writerow({k: row[k] for k in column_names})
            output.seek(0)
            return app.response_class(
                output.getvalue(),
                mimetype='text/csv', # или 'application/csv'
                headers={
                    'Content-Disposition': f'attachment; filename={table_name}.csv'
                },
                # Добавляем кодировку в Content-Type (опционально, но полезно)
...
def create_module_file(modul_command: str):
    ...
    # Получаем путь к установленному пакету
    package_path = pathlib.Path(pkg_resources.resource_filename('pgadmin_site', ''))
    src = package_path / 'modul' / filename
