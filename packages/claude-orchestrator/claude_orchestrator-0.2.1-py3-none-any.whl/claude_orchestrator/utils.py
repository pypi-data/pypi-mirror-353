...
def run_claude_command(cmd: List[str], cwd: str, logger: logging.Logger, auto_exit: bool = False) -> subprocess.CompletedProcess:
    """
    ...
    Args:
        ...
        auto_exit: If True, automatically terminate when Claude outputs /exit
    ...
    def read_stdout(pipe, queue, check_exit):
        """Read stdout in a separate thread, optionally checking for /exit"""
        for line in iter(pipe.readline, ''):
            if line:
                queue.put(line)
                if check_exit and '/exit' in line.strip():
                    logger.info("Detected /exit command in Claude output")
                    exit_detected.set()
        pipe.close()
    ...
    process = subprocess.Popen(
        cmd,
        cwd=cwd,
        stdin=subprocess.PIPE if auto_exit else sys.stdin,
        stdout=subprocess.PIPE if auto_exit else sys.stdout,
    ...
    # Start thread to read stdout if auto_exit is enabled
    stdout_thread = None
    if auto_exit:
        stdout_thread = threading.Thread(target=read_stdout, args=(process.stdout, stdout_queue, True))
        stdout_thread.daemon = True
        stdout_thread.start()
    
    # Process output lines while the command runs
    while True:
        # Check if process is still running
        poll_status = process.poll()
        
        # If auto_exit is enabled and /exit detected, terminate the process
        if auto_exit and exit_detected.is_set() and poll_status is None:
            logger.info("Terminating Claude process after detecting /exit")
            process.terminate()
        ...
        # Process any stdout lines if auto_exit is enabled
        if auto_exit:
            try:
                while True:
    ...
    return subprocess.CompletedProcess(
        ...
        stdout=''.join(stdout_lines) if auto_exit else None,
