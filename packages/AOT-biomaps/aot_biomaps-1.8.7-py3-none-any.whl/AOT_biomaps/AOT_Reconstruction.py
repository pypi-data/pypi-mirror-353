...
class AnalyticRecon(Recon):
    ...
    def _iFourierRecon(self):
        """
        This method is a placeholder for the inverse Fourier reconstruction process.
        It currently does not perform any operations but serves as a template for future implementations.
        """
        # Placeholder for inverse Fourier reconstruction logic
        pass
    ...
    def _iRadonRecon(self):
        ...
        :param s_tilde: Signal transformé de Fourier, de dimensions (nombre d'angles, nombre de fréquences).
        :param theta: Angles de projection.
        :param f_s: Fréquences spatiales.
        :param x: Coordonnées x de l'image.
        :param z: Coordonnées z de l'image.
        :param theta_m: Angle maximum de projection.
        :param f_s_max: Fréquence spatiale maximale.
        ...
        for i in range(len(self.experiment.AcousticFields.shape[0])):
            pass


        # Calcul des coordonnées transformées
        for i, th in enumerate(theta):
            if np.abs(th) > theta_m:
                continue
            x_prime = x[:, np.newaxis] * np.cos(th) - z[np.newaxis, :] * np.sin(th)
            z_prime = z[np.newaxis, :] * np.cos(th) + x[:, np.newaxis] * np.sin(th)
        ...
        for i, th in enumerate(theta):
            ...
            integrand = s_tilde[i, f_s == 0] * np.exp(2j * np.pi * z_prime * f_s)
            integral = trapz(integrand * f_s, f_s[f_s <= f_s_max])
            I_rec += integral
