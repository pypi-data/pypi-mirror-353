...
def load_shape_dict(args, returnInfo = False, origin = False, scale = 1):
    ...
    for k in shapeDict.keys():
        ...
        backDict[str(k)] = load_mesh_plain(loadPath,  args.ZRotNum, 'BoundingBox', scale)
        infoDict[str(k)] = []

        for idx in range(len(backDict[str(k)])):
            infoDict[str(k)].append({'volume': backDict[str(k)][idx].volume, 'extents': backDict[str(k)][idx].extents})
    if returnInfo:
