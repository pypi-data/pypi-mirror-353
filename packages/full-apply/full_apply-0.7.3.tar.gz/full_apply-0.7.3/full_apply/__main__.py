...
def main(
    ...
    git_stage: bool = typer.Option(
        False,
        "--git-stage",
        help="stage changes to files tracked by Git "
        "(NOTE: only works when run within the Git repo, otherwise undefined "
        "behavior)",
    ),
) -> None:
    """
    ...
    if yes:
        ...
        for change in changes:
            change.apply_to_fs(git_stage=git_stage)
        print("all done")
