...
class Server:
    ...
    password:str = None):
        ...
        if(analyzer_major_version >= 3):
            if(analyzer_major_version >= 14):
                self._server = "{0}\ZA2022".format(server)
            elif(analyzer_major_version >= 10):
            ...
            if(self._analyzer_major_version >= 14):
                if (self._version != (self._analyzer_major_version - 2)): #for Report Engine 14 and higher, the database version is always the major version minus 2.
                    raise RuntimeError(f"Report Engine metadata database {self._metadb} is out of date. It can be updated with Reporting Studio.")
            elif(self._analyzer_major_version >= 10):
                if (self._version != (self._analyzer_major_version - 1)): #for Report Engine 10 - 12, the database version is always the major version minus 1.
                    raise RuntimeError(f"Report Engine metadata database {self._metadb} is out of date. It can be updated with Reporting Studio.")
            elif(self._version < 7): #for Analyzer 3.XX, at least database version 7 is required for using this class
    ...
    def read_MetaData_Archives(self)->pd.DataFrame:
        ...
        Returns
        --------
        version >= 14 --> DataFrame: [ID, PROJECT_ID, REFERENCE, VISUALNAME, DESCRIPTION, CYCLETIME, LOTARCHIV, PARENT_ID, DESCRIPTION_TRANSLATABLE, TIMEOUT]
        ...
        elif(self._analyzer_major_version < 14):
            sqlCmd = f"""SELECT [ID],
                        [PROJECT_ID],
                        [REFERENCE],
                        [VISUALNAME],
    ...
    def read_MetaData_Projects(self)->pd.DataFrame:
        ...
        Returns
        --------
        version >= 15 --> DataFrame: [ID, REFERENCE, VISUALNAME, DESCRIPTION, SERVER, STANDBY, SERVICE_ENGINE_CONNECTOR_AVAILABLE]
        ...
        if(self._analyzer_major_version < 11):
        ...
        elif(self._analyzer_major_version < 15):
            sqlCmd = f"""SELECT [ID],
                    [REFERENCE],
                    [VISUALNAME],
                    [DESCRIPTION],
            ...
            sqlCmd = f"""SELECT [ID],
                    ...
                    [STANDBY],
                    CASE
                      WHEN [SERVICE_ENGINE_CONNECTOR_AVAILABLE] IS NULL
                      THEN 1
                      ELSE [SERVICE_ENGINE_CONNECTOR_AVAILABLE]
                    END AS [SERVICE_ENGINE_CONNECTOR_AVAILABLE]
                    FROM [{self._metadb}].[dbo].[PROJECT]"""
        try:
            return pd.read_sql(sqlCmd, self._cnxn)
        except:
    ...
    show_visualnames:bool = False
        ...
        """
        self._check_Connector(connector, project_reference)
    ...
    def _check_Connector(self, connector:Connector, project_reference:str):
        ...
        if(self._analyzer_major_version >= 15 and connector == Connector.ServiceEngine.value):
            sqlCmd = f"""SELECT CASE
                                  WHEN [SERVICE_ENGINE_CONNECTOR_AVAILABLE] IS NULL
                                  THEN 1
                                  ELSE [SERVICE_ENGINE_CONNECTOR_AVAILABLE]
                                END AS [SERVICE_ENGINE_CONNECTOR_AVAILABLE]                     
                         FROM [{self._metadb}].[dbo].[PROJECT]
                         WHERE [REFERENCE] = N'{project_reference}'"""
            data = pd.read_sql(sqlCmd, self._cnxn)
            if(data.iat[0, 0] == 0):
                raise RuntimeError(f"The Service Engine connector of the project '{project_reference}' is disabled")