...
def walk(path: Path):
    ...
    path = Path(path)
    return [Path(i) for i in os.walk(path, topdown=True, onerror=None, followlinks=False)]

def walkdirs(path):
    """Get all the dirs in path."""
    path = Path(path)
    return [Path(i[0]) for i in os.walk(path, topdown=True, onerror=None, followlinks=False)]

def walkfiles(path):
    """Get all the dirs in path."""
    path = Path(path)
    return [Path(i[0]/ii) for i in os.walk(path, topdown=True, onerror=None, followlinks=False) for ii in i[2] if ii not in [".DS_Store", ".gitkeep"]]
