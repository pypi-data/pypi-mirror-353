...
class ProjectCommandDetector:
            ...
            try:
                ...
                if parser and hasattr(parser, 'parse'):
                    ...
                    try:
                        ...
                        if file_commands:
                            logger.debug("Found %d commands in %s", len(file_commands), file_path)
    ...
    def _get_parser_for_file(self, file_path: Path) -> Optional[BaseParser]:
        ...
        logger.debug("Available parsers: %s", [p.__class__.__name__ for p in self.parsers])
        ...
        for parser in self.parsers:
            try:
                ...
                if can_parse:
                    ...
                    is_mock = hasattr(parser, "_is_mock") or hasattr(parser, "_mock_return_value")
                    has_file_path = hasattr(parser, 'file_path')
                    
                    logger.debug("  is_mock: %s, has_file_path: %s", is_mock, has_file_path)
                    
                    if is_mock or has_file_path:
                        logger.debug("Using existing parser: %s (is_mock=%s, has_file_path=%s)", 
                                    parser, is_mock, has_file_path)
                        # Update the file path on the parser
                        parser.file_path = file_path
                        if not hasattr(parser, 'project_path') or parser.project_path is None:
                    ...
                    try:
                        return parser.__class__(file_path=file_path, project_path=self.project_path)
                    except Exception as e:
                        logger.debug("Failed to initialize with kwargs, trying positional args: %s", e)
