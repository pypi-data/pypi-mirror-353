"""
Example of using Claude with structured output
Claudeで構造化された出力を使用する例
"""
...
async def main():
    """
    Main function to demonstrate structured output with Claude
    Claudeでの構造化出力を実演するメイン関数
    """
    ...
    # ClaudeモデルでAgentを作成
    model = ClaudeModel(
        model="claude-3-sonnet-20240229",  # Using Sonnet for better structured output
        temperature=0.3,
        api_key=api_key
    )
    agent = Agent(
