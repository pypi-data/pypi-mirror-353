...
from .utils import get_scenario, get_priority, get_column_widths
...
class ScenarioContext:
    ...
    def __init__(
        self, summary: str, description: str, priority: str, steps: list[StepContext]
    ):
    ...
    def parse(scenario: Scenario | ScenarioOutline) -> "ScenarioContext":
        return ScenarioContext(
            ...
            get_priority(scenario.effective_tags),
            [StepContext.parse(step) for step in scenario.steps],
        )
...
class TestCaseContext:
    ...
    def _get_attributes(self) -> TestCaseAttributes:
        return TestCaseAttributes(
            ...
            pmi="not",
            automated="yes",
            precondition=str(self.background),
            test_step=self._get_test_steps(),
        )

    def to_create_request(self, space: str) -> TestCaseCreateRequest:
