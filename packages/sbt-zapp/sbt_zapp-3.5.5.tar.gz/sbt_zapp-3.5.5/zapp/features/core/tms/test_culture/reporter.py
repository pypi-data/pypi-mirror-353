...
class TestCultureReporter:
    ...
    def before_scenario(self, context, scenario):
        ...
        try:
            tms_code, label, owner = self._extract_attributes(scenario)
            ...
            if code:
                code = self._update(context, code, folder, owner)
            elif label:
                code = self._create(context, label, owner)
            else:
    ...
    def _extract_attributes(
        ...
        """Поиск/извлечение атрибутов сценария: кода, метки, владельца. Данные атрибуты могут отсутствовать"""
        tags = scenario.effective_tags

        tms_code = ScenarioTag.TMS.find(tags)
        label = ScenarioTag.LABEL.find(tags)
        owner = ScenarioTag.OWNER.find(tags)
    ...
    def _create(self, context, label: str, owner: str | None) -> str:
