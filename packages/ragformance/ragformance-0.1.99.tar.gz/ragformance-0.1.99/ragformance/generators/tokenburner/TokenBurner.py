...
def _run_smoke_test(
    ...
    if not api_key:
        print("⚠️ API_KEY not set for smoke test. Skipping.")
        return
    try:
        ...
        print(
            f"🔹 Chat OK with {model_name} → {response.choices[0].message.content[:80]}…"
        )
    except Exception as e:
        # Softer failure for smoke test, as it's not critical path for generator
        print(f"⚠️ Chat smoke-test failed for {model_name}: {e}")
...
def extract_layout(
    ...
    prompt_content = (
        ...
        "avec leurs numéros de page. Présente le résultat sous forme de liste hiérarchique. "
        "Sois précis et n'invente pas d'information :\n" + full_text
    )
    response = litellm.completion(
        model="openai/" + model_name,
...
def analyze_pdf_unified(
    ...
    prompt = f"""
    Vous allez recevoir la structure du document et son contenu intégral. Votre mission est d'en produire une analyse complète, en une seule réponse, structurée selon les instructions suivantes.
    ...
    **Contenu complet :**
    ...
    ### 1. Résumé structuré (par thématique fonctionnelle)
    Pour chacune des rubriques suivantes :
      ...
      6. Sécurité et perspectives
    - Résumez en 1 phrase l'objectif global de chaque rubrique
    - Puis ajoutez 1 phrase par paragraphe important
    - Ajoutez tout autre point saillant si pertinent

    ### 2. Résumé hiérarchique précis (par structure)
    Respectez précisément chaque niveau indiqué dans la structure (ex : 1, 1.1, 1.1.1...) :
    - Pour chaque niveau :
      - Résumé global : 1 phrase
      - Résumé détaillé : 1 phrase par paragraphe

    ### 3. Extraction hiérarchique des entités et tags
    Pour chaque niveau (section, sous-section, sous-sous-section exactement comme indiqué dans la structure) :
    - Listez les entités nommées clairement identifiées (personnes, organisations, lieux, technologies)
    - Proposez des tags pertinents spécifiques à chaque niveau

    Générez ensuite un diagramme Mermaid hiérarchisé clair, par exemple :
    ...
    ### 4. Extraction hiérarchique des concepts
        Pour chaque niveau indiqué dans la structure :
        - Listez les concepts-clés (méthodologies, composants, technologies, principes techniques) qui se trouvent dans les paragraphes

    Générez un diagramme Mermaid hiérarchisé correspondant :
    ...
    Répondez dans l’ordre exact ci-dessus. Soyez clair, synthétique, précis et strictement fidèle au texte, sans inventer d'information.
    """
...
def find_relevant_sections_detailed(
    ...
    prompt = f"""
    À partir de l'analyse complète suivante du document, incluant les sections, les entités, les tags, et les concepts (présentés en format Mermaid), identifie précisément les sections pertinentes pour répondre à la question suivante:

    Analyse complète avec graph Mermaid :
    {analysis_result}
    ...
    Utilise explicitement la hiérarchie (chapitres, sections, jusqu'aux sous-sections), les résumés ainsi que ce qui a été trouvé en MERMAID: les noms, les résumés, les tags, entités et concepts pour identifier et justifier les sections pertinentes. Réponds avec les numéros et titres exacts des sections pertinentes ainsi qu'une courte justification explicite de leur pertinence basée sur les tags, entités, concepts et résumés fournis.

    produit un mermaid des liens trouvés entre les résumés/chapitres, sections, sous sections, noms, concepts tags, entités et concepts afin d'exprimer ton raisonement

    à la fin liste les sections et sous-sections utilisées, il est important d'avoir les 2.
    """
    if not api_key:
        raise ValueError(
...
def extract_raw_text_sections_llm(
    ...
    prompt = f"""
    Extrait précisément le texte brut des sections et sous-sections suivantes à partir du document fourni :
    ...
    Retourne uniquement le texte brut exact des sections demandées, sans ajouter ni modifier d'information.
    """
    if not api_key:
        raise ValueError(
...
def generate_questions_from_sections(
    ...
    prompt = f"""
    À partir du texte extrait des sections suivantes :
    {final_sections}

    et de l'analyse complète (incluant le graphe Mermaid) :
    {analysis_result}

    Catégories de questions :
    {categories_definition}

    Génère des questions pertinentes pour chaque section et sous-section en lien direct avec la requête utilisateur suivante :
    {user_query}

    Ensuite, utilise les liens indiqués dans le graphe Mermaid pour créer des questions additionnelles reliant ces sections, concepts, entités entre elles.

    Structure clairement chaque série de questions par section, entités, concepts et indique explicitement les liens utilisés pour générer les questions additionnelles.
    Pour chaque question précise les concepts, entités utilisées.

    enfin de toute ces questions essais d'en générer des plus globales toujours en liant sections, entités, concepts.

    n'oublie pas de classifier chaque question selon les catégories fournies dans le fichier

    remember those question must be related to concepts and entities of the user input

    n'oublie pas de générer aussi les réponses à chaque fois!
    """
    if not api_key:
        raise ValueError(
...
def refine_questions(
    ...
    Sélectionne les 5 questions les plus pertinentes par rapport à la question utilisateur initiale, en tenant compte de la proximité sémantique, des concepts clés abordés, les entités et en assurant une diversité des catégories représentées.
    Les questions doivent avoir une proximité des concepts et entités avec les questions générées.
    Retourne ces 5 questions raffinées clairement classifiées selon les catégories fournies ainsi que les réponses à ne pas oublier.
    """
    if not api_key:
        raise ValueError(
