...
class BlaskPipeline:
    ...
    def __init__(self, username: Optional[str] = None, password: Optional[str] = None):
        ...
        self.country_extractor = CountryExtractor(api_agent=self.api_agent)
    ...
    def process_query(self, query: str) -> Dict[str, Any]:
        ...
        # Step 2: Extract country IDs from query
        country_mapping = self.country_extractor.extract_country_ids_from_query(query)
        logger.info(f"Extracted country mapping: {country_mapping}")

        # Step 3: Create API call plan
        actions, explanation = self.planner.get_api_plan(query, country_mapping)
        ...
        for action in sorted_actions:
            ...
            # Convert processor results to JSON
            previous_processed_results = (
                json.dumps(complete_processor_results, indent=2)
                if complete_processor_results
                else "{}"
            )
            ...
            if result.get("multiple_executions", False) and result.get(
                ...
                for individual_result_data in result["individual_results"]:
                    ...
                    if individual_raw_api_response:
                        ...
                        if individual_processed.get("execution_trace"):
                            combined_execution_traces.extend(
                                individual_processed["execution_trace"]
                            )
                    else:
                        logger.warning(
                ...
                # Create combined processed result
                processed_result = {
                    "processed_data": (
                        combined_processed_data if combined_processed_data else None
                    ),
                    "execution_trace": combined_execution_traces,
            ...
            if processed_result and processed_result.get("processed_data"):
                ...
                if extracted_brand_ids:
                    ...
                    new_ids_str = [
                        str(bid) for bid in extracted_brand_ids if bid is not None
                    ]
                    existing_ids_str = [
                        str(bid) for bid in current_brand_ids if bid is not None
                    ]
                    ...
                    for bid_str in updated_brand_ids:
                        try:
                            ...
                            final_brand_ids.append(bid_str)
            ...
            # Store processed data result
            if processed_result and processed_result.get("processed_data") is not None:
                accumulated_results[result_key] = processed_result["processed_data"]
                logger.info(f"Processed result for {result_key}")
            else:
                accumulated_results[result_key] = raw_result_data
                logger.info(
                    f"Using raw result for {result_key} (processing failed or not needed)"
                )
        ...
        summary = self.synthesizer.synthesize_results(
            ...
            api_results=accumulated_results,
            explanation=explanation,
        )
        ...
        return {
            ...
            "data": {
                ...
                "processed_results": accumulated_results,
                "complete_processor_results": complete_processor_results,
            },
            "summary": summary,
        }
