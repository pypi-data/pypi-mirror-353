...
class APIAction(BaseModel):
    ...
    reason: str = Field(
        description="A clear reason that links this call to the user's request and explains what needs to be extracted from the expected data."
    )
    priority: int = Field(
...
class PlannerTool:
    ...
    def __init__(
        ...
        self.llm = llm or ChatOpenAI(
            model="google/gemini-2.0-flash-001",  # perplexity/r1-1776
            temperature=0.0,
    ...
    def get_api_plan(
        self, query: str, country_mapping: Optional[Dict[str, int]] = None
    ) -> Tuple[List[Dict], str]:
        ...
        country_ids = None
        if country_mapping:
            country_ids = list(country_mapping.values()) if country_mapping else None
