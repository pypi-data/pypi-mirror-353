...
from .config import load_config, PERMISSIONS
...
async def execute_sql(sqls: str) -> str:
    """
    在MySql数据库上执行";"分割的SQL语句并返回结果(Execute the SQL statements separated by ";" on the MySql database and return the results)
    """
    role = config["role"]
    results = []
    try:
...
async def get_table_indexes(table_names: str) -> str:
    """
    获取指定表的索引信息
    :param table_names:要查询的表名，多个表名以逗号分隔
    :return:- 返回表的索引名、索引字段、索引类型等信息
            - 结果按表名、索引名和索引顺序排序
            - 结果以CSV格式返回，包含列名和数据
    """
