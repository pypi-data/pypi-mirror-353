...
class OpenRouterSync:
    def __init__(
        ...
        :param auto_latest_free: 启用最新免费模型作为最后的容灾模型，默认开启
        ...
        self.auto_latest_free = auto_latest_free
    ...
    def chat(
            ...
            primary_model: Optional[str] = None,
            backup_models: Optional[List[str]] = None,
            auto_latest_free: Optional[bool] = None,
        ...
        :param backup_models: 备用模型列表，主模型失效时自动尝试
        :param auto_latest_free: 启用最新免费模型作为最后的容灾模型
        ...
        if auto_fallback or not model_try_list:
            free_models = self.get_free_models(fallback_max, only_id=True)
...
class OpenRouterAsync:
    ...
    async def chat(
        ...
        # 所有主模型和备用模型都失败后，尝试免费模型
        if auto_fallback or not model_try_list:
            free_models = await self.get_free_models(fallback_max, only_id=True)
