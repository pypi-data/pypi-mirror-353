...
def extract_binary_result(solution_latex):
    ...
    Returns:
        str: Chuỗi kết quả nhị phân (ví dụ: "11010011"),
             ...
             hoặc None nếu không thể trích xuất kết quả.
    """
    # 1. Kiểm tra dấu hiệu "OVERFLOW" rõ ràng từ logic tạo giải pháp
    # Điều này cho biết số nằm ngoài phạm vi có thể biểu diễn.
    if "Result: OVERFLOW" in solution_latex:
        return "OVERFLOW"

    # 2. Cố gắng trích xuất kết quả nhị phân cho các chuyển đổi thành công
    # Mẫu cho định dạng "Final answer" chuẩn
    # Ví dụ: \\begin{center} Final answer: The 8-bit ... representation of ... is $11010011_2$ \\end{center}
    # Cho phép có dòng mới tùy chọn sau \\begin{center} và trước \\end{center}
    final_answer_match = re.search(
        r"\\\\begin{center}\\s*Final answer: The \\d+-bit .+? representation of .+? is \\\\$([01]+)_2\\\\$\\s*\\\\end{center}",
        solution_latex
    )
    if final_answer_match:
        return final_answer_match.group(1)

    # Mẫu dự phòng cho các định dạng "Result: $BINARY_2$" đơn giản hơn (thường cho số dương)
    # Ví dụ: \\begin{center} Result: $00100001_2$ \\end{center} 
    # Hoặc đôi khi chỉ là "Result: $00100001_2$" không có môi trường center trong các trường hợp đơn giản hơn
    simple_result_match = re.search(r"Result: \\\\$([01]+)_2\\\\$", solution_latex)
    if simple_result_match:
        return simple_result_match.group(1)
    
    # Một mẫu phổ biến khác cho các số dương trong d2b_solutions.py
    # Ví dụ: "... Two's Complement representation is: $00100001_2$" (không phải lúc nào cũng trong \\begin{center})
