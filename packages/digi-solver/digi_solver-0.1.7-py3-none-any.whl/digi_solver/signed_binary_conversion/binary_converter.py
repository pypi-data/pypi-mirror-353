...
def extract_binary_result(solution_latex):
    ...
    Returns:
        str: Chuỗi kết quả nhị phân (ví dụ: "11010011"),
    ...
    # Các hàm này trả về chuỗi bắt đầu bằng "Overflow: Cannot represent..."
    ...
    # Mẫu cho định dạng "Final answer" chuẩn, có thể có hoặc không có môi trường center
    # Ví dụ: \\begin{center} Final answer: The 8-bit ... representation of ... is $11010011_2$ \\end{center}
    # Hoặc: Final answer: The 8-bit ... representation of ... is $11010011_2$
    final_answer_match = re.search(
        r"Final answer: The \\d+-bit .+? representation of .+? is \\\\$([01]+)_2\\\\$",
        solution_latex
    )
    if final_answer_match:
        return final_answer_match.group(1)

    # Mẫu cho các trường hợp số dương mà kết quả có thể nằm ngay sau "... representation is:"
    # Ví dụ: "... One's Complement representation is: $0110101_2$"
    # hoặc "... Two's Complement representation is: $00100001_2$"
    positive_representation_match = re.search(r"representation is: \\\\$([01]+)_2\\\\$", solution_latex)
    if positive_representation_match:
        return positive_representation_match.group(1)

    # Mẫu dự phòng cho các định dạng "Result: $BINARY_2$" đơn giản hơn
    # Thường ít gặp hơn trong các hàm solution hiện tại nhưng để dự phòng
    simple_result_match = re.search(r"Result: \\\\$([01]+)_2\\\\$", solution_latex)
    if simple_result_match:
        return simple_result_match.group(1)
    
    # Nếu không tìm thấy kết quả cụ thể hoặc dấu hiệu "OVERFLOW" sau khi thử tất cả các mẫu
    # Điều này chỉ ra lỗi parsing LaTeX hoặc một định dạng LaTeX không mong muốn.
    return None
