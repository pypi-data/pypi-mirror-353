...
def extract_binary_result(solution_latex):
    ...
    Hàm này được thiết kế để ưu tiên tìm kết quả trong phần "Final answer" trước,
    sau đó mới quay lại các mẫu khác để đảm bảo độ chính xác.
    ...
    """
    # 1. Check for a definitive overflow message first.
    if solution_latex.startswith("Overflow: Cannot represent"):
        return "OVERFLOW"

    # 2. Prioritize the "Final answer" section to get the most accurate result.
    final_answer_marker = "Final answer:"
    try:
        # Find the last occurrence of "Final answer:" to handle complex strings
        final_answer_index = solution_latex.rfind(final_answer_marker)
        if final_answer_index != -1:
            substring_from_final_answer = solution_latex[final_answer_index:]
            
            # Search for the first binary number in this specific part of the string.
            match = re.search(r"\$([01]+)_2\$", substring_from_final_answer)
            if match:
                return match.group(1)
    except Exception:
        # If any error occurs in this block, we'll just fall through to the other methods.
        pass

    # 3. If "Final answer" isn't found, try other patterns. As a last resort,
    # find the *last* binary string in the solution, as it's most likely the final one.
    all_binary_matches = re.findall(r"\$([01]+)_2\$", solution_latex, re.S)
    if all_binary_matches:
        # Return the last match, as it's most likely to be the final result,
        # avoiding intermediate calculation results.
        return all_binary_matches[-1]
    
    # Ghi lại lỗi parsing nếu không có mẫu nào khớp
    return None
