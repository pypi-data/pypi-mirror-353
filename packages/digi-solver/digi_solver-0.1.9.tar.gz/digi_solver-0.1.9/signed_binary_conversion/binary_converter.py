...
def extract_binary_result(solution_latex):
    """
    Trích xuất kết quả nhị phân hoặc xác định tình trạng tràn số thực sự từ chuỗi giải pháp LaTeX.

    Args:
        solution_latex (str): Chuỗi LaTeX chứa các bước giải.

    Returns:
        str: Chuỗi kết quả nhị phân (ví dụ: "11011001"),
             "OVERFLOW" nếu có tình trạng tràn số thực sự được chỉ định trong giải pháp,
             hoặc None nếu không thể trích xuất kết quả (lỗi parsing).
    """
    # 1. Kiểm tra dấu hiệu OVERFLOW dựa trên cách các hàm d2b_solutions.py trả về lỗi
    if solution_latex.startswith("Overflow: Cannot represent"):
        return "OVERFLOW"

    # 2. Cố gắng trích xuất kết quả nhị phân cho các chuyển đổi thành công

    # Mẫu ưu tiên 1: Tìm "Final answer: The X-bit ... representation of $Y_{10}$ is $BINARY_2$"
    final_answer_match = re.search(
        r"Final answer:\s*The\s+\d+-bit\s+(?:Signed Magnitude|One's Complement|Two's Complement)\s+representation\s+of\s+\$[-+]?\d+\$_{10}\s+is\s+\$([01]+)_2\$",
        solution_latex,
        re.S
    )
    ...
    # Mẫu ưu tiên 2: Tìm "... representation is: $BINARY_2$" (thường cho số dương)
    positive_representation_match = re.search(
        r"(?:Signed Magnitude|One's Complement|Two's Complement)\s+representation\s+is:\s*\$([01]+)_2\$",
        solution_latex,
        re.S
    )
    if positive_representation_match:
        return positive_representation_match.group(1)
    
    # Mẫu dự phòng cho "Result: $BINARY_2$"
    simple_result_match = re.search(r"Result:\s*\$([01]+)_2\$", solution_latex, re.S)
    if simple_result_match:
        return simple_result_match.group(1)

    # Mẫu fallback cuối cùng: Tìm bất kỳ $BINARY_2$ nào
    # Điều này giúp xác định xem vấn đề là do context hay do không tìm thấy mẫu $..._2$ überhaupt
    fallback_binary_match = re.search(r"\$([01]+)_2\$", solution_latex, re.S)
    if fallback_binary_match:
        # Nếu fallback này khớp, có nghĩa là các regex cụ thể hơn ở trên có vấn đề với context xung quanh.
        # Bạn có thể muốn ghi log trường hợp này hoặc xử lý khác đi.
        # print(f"DEBUG: Fallback regex matched: {fallback_binary_match.group(1)}. Contextual regexes failed. LaTeX content:\n{solution_latex[:500]}...") # Uncomment for debugging
        return fallback_binary_match.group(1) # Trả về kết quả nếu tìm thấy, nhưng cảnh báo rằng context bị lỗi
    
    # Ghi lại lỗi parsing nếu không có mẫu nào khớp (kể cả fallback $..._2$)
    # print(f"DEBUG: All LaTeX Parsing Failed. LaTeX content:\n{solution_latex[:500]}...") # Uncomment for intensive debugging
    return None
...
def auto_generate_dataset(count=100, bit_length=8, type_filter=None, filename="auto_generated_dataset.csv"):
    ...
    while generated_count < count and attempts < max_attempts:
        ...
        # Extract the binary result or handle overflow/parsing error
        binary_result_extracted = extract_binary_result(solution)
        if binary_result_extracted == "OVERFLOW":
            formatted_result = "Overflow"
        elif binary_result_extracted is not None: # It's a binary string
            formatted_result = f"${binary_result_extracted}_2$"
        else: # binary_result_extracted is None, meaning LaTeX parsing failed
            formatted_result = "Error: LaTeX Parsing Failed" # Indicate parsing error
...
def handle_manual_conversion(args):
    ...
    if args.direction == "d":
        try:
            ...
            binary_representation_extracted = extract_binary_result(solution)
            if binary_representation_extracted == "OVERFLOW":
                formatted_result = "Overflow"
            elif binary_representation_extracted is not None: # It's a binary string
                formatted_result = f"${binary_representation_extracted}_2$"
            else: # binary_representation_extracted is None, meaning LaTeX parsing failed
                formatted_result = "Error: LaTeX Parsing Failed" # Indicate parsing error
