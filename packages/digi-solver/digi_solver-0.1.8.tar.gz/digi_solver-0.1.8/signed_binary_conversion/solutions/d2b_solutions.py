def get_signed_magnitude_solution(decimal_num, bit_length):
    ...
    if decimal_num > max_val or decimal_num < min_val:
        return f"Overflow: Cannot represent {decimal_num} in {bit_length}-bit Signed Magnitude. Valid range is [$-2^{{{bit_length}-1}} - 1$, $2^{{{bit_length}-1}} - 1$] or [{min_val}, {max_val}]."
...
def get_ones_complement_solution(decimal_num, bit_length):
    ...
    if decimal_num > max_val or decimal_num < min_val:
        return f"Overflow: Cannot represent {decimal_num} in {bit_length}-bit One's Complement. Valid range is [$-2^{{{bit_length}-1}} - 1$, $2^{{{bit_length}-1}} - 1$] or [{min_val}, {max_val}]."
...
def get_twos_complement_solution(decimal_num, bit_length):
    ...
    if decimal_num > max_val or decimal_num < min_val:
        return f"Overflow: Cannot represent {decimal_num} in {bit_length}-bit Two's Complement. Valid range is [$-2^{{{bit_length}-1}}$, $2^{{{bit_length}-1}} - 1$] or [{min_val}, {max_val}]."
