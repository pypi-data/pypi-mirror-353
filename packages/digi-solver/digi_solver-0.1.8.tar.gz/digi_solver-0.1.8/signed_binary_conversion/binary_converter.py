...
def extract_binary_result(solution_latex):
    ...
    # Mẫu ưu tiên: Tìm "Final answer: ... is $BINARY_2$"
    # re.S (hoặc re.DOTALL) cho phép . khớp với cả ký tự xuống dòng.
    # \s* xử lý các khoảng trắng/dòng mới xung quanh các phần chính.
    final_answer_match = re.search(
        r"Final answer:.*?The \\d+-bit .+? representation of .+? is \\s*\\\$([01]+)_2\\\\$",
        solution_latex,
        re.S  # Cho phép . khớp với newline
    )
    if final_answer_match:
        return final_answer_match.group(1)

    # Mẫu thứ hai: Tìm "... representation is: $BINARY_2$"
    # Thường dùng cho các số dương khi không có phần "Final answer" đầy đủ.
    positive_representation_match = re.search(
        r"representation is:\\s*\\\$([01]+)_2\\\\$", 
        solution_latex,
        re.S # Cho phép . khớp với newline
    )
    ...
    # Mẫu dự phòng cuối cùng (ít có khả năng xảy ra với cấu trúc LaTeX hiện tại từ d2b_solutions)
    ...
    return None # Lỗi parsing nếu không khớp mẫu nào
