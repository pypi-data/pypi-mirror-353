...
@retry_on_json_error
def number_question_AMI(openai_model, prompt,sync_layer, general_tagged):
        schema = """
        {
        ...
        messages=[
                ...
                you cannot miss even a single "[" or "]". Each and everything should be captured.
...
@retry_on_json_error
def number_logic_AMI(openai_model, input_json,user_logic,input_text, logic_tagged):
    ...
    parsed_text = openai_model.client_open_ai.chat.completions.create(
        ...
        messages=[
            ...
            - If you observe any logic that we support, add a new field called `"number_logic"` with the following format:
            ...
            Important: There can be multiple logics in the number_logic field, so ensure that you add all the logics you can infer from the provided data.
