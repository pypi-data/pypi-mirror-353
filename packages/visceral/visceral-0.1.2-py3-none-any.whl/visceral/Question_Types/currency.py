...
def currency_question(openai_model, prompt):
        schema = """
        {
            ...
            "type"="currency"
            "currency":"USD" #capture the currency if mentioned in the text or in front of currency, by default it will be USD, for example if we have ¥, then it will be CNY
        }
        """
        ...
        parsed_text = openai_model.client_open_ai.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are a precise survey question parser that captures every detail without losing any information. The most important thing is that we should not delete even a single dot. You are dealing with currency question type"},
                {"role": "user", "content": f"""
                ...
                ```
                Ask if Q3==0 \n
                QM. Demo currency Question? Disqualify if <18 \n random text
                ```
                ...
                {{
                    ...
                    "questionLabel": "QM",
                    "question": "Demo currency Question? `{{"Disqualify if <18": None}}` \n random text",
                    "extra_text":""
                    "type":"currency"
...
def currency_logic(openai_model, input_json,user_logic):
    ...
    parsed_text = openai_model.client_open_ai.chat.completions.create(
        ...
        messages=[
            ...
            "extra_text":"",
            "type":"currency",
            "currency_logic": {{"Terminating":"QM==0"}}
