...
def ranking_question(oepnai_model, prompt,keymap_name):
        schema = """
        {
        ...
        parsed_text = oepnai_model.client_open_ai.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are a precise survey question parser that captures every detail without losing any information. The most important thing is that we should not delete even a single dot. You are dealing with ranking question type"},
                {"role": "user", "content": f"""
                ...
                - Example: "This is a a Demo Ranking Question. (Disqualify if <18)" becomes:
                "This is a a Demo Ranking Question. `{{"Disqualify if <18": None}}`"
                ...
                Important: If we dont have any options, but the question says to create plaeholders. then please create appropriate options based on the questions text.
...
def ranking_logic(openai_model, input_json,user_logic):
