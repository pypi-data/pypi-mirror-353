...
import time
from CreateSection.Functions.process_logic import *
from CreateSection.Functions.process_hidden_variable import *
from CreateSection.Functions.process_maxdiff import *
from CreateSection.Functions.process_grid import process_grid_question
...
def process_question(question_data: dict, raw_data:str, key_id=None, variable_list_keyword=None) -> dict:
    try:
        if not isinstance(question_data, dict):
            raise ValueError("Question data must be a dictionary")
        
        # Get basic question properties
        print("we are processing for question", question_data)
        ...
        annotated_text=question_data.get("annotated_text"," Not found")
        ...
        if question_type in ['van_westendorp', 'van_westerndorp']:
            ...
            return {
                ...
                "mdText": question_text_for_van_westerndorp,
                "created": current_time,
                "subtype": "",
                "updated": current_time,
                "condition": condition,
        ...
        if question_type == 'max_diff' or question_type=="maxdiff":
            ask_logic = question_data.get('ask_logic', '')
            condition = ask_logic.get('1', '') if isinstance(ask_logic, dict) else ask_logic
            
            result = process_max_diff_question(question_data, current_time,raw_data)
        ...
        return {
            ...
            "mdText": question_data.get('question', ''),
            "label": label,
            "options": [],
            "updated": current_time,
            "subtype": None,
