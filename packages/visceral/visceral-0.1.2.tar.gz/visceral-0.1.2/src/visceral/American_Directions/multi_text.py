...
@retry_on_json_error
def multi_text_question_AMI(openai_model, prompt,sync_layer, general_tagged):
        schema = """
        {
        ...
        parsed_text = openai_model.client_open_ai.chat.completions.create(
            ...
            messages=[
                ...
                Important : If the question says to create response boxes but does not explicitly mention helper text, create options labeled as 'NA1,' 'NA2,' etc., with values as None
                important  but if it specifies helper text, create relevant options with placeholders based on the question's content and not NA1, NA2..with values as None
                ...
                6. Please capture the text after 'Ref' if it is given in the input text into a field called 'Ref'. For example, if the input text contains 'Ref:PHONEINTRO', then your output should include 'Ref': 'PHONEINTRO'.
                ...
                Make sure to add the type in the json
                
            
                
                Important: Give me a valid json.
...
@retry_on_json_error
def multi_text_logic_AMI(openai_model, input_json,user_logic,input_text, logic_tagged):
    ...
    parsed_text = openai_model.client_open_ai.chat.completions.create(
        ...
        messages=[
            ...
            You will receive a JSON containing a multi-text question. 
            ...
            - If you observe any logic that we support, add a new field called `"multi_text_logic"` with the following format:
            ...
            Important: There can be multiple logics in the multi_text_logic_logic field, so ensure that you add all the logics you can infer from the provided data.
            Please pay special attention to the instructions for the logics and double-check the logic marking to ensure correctness.
            Most Important: No data should be deleted—not even a single dot or bracket.
