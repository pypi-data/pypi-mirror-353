    ...
    parsed_text = await asyncio.get_event_loop().run_in_executor(
        ...
        messages=[
            ...
            {"role": "user", "content": f"""
                ...
                Also Important,  If there is a newline character in the input ("example backslash n"), it should also be captured as a linebreak. please make sure to capture it properly. 
...
@retry_on_json_error
async def annotation_transition(openai_model,prompt,keymap, type_of_question,user_tags, variable_list_keyword=None):
