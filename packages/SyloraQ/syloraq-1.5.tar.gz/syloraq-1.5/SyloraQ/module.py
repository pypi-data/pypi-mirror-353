...
class Claw:
    def __init__(self, main_html="<h1>Default Page</h1>", subdomains=None, ip="0.0.0.0", port=8000,
                 logs=False, delete_if_inactive_for=None, custom_404_html=None,
                 expected_auth_token="YOUR_SECRET_TOKEN"):
        self.main_html = main_html
        self.subdomains = {}
        if subdomains:
            for name, html in subdomains.items():
                self.add_subdomain(name, html)
        self.ip = ip
        self.port = port
        self.logs = logs
        self.delete_if_inactive_for = delete_if_inactive_for
        ...
        class ClawHandler(http.server.BaseHTTPRequestHandler):
            ...
            def do_GET(self_inner):
                ...
                for sub_id, data in claw.subdomains.items():
                    if data["name"] == path:
                        data["last_active"] = time.time()
                        self_inner.send_response(200)
                        self_inner.send_header("Content-Type", "text/html")
                        self_inner.end_headers()
                        self_inner.wfile.write(data["html"].encode())
                        return
                if path == "/":
                    self_inner.send_response(200)
                    self_inner.send_header("Content-Type", "text/html")
            ...
            def do_POST(self_inner):
                ...
                if path.startswith("/api/"):
                    auth_header = self_inner.headers.get("auth") or self_inner.headers.get("Authorization")
                    if auth_header != claw.expected_auth_token:
                        self_inner.send_response(401)
                        self_inner.send_header("Content-Type", "application/json")
                        self_inner.end_headers()
                        self_inner.wfile.write(json.dumps({"error": "Unauthorized"}).encode())
                        return
                    ...
                    try:
                        ...
                        self_inner.send_header("Content-Type", "application/json")
                        self_inner.end_headers()
                        self_inner.wfile.write(json.dumps({"error": "Invalid JSON"}).encode())
                        return
                    ...
                    if path == "/api/message":
                        ...
                        if not msg:
                            ...
                            self_inner.send_header("Content-Type", "application/json")
                            self_inner.end_headers()
                            self_inner.wfile.write(json.dumps({"error": "Missing 'message' field"}).encode())
                            return

                        claw.received_messages.append(msg)
                        if claw.logs:
                        ...
                        self_inner.send_header("Content-Type", "application/json")
                        self_inner.end_headers()
                        self_inner.wfile.write(json.dumps({"status": "message received"}).encode())
                        return
        ...
        self.handler_class = ClawHandler

    def _nn(self, name: str) -> str:
        return name if name.startswith("/") else "/" + name

    def add_subdomain(self, name: str, html: str) -> str:
        if not name.startswith("/"):
            name = "/" + name
        sub_id = str(uuid.uuid4())
        self.subdomains[sub_id] = {
            "name": name,
            "html": html,
            "last_active": time.time()
        }
        if self.logs:
            print(f"[Claw] Added subdomain: {name} with id {sub_id}")
        return sub_id
