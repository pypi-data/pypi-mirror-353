...
from ctypes import byref, cast, c_char_p, c_void_p, c_size_t, c_uint32, POINTER
...
class Entry:
  ...
  __slots__ = ['_family', '_ip_address', '_mac_address', '_name']

  def __init__(self, name: str, family: str, mac_address: MacAddress, ip_address: int):
    self._name = name
    self._family = family
...
class EntryList(UnmanagedObject):
  ...
  def __get_entry(self, index):
    ...
    ip_addr = c_uint32()
    if (api.hebiLookupEntryListGetIpAddress(self._internal, index, byref(ip_addr)) != StatusSuccess):
      return None

    if name is None or family is None:
      return None
    return Entry(name, family, MacAddress(*tmp_buffer.bytes_), ip_addr.value)
...
class LookupDelegate(UnmanagedObject):
  ...
  def __init__(self):
    super().__init__(api.hebiLookupCreate(None, 0), on_delete=api.hebiLookupRelease)

  def reset(self):
    with LookupDelegate.__singleton_lock:
      api.hebiLookupReset(self, None, 0)
