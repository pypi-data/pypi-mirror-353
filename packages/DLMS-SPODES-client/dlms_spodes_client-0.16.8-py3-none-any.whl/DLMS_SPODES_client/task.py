...
from typing import Callable, Any, Optional, TypeVar, Generic, Protocol
...
class Base(ABC, Generic[T]):
    ...
    async def run(self, c: Client) -> result.Result[T]:
...
class ConditionalTask[T](Base):
    ...
    async def exchange(self, c: Client) -> result.Result:
        ...
        return result.Error().append_err(ValueError("Condition not satisfied"))


@dataclass
class Scheduler(Base[bool]):
    ...
    async def run(self, c: Client) -> result.Result[bool]:
