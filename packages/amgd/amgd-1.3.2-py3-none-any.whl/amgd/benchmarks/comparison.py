...
def compare_optimizers(X, y, optimizers=['amgd', 'adam', 'adagrad'], 
                      penalties=['l1', 'elasticnet'], cv_folds=5, test_size=0.2):
    """
    Compare different optimizers on the given dataset.
    ...
    -------
    dict
        Results dictionary with test results and metrics
    """
    ...
    for optimizer in optimizers:
        ...
        try:
            # Create model based on optimizer
            if optimizer.lower() == 'amgd':
            ...
            results['test_results'][optimizer] = {
                'score': score,
                'status': 'success'
            }
