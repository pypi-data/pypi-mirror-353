...
def diff_log(arr, periods=1) -> np.ndarray:
    # 1) signed-log1p transform
    x = signed_log1p(np.asarray(arr, dtype=float))
    # 2) difference at given lag
    return diff(x, periods=periods)
