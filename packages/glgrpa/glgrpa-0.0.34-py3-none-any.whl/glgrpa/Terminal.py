...
class Terminal:
    ...
    def __init__(self, dev:bool=False):
        ...
        if not Terminal._carpeta_raiz:
            ...
            self.mostrar(f"Carpeta raíz para logs: {Terminal._carpeta_raiz}")
        # Solo inicializa el log si aún no está inicializado
        if not Terminal._log_activo:
            Terminal._log_activo = self.__iniciar_log(Terminal._carpeta_raiz)
        self.log_activo = Terminal._log_activo
        if Terminal._log_activo:
            self.inicio_ejecucion(Terminal._carpeta_raiz)
            ...
            if filename:
                ...
                for parent in [path] + list(path.parents):
                    if parent.name == "src":
                        ...
                        return str(parent)
                frame = frame.f_back
        # Si no encuentra 'src', usar el directorio actual
        cwd = Path.cwd()
        logs_dir = cwd / "logs"
        logs_dir.mkdir(exist_ok=True)
        return str(cwd)
    ...
    def inicio_ejecucion(self, carpeta_raiz:str = "") -> None:
        ...
        if not Terminal._log_activo:
            Terminal._log_activo = self.__iniciar_log(carpeta_raiz)
        self.log_activo = Terminal._log_activo
