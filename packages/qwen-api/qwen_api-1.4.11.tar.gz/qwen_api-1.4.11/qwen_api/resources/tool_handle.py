...
def action_selection(messages, tools, model, temperature, max_tokens, stream, client):
    choice_messages = [
        ChatMessage(
            role="system",
            content=CHOICE_TOOL_PROMPT.format(
            list_tools=tools
            )
        ),
        ChatMessage(
...
def using_tools(messages, tools, model, temperature, max_tokens, stream, client):
    ...
    if not response_tool.ok:
        ...
        client.logger.error(
            f"API Error: {response_tool.status_code} {error_text}")
        raise QwenAPIError(
            f"API Error: {response_tool.status_code} {error_text}")
    ...
    return client._process_response_tool(response_tool)

async def async_action_selection(messages, tools, model, temperature, max_tokens, stream, client):
    choice_messages = [
        ChatMessage(
            role="system",
            content=CHOICE_TOOL_PROMPT.format(
...
async def async_using_tools(messages, tools, model, temperature, max_tokens, stream, client):
    tool = [tool if isinstance(tool, dict) else tool.dict() for tool in tools]
    tools_list = "\n".join([str(tool['function']) for tool in tool])
    msg_tool = [
        ChatMessage(
    ...
    if not response_tool.ok:
        ...
        client.logger.error(
            f"API Error: {response_tool.status} {error_text}")
        raise QwenAPIError(
            f"API Error: {response_tool.status} {error_text}")
