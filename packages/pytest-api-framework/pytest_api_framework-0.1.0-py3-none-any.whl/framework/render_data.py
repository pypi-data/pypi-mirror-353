...
from framework.settings import DATA_DIR
...
class RenderData(object):
    def __init__(self, data):
        ...
        self.parametrize = data.get("_parametrize")
    ...
    def render(self):
        ...
        with allure.step("渲染数据"):
            if self.parametrize:
                self.request = self.replace_variables_recursive(self.request, self.parametrize)
    ...
    def replace_variables_recursive(self, data, params):
        ...
        if isinstance(data, dict):  # 处理字典
            ...
            if key in params.keys():
                return params[key]  # 直接赋值（不存在则返回原值）
            return data
    ...
    def get_func_variable(self, keyword, func_name, *args):
        ...
        if not value:
            logger.error(f"未从common.py中或faker对象中获取到对应函数{keyword}")
