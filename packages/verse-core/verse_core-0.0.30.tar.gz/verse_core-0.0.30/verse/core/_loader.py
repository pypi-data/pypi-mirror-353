...
class Loader:
    ...
    def _prepare_component_dependency(
        ...
        elif cnamespace == "customc":
            cdep.kind = "custom"
            cdep.py_path = cpackage
        else:
            cdep.kind = "base"
            cdep.py_path = (
    ...
    def _prepare_provider_dependency(
        ...
        cnamespace, cpackage = component_type.split(".", 1)
        pdep = ProviderDep()
        pdep.handle = handle
        ...
        if "." in type:
            pnamespace, ppackage = type.split(".", 1)
            if pnamespace == "custom" or pnamespace == "customp":
                pdep.kind = "custom"
                pdep.py_path = ppackage
            elif pnamespace == "composite":
                pdep.kind = "composite"
                ccomposite, pcomposite = (
                    self._get_composite_component_provider(type)
                )
                pdep.py_path = (
                    f"{ROOT_PACKAGE_NAME}.composite.{ccomposite}.providers."
                    f"{pcomposite}"
                )
        elif cnamespace == "custom":
            pdep.kind = "custom"
            ppackage = type
            ...
            pdep.py_path = f"{cnamespace}.providers.{type}"
        pdep.resolved = False
        return pdep
    ...
    def generate_requirements(
        ...
        base_requirements: dict[str, list[str]] = {"verse-core": []}
        custom_requirements: set[str] = set()

        def traverse_provider(cnode: ComponentNode, pnode: ProviderNode):
            if pnode.requirements:
                ...
                if pnode.kind == "composite":
                    ...
                    cnamespace, cpackage = cnode.type.split(".", 1)
    ...
    def get_dependency_graph(
        ...
        def _get_component_node(cdep: ComponentDep) -> ComponentNode:
            ...
            node = ComponentNode(
                ...
                provider=_get_provider_node(cdep=cdep, pdep=cdep.provider),
            )
        ...
        def _prepare_unresolved(
            ...
            for chandle in self.manifest.components:
                ...
                for phandle, pconfig in cconfig.providers.items():
