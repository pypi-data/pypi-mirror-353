...
class ComponentSpec(DataModel):
    ...
    def get_py_path(self) -> str | None:
        ...
        if self.kind == Kind.CUSTOM:
            return "./component.py"
        namespace = f"{ROOT_PACKAGE_NAME}.components.{self.type}"
    ...
    def get_provider_py_path(self, name: str) -> str:
        ...
        if self.kind == Kind.CUSTOM or provider_spec.kind == Kind.CUSTOM:
            return f"./providers/{provider_spec.type}.py"
        namespace = f"{ROOT_PACKAGE_NAME}.components.{self.type}"
        return f"{namespace}.providers.{provider_spec.type}"
...
class SpecBuilder:
    ...
    def build_provider_spec(
        ...
        cnamespace, _ = component_type.split(".", 1)
        kind = Kind.CUSTOM if cnamespace == "custom" else Kind.BASE
        py_path = SpecBuilder._get_provider_path(component_type, provider_type)
        provider = Loader.load_class(py_path, Provider)
        title = " ".join(
            word.capitalize() for word in provider_type.split("_")
        )
    ...
    @staticmethod
    def _get_provider_path(component_type: str, provider_type: str) -> str:
        cnamespace, cpackage = component_type.split(".", 1)
        if cnamespace == "custom":
            py_path = f"{cpackage}"
            py_path = f"{py_path}.providers.{provider_type}"
        else:
