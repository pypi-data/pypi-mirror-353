...
from codecraft.args_formatter import (
    DotEnvFormatter,
    MarkdownHelpFormatter,
    YamlHelpFormatter,
)
from codecraft.deprecated import add_deprecated_model_args

from .dump import dump  # noqa: F401


def resolve_codecraftignore_path(path_str, git_root=None):
    path = Path(path_str)
    if path.is_absolute():
        return str(path)
    elif git_root:
...
def get_parser(default_config_files, git_root):
    parser = configargparse.ArgumentParser(
        description="codecraft is AI pair programming in your terminal",
        add_config_file_help=True,
        default_config_files=default_config_files,
        config_file_parser_class=configargparse.YAMLConfigFileParser,
        auto_env_var_prefix="CODECRAFT_",
    )
    ...
    group.add_argument(
        ...
        default=".codecraft.model.settings.yml",
        help="Specify a file with codecraft model settings for unknown models",
    )
    group.add_argument(
        "--model-metadata-file",
        metavar="MODEL_METADATA_FILE",
        default=".codecraft.model.metadata.json",
    ...
    default_input_history_file = (
        os.path.join(git_root, ".codecraft.input.history") if git_root else ".codecraft.input.history"
    )
    default_chat_history_file = (
        os.path.join(git_root, ".codecraft.chat.history.md") if git_root else ".codecraft.chat.history.md"
    )
    group.add_argument(
        "--input-history-file",
    ...
    group.add_argument(
        ...
        default=None,
        help="Log the conversation with the LLM to this file (for example, .codecraft.llm.history)",
    )
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable/disable adding .codecraft* to .gitignore (default: True)",
    )
    default_codecraftignore_file = (
        os.path.join(git_root, ".codecraftignore") if git_root else ".codecraftignore"
    )

    group.add_argument(
        "--codecraftignore",
        metavar="CODECRAFTIGNORE",
        type=lambda path_str: resolve_codecraftignore_path(path_str, git_root),
        default=default_codecraftignore_file,
        help="Specify the codecraft ignore file (default: .codecraftignore in git root)",
    )
    group.add_argument(
        "--subtree-only",
        action="store_true",
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Attribute codecraft code changes in the git author name (default: True). If explicitly set"
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Attribute codecraft commits in the git committer name (default: True). If explicitly set"
            " to True, overrides --attribute-co-authored-by precedence for codecraft edits."
        ),
    )
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix commit messages with 'codecraft: ' if codecraft authored the changes (default: False)",
    )
    group.add_argument(
        "--attribute-commit-message-committer",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix all commit messages with 'codecraft: ' (default: False)",
    )
    group.add_argument(
        ...
        default=False,
        help=(
            "Attribute codecraft edits using the Co-authored-by trailer in the commit message"
    ...
    group.add_argument(
        "--check-update",
        action=argparse.BooleanOptionalAction,
        help="Check for new codecraft versions on launch",
        default=True,
    )
    group.add_argument(
        "--show-release-notes",
    ...
    group.add_argument(
        ...
        "--update",
        action="store_true",
        help="Upgrade codecraft to the latest version from PyPI",
        default=False,
    )
    group.add_argument(
        "--version",
    ...
    group.add_argument(
        ...
        "--browser",
        action=argparse.BooleanOptionalAction,
        help="Run codecraft in your browser (default: False)",
        default=False,
    )
    group.add_argument(
        "--copy-paste",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable automatic copy/paste of chat between codecraft and web UI (default: False)",
    )
    group.add_argument(
        "--apply",
        metavar="FILE",
    ...
    group.add_argument(
        ...
        metavar="CONFIG_FILE",
        help=(
            "Specify the config file (default: search for .codecraft.conf.yml in git root, cwd"
            " or home directory)"
        ),
    )
    ...
    group.add_argument(
        ...
        help=(
            ...
            f" {', '.join(supported_shells_list)}. Example: codecraft --shell-completions bash"
        ),
    )
    ...
    if command == "md":
        ...
        if len(sys.argv) > 2:
            ...
            if shell not in shtab.SUPPORTED_SHELLS:
                ...
                sys.exit(1)
            parser = get_parser([], None)
            parser.prog = "codecraft"  # Set the program name on the parser
