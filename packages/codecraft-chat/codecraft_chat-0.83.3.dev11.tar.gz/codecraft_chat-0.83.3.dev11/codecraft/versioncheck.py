...
import codecraft
from codecraft import utils
from codecraft.dump import dump  # noqa: F401

VERSION_CHECK_FNAME = Path.home() / ".codecraft" / "caches" / "versioncheck"


def install_from_main_branch(io):
    """
    Install the latest version of codecraft from the main branch of the GitHub repository.
    """

    return utils.check_pip_install_extra(
        io,
        None,
        "Install the development version of codecraft from the main branch?",
        ["git+https://github.com/CodeCraft-AI/codecraft.git"],
        self_update=True,
    )
...
def install_upgrade(io, latest_version=None):
    """
    Install the latest version of codecraft from PyPI.
    """

    if latest_version:
        new_ver_text = f"Newer codecraft version v{latest_version} is available."
    else:
        new_ver_text = "Install latest version of codecraft?"

    docker_image = os.environ.get("CODECRAFT_DOCKER_IMAGE")
    if docker_image:
        text = f"""
...
"""
    ...
    if success:
        io.tool_output("Re-run codecraft to use new version.")
        sys.exit()
...
def check_version(io, just_check=False, verbose=False):
    ...
    try:
        response = requests.get("https://pypi.org/pypi/codecraft-chat/json")
        data = response.json()
        latest_version = data["info"]["version"]
        current_version = codecraft.__version__
