...
# Set UTF-8 mode at the earliest possible moment
...
from codecraft import __version__, models, urls, utils
from codecraft.analytics import Analytics
from codecraft.args import get_parser
from codecraft.coders import Coder
from codecraft.coders.base_coder import UnknownEditFormat
from codecraft.commands import Commands, SwitchCoder
from codecraft.copypaste import ClipboardWatcher
from codecraft.deprecated import handle_deprecated_model_args
from codecraft.format_settings import format_settings, scrub_sensitive_info
from codecraft.history import ChatSummary
from codecraft.io import InputOutput
from codecraft.llm import litellm  # noqa: F401; properly init litellm on launch
from codecraft.models import ModelSettings
from codecraft.onboarding import offer_openrouter_oauth, select_default_model
from codecraft.repo import ANY_GIT_ERROR, GitRepo
from codecraft.report import report_uncaught_exceptions
from codecraft.versioncheck import check_version, install_from_main_branch, install_upgrade
from codecraft.watch import FileWatcher
...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning(
            "You should probably run codecraft in your project's directory, not your home dir."
        )
        return
    elif cwd and io.confirm_ask(
        "No git repo found, create one to track codecraft's changes (recommended)?"
    ):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        ...
        if not repo.ignored(".codecraft"):
            patterns_to_add.append(".codecraft*")
...
def check_streamlit_install(io):
    return utils.check_pip_install_extra(
        io,
        "streamlit",
        "You need to install the codecraft browser feature",
...
def write_streamlit_credentials():
    ...
    # See https://github.com/CodeCraft-AI/codecraft/issues/772
...
def launch_gui(args):
    ...
    # https://github.com/CodeCraft-AI/codecraft/issues/2193
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    ...
    resource_settings = importlib_resources.files("codecraft.resources").joinpath("model-settings.yml")
    model_settings_files.append(str(resource_settings))
    
    model_settings_files += generate_search_path_list(
        ".codecraft.model.settings.yml", git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading codecraft model settings: {e}")
        return 1
...
def load_dotenv_files(git_root, dotenv_fname, encoding="utf-8"):
    ...
    oauth_keys_file = Path.home() / ".codecraft" / "oauth-keys.env"
...
def register_litellm_models(git_root, model_metadata_fname, io, verbose=False):
    ...
    # Add the resource file path
    resource_metadata = importlib_resources.files("codecraft.resources").joinpath("model-metadata.json")
    model_metadata_files.append(str(resource_metadata))

    model_metadata_files += generate_search_path_list(
        ".codecraft.model.metadata.json", git_root, model_metadata_fname
    )
...
def sanity_check_repo(repo, io):
    ...
    if bad_ver:
        io.tool_error("CodeCraft only works with git repos with version number 1 or 2.")
        io.tool_output("You may be able to convert your repo: git update-index --index-version=2")
        io.tool_output("Or run codecraft --no-git to proceed without using git.")
        io.offer_url(urls.git_index_version, "")
        return False
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    # Force UTF-8 encoding to avoid UnicodeDecodeError with litellm
    ...
    conf_fname = Path(".codecraft.conf.yml")
    ...
    if args.analytics is not False:
        if analytics.need_to_ask(args.analytics):
            io.tool_output(
                "CodeCraft respects your privacy and never collects your code, chat messages, keys or"
                " personal info."
            )
            io.tool_output(f"For more info: {urls.analytics}")
            disable = not io.confirm_ask(
                "Allow collection of anonymous analytics to help improve codecraft?"
            )
    ...
    if args.model.startswith("openrouter/") and not os.environ.get("OPENROUTER_API_KEY"):
        ...
        if offer_openrouter_oauth(io, analytics):
            ...
            io.offer_url(urls.models_and_keys, "")
            analytics.event(
                "exit",
    ...
    if args.show_model_warnings:
        ...
        if problem:
            ...
            # If the model has no info (unknown model), exit with error
            if not main_model.info:
                io.tool_error(f"Error: Model '{main_model.name}' does not exist.")
                
                # Show possible matches if available
                possible_matches = models.fuzzy_match_models(main_model.name)
                if possible_matches:
                    io.tool_output("Did you mean one of these?")
                    for match in possible_matches:
                        io.tool_output(f"- {match}")
                
                analytics.event("exit", reason="Unknown model")
                return 1
            
            # For other warnings, show the message but continue
            io.tool_output("You can skip this check with --no-show-model-warnings")

            try:
                io.offer_url(urls.model_warnings, "")
                io.tool_output()
    ...
    if args.codecraftignore:
        ignores.append(args.codecraftignore)
    ...
    if args.apply:
        ...
        # For testing #2879
        # from codecraft.coders.base_coder import all_fences
...
def is_first_run_of_new_version(io, verbose=False):
    ...
    installs_file = Path.home() / ".codecraft" / "installs.json"
    ...
    try:
        if is_first_run:
            ...
            try:
                ...
                io.tool_error(str(err))
                io.tool_output("Error loading required imports. Did you install codecraft properly?")
...
def main_with_utf8():
    """
    Wrapper function to enable UTF-8 mode for proper handling of non-ASCII characters.
    """
    # Force the UTF-8 encoding at the system level
    import sys
    import os
    import locale
    ...
    # Try to set the preferred encoding for stdin/stdout/stderr
    try:
    ...
    # On Windows, explicitly set console to UTF-8 mode
    if sys.platform == "win32":
        try:
            ...
            # Try to set locale for Windows
            try:
                locale.setlocale(locale.LC_ALL, '.UTF-8')
            except locale.Error:
                # Fall back to default locale with UTF-8 if en_US.UTF-8 is not available
                pass
        except Exception as e:
            # Just log the error but continue execution
            print(f"Warning: Unable to set Windows console to UTF-8: {e}")
            
    # Patch the JSON module to always use UTF-8
    import json
    ...
    # Patch open to ensure UTF-8 encoding
    original_open = open
    ...
    # Replace built-in open with our UTF-8 version
    __builtins__['open'] = utf8_open
    
    # Patch sys.modules to ensure imports use UTF-8
    import sys
    original_import = __import__
    
    def utf8_import(name, *args, **kwargs):
        try:
            return original_import(name, *args, **kwargs)
        except UnicodeDecodeError:
            # If there's a UnicodeDecodeError, try to reload with UTF-8
            import importlib
            import types
            ...
            if name in sys.modules:
                ...
                if hasattr(module, '__file__') and module.__file__:
                    ...
                    module_dict = types.ModuleType(name).__dict__
                    exec(code, module_dict)
                    sys.modules[name] = types.ModuleType(name, module_dict)
            return original_import(name, *args, **kwargs)
    
    __builtins__['__import__'] = utf8_import
    
    # Now we can safely call the main function
    try:
        return main()
    except UnicodeDecodeError as e:
        print(f"\nUnicodeDecodeError encountered: {e}")
        print("This appears to be an encoding issue. Try using the run_codecraft.bat script or")
        print("run the following command instead:")
        print("  python -X utf8 -m codecraft [your_arguments]")
        return 1
