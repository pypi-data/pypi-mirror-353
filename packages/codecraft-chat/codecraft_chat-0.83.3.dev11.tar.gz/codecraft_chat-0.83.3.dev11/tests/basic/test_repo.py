...
from codecraft.models import Model
from codecraft.repo import GitRepo
from codecraft.utils import GitTemporaryDirectory
...
class TestRepo(unittest.TestCase):
    ...
    @patch("codecraft.models.Model.simple_send_with_retries")
    ...
    @patch("codecraft.repo.GitRepo.get_commit_message")
    def test_commit_with_custom_committer_name(self, mock_send):
        ...
        with GitTemporaryDirectory():
            ...
            # commit a change with codecraft_edits=True (using default attributes)
            fname.write_text("new content")
            commit_result = git_repo.commit(fnames=[str(fname)], codecraft_edits=True)
            self.assertIsNotNone(commit_result)

            # check the committer name (defaults interpreted as True)
            commit = raw_repo.head.commit
            self.assertEqual(commit.author.name, "Test User (codecraft)")
            self.assertEqual(commit.committer.name, "Test User (codecraft)")

            # commit a change without codecraft_edits (using default attributes)
            fname.write_text("new content again!")
            commit_result = git_repo.commit(fnames=[str(fname)], codecraft_edits=False)
            self.assertIsNotNone(commit_result)
            ...
            commit_result = git_repo_explicit_false.commit(fnames=[str(fname)], codecraft_edits=True)
            self.assertIsNotNone(commit_result)
            commit = raw_repo.head.commit
            ...
            commit_result = git_repo_user_no_committer.commit(
                fnames=[str(fname)], codecraft_edits=False
            )
            self.assertIsNotNone(commit_result)
            commit = raw_repo.head.commit
            self.assertEqual(
    ...
    def test_commit_with_co_authored_by(self):
        with GitTemporaryDirectory():
            ...
            # commit a change with codecraft_edits=True and co-authored-by flag
            fname.write_text("new content")
            commit_result = git_repo.commit(
                fnames=[str(fname)], codecraft_edits=True, coder=mock_coder, message="codecraft edit"
            )
            self.assertIsNotNone(commit_result)

            # check the commit message and author/committer
            commit = raw_repo.head.commit
            self.assertIn("Co-authored-by: codecraft (gpt-test) <noreply@codecraft.chat>", commit.message)
            self.assertEqual(commit.message.splitlines()[0], "codecraft edit")
    ...
    def test_commit_co_authored_by_with_explicit_name_modification(self):
        ...
        with GitTemporaryDirectory():
            ...
            # commit a change with codecraft_edits=True and combo flags
            fname.write_text("new content combo")
            commit_result = git_repo.commit(
                fnames=[str(fname)], codecraft_edits=True, coder=mock_coder, message="codecraft combo edit"
            )
            self.assertIsNotNone(commit_result)
            ...
            commit = raw_repo.head.commit
            self.assertIn(
                "Co-authored-by: codecraft (gpt-test-combo) <noreply@codecraft.chat>", commit.message
            )
            self.assertEqual(commit.message.splitlines()[0], "codecraft combo edit")
    ...
    def test_commit_ai_edits_no_coauthor_explicit_false(self):
        # Test AI edits (codecraft_edits=True) when co-authored-by is False,
        ...
        with GitTemporaryDirectory():
            ...
            commit_result = git_repo_no_author.commit(
                fnames=[str(fname)],
                codecraft_edits=True,
                coder=mock_coder_no_author,
                message="codecraft no author",
            )
            self.assertIsNotNone(commit_result)
            commit = raw_repo.head.commit
            self.assertNotIn("Co-authored-by:", commit.message)
            self.assertEqual(commit.author.name, "Test User")  # Explicit False
            self.assertEqual(commit.committer.name, "Test User (codecraft)")  # Default True
            ...
            commit_result = git_repo_no_committer.commit(
                ...
                message="codecraft no committer",
            )
            self.assertIsNotNone(commit_result)
            commit = raw_repo.head.commit
            self.assertNotIn("Co-authored-by:", commit.message)
    ...
    def test_get_tracked_files_with_codecraftignore(self):
        with GitTemporaryDirectory():
            # new repo
            raw_repo = git.Repo()
            ...
            codecraftignore = Path(".codecraftignore")
            git_repo = GitRepo(InputOutput(), None, None, str(codecraftignore))
            ...
            codecraftignore.write_text("new.txt\n")
            time.sleep(2)
            ...
            #
            # codecraftignore.write_text("new2.txt\n")
