...
from codecraft import models, sendchat
from codecraft.coders import Coder, base_coder
from codecraft.dump import dump  # noqa: F401
from codecraft.io import InputOutput

BENCHMARK_DNAME = Path(os.environ.get("CODECRAFT_BENCHMARK_DIR", "tmp.benchmarks"))
...
def main(
    ...
    replay: str = typer.Option(
        None,
        "--replay",
        help="Replay previous .codecraft.chat.history.md responses from previous benchmark run",
    ),
    ...
    no_codecraft: bool = typer.Option(False, "--no-codecraft", help="Do not run codecraft"),
    ...
    read_model_settings: str = typer.Option(
        None, "--read-model-settings", help="Load codecraft model settings from YAML file"
    ),
    ...
    if "CODECRAFT_DOCKER" not in os.environ:
...
def show_diffs(dirnames):
    ...
    for testcase in testcases:
        ...
        for outcome, dirname in zip(all_outcomes, dirnames):
            print(outcome, f"{dirname}/{testcase}/.codecraft.chat.history.md")
...
def load_results(dirname, stats_languages=None):
    ...
    if stats_languages:
        ...
        glob_patterns = [f"{lang}/exercises/practice/*/.codecraft.results.json" for lang in languages]
    else:
        glob_patterns = ["*/exercises/practice/*/.codecraft.results.json"]
...
def summarize_results(dirname, stats_languages=None):
    ...
    if variants["model"]:
        ...
        command = f"codecraft --model {a_model}"
    ...
    for hsh in commit_hashes:
        ...
        try:
            version = subprocess.check_output(
                ["git", "show", f"{hsh}:codecraft/__init__.py"], universal_newlines=True
            )
...
def get_replayed_content(replay_dname, test_dname):
    ...
    replay_fname = replay_dname / test_name / ".codecraft.chat.history.md"
    dump(replay_fname)
...
def run_test(original_dname, testdir, *args, **kwargs):
    try:
        ...
        results_fname = testdir / ".codecraft.results.json"
...
def run_test_real(
    ...
    history_fname = testdir / ".codecraft.chat.history.md"
...
def run_unit_tests(original_dname, testdir, history_fname, test_files):
    ...
    TEST_COMMANDS = {
        ...
        ".go": ["go", "test", "./..."],
        ".js": ["/codecraft/benchmark/npm-test.sh"],
        ".cpp": ["/codecraft/benchmark/cpp-test.sh"],
        ".java": ["./gradlew", "test"],
    }
