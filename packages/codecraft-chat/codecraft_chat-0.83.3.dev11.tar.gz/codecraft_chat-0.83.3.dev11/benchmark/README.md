...
# CodeCraft benchmark harness

CodeCraft uses benchmarks to quantitatively measure how well it works
with various LLMs.
...
This
benchmark evaluates how effectively codecraft and LLMs can translate a
...
and *format those code edits* so that codecraft can save the
edits to the local source files.

See [this writeup for a longer discussion about the benchmark](https://codecraft.chat/2024/12/21/polyglot.html).
...
There are 3 main tasks involved in benchmarking codecraft:
...
```
# Clone the codecraft repo
git clone https://github.com/CodeCraft-AI/codecraft.git

# Create the scratch dir to hold benchmarking results inside the main codecraft dir:
cd codecraft
mkdir tmp.benchmarks

# Clone the repo with the exercises
git clone https://github.com/CodeCraft-AI/polyglot-benchmark tmp.benchmarks/polyglot-benchmark
...
# Inside the container, install codecraft as a development build.
...
- `--model` is the name of the model, same as you would pass directly to `codecraft`.
- `--edit-format` is the name of the edit format, same as you would pass directly to `codecraft`. When working with an experimental LLM, I recommend starting with `whole`
...
[codecraft leaderboard data files](https://github.com/CodeCraft-AI/codecraft/blob/main/codecraft/website/_data/).
...
[codecraft leaderboard data files](https://github.com/CodeCraft-AI/codecraft/blob/main/codecraft/website/_data/).
- These scripts are not intended for use by typical codecraft end users.
