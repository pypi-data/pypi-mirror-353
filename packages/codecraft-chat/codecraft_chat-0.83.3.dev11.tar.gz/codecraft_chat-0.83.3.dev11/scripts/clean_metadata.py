...
def main():
    ...
    # Adjust path relative to the script's location in the codecraft repo
    litellm_path = script_dir.parent / "../litellm/model_prices_and_context_window.json"
    codecraft_path = script_dir / "../codecraft/resources/model-metadata.json"

    if not litellm_path.exists():
        print(f"Error: LiteLLM metadata file not found at {litellm_path}")
        return

    if not codecraft_path.exists():
        print(f"Error: CodeCraft metadata file not found at {codecraft_path}")
        return
    ...
    try:
        # Use json5 for the codecraft metadata file as it might contain comments
        with open(codecraft_path, "r") as f:
            codecraft_data = json5.load(f)
    except json.JSONDecodeError as e:
        print(f"Error decoding JSON from {codecraft_path}: {e}")
        return
    except Exception as e:
        print(f"Error reading {codecraft_path}: {e}")
        return

    litellm_keys = set(litellm_data.keys())
    codecraft_keys = set(codecraft_data.keys())

    common_keys = sorted(list(litellm_keys.intersection(codecraft_keys)))
    removed_count = 0

    if common_keys:
        print("Comparing common models found in both files:\n")
        for key in common_keys:
            print(f"--- {key} (codecraft) ---")
            print(f"+++ {key} (litellm) +++")

            litellm_entry = litellm_data.get(key, {})
            codecraft_entry = codecraft_data.get(key, {})

            # Convert dicts to formatted JSON strings for comparison
            # First, compare the dictionaries directly for semantic equality
            if litellm_entry == codecraft_entry:
            ...
            codecraft_entry_copy = codecraft_entry.copy()
            dummy_key = "zzzdummykey"
            litellm_entry_copy[dummy_key] = True
            codecraft_entry_copy[dummy_key] = True

            litellm_json_lines = json.dumps(
                litellm_entry_copy, indent=4, sort_keys=True
            ).splitlines()
            codecraft_json_lines = json.dumps(codecraft_entry_copy, indent=4, sort_keys=True).splitlines()
            ...
            codecraft_json_filtered = [line for line in codecraft_json_lines if dummy_key not in line]

            diff = difflib.unified_diff(
                codecraft_json_filtered,
                litellm_json_filtered,
                fromfile=f"{key} (codecraft)",
                tofile=f"{key} (litellm)",
                lineterm="",
                n=max(len(litellm_json_filtered), len(codecraft_json_filtered)),  # Show all lines
            )
            ...
            # Ask user if they want to remove the entry from codecraft's metadata
            response = (
                input(f"Remove '{key}' from codecraft/resources/model-metadata.json? (y/N): ")
                .strip()
                .lower()
            )
            if response == "y":
                # Perform surgical removal from the text file
                if remove_block_surgically(codecraft_path, key):
                    removed_count += 1
                    # Optional: Also remove from the in-memory dict if needed later,
                    # but it's not strictly necessary if we reload or finish now.
                    # if key in codecraft_data: del codecraft_data[key]
                else:
                    print(f"Failed to remove '{key}' block surgically.")
                    # Key might still be in codecraft_data if removal failed
            else:
                print(f"Keeping '{key}'.")
            print("-" * 40 + "\n")  # Separator for the next model
