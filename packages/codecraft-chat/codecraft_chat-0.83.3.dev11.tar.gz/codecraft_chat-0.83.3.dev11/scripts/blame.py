...
website_files = [
    ...
    "codecraft/website/share/index.md",
    "codecraft/website/_includes/head_custom.html",
    "codecraft/website/_includes/home.css",
    "codecraft/website/docs/leaderboards/index.md",
]

exclude_files = [
    "codecraft/website/install.ps1",
    "codecraft/website/install.sh",
]
...
def blame(start_tag, end_tag=None):
    ...
    files = [
        ...
        or (f.startswith("codecraft/resources/") and f.endswith(".yml"))
    ...
    for file in files:
        ...
        if file_counts:
            ...
            for author, count in file_counts.items():
                ...
                if "(codecraft)" in author.lower():
                    codecraft_total += count

    total_lines = sum(grand_total.values())
    codecraft_percentage = (codecraft_total / total_lines) * 100 if total_lines > 0 else 0

    end_date = get_tag_date(end_tag if end_tag else "HEAD")

    return all_file_counts, grand_total, total_lines, codecraft_total, codecraft_percentage, end_date
...
def get_commit_authors(commits):
    ...
    for commit in commits:
        ...
        if commit_message.lower().startswith("codecraft:"):
            author += " (codecraft)"
...
def process_all_tags_since(start_tag):
    ...
    for i in tqdm(range(len(tags) - 1), desc="Processing tags"):
        ...
        all_file_counts, grand_total, total_lines, codecraft_total, codecraft_percentage, end_date = blame(
            start_tag, end_tag
        )
        results.append(
            {
                ...
                "codecraft_total": codecraft_total,
                "codecraft_percentage": round(codecraft_percentage, 2),
            }
        )
    return results
...
def main():
    parser = argparse.ArgumentParser(description="Get codecraft/non-codecraft blame stats")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Find all tags since the specified tag and print codecraft percentage between each pair of"
            " successive tags"
        ),
    )
    parser.add_argument(
    ...
    if not args.all_since:
        print(f"- CodeCraft wrote {round(codecraft_percentage)}% of the code in this release.")
