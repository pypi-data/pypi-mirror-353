...
def get_latest_version_from_history():
    ...
    match = re.search(r"### CodeCraft v(\d+\.\d+\.\d+)", history_content)
    if not match:
...
def main():
    ...
    version_header = f"### codecraft v{latest_ver}"
    ...
    next_version_idx = history_content.find("\n### codecraft v", version_idx + len(version_header))
    ...
    # Run blame to get codecraft percentage
    blame_result = subprocess.run(["python3", "scripts/blame.py"], capture_output=True, text=True)
    codecraft_line = blame_result.stdout.strip().split("\n")[-1]  # Get last line with percentage

    # Construct and run the codecraft command
    message = history_prompt.format(codecraft_line=codecraft_line)
    ...
    # Run update-docs.sh after codecraft
