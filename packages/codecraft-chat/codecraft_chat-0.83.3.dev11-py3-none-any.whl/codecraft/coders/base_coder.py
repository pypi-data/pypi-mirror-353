...
from codecraft import __version__, models, prompts, urls, utils
from codecraft.analytics import Analytics
from codecraft.commands import Commands
from codecraft.exceptions import LiteLLMExceptions
from codecraft.history import ChatSummary
from codecraft.io import ConfirmGroup, InputOutput
from codecraft.linter import Linter
from codecraft.llm import litellm
from rich.console import Console
from codecraft.models import RETRY_TIMEOUT
from codecraft.reasoning_tags import (
    REASONING_TAG,
...
from codecraft.repomap import RepoMap
from codecraft.run_cmd import run_cmd
from codecraft.utils import format_content, format_messages, format_tokens, is_image_file
...
class Coder:
    ...
    repo = None
    last_codecraft_commit_hash = None
    codecraft_edited_files = None
    ...
    def create(
        ...
        **kwargs,
        ...
        import codecraft.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                codecraft_commit_hashes=from_coder.codecraft_commit_hashes,
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"CodeCraft v{__version__}")
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .codecraftignore"
                )
    ...
    def __init__(
        ...
        self.codecraft_commit_hashes = set()
        ...
        if codecraft_commit_hashes:
            self.codecraft_commit_hashes = codecraft_commit_hashes
        else:
            self.codecraft_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches codecraftignore spec.")
                continue
    ...
    def init_before_message(self):
        self.codecraft_edited_files = set()
    ...
    def warm_cache(self, chunks):
        ...
        delay = 5 * 60 - 5
        delay = float(os.environ.get("CODECRAFT_CACHE_KEEPALIVE_DELAY", delay))
    ...
    def send_message(self, inp):
        ...
        if edited:
            self.codecraft_edited_files.update(edited)
    ...
    def auto_commit(self, edited, context=None):
        ...
        try:
            res = self.repo.commit(fnames=edited, context=context, codecraft_edits=True, coder=self)
            if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_codecraft_commit_hash = commit_hash
        self.codecraft_commit_hashes.add(commit_hash)
        self.last_codecraft_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head_commit_sha():
            self.io.tool_output("You can use /undo to undo and discard each codecraft commit.")
