...
from codecraft import prompts, utils
...
class GitRepo:
    repo = None
    codecraft_ignore_file = None
    codecraft_ignore_spec = None
    codecraft_ignore_ts = 0
    codecraft_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def __init__(
        ...
        if codecraft_ignore_file:
            self.codecraft_ignore_file = Path(codecraft_ignore_file)

    def commit(self, fnames=None, context=None, message=None, codecraft_edits=False, coder=None):
        """
        ...
        Args:
            ...
            codecraft_edits (bool, optional): Whether the changes were made by codecraft. Defaults to False.
        ...
        codecraft generated the changes (`codecraft_edits`).
        ...
        - codecraft_edits=True: Changes were generated by codecraft (LLM).
        - codecraft_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        Flags:
        - --attribute-author: Modify Author name to "User Name (codecraft)".
        - --attribute-committer: Modify Committer name to "User Name (codecraft)".
        - --attribute-co-authored-by: Add
          "Co-authored-by: codecraft (<model>) <noreply@codecraft.chat>" trailer to commit message.

        Behavior Summary:

        1. When codecraft_edits = True (AI Changes):
        ...
        2. When codecraft_edits = False (User Changes):
           - --attribute-co-authored-by is IGNORED (trailer never added).
           - Author name is NEVER modified (--attribute-author ignored).
           - Committer name IS modified by default (implicit True, as codecraft runs `git commit`).
           - EXCEPTION: If --attribute-committer is EXPLICITLY False, the name is NOT modified.

        Resulting Scenarios:
        - Standard AI edit (defaults): Co-authored-by=False -> Author=You(codecraft),
          Committer=You(codecraft)
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(codecraft), Committer=You, Trailer added
        - User commit (defaults): codecraft_edits=False -> Author=You, Committer=You(codecraft)
        - User commit with explicit no-committer: codecraft_edits=False,
        ...
        prefix_commit_message = codecraft_edits and (
        ...
        if codecraft_edits and attribute_co_authored_by:
            model_name = "unknown-model"
            if coder and hasattr(coder, "main_model") and coder.main_model.name:
                model_name = coder.main_model.name
            commit_message_trailer = (
                f"\n\nCo-authored-by: codecraft ({model_name}) <noreply@codecraft.chat>"
            )

        # Determine if author/committer names should be modified
        # Author modification applies only to codecraft edits.
        # It's used if effective_author is True AND
        # (co-authored-by is False OR author was explicitly set).
        use_attribute_author = (
            codecraft_edits and effective_author and (not attribute_co_authored_by or author_explicit)
        )

        # Committer modification applies regardless of codecraft_edits (based on tests).
        # It's used if effective_committer is True AND
        # (it's not an codecraft edit with co-authored-by OR committer was explicitly set).
        use_attribute_committer = effective_committer and (
            not (codecraft_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        if prefix_commit_message:
            commit_message = "codecraft: " + commit_message
        ...
        committer_name = f"{original_user_name} (codecraft)"
    ...
    def refresh_codecraft_ignore(self):
        if not self.codecraft_ignore_file:
            return

        current_time = time.time()
        if current_time - self.codecraft_ignore_last_check < 1:
            return

        self.codecraft_ignore_last_check = current_time

        if not self.codecraft_ignore_file.is_file():
            return

        mtime = self.codecraft_ignore_file.stat().st_mtime
        if mtime != self.codecraft_ignore_ts:
            self.codecraft_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.codecraft_ignore_file.read_text().splitlines()
            self.codecraft_ignore_spec = pathspec.PathSpec.from_lines(
    ...
    def ignored_file_raw(self, fname):
        ...
        if not self.codecraft_ignore_file or not self.codecraft_ignore_file.is_file():
            return False
        ...
        return self.codecraft_ignore_spec.match_file(fname)
