...
from codecraft.mdstream import MarkdownStream
...
# Constants
NOTIFICATION_MESSAGE = "CodeCraft is waiting for your input"
...
class InputOutput:
    ...
    def __init__(
        ...
        self.append_chat_history(f"\n# codecraft chat started at {current_time}\n\n")
    ...
    def rule(self):
        if self.pretty:
            ...
            # Add extra newlines to ensure proper spacing in non-pretty mode (like PowerShell)
            print("\n" + "-" * 120)
    ...
    def get_input(
        ...
        # Add a print() before rule() call to create better separation from shell prompt
        print()
        self.rule()
    ...
    def get_default_notification_command(self):
        ...
        if system == "Darwin":  # macOS
            ...
            if shutil.which("terminal-notifier"):
                return f"terminal-notifier -title 'CodeCraft' -message '{NOTIFICATION_MESSAGE}'"
            # Fall back to osascript
            return (
                f'osascript -e \'display notification "{NOTIFICATION_MESSAGE}" with title "CodeCraft"\''
            )
        elif system == "Linux":
            ...
            for cmd in ["notify-send", "zenity"]:
                if shutil.which(cmd):
                    if cmd == "notify-send":
                        return f"notify-send 'CodeCraft' '{NOTIFICATION_MESSAGE}'"
                    elif cmd == "zenity":
