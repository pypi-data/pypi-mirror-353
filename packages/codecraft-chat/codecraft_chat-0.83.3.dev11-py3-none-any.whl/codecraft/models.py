...
from codecraft.sendchat import ensure_alternating_roles, sanity_check_messages
from codecraft.utils import check_pip_install_extra
...
MODEL_SETTINGS = []
with importlib.resources.open_text("codecraft.resources", "model-settings.yml") as f:
...
class ModelInfoManager:
    ...
    def __init__(self):
        self.cache_dir = Path.home() / ".codecraft" / "caches"
...
class Model(ModelSettings):
    def __init__(
        ...
        self.extra_model_settings = next(
            (ms for ms in MODEL_SETTINGS if ms.name == "codecraft/extra_params"), None
        )
    ...
    def configure_model_settings(self, model):
        ...
        if (
            ...
            and self.extra_model_settings.name == "codecraft/extra_params"
        ):
    ...
    def send_completion(self, messages, functions, stream, temperature=None):
        if os.environ.get("CODECRAFT_SANITY_CHECK_TURNS"):
    ...
    def simple_send_with_retries(self, messages):
        ...
        while True:
            try:
                ...
                from codecraft.reasoning_tags import remove_reasoning_content
...
def sanity_check_model(io, model):
    ...
    if not model.info:
        show = True
        # Don't print any error message here, main.py will handle it
...
def fuzzy_match_models(name):
    ...
    # If name is empty, return all models
    if not name:
