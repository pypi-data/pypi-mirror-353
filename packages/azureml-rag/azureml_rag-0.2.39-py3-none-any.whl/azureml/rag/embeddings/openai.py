...
class OpenAIEmbedder(Embeddings):
    ...
    def _embed(self, texts: List[str]) -> List[List[float]]:
        ...
        if self.api_type == "serverless":
            logger.info("Using serverless embedding model, no tokenization required.")
            ...
            logger.info(f"API Type is {self.api_type}, Using model {self.model} for embedding.")

            try:
                self._cl100k_local_cache_check()
            except Exception as e:
                logger.error(f"Failed to check local cache for cl100k_base encoding: {e}")
    ...
    def _cl100k_local_cache_check(self) -> None:
        """Check if the local cache for cl100k_base encoding is valid."""
        import hashlib
        ...
        logger.info(f"Using encoding {encoding_name} for embedding.")
        if encoding_name == "cl100k_base" and "TIKTOKEN_CACHE_DIR" in os.environ:
            tiktoken_cache_dir = os.environ["TIKTOKEN_CACHE_DIR"]
            cl100k_blob_name = "https://openaipublic.blob.core.windows.net/encodings/cl100k_base.tiktoken"
            cl100k_cache_key = hashlib.sha1(cl100k_blob_name.encode()).hexdigest()

            cache_file = Path(tiktoken_cache_dir, cl100k_cache_key)
            if cache_file.exists():
                with cache_file.open(mode="rb") as f:
                    data = f.read()
                    expected_hash = "223921b76ee99bde995b7ff738513eef100fb51d18c93597a113bcffe865b2a7"
                    if self._check_hash(data, expected_hash):
                        logger.info(f"Cache file is valid for {cl100k_blob_name}")
                    else:
                        logger.warning(f"Cache file is invalid for {cl100k_blob_name}")
            else:
                logger.info(f"Cache file {cache_file} does not exist for {cl100k_blob_name}")

    def _check_hash(self, data: bytes, expected_hash: str) -> bool:
        """Check if the SHA-256 hash of the data matches the expected hash."""
        import hashlib

        actual_hash = hashlib.sha256(data).hexdigest()
        logger.info(f"Actual hash: {actual_hash}, Expected hash: {expected_hash}")
