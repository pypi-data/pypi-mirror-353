...
class SafeStore:
    ...
    def _query_impl(
        ...
        assert self.conn is not None
        _vectorizer_name_requested = vectorizer_name or self.DEFAULT_VECTORIZER
        _vectorizer_name_to_use = _vectorizer_name_requested

        ASCIIColors.info(f"Received query. Vectorizer: '{_vectorizer_name_requested}', Metric: '{similarity_metric}', top_k={top_k}, min_similarity_percent={min_similarity_percent}%.")
        ...
        try:
            ...
            try:
                vectorizer, method_id = self.vectorizer_manager.get_vectorizer(_vectorizer_name_requested, self.conn, None)
                _vectorizer_name_to_use = _vectorizer_name_requested
                ASCIIColors.debug(f"Using requested vectorizer '{_vectorizer_name_to_use}' (method_id={method_id})")
            ...
            if vectorizer is None:
                if use_available_vectorization_if_vectorizer_not_present:
                    ASCIIColors.info(f"Vectorizer '{_vectorizer_name_requested}' not found/loaded. Trying to find an available one.")
                    cursor.execute("SELECT method_name FROM vectorization_methods ORDER BY method_id LIMIT 1")
                    available_method_row = cursor.fetchone()
                    if available_method_row:
                        _vectorizer_name_to_use = available_method_row[0]
                        ASCIIColors.info(f"Found available vectorizer: '{_vectorizer_name_to_use}'. Attempting to use it.")
                        try:
                            vectorizer, method_id = self.vectorizer_manager.get_vectorizer(_vectorizer_name_to_use, self.conn, None)
                        except (DatabaseError, ConfigurationError, VectorizationError) as e_fallback_vec:
                            ASCIIColors.error(f"Failed to load fallback vectorizer '{_vectorizer_name_to_use}': {e_fallback_vec}")
                            vectorizer = None
                    else:
                        ASCIIColors.warning("No vectorization methods found in the database at all.")
                
                if vectorizer is None and add_vectorizer_if_vectorizer_not_present:
                    ASCIIColors.warning(f"No suitable vectorizer found. Attempting to add '{_vectorizer_name_requested}' to all documents.")
                    self.conn.commit() 
                    try:
                        self._add_vectorization_impl(_vectorizer_name_requested, target_doc_path=None, vectorizer_params=None, batch_size=64)
                        self.conn.execute("BEGIN")
                        vectorizer, method_id = self.vectorizer_manager.get_vectorizer(_vectorizer_name_requested, self.conn, None)
                        _vectorizer_name_to_use = _vectorizer_name_requested
                    except Exception as e_add_vec:
                        if self.conn.in_transaction: self.conn.rollback()
                        raise QueryError(f"Failed to add and load vectorizer '{_vectorizer_name_requested}': {e_add_vec}") from e_add_vec
                
                if vectorizer is None:
                    self.conn.rollback()
                    raise QueryError(f"No suitable vectorizer found or could be prepared. Searched for '{_vectorizer_name_requested}'.")
            ...
            if not all_vectors_data:
                ASCIIColors.warning(f"No vectors found for method '{_vectorizer_name_to_use}'.")
                self.conn.commit()
                return []
            ...
            method_details_for_query = self.vectorizer_manager._get_method_details_from_db(self.conn, _vectorizer_name_to_use)
            if not method_details_for_query:
                raise DatabaseError(f"Could not retrieve method details for '{_vectorizer_name_to_use}'.")
            ...
            if candidate_vectors.shape[0] > 0 and candidate_vectors.shape[1] != query_vector.shape[0]:
                raise QueryError(f"Query vector dimension ({query_vector.shape[0]}) does not match stored vector dimension ({candidate_vectors.shape[1]}) for method '{_vectorizer_name_to_use}'.")
            ...
            if similarity_metric == 'cosine':
                ...
                if not is_likely_l2_normalized:
                    ASCIIColors.warning(
                        f"Dot product with vectorizer '{_vectorizer_name_to_use}' (type: {method_type}) may use non-L2-normalized vectors. "
                ...
                if not is_likely_l2_normalized:
                    ASCIIColors.warning(
                        f"Euclidean distance with vectorizer '{_vectorizer_name_to_use}' (type: {method_type}) may use non-L2-normalized vectors. "
