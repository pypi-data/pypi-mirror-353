...
class icCMS(pd.DataFrame):
    def __init__(self, iov=None, name='ICs', error=True):
        ...
        if isinstance(iov, icCMS):
            ...
            try:
                data = pd.read_csv(iov, sep='\s+', header=None, names=['ix', 'iy', 'iz', 'ic', 'eic'],
                                   usecols=[0, 1, 2, 3, 4])
            except pd.errors.ParserError:
                data = pd.read_csv(iov, sep='\s+', header=None, names=['ix', 'iy', 'iz', 'ic'],
                                   usecols=[0, 1, 2, 3])
                data['eic'] = 1
            ...
            data['eic'] = data['eic'].where(data['eic'] < 99, 0)
            data['ic'] = data['ic'].where(data['eic'] > 1e-8, np.nan)
