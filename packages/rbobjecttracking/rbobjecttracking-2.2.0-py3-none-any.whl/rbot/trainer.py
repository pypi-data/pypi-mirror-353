...
class Trainer:
    ...
    def quantize_model(self, model_path=None, save_path="classifier_quantized.pth", quantization_size="int8"):
        """Loads a trained model (if provided), applies the specified quantization precision, and saves it."""
        print(f"⚙️ Applying quantization with {quantization_size} precision...")

        if model_path:
            print(f"📥 Loading model from {model_path}")
            self.model.load_state_dict(torch.load(model_path, map_location="cpu"))

        # Select dtype based on user choice
        quantization_map = {
            "int8": torch.qint8,
            "int4": torch.quint4,  # Placeholder—PyTorch may not support int4 natively
            "int16": torch.qint16
        }
        quant_dtype = quantization_map.get(quantization_size, torch.qint8)  # Default to int8 if invalid choice

        quantized_model = torch.quantization.quantize_dynamic(self.model.cpu(), {nn.Linear}, dtype=quant_dtype)

        torch.save(quantized_model.state_dict(), save_path)
        print(f"✅ Quantized model saved as '{save_path}' with {quantization_size} precision.")
