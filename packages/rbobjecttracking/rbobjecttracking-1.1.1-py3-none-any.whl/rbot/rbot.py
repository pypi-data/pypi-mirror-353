...
class ColorDetector:
    ...
    def extract_rois(self, frame, minimum_contour_size, target_size=(128, 128)):
        ...
        rois = [cv2.resize(frame[y:y+h, x:x+w], target_size) for x, y, w, h in bounding_boxes] if bounding_boxes else []
        return rois, bounding_boxes

class RBOT:
    """Main RBOT tracking system."""
    def __init__(self, hsvValues, image_size, minimum_contour_size=500, minimum_confidence=0.8, model_path="classifier.pth"):
    ...
    def detect_object(self, frame, target_size=(128, 128)):
        ...
        rois, bounding_boxes = self.color_detector.extract_rois(frame, target_size=target_size, minimum_contour_size=self.minimum_contour_size)
    ...
    def track_object(self, frame, color, width, x_offset=15, y_offset=15):
        """Tracks object using bounding box visualization."""
        ROI, bounding_box = self.get_object(frame, self.image_size)
        if not isinstance(ROI, str):
            x, y, w, h = bounding_box
            bounding_box_frame = cv2.rectangle(frame, (x-x_offset, y-y_offset), (x+w+x_offset, y+h+y_offset), color, width)
