...
class Recorder:
    ...
    def lookup_address(self, address: str) -> tuple[int | None, str | None]:
        ...
        return cast(tuple[int | None, str | None], self.asndb.lookup(address))
    ...
    def lookup_asn(self, asn: int) -> str | None:
        ...
        return cast(str | None, self.asnamesdb.get(str(asn)))
    ...
    def asn_all_prefixes(self, asn: int, aggregate: bool = False) -> set[str]:
        ...
        if not aggregate:
            # upstream lacks type signatures
            return cast(set[str], self.asndb.get_as_prefixes(asn))
...
class CollectionsRecorder(Recorder):
    ...
    def record(
        ...
        if not args.no_asn:
            self.asn_counter[asn] += round(count)
        # record the prefix, if relevant
        if not args.no_prefixes:
            self.prefix_counter[prefix] += round(count)
...
class Collector:
    ...
    def _collect(
        ...
        parser: Callable[[str], tuple[str | None, float]] | None = None,
    ) -> None:
        raise NotImplementedError()
...
class LineCollector(Collector):
    def _collect(
        ...
        if parser is None:
            ...
            def parser(address: str | None = None) -> tuple[str | None, float]:
                return address, 1.0
...
class TupleCollector(LineCollector):
    def _collect(
        self,
        stream: TextIO = sys.stdin,
        parser: Callable[[str], tuple[str | None, float]] | None = None,
        ...
        if parser is None:
            ...
            def parser(line: str | None = None) -> tuple[str | None, float]:
                if not line:
                    return None, 0.0
                try:
