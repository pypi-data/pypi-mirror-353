...
def eread(addr, length=1, dtype="u1", ch=0, timeout=3, cwd=None):
    """
    ...
    try:
        raw = check_output(f"e {addr}l{length}", stderr=STDOUT, timeout=timeout, env={"ch":f"{ch}"}, cwd=cwd).decode("utf8")
    except CalledProcessError as e:
        print("e.exe error: " + e.output.decode("utf8"))
        raise e
    return e2int(raw, dtype=dtype)[0]

def ewrite(addr, value, ch=0, timeout=3, cwd=None, length=None):
    """
    ...
    try:
        check_output(f"e {addr} {value}", stderr=STDOUT, timeout=timeout, env={"ch":f"{ch}"}, cwd=cwd)
    except CalledProcessError as e:
        print("e.exe error: " + e.output.decode("utf8"))
        raise e
