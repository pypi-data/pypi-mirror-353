...
CASSETTE_PATH = os.path.join(
    ...
    "../tests/cassettes/ridewithgps_integration.yaml"
)
...
def scrub_user(user, id_maps):
    ...
    if 'gear' in user and isinstance(user['gear'], list):
        for gear in user['gear']:
            ...
            gear['group_membership_id'] = fake_id(gear.get('group_membership_id'), id_maps['group'], 3)
            gear['name'] = "FakeGear"
...
def scrub_results(results, id_maps):
    for ride in results:
        ...
        if 'group_membership_id' in ride and ride['group_membership_id']:
            ride['group_membership_id'] = fake_id(ride['group_membership_id'], id_maps['group'], 3)
...
def main():
    ...
    for interaction in cassette['interactions']:
        if '/users/current.json' in interaction['request']['uri']:
            ...
            interaction['response']['body']['string'] = json.dumps(resp, ensure_ascii=False, separators=(',', ':'))
        if '/trips.json' in interaction['request']['uri']:
            ...
            interaction['response']['body']['string'] = json.dumps(resp, ensure_ascii=False, separators=(',', ':'))
        interaction['response']['body']['string'] = scrub_string(interaction['response']['body']['string'])
        interaction['request']['uri'] = scrub_uri(interaction['request']['uri'], id_maps)
