...
def parse_functions_to_schema(func: Callable) -> dict:
    ...
    try:
        if isinstance(func, BaseTool):
            tool_schema = build_openai_schema(func)
            return tool_schema

        # This will handle the tools decorated with @tool
        if isinstance(func, StructuredTool):
            cur_openai_tool = convert_to_openai_function(
                func.args_schema)
            parameters = cur_openai_tool["parameters"]["properties"]
            parameters["required"] = cur_openai_tool["parameters"]["required"]
            tools_schema = {
                "name": func.name, "description": func.description, "parameters": parameters}
            return tools_schema

        # This will handle the normal functions
        elif callable(func):
            sig = signature(func)
            properties = {}
            required = []
