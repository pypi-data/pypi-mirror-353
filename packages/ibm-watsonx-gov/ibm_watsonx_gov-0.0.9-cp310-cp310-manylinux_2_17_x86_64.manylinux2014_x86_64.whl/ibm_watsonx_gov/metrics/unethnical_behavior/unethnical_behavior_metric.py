...
UNETHNICAL_BEHAVIOR = "unethnical_behavior"

class UnethnicalBehaviorResult(RecordMetricResult):
    name: str = UNETHNICAL_BEHAVIOR
    group: MetricGroup = MetricGroup.CONTENT_SAFETY


class UnethnicalBehaviorMetric(GenAIMetric):
    name: Annotated[Literal["unethnical_behavior"],
                    Field(default=UNETHNICAL_BEHAVIOR)]
    method: Annotated[
        Literal["granite_guardian"],
        Field(description=f"The method used to compute unethnical behavior metric.",
    ...
    def evaluate(
        ...
        kwargs["detector_params"] = {"risk": UNETHNICAL_BEHAVIOR}
