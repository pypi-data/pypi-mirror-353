...
class PTALifecycleEvaluator:
    ...
    def __validate_setup_stages(self, stages: list[EvaluationStage], prompt_template_id: str | None) -> None:
        self.logger.info(f"Validating setup stages: {stages}")

        # Require one of the set up config of the development project or an existing prompt template id
        if EvaluationStage.DEVELOPMENT not in stages and not prompt_template_id:
            message = f"{EvaluationStage.DEVELOPMENT} setup stage or prompt_template_id must be passed to the prompt evaluator."
            self.logger.error(message)
            raise Exception(message)
    ...
    def setup(
        ...
        Args:
            ...
            prompt_template (PropmtTemplate): The prompt template to evaluate.
    ...
    def __parse_configuration(self, config: dict) -> None:
        ...
        # Parse development related configurations. Development info are always required to be parsed
