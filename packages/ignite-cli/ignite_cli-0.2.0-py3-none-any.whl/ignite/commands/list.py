...
from igniteops_sdk.api.api_keys.list_keys import sync as list_keys_sync

app = Typer(help="List commands.")


@app.command("keys")
def list_keys(output: str = Option(get_default_output(), '-o', '--output', help='Output format', case_sensitive=False, show_choices=True), output_force: str = Option(None, '-ow', '--output-force', help='Set default output format', case_sensitive=False, show_choices=True), debug: bool = Option(False, '--debug', help='Enable HTTP request/response logging')):
    ...
    client = _sdk_client()
    try:
        resp = list_keys_sync(client=client)
