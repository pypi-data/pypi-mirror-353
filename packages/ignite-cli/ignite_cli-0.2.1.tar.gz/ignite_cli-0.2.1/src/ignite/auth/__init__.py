...
class BrowserAuthenticator:
    ...
    def __init__(self, client_id: str | None = None, identity_provider: str | None = None):
        ...
        self.identity_provider = identity_provider  # None by default to show all available options
        if not self.client_id:
            raise RuntimeError("Cognito Client ID is missing. Set IGNITE_CLIENT_ID env var or hard-code in auth module.")

    def login(self, force_email_otp: bool = False) -> Dict[str, str]:
        ...
        # Only add identity_provider if explicitly set
        if self.identity_provider:
            auth_params["identity_provider"] = self.identity_provider
        elif force_email_otp:
            # Try to force EMAIL_OTP mode - try different approaches
            # Don't set identity_provider, let Cognito show all available options
            # Add ui_locales to help with EMAIL_OTP display
            auth_params["ui_locales"] = "en"

        authorize_url = f"{AUTH_DOMAIN}/oauth2/authorize?" + urlparse.urlencode(auth_params)
