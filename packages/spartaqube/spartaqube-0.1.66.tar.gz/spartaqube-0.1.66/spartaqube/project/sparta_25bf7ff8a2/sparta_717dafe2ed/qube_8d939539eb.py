...
class PipInstallWS(WebsocketConsumer):
	...
	def receive(C,text_data):
		...
		if len(F)>0:
			G=json.loads(F);H=G['pipInstallCmd'].strip();L=G['env_name'];M=qube_8f5bff304e.sparta_6dd9319d8c(L);N=H.replace('pip',M)
			if not sparta_ea453aff21(H):A={E:-1,'errorMsg':'Invalid syntax'};B=json.dumps(A);C.send(text_data=B);return
