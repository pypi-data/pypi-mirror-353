...
def _get_error_logging_producer():
    """Get error logging producer for Kafka."""
    try:
        from confluent_kafka import Producer

        producer_config = {
            "bootstrap.servers": os.environ.get(
                "KAFKA_BOOTSTRAP_SERVERS", "34.66.122.137:9092"
            ),
            "acks": "all",
            "retries": 3,
        ...
        # Handle case where kafka is not available
        logging.warning("Kafka not available, error logging to Kafka disabled")
        return None
...
def send_error_log(
    ...
    try:
        ...
        if producer:
            ...
            producer.produce(topic="error_logs", value=json.dumps(message_value))
    except Exception as e:
