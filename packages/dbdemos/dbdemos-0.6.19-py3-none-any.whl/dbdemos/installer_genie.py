...
class InstallerGenie:
    ...
    def load_data_to_volume(self, ws: WorkspaceClient, data_folder: DataFolder, demo_conf: DemoConf, debug=True):
        ...
        try:
            ...
            # Check if the response is an error message or dictionary instead of a list
            if isinstance(files_data, dict) and 'message' in files_data:
                print(f"Error getting files from {github_path}: {files_data}")
                raise DataLoaderException(f"GitHub API error: {files_data['message']}")
            
            # Make sure we have a list of file objects
            if not isinstance(files_data, list):
                print(f"Unexpected response format from GitHub API: {type(files_data)}")
                raise DataLoaderException(f"Unexpected response format from GitHub API: got {type(files_data)} instead of list")
            ...
            if not files:
                print(f"No valid files found in GitHub repo for {data_folder.source_folder}")
                raise DataLoaderException(f"No valid files found in GitHub repo for {data_folder.source_folder}")
