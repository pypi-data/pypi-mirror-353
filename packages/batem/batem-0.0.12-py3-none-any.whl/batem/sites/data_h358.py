...
def make_data_provider(starting_stringdate: str = '15/02/2015', ending_stringdate: str = '15/02/2016', control: bool = False) -> DataProvider:
    ...
    dp.add_parameterized('office:Pheat', lambda k: dp('total_electric_power', k) + dp('occupancy', k) * dp('body_metabolism') + dp('office-outdoor:solar_factor') * dp('Psun_hitting_window', k) + dp('office:permanent_power') + (dp('office:heater_power_per_delta_surface_temperature') * (dp('dT_heat', k) if dp('dT_heat', k) > 1 else 0)), default=0, resolution=10)
