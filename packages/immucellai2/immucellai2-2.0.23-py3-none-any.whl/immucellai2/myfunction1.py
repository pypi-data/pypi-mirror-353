...
def gibbs_proposal_function(coords, random_state, phi, alpha, sp):
    ...
    for n in range(len(coords)):
        ...
        Z_n = []
        for g in range(G):
            row_sum = np.sum(prob_mat.iloc[g, :])  # 使用 NumPy 方法计算行和
            pvals = np.full(K, 1/K) if row_sum < min_threshold else prob_mat.iloc[g, :] / row_sum
...
def MainRun(RunObject,seed=42, MultithreadModule = 'multiprocessing'): 
    np.random.seed(seed)
    random.seed(seed)
    EnvironmentRun = RunObject.EnvironmentRun
    # print("###---->Main program Begin Run....")
