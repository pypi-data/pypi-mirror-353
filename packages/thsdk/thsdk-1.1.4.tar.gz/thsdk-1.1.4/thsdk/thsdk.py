...
from .constants import FieldNameMap, MARKETS, BLOCK_MARKETS, rand_account

__all__ = ['THS', 'Adjust', 'Interval']
...
class Lib:
    ...
    def _get_lib_path(self) -> str:
        ...
        if system == 'Linux':
            return os.path.join(base_dir, "lib", "hq.so")
        elif system == 'Darwin':
            if arch == 'arm64':
                raise THSAPIError('Apple M系列芯片暂不支持')
            return os.path.join(base_dir, "lib", 'hq.dylib')
        elif system == 'Windows':
            return os.path.join(base_dir, "lib", 'hq.dll')
...
class Payload:
    ...
    def __init__(self, data_dict: Dict[str, Any]):
        ...
        self.data: Union[List[Dict], Dict] = data_dict.get('data', data_dict.get('map', {}))
        self.dic_extra: Dict[str, Any] = data_dict.get('dic_extra', {})
        self.extra: Any = data_dict.get('extra')
        self.subscribe_id: Optional[str] = data_dict.get('subscribe_id')
        self.help: Optional[str] = data_dict.get('help')
    ...
    def __repr__(self) -> str:
        ...
        if isinstance(self.data, list):
            ...
            data_str = f"{data_preview!r}... ({len(self.data)} items)"
        else:
...
class THS:
    ...
    def __init__(self, ops: Optional[Dict[str, Any]] = None):
        ...
        self._lib = Lib()
        self._login = False
        self._shareInstanceId = random.randint(6666666, 8888888)
    ...
    def nextShareInstanceId(self) -> int:
        """获取共享实例 ID。

        Returns:
            int: 自增的共享实例 ID。
        """
        id_ = self._shareInstanceId
        self._shareInstanceId += 1
        return id_
    ...
    def call(self, method: str, params: Optional[Union[str, dict, list]] = "",
        ...
        if result_code == 0:
            ...
            return error_response(f"错误代码: {result_code}, 未找到请求数据: {method}, 参数:{params}")

    def cmd_query_data(self, req: str, service_type: str = "zhu", buffer_size: int = 1024 * 1024 * 2,
        ...
        while attempt < max_attempts:
            result_code, result = self._lib.call(method=f'cmd.query_data.{service_type}', params=req,
    ...
    def history_minute_time_data(self, ths_code: str, date: str, fields: Optional[List[str]] = None) -> Response:
        ...
        req = (f"id=207&instance={self.nextShareInstanceId()}&zipversion={self.zip_version()}"
               f"&code={short_code}&market={market}&datatype={data_type}&date={date}")
        response = self.cmd_query_data(req)
    ...
    def security_bars(self, ths_code: str, start: int, end: int, adjust: str, interval: int) -> Response:
        ...
        market = ths_code[:4]
        short_code = ths_code[4:]
        req = (f"id=210&instance={self.nextShareInstanceId()}&zipversion={self.zip_version()}"
    ...
    def get_block_data(self, block_id: int) -> Response:
        ...
        req = (f"id=7&instance={self.nextShareInstanceId()}&zipversion={self.zip_version()}"
               f"&sortbegin=0&sortcount=0&sortorder=D&sortid=55&blockid={block_id:x}&reqflag=blockserver")
        return self.cmd_query_data(req, "bk")
    ...
    def get_transaction_data(self, ths_code: str, start: int, end: int) -> Response:
        ...
        market = ths_code[:4]
        short_code = ths_code[4:]
        req = (f"id=205&instance={self.nextShareInstanceId()}&zipversion={self.zip_version()}"
               f"&code={short_code}&market={market}&start={start}&end={end}"
               f"&datatype={data_type}&TraceDetail=0")
        return self.cmd_query_data(req)
    ...
    def get_l2_transaction_data(self, ths_code: str, start: int, end: int) -> Response:
        ...
        market = ths_code[:4]
        short_code = ths_code[4:]
        req = (f"id=220&instance={self.nextShareInstanceId()}&zipversion={self.zip_version()}"
    ...
    def block_market_data(self, block_code: Any) -> Response:
        ...
        req = (f"id=200&instance={self.nextShareInstanceId()}&zipversion={self.zip_version()}"
               f"&codelist={short_codes}&market={market}"
               f"&datatype={data_type}")
        return self.cmd_query_data(req, "fu")
    ...
    def block_market_data_extra(self, block_code: Any) -> Response:
        ...
        req = (f"id=202&instance={self.nextShareInstanceId()}&zipversion={self.zip_version()}"
               f"&codelist={short_codes}&market={market}"
               f"&datatype={data_type}")
        return self.cmd_query_data(req, "fu")
