...
from .__constants__ import FieldNameMap, MARKETS, BLOCK_MARKETS, rand_account
...
class Payload:
    ...
    def from_json(cls, data: Dict[str, Any]) -> 'Payload':
        ...
        if isinstance(result, list):
            result = _convert_data_keys(result or [])
...
class THS:
    ...
    def next_share_instance_id(self) -> int:
        """获取共享实例 ID。
    ...
    def cmd_query_data(self, req: str, service_key: str = "zhu", buffer_size: int = 1024 * 1024 * 2,
                       max_attempts=5) -> Response:
        """查询行情数据。

        Args:
            req (str): 查询请求字符串。
            service_key (str, optional): 查询类型，默认为 "zhu"。
        ...
        while attempt < max_attempts:
            result_code, result = self._lib.call(method=f'cmd.query_data.{service_key}', params=req,
            ...
            if result_code == 0:
                ...
                if response.errInfo != "":
                    logger.info(f"查询数据错误信息: {response.errInfo}")
                logger.debug(f"查询执行: {req}, 类型: {service_key}")
                return response
            elif result_code == -1:
                current_size_mb = buffer_size / (1024 * 1024)
    ...
    def history_minute_time_data(self, ths_code: str, date: str, fields: Optional[List[str]] = None) -> Response:
        ...
        req = (f"id=207&instance={self.next_share_instance_id()}&zipversion={self.zip_version()}"
        ...
        if fields:
            response.payload.result = [entry for entry in response.payload.result if
    ...
    def get_block_data(self, block_id: int) -> Response:
        ...
        req = (f"id=7&instance={self.next_share_instance_id()}&zipversion={self.zip_version()}"
    ...
    def get_transaction_data(self, ths_code: str, start: int, end: int) -> Response:
        ...
        market = ths_code[:4]
        short_code = ths_code[4:]
        req = (f"id=205&instance={self.next_share_instance_id()}&zipversion={self.zip_version()}"
    ...
    def get_l2_transaction_data(self, ths_code: str, start: int, end: int) -> Response:
        ...
        market = ths_code[:4]
        short_code = ths_code[4:]
        req = (f"id=220&instance={self.next_share_instance_id()}&zipversion={self.zip_version()}"
    ...
    def stock_zh_lists(self):
        """A股"""
        return self.get_block_data(0xE)

    def stock_us_lists(self):
        """美股"""
        return self.get_block_data(0xDC47)

    def stock_hk_lists(self):
        """港股"""
        return self.get_block_data(0xB)

    def stock_zh_b_lists(self):
        """B股"""
        return self.get_block_data(0xF)

    def cbond_lists(self) -> Response:
        """可转债"""
    ...
    def block_market_data(self, block_code: Any) -> Response:
        ...
        req = (f"id=200&instance={self.next_share_instance_id()}&zipversion={self.zip_version()}"
    ...
    def block_market_data_extra(self, block_code: Any) -> Response:
        ...
        req = (f"id=202&instance={self.next_share_instance_id()}&zipversion={self.zip_version()}"
               f"&codelist={short_codes}&market={market}"
               f"&datatype={data_type}")
        return self.cmd_query_data(req, "fu")
