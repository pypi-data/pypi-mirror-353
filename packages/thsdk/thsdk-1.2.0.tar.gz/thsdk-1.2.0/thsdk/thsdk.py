...
class THS:
    ...
    def historical_daily_minute_data(self, ths_code: str, date: str, ) -> Response:
        """获取分钟级历史数据。

        Args:
            ths_code (str): 证券代码，格式为10位，以 'USHA' 或 'USZA' 开头。
            date (str): 查询日，格式为 YYYYMMDD。
        ...
        response = self.call(method="historical_daily_minute_data", params=params)
    ...
    def depth_full(self, ths_code: Union[str, list]) -> Response:
        """获取深度数据 全档。
        ...
        response = self.call(method="depth.full", params=params)
        return response
