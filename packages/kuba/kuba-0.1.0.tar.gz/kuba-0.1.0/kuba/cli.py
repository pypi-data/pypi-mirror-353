...
def logs_cmd(
    ...
    cmd = get_kubectl_logs_command(ctx, kubectl, pods, containers, debug, since, follow)
    log(f"cmd: {cmd}", debug)
...
def get_kubectl_logs_command(ctx: click.Context, kubectl: str, pods: Resources, containers: list[str], debug: bool, since: str, follow: bool) -> Command:
    ...
    if len(pods) > 1:
        log_cmd = remove_empty(
            [
                ...
                f"--namespace={ns}" if ns else "",
                f"--context={cluster}" if cluster else "",
                f'^({"|".join([pod.name for pod in pods])})$',  # stern supports regex for multiple pods
                *ctx.args,
