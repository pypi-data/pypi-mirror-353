...
class Cookies(MutableMapping):
    ...
    def create(self, name, value, domain=None, expires=None, path=None,
    ...
    def _change(self, ck, value=None, domain=None, expires=None, path=None,
        secure=None, comment=None, commenturl=None, port=None, now=None):
        if value is None:
            value = ck.value
        if domain is None:
            domain = ck.domain
    ...
    def _setCookie(self, name, value, domain, expires, path, secure, comment,
        ...
        for nm, val in list(self._req_headers.items()):
        ...
        if six.PY3:
            ...
            c[name.encode('latin1')] = value.encode('utf8')
        if secure:
            c[name]['secure'] = True
        if domain:
