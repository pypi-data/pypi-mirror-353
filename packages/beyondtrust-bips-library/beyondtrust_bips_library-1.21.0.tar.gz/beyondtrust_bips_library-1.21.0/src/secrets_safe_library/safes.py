...
class Safe(APIObject):
    ...
    def get_safe_by_id(self, safe_id: str) -> dict:
        """
        Find a safe by ID.

        API: GET Secrets-Safe/Safes/{id}

        Args:
            safe_id (str): The safe ID (GUID).

        Returns:
            dict: Safe object according requested API version.
        """
        ...
        utils.print_log(
            self._logger,
            f"Calling get_safe_by_id endpoint: {endpoint}",
            logging.DEBUG,
        )
    ...
    def list_safes(self) -> list:
        """
        Returns all safes.

        API: GET Secrets-Safe/Safes/
        ...
        utils.print_log(
            self._logger, f"Calling list_safes endpoint: {endpoint}", logging.DEBUG
        )
    ...
    def create_safe(
        ...
        utils.print_log(
            self._logger, f"Calling create safe endpoint: {endpoint}", logging.DEBUG
        )
        response = self._run_post_request(endpoint, payload, include_api_version=False)

        return response.json()

    def delete_safe_by_id(self, safe_id: str) -> None:
        """
        Delete a safe by ID.

        API: DELETE Secrets-Safe/Safes/{id}
        ...
        utils.print_log(
            self._logger,
            f"Calling delete_safe_by_id endpoint: {endpoint}",
            logging.DEBUG,
        )
        self._run_delete_request(endpoint, expected_status_code=204)
    ...
    def update_safe(
        ...
        utils.print_log(
            self._logger, f"Calling update safe endpoint: {endpoint}", logging.DEBUG
        )
