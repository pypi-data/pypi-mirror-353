...
class NotionSyncBackend(BaseSyncBackend):
    ...
    # notionmanager/backends.py  (add to NotionSyncBackend)
    def fetch_all_pages(self) -> list[dict]:
        """Return EVERY row in the database, already transformed."""
        pages_raw = self.notion_manager.get_pages(retrieve_all=True)
        return self.notion_manager.transform_pages(
            pages_raw, self.notion_db_config.forward_mapping
        )
