...
class ResourceRepository:
    ...
    def _reload_resources_if_stale(self, threshold_hours: int = 3):
        """
        Reloads the resources CSV if the last load time is older than the specified threshold.
        This ensures we always work with a reasonably fresh view of the data.
        """
        if not self.resources_list_path.exists():
            return  # Nothing to reload

        now = datetime.datetime.now()
        if (now - self._resources_last_loaded) > timedelta(hours=threshold_hours):
            logger.info("Reloading resources.csv due to stale cache...")
            self._reload_resources_csv()
    ...
    def write_dataframe_for_task(
        ...
        **kwargs,
        ...
        try:
            ...
            self._reload_resources_if_stale()
    ...
    def convert_dataframe(
        ...
        for column_name, (classif_name, classif_type) in from_class.items():
            ...
            try:
                ...
                logger.warning(f"Correspondence for {classif_name} to {classifications[classif_type_name]} not found. Skipping")
                continue
        ...
        if isinstance(concordance, pd.DataFrame) and "accounttype" in concordance.columns:
