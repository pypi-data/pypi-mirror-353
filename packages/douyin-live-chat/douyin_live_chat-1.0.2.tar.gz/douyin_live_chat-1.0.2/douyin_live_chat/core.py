...
def generateSignature(wss, script_file=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'DouyinLiveWebFetcher', 'sign.js')):
    """
...
class DouyinLiveWebFetcher:
    ...
    def _parseChatMsg(self, payload):
        ...
        print(f"收到聊天消息: 抖音号:{message.user.display_id} 用户[{message.user.nick_name}] 说: {message.content}")  # 打印聊天消息
        if self.chat_callback:
            self.chat_callback({
                'payload': payload,
                'user_id': message.user.display_id
            })
