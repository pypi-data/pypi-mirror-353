...
def generateSignature(wss, script_file=os.path.join(os.path.dirname(os.path.abspath(__file__)), '', 'sign.js')):
    """
...
class DouyinLiveWebFetcher:
    ...
    def __init__(self, live_id, chat_callback=None, protobuf_path=None):
    ...
    # def _wsOnMessage_back(self, ws, message):
    #     """
    #     最初代码的模样新写了一个 _wsonMessage()
    #     接收到数据
    ...
    def _wsOnError_back(self, ws, error):
        print("WebSocket error: ", error)
    
    def _wsOnClose_back(self, ws, *args):
    ...
    def _parseChatMsg(self, payload):
        try:
            ...
            if not hasattr(message, 'user') or not hasattr(message.user, 'display_id'):
                return
                
            print(f"收到聊天消息: 抖音号:{message.user.display_id} 用户[{message.user.nick_name}] 说: {message.content}")
            ...
            if self.chat_callback:
                ...
                callback_data = {
                    ...
                    'user_id': str(message.user.display_id) if hasattr(message.user, 'display_id') else '',
                    'content': str(message.content) if hasattr(message, 'content') else '',
                    'nick_name': str(message.user.nick_name) if hasattr(message.user, 'nick_name') else '',
                    'pay_grade': {
                        'level': int(message.user.pay_grade.level) if hasattr(message.user, 'pay_grade') else 0
                    }
                }
                self.chat_callback(callback_data)
        except Exception as e:
            print(f"解析聊天消息错误: {str(e)}")


    def _parseChatMsg_back(self, payload):
        """聊天消息"""
        message = ChatMessage().parse(payload)
    ...
    def _parseControlMsg_back(self, payload):
        '''直播间状态消息'''
        message = ControlMessage().parse(payload)
