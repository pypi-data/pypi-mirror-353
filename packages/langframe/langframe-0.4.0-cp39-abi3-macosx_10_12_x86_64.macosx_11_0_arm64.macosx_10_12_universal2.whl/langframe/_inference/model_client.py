...
class RateLimitBucket:
    ...
    def _get_available_capacity(self, curr_time: float) -> int:
        ...
        time_since_last_check = curr_time - self._last_update_time
        replenished_amount = (
            self._max_capacity * time_since_last_check / MINUTE_IN_SECONDS
        )
        return min(
            replenished_amount + self._current_capacity,
            self._max_capacity,
        )
...
class ModelClient(Generic[RequestT, ResponseT], ABC):
    """
    Base client for interacting with a specific language model type.
    Handles rate limiting, request queuing, retries, and deduplication.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            queue_size: The maximum size of the request queue.
            initial_backoff_seconds: The initial delay in seconds for exponential backoff.
            backoff_factor: The factor by which the backoff time increases.
            max_backoffs: The maximum number of retry attempts for a transient error.
        """
        self.model = model

        # Rate limiting
        self.tokens_bucket = RateLimitBucket(tokens_per_minute)
        self.requests_bucket = RateLimitBucket(requests_per_minute)
        ...
        logger.info(
            f"Initialized client for model {model} with {requests_per_minute} RPM and {tokens_per_minute} TPM"
        )
    ...
    async def make_single_request(
        ...
        Returns:
            The API response, None if the request was empty, or an exception wrapper
            (TransientException or FatalException).
        """
        pass

    @abstractmethod
    def estimate_tokens_for_request(self, request: RequestT) -> int:
        """
        Estimate the number of tokens that will be used for the request.
        Must be implemented by subclasses.
    ...
    def get_request_key(self, request: RequestT) -> Any:
        """
        Return a key that uniquely identifies a request for deduplication purposes.
        The key must be hashable (so it can be used as a dictionary key).

        Args:
            request: The request to get the key for.

        Returns:
            A hashable value that uniquely identifies this request
        """
        pass
    ...
    def shutdown(self):
        """
        Shuts down this model client, cancels all pending and inflight requests,
        and unregisters it from the ModelClientManager.
        """
    ...
    def make_batch_requests(
        ...
        """
        Submits a batch of requests to be processed asynchronously.

        Args:
            requests: A list of requests to process. None entries are handled as empty responses.
            operation_name: A name for logging purposes to identify the operation.

        Returns:
            A list of responses in the same order as the input requests.
        """
        ...
        with tqdm(
            ...
            for request in requests:
                ...
                if request is None:
                    ...
                    pbar.update(1)
                    pbar.set_postfix(tokens=total_estimated_tokens)
                    continue
        ...
        logger.info(
            f"Batch {batch_id}: Submitted {num_unique_requests} unique requests with {total_estimated_tokens} total tokens"
        )
    ...
    def _get_or_create_request_future(
        ...
        if key in unique_futures:
            ...
            return new_future, 0  # No tokens for duplicate requests

        # If it's a new request, create a future and estimate its token cost
        new_future = Future()
        unique_futures[key] = new_future
        estimated_tokens = self.estimate_tokens_for_request(request)
        return new_future, estimated_tokens
    ...
    async def _process_queue(self):
        """
        Continuously processes requests from the request and retry queues.
        This method runs on the shared asyncio event loop.
        """
        try:
            while True:
    ...
    async def _maybe_backoff(self):
        ...
        if time_since_last_transient_exception < backoff_time:
            ...
            self.tokens_bucket._set_capacity(0, now)
            self.requests_bucket._set_capacity(0, now)
