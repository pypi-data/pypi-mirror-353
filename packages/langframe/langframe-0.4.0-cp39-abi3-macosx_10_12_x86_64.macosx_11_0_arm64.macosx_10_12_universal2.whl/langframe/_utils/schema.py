...
def convert_markdown_schema_to_custom_dtype(json_value: Any) -> DataType:
    """
    Recursively converts a JSON schema (as a Python object) into a DataType.

    The rules are:
    - Array: if empty, returns ArrayType(PrimitiveType.STRING), otherwise returns ArrayType(inner_type)
    - Object: if empty, returns PrimitiveType.STRING, otherwise returns StructType with one field per key.
      When encountering a key named "section", we also add a child key "title": PrimitiveType.STRING.
    - Otherwise, returns PrimitiveType.STRING.
    """
    # Array case.
    if isinstance(json_value, list):
        if len(json_value) == 0:
            ...
            inner_dtype = convert_markdown_schema_to_custom_dtype(json_value[0])
            return ArrayType(inner_dtype)
    ...
    # Object case.
        ...
        if not json_value:
            ...
            for key, value in json_value.items():
                field_dtype = convert_markdown_schema_to_custom_dtype(value)
                # Special case for "section": add a "title" field of type STRING.
                if key == "section":
                    if isinstance(field_dtype, StructType):
                        # If "title" is not already present, add it.
                        if "title" not in [
                            field.name for field in field_dtype.struct_fields
                        ]:
                            field_dtype.struct_fields.append(
                                StructField("title", StringType)
                            )
                fields.append(StructField(key, field_dtype))
...
def _convert_polars_dtype_to_custom_dtype(
    ...
    if isinstance(polars_dtype, (pl.Int32, pl.Int64, pl.Int128, pl.Int16, pl.Int8)):
        ...
        # Convert the inner type for an ArrayType
