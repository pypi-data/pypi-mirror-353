...
def struct(
    *args: Union[ColumnOrName, List[ColumnOrName], Tuple[ColumnOrName, ...]]
    ...
    Args:
        *args: Columns or column names to combine into a struct. Can be individual
            arguments, lists, or tuples of columns/column names.
...
def array(
    *args: Union[ColumnOrName, List[ColumnOrName], Tuple[ColumnOrName, ...]]
    ...
    Args:
        *args: Columns or column names to combine into an array. Can be individual
            arguments, lists, or tuples of columns/column names.
...
def asc(column: ColumnOrName) -> Column:
    """Creates a Column expression representing an order to sort by.

    Args:
        value: the column to apply the ordering

    Returns:
        A Column expression representing the column and the order to sort by
...
def array_contains(
    ...
    Raises:
        TypeError: If value type is a different type than the elements in the array
        TypeError: If the column does not contain array data
...
def when(condition: Column, value: Column) -> Column:
    ...
    Returns:
        A Column expression that evaluates the condition and returns the specified value when true and returns None
...
def coalesce(*cols: ColumnOrName) -> Column:
    ...
    Accepts one or more column expressions or column names. If a nested list or tuple
    of columns is provided, they are flattened before evaluation. The resulting column
    contains the first non-null value from the input columns, evaluated in order.

    This mimics the behavior of SQL's COALESCE function.

    Args:
        *cols: Column expressions or column names, optionally nested in lists or tuples.

    Returns:
        Column: A column expression representing the coalesced result.
