...
from langframe._backends.cloud.engine_config import CloudSessionConfig
from langframe._backends.cloud.execution import CloudExecution
...
class CloudSessionState(BaseSessionState):
    ...
    def stop(self):
        ...
        future = asyncio.run_coroutine_threadsafe(
            self._cleanup_session(), self.asyncio_loop
        )
    ...
    def get_engine_grpc_metadata(self):
        ...
        if not self.client_token:
            raise ValueError("User token not set. Call authenticate_user first.")
    ...
    async def _entrypoint_register_app(self):
        ...
        if not self.app_name:
            raise ValueError("App name is required to create a session")
    ...
    async def _send_config_session_request_to_engine(self):
        ...
        try:
            ...
            if (
                ...
                raise RuntimeError(f"Failed to configure session: {e}") from e
