...
class TableIdentifier(BaseIdentifier):
    ...
    def from_string(cls, full_name: str) -> "TableIdentifier":
        ...
        if len(parts) == 2:
            return TableIdentifier(table=parts[1], db=parts[0])
        if len(parts) == 3:
            return TableIdentifier(catalog=parts[0], db=parts[1], table=parts[2])
        raise ValueError(
...
class DBIdentifier(BaseIdentifier):
    ...
    def from_string(cls, full_name: str) -> "DBIdentifier":
        ...
        parts = QualifiedNameParser._split(full_name)
        if len(parts) == 1:
            return DBIdentifier(db=parts[0])
        if len(parts) == 2:
            return DBIdentifier(catalog=parts[0], db=parts[1])
        raise ValueError(
