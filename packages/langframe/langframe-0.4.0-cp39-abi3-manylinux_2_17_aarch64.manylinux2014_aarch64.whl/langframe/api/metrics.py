...
class LMMetrics:
    ...
    Attributes:
        num_input_tokens: Number of tokens in the prompt/input
...
class QueryMetrics:
    ...
    def get_execution_plan_details(self) -> str:
        ...
        def _format_node(node: PhysicalPlanRepr, indent: int = 1) -> str:
            ...
            if op.lm_metrics.cost > 0:
                details.extend(
                    [
                        f"{indent_str}  Language Model Usage: {op.lm_metrics.num_input_tokens:,} input tokens, {op.lm_metrics.num_cached_input_tokens:,} cached input tokens, {op.lm_metrics.num_output_tokens:,} output tokens",
    ...
    def __str__(self) -> str:
        return (
            ...
            f"Language Model Cost: ${self.total_lm_metrics.cost:.6f}\n"
            f"Language Model Tokens: {self.total_lm_metrics.num_input_tokens:,} input tokens, {self.total_lm_metrics.num_cached_input_tokens:,} cached input tokens, {self.total_lm_metrics.num_output_tokens:,} output tokens\n"
