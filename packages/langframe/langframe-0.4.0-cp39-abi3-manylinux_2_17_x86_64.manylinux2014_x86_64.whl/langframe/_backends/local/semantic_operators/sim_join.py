...
from langframe._constants import VECTOR_INDEX_DIR
...
class SimJoin:
    ...
    def execute(self) -> pl.DataFrame:
        ...
        """
        left = filter_null_embeddings(self.left, LEFT_ON_COL_NAME)
        right = filter_null_embeddings(self.right, RIGHT_ON_COL_NAME)
...
def filter_null_embeddings(df: pl.DataFrame, embedding_column: str) -> pl.DataFrame:
    non_null_filter = (
        pl.col(embedding_column).is_not_null()
        & ~pl.col(embedding_column)
        .list.eval(pl.element().is_null() | pl.element().is_nan())
        .list.any()
    )
