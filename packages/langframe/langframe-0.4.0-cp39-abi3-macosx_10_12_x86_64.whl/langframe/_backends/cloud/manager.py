...
from langframe._backends.cloud.settings import CloudSettings
...
class CloudSessionManager:
    ...
    def configure(self, dependencies: CloudSessionManagerConfigDependencies):
        ...
        if self.initialized:
            logger.warning("CloudSessionManager already initialized, should not call configure again.")
            return
    ...
    @staticmethod
    def create_global_session_dependencies() -> Optional[CloudSessionManagerConfigDependencies]:
        """Create the dependencies for the cloud session manager"""
        if CloudSessionManager._class_instance and CloudSessionManager._class_instance.initialized:
            return None
        ...
        logger.debug(
            f"Initialized entrypoint stub with channel: {entrypoint_channel}"
        )
        hasura_client = HasuraClient(
        ...
        return settings, asyncio_loop, background_thread, entrypoint_channel, entrypoint_stub, hasura_client


    async def get_or_create_session(self, session_config: SessionConfig) -> Session:
        """Get or create a cloud session"""
        if not self.initialized:
            raise ValueError("CloudSessionManager not initialized. Call configure first.")

        async with self._lock:
            if session_config.app_name in self._live_sessions:
                session = self._live_sessions[session_config.app_name]
            ...
            session = Session._create_cloud_session(cloud_session_state)
            self._live_sessions[session_config.app_name] = session
            return session
    ...
    def authenticate_user_and_create_hasura_client(self):
        ...
        if self.client_token:
            ...
            logger.info(
                f"{self._settings.typedef_instance} Authenticating user {self._settings.client_id} with secret {self._settings.client_secret}"
            )
    ...
    def remove_session(self, app_name: str):
        ...
        async def _remove_session():
            async with self._lock:
                self._live_sessions.pop(app_name)
                if len(self._live_sessions) == 0:
