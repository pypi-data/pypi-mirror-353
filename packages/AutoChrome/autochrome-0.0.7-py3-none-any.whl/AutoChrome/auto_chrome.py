...
class AutoChrome(Chromium):
    ...
    def ele_for_action(
            ...
            timeout: Optional[float] = None,
    ) -> Union[ChromiumElement, ChromiumFrame, SessionElement, None]:
        """
        定位单个元素用于执行操作
    ...
    def wait_ele_displayed(
        ...
        try:
            ...
            if result:
                self.log.info(f"✅ 元素已出现: {sel_or_ele}")
            else:
    ...
    def _collect_options(
            self,
            first_opt: Union[ChromiumElement, None],
            timeout: float = 3,
    ) -> List[Tuple[ChromiumElement, str]]:
        """
        【静态方法】收集列表中的所有选项及其文本内容。
        ...
        try:
            # 获取第一个元素及后续兄弟节点
            next_elements = first_opt.nexts(timeout=timeout)
    ...
    def list_option(self, options: SelectType,
        ...
        # 获取列表的选项文本
        option_texts = self._collect_options(first_opt, timeout=timeout)
