...
class ProfilerMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            # ✅ EARLY RETURN : Pas HTML ou pas de profiler
            if not is_html or not profiler_enabled or not token:
                return response

            # ✅ EARLY RETURN : Redirections
            if response.status_code in (301, 302, 303, 307, 308):
                return response

            # ✅ TRAITEMENT : Seulement si on a un token valide
            is_streaming = (
    ...
    def _generate_profiler_bar(
        ...
        # ✅ CORRECTION : Récupérer correctement les données mémoire
        memory_data = profile.get("memory", {})
        memory = 0
        
        if memory_data:
            # Essayer différentes clés selon la version du collector
            memory = (
        ...
        context = {
            ...
            "memory": memory,  # ✅ Maintenant correctement défini
            "route": route,
