# framefox/core/di/service_container.py
import importlib
import inspect
import logging
import os
...
class ServiceContainer:
    ...
    - Performance optimizations with caching (via ServiceCacheManager)
    ...
    def __init__(self):
        ...
        # ✅ CACHE MANAGER : Gestionnaire de cache séparé
    ...
    def _initialize_container(self) -> None:
        ...
        try:
            # ✅ CONFIGURER : Le cache manager avec les settings
    ...
    def _register_essential_services(self) -> None:
        ...
        for service_path in essential_services:
            try:
                ...
                definition = ServiceDefinition(service_cls, public=True, autowire=True, synthetic=True, tags=["essential"])  # ✅ AJOUT : synthetic=True
                self._registry.register_definition(definition)

                # ✅ CRITIQUE : Instantiation immédiate comme dans service_containerbis
    ...
    def _discover_and_register_services(self) -> None:
        ...
        # ✅ CACHE : Essayer de charger depuis le cache
        ...
        # ✅ SCAN : Contrôleurs immédiatement (nécessaires)
        ...
        # ✅ CACHE : Sauvegarder le cache initial
        self._save_initial_cache()
        
        # ✅ LAZY : Le reste en arrière-plan si nécessaire
    ...
    def _setup_exclusions(self) -> None:
        """Configure les exclusions pour le scan."""
    ...
    def _save_initial_cache(self) -> None:
        """Sauvegarde le cache initial via le cache manager."""
        try:
    ...
    def _process_module(self, module_name: str) -> None:
        ...
        # Skip unnecessary modules early
        ...
        try:
            ...
            # Scan optimized: only relevant classes
    ...
    def _can_be_service(self, cls: Type) -> bool:
        ...
        external_libraries = [
            ...
            "bs4", "lxml", "markupsafe", "click", "rich", "typer", "uvicorn",
            "gunicorn", "pytest", "coverage", "mypy"
        ]
        ...
        if (hasattr(cls, "__abstractmethods__") and len(cls.__abstractmethods__) > 0 and 
    ...
    def get(self, service_class: Type[Any]) -> Any:
        """Get a service instance with Symfony-like autoloading."""
        ...
        # Autoloading: scan only requested module (PSR-4 like)
        ...
        # Dynamic creation like Symfony autowiring
    ...
    def _scan_specific_module(self, module_name: str) -> None:
        """Scan a specific module optimized."""
        if module_name in self._scanned_modules:
            return
    ...
    def _start_background_src_scan(self) -> None:
        ...
        def background_scan():
            try:
                ...
                # Save complete cache after scan
                cache_data = self._cache_manager.create_cache_snapshot(self._registry)
                self._cache_manager.save_cache(cache_data)
    ...
    def cleanup_memory(self) -> None:
        ...
        # Keep only essential modules in cache
