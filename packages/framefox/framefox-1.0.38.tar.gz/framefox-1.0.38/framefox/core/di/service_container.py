...
class ServiceContainer:
    ...
    def _discover_and_register_services(self) -> None:
        ...
        print(f"🔍 [DEBUG] Core path exists: {core_path.exists()}")
        ...
        print(f"🔍 [DEBUG] Excluded modules: {excluded_modules}")

        # Avant scan
        initial_count = len(self._registry.get_all_definitions())
        print(f"🔍 [DEBUG] Services before core scan: {initial_count}")

        # Scan core framework
        print(f"🔍 [DEBUG] Starting core framework scan...")
        ...
        print(f"🔍 [DEBUG] Services after core scan: {after_core_count} (added: {after_core_count - initial_count})")

        # Scan user source code
        for i, src_path in enumerate(src_paths):
            if src_path.exists():
                print(f"🔍 [DEBUG] Scanning user source path {i+1}/{len(src_paths)}: {src_path}")
                ...
                print(f"🔍 [DEBUG] Services after src scan {i+1}: {after_src} (added: {after_src - before_src})")
            else:
                print(f"🔍 [DEBUG] Source path {i+1} does not exist: {src_path}")
    ...
    def _scan_for_service_definitions(
        ...
        print(f"🔍 [SCAN] Scanning {base_package} at {base_path}")
        ...
        for root, dirs, files in os.walk(base_path):
            ...
            # Debug exclusion de répertoires
            if self._should_exclude_directory(root_path, excluded_dirs):
                print(f"🔍 [SCAN] Excluded directory: {root_path}")
                continue

            print(f"🔍 [SCAN] Processing directory: {root_path}")
            print(f"🔍 [SCAN] Files in directory: {files}")

            for file in files:
                if not self._should_process_file(file):
                    files_skipped += 1
                    print(f"🔍 [SCAN] Skipped file: {file}")
                    continue

                files_processed += 1
                print(f"🔍 [SCAN] Processing file: {file}")

                try:
                    module_name = self._build_module_name(root_path / file, base_path, base_package)
                    print(f"🔍 [SCAN] Built module name: {module_name}")

                    if self._should_exclude_module(module_name, excluded_modules):
                        print(f"🔍 [SCAN] Excluded module: {module_name}")
                        continue
                    ...
                    if services_added > 0:
                        print(f"🔍 [SCAN] ✅ Found {services_added} service(s) in {module_name}")
                    else:
                        print(f"🔍 [SCAN] ❌ No services found in {module_name}")

                except Exception as e:
                    modules_failed += 1
                    print(f"🔍 [SCAN] ❌ Error processing file {root_path / file}: {e}")

        print(f"🔍 [SCAN] Summary for {base_package}:")
        print(f"🔍 [SCAN] - Files processed: {files_processed}")
        print(f"🔍 [SCAN] - Files skipped: {files_skipped}")
        print(f"🔍 [SCAN] - Modules processed: {modules_processed}")
        print(f"🔍 [SCAN] - Modules failed: {modules_failed}")
        print(f"🔍 [SCAN] - Services found: {services_found}")
    def _should_exclude_directory(self, path: Path, excluded_dirs: List[str]) -> bool:
        """Check if a directory should be excluded from scanning."""
        dir_name = path.name.lower()
        
        # Liste d'exclusion basée sur le nom exact du répertoire
        exact_exclusions = {
            "entity", "entities", 
        ...
        # Vérifier si le nom du répertoire correspond exactement
        if dir_name in exact_exclusions:
            return True
    ...
    def _process_module(self, module_name: str) -> None:
        ...
        print(f"🔍 [MODULE] Processing module: {module_name}")
        
        try:
            module = importlib.import_module(module_name)
            print(f"🔍 [MODULE] ✅ Successfully imported: {module_name}")
            ...
            for attr_name in dir(module):
                ...
                try:
                    ...
                    if inspect.isclass(attr):
                        classes_found += 1
                        print(f"🔍 [MODULE] Found class: {attr_name} in {module_name}")
                        print(f"🔍 [MODULE] Class module: {attr.__module__}")
                        print(f"🔍 [MODULE] Is same module: {attr.__module__ == module_name}")
                        
                        if attr.__module__ == module_name:  # Classe définie dans ce module
                            can_be_service = self._can_be_service(attr)
                            print(f"🔍 [MODULE] Can be service: {can_be_service}")
                            
                            if can_be_service:
                                services_registered += 1
                                self._create_and_register_definition(attr)
                                print(f"🔍 [MODULE] ✅ Registered service: {attr_name}")
                            else:
                                print(f"🔍 [MODULE] ❌ Cannot be service: {attr_name}")

                except Exception as e:
                    if "access" not in str(e).lower():
                        print(f"🔍 [MODULE] Error inspecting {attr_name}: {e}")

            print(f"🔍 [MODULE] Module {module_name} summary:")
            print(f"🔍 [MODULE] - Total attributes: {attrs_found}")
            print(f"🔍 [MODULE] - Classes found: {classes_found}")
            print(f"🔍 [MODULE] - Services registered: {services_registered}")

        except (ModuleNotFoundError, ImportError) as e:
            print(f"🔍 [MODULE] ❌ Failed to import {module_name}: {e}")
            debug_modules = ["icecream", "devtools", "pytest", "IPython"]
            if not any(debug_module in str(e) for debug_module in debug_modules):
                print(f"🔍 [MODULE] Import error (not debug): {e}")
    ...
    def _can_be_service(self, cls: Type) -> bool:
        ...
        print(f"🔍 [FILTER] Checking if {cls.__name__} can be service...")
        
        if not inspect.isclass(cls):
            print(f"🔍 [FILTER] ❌ Not a class: {cls}")
            return False

        # Ignore exceptions
        if issubclass(cls, Exception):
            print(f"🔍 [FILTER] ❌ Is exception: {cls.__name__}")
            return False

        module_name = cls.__module__
        print(f"🔍 [FILTER] Module: {module_name}")

        # Vérifier le module
        if not (module_name.startswith("framefox.") or module_name.startswith("src.")):
            print(f"🔍 [FILTER] ❌ Not framefox or src module: {module_name}")
            return False
        ...
        if (hasattr(cls, "__pydantic_self__") or 
            ...
            print(f"🔍 [FILTER] ❌ Is Pydantic model: {cls.__name__}")
            return False

        if cls.__name__.startswith("_"):
            print(f"🔍 [FILTER] ❌ Private class: {cls.__name__}")
            return False

        if hasattr(cls, "__service__") and not cls.__service__:
            print(f"🔍 [FILTER] ❌ Explicitly marked as non-service: {cls.__name__}")
            return False

        print(f"🔍 [FILTER] ✅ Can be service: {cls.__name__}")
        return True
