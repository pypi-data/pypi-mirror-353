...
class ServiceContainer:
    ...
    def _should_use_background_scan(self) -> bool:
        """Version optimisée pour éviter le scan sur les petits projets."""
        ...
        for src_path in self._src_paths:
            if src_path.exists():
                ...
                if controllers_path.exists():
                    try:
                        controller_files = list(controllers_path.rglob("*.py"))
    
                        valid_controllers = [
                            f for f in controller_files 
                            if f.name != "__init__.py" and not f.name.startswith("test_")
                        ]
                        total_controllers += len(valid_controllers)
                    except Exception:
                        pass
        
        should_async = total_controllers > 50
