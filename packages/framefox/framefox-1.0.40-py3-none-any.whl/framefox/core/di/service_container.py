...
class ServiceContainer:
    ...
    def _should_exclude_directory(self, path: Path, excluded_dirs: List[str]) -> bool:
        ...
        parent_name = path.parent.name.lower() if path.parent != path else ""
        ...
        # Exclusions CONTEXTUELLES : seulement dans certains contextes
        contextual_exclusions = {
            "test": ["src", "framefox"],          # Exclure test/ dans src/ et framefox/
            "tests": ["src", "framefox"],         # Exclure tests/ dans src/ et framefox/
            "entity": ["src"],                    # Exclure entity/ SEULEMENT dans src/
            "entities": ["src"],                  # Exclure entities/ SEULEMENT dans src/
            "migration": ["src", "framefox"],     # Exclure migration/ dans src/ et framefox/
            "migrations": ["src", "framefox"],    # Exclure migrations/ dans src/ et framefox/
        }
        
        # Vérifier les exclusions contextuelles
        if dir_name in contextual_exclusions:
            allowed_parents = contextual_exclusions[dir_name]
            # Chercher si un des parents autorisés est dans le chemin
