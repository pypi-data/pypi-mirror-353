...
#         # ✅ ÉTAPE 0 : Essayer de charger depuis le cache
#         cache_data = self._create_service_cache()
#         if cache_data and self._load_from_cache(cache_data):
...
#         # ✅ ÉTAPE 3 : Scanner seulement les contrôleurs immédiatement
...
#                     controllers_path, 
#                     "src.controllers", 
#                     [],  # Pas d'exclusions pour controllers
#                     []
#                 )
        
#         # ✅ ÉTAPE 4 : Sauvegarder le cache initial (core + controllers)
#         self._save_initial_cache()
        
#         # ✅ ÉTAPE 5 : Le reste en lazy seulement si vraiment nécessaire
...
#     def _process_module(self, module_name: str) -> None:
#         """Version ultra-optimisée avec cache agressif."""
        
#         # ✅ CACHE : Éviter de rescanner
#         if module_name in self._scanned_modules:
#             return
        
#         # ✅ CACHE : Utiliser le cache même pour les échecs
#         if module_name in self._module_scan_cache:
#             for service_class in self._module_scan_cache[module_name]:
#                 if not self._registry.has_definition(service_class):
#                     self._create_and_register_definition(service_class)
...
#         # ✅ FILTRE PRÉCOCE : Éviter les imports inutiles
#         if self._should_skip_module(module_name):
#             self._scanned_modules.add(module_name)
#             self._module_scan_cache[module_name] = []  # Cache vide
#             return
...
#             # ✅ SCAN OPTIMISÉ : Seulement les classes pertinentes
#             for attr_name in dir(module):
#                 if not attr_name[0].isupper():  # Skip non-classes rapidement
#                     continue
...
#                         self._create_and_register_definition(attr)
#                 except Exception:
#                     pass  # Ignore les erreurs individuelles
            
#             # ✅ CACHE : Stocker même les résultats vides
#             self._module_scan_cache[module_name] = discovered_services
#             self._scanned_modules.add(module_name)
            
#         except (ModuleNotFoundError, ImportError):
#             # ✅ CACHE : Marquer les échecs pour éviter les tentatives répétées
#             self._scanned_modules.add(module_name)
#             self._module_scan_cache[module_name] = []

#     def _should_skip_module(self, module_name: str) -> bool:
#         """Filtre précoce pour éviter les imports inutiles."""
#         skip_patterns = [
...
#         """Version Symfony-like avec autoloading à la demande."""
...
#         # ✅ AUTOLOADING : Scanner SEULEMENT le module demandé (comme PSR-4)
#         if not definition and hasattr(service_class, '__module__'):
#             module_name = service_class.__module__
            
#             if module_name.startswith('src.') and module_name not in self._scanned_modules:
#                 self._logger.debug(f"Autoloading module: {module_name}")
#                 self._scan_specific_module(module_name)
#                 definition = self._registry.get_definition(service_class)
        
#         # ✅ CRÉATION DYNAMIQUE : Comme Symfony autowiring
...
#         # ✅ DÉSACTIVER le scan automatique pour les gros projets
#         return False

#     def _start_background_src_scan(self) -> None:
#         """Démarrer le scan src en arrière-plan avec cache."""
...
#                 # ✅ NOUVEAU : Sauvegarder le cache complet après scan
#                 complete_cache = self._create_cache_snapshot()
#                 self._save_service_cache(complete_cache)
...
#     def _create_service_cache(self) -> Dict[str, Any]:
#         """Cache persistant même en mode reload."""
#         cache_file = Path("var/cache/dev_services.json")
        
#         if cache_file.exists():
#             try:
#                 # ✅ EN DEV : Cache valide pendant 5 minutes seulement
#                 cache_age = time.time() - cache_file.stat().st_mtime
#                 if cache_age < 300:  # 5 minutes au lieu de 24h
#                     with open(cache_file) as f:
#                         cache_data = json.load(f)
                        
#                     # ✅ VÉRIFICATION : Fichiers sources modifiés ?
#                     if not self._are_source_files_modified(cache_data.get('scan_timestamp', 0)):
#                         return cache_data
#                     else:
#                         self._logger.debug("Source files modified, invalidating cache")
#                 else:
#                     self._logger.debug("Dev cache expired (5min)")
#             except Exception as e:
#                 self._logger.debug(f"Failed to read dev cache: {e}")
...
#             # ✅ VÉRIFIER : Core framefox seulement si nécessaire
...
#         """Sauvegarder le cache de services."""
#         if self.settings.app_env == "prod":
#             cache_file = Path("var/cache/services.json")
...
#             except Exception as e:
#                 self._logger.warning(f"Could not save service cache: {e}")
#     def _load_from_cache(self, cache_data: Dict[str, Any]) -> bool:
#         """Charger les services depuis le cache."""
#         try:
#             if "services" not in cache_data or "version" not in cache_data:
#                 return False
            
#             # Vérifier la version du cache
#             if cache_data["version"] != "1.0":
#                 self._logger.debug("Cache version mismatch, rebuilding")
#                 return False
...
#                     self._registry.register_definition(definition)
#                     self._scanned_modules.add(service_info["module"])
#                     loaded_count += 1
...
#             self._src_scanned = True  # Marquer comme scanné
#             return True
            
#         except Exception as e:
#             self._logger.debug(f"Failed to load from cache: {e}")
#             return False

#     def _save_initial_cache(self) -> None:
#         """Sauvegarder le cache initial après scan core + controllers."""
#         cache_data = self._create_cache_snapshot()
#         self._save_service_cache(cache_data)

#     def _create_cache_snapshot(self) -> Dict[str, Any]:
#         """Créer un snapshot du cache actuel."""
#         services = []
        
#         try:
#             all_definitions = self._registry.get_all_definitions()
            
#             for item in all_definitions:
#                 try:
#                     # ✅ VÉRIFICATION : item est-il un ServiceDefinition ou une classe ?
#                     if hasattr(item, 'service_class'):
#                         # C'est un ServiceDefinition
#                         definition = item
#                         service_class = definition.service_class
#                     elif inspect.isclass(item):
#                         # C'est directement une classe
#                         service_class = item
#                         # Créer une définition temporaire pour récupérer les attributs
#                         definition = self._registry.get_definition(service_class)
#                         if not definition:
#                             # Créer une définition par défaut
#                             definition = ServiceDefinition(service_class, autowire=True)
#                     else:
#                         self._logger.warning(f"Unknown item type in registry: {type(item)}")
#                         continue
...
#                 "timestamp": time.time(),
#                 "services": services,
#                 "scanned_modules": list(self._scanned_modules)
#             }
...
#     def _create_service_cache(self) -> Dict[str, Any]:
#         """Charger le cache de services depuis le disque."""
#         cache_file = Path("var/cache/services.json")
        
#         if cache_file.exists():
#             try:
#                 # Vérifier l'âge du cache (invalide après 24h en dev)
#                 if hasattr(self, 'settings') and self.settings.app_env == "dev":
#                     cache_age = time.time() - cache_file.stat().st_mtime
#                     if cache_age > 86400:  # 24 heures
#                         self._logger.debug("Cache expired in dev mode")
#                         return {}
                
#                 with open(cache_file) as f:
#                     return json.load(f)
#             except Exception as e:
#                 self._logger.debug(f"Failed to read cache: {e}")
...
#             self._logger.debug(f"Service cache saved with {len(cache_data.get('services', []))} services")
#         except Exception as e:
#             self._logger.warning(f"Could not save service cache: {e}")