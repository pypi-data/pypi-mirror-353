...
class ServiceContainer:
    ...
    def _discover_and_register_services(self) -> None:
        ...
        self._logger.debug("Starting service discovery...")
    
        # Core framework path
        core_path = Path(__file__).resolve().parent.parent
        self._logger.debug(f"Scanning core path: {core_path}")
        
        # Avant scan
        initial_count = len(self._registry.get_all_definitions())
        self._logger.debug(f"Services before core scan: {initial_count}")
        ...
        self._logger.debug(f"Services after core scan: {after_core_count}")
