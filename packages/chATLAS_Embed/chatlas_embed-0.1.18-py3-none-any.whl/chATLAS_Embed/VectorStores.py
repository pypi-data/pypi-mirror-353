...
from chATLAS_Embed.Base import Document, EmbeddingModel, VectorStore
...
class PostgresParentChildVectorStore(VectorStore):
    ...
    def delete(self, document_ids: list[str] = None, document_name: str = None, verbose=False) -> None:
        """
    ...
    def optimize_index_for_size(self):
        ...
        **NOTE**: Optimal is taken very loosely here!
        ...
        with self.engine.connect() as conn:
            ...
            optimal_probes = max(4, int(optimal_lists * 0.14))  # optimal bins roughly 10% of number of lists
    ...
    def enable_benchmarking_mode(self):
        ...
        try:
            ...
            print(
                "Could not outright disable index, defaulting to removing all indexes "
                "- check vectorstore.explain_analyse query output!"
            )
        self.search_hyperparams = """
    ...
    def search(
        ...
        final_results = sorted(combined_results.values(), key=lambda x: x[2], reverse=True)
    ...
    def _vector_search(
        ...
        with self.engine.connect() as conn:
            ...
            for i, row in enumerate(results):
                ...
                parent_metadata = row.parent_metadata.copy() if row.parent_metadata else {}
