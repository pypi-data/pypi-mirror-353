...
def test_wrong_embedding_size(test_db):
    class wrongEmbedder(EmbeddingModel):
        ...
        def embed(self, texts: list[str] | str, show_progress_bar: bool = None, batch_size=1) -> list[list[float]]:
            return [[0.1, 0.1, 0.1]]
...
def test_delete_dcouments(populated_vector_store):
    ...
    assert all("ATLAS_Experiment" not in result[0].metadata["name"] for result in results)
