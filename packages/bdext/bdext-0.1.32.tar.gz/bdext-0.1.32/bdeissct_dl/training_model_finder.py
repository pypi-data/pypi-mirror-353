...
def get_train_data(n_input, columns_x, labels, file_pattern=None, filenames=None, scaler_x=None, \
    ...
    def read_xz_lines(filenames, labels):
    ...
    dataset = tf.data.Dataset.from_generator(
        lambda: read_xz_lines(filenames, labels),
    ...
    #         .skip(1)
    #         # .filter(lambda line: tf.strings.length(line) > 0)
    #         .map(parse_line) #, num_parallel_calls=tf.data.AUTOTUNE)
    #     ),
    #     cycle_length=1,  # how many files to read in parallel
...
def main():
    ...
    x_indices, _, n_col, n_examples = get_data_characteristics(paths=params.train_data, scaler_x=scaler_x)
